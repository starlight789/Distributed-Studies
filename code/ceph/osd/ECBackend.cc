...
struct SubWriteCommitted : public Context {
  ...
  void finish(int) override {
    ...
    pg->sub_write_committed(tid, version, last_complete, trace, msg);
  }
};

void ECBackend::sub_write_committed(
  ceph_tid_t tid, eversion_t version, eversion_t last_complete,
  const ZTracer::Trace &trace, OpRequestRef &msg) {
  if (get_parent()->pgb_is_primary()) {
    ...
    // record log
    if (msg) {
      utime_t latency = ceph_clock_now() - msg->get_req()->get_recv_stamp();
      uint64_t inb = msg->get_req()->get_data().length();
      PerfCounters *logger = get_parent()->get_logger();
      logger->inc(l_osd_sop);
      logger->tinc(l_osd_sop_lat, latency);
      logger->inc(l_osd_sop_inb, inb);
      logger->inc(l_osd_sop_w);
  ...
  const ZTracer::Trace &trace)
{
  ...
  if (get_parent()->pgb_is_primary() && i->second.client_op) {
    ostringstream ss;
    ss << "sub_op_ec_commit_rec from " << from;
    i->second.client_op->mark_event_string(ss.str());
  }
