...
public:
  ...
  int get_osd_adjusted_crush_weight(int ruleno, int osd) const {
    int crush_weight = crush->get_item_weight(ruleno, osd);
    return ((float) crush_weight) * get_weightf(osd);
  }

  int calc_osd_parent_adjusted_weight(int parent_id) const;
...
public:
  ...
  bool does_rule_is_used_by_pool(int rule_id) const {
    for (auto &p: pools) {
      if (p.second.get_crush_rule() == rule_id)
        return true;
    }
