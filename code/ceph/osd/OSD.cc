...
#include "messages/MOSDLicense.h"
#include "messages/MOSDSlowDisk.h"
...
// begin_weekday:  [0, 6], include, 0 is Sunday
// end_weekday: [0, 7], exclude
// begin_time_second_of_day: [0, 86400], include
// end_time_second_of_day: [0, 86400], exclude
static bool utime_permit(utime_t now,
                         int begin_weekday,
                         int end_weekday,
                         int begin_time_second_of_day,
                         int end_time_second_of_day)
{
  struct tm bdt;
  time_t tt = now.sec();
  localtime_r(&tt, &bdt);
  ...
  if (begin_weekday < end_weekday) {
    if (bdt.tm_wday >= begin_weekday && bdt.tm_wday < end_weekday) {
      day_permit = true;
    }
  } else {
    if (bdt.tm_wday >= begin_weekday || bdt.tm_wday < end_weekday) {
  ...
  int now_time = bdt.tm_hour * 3600 + bdt.tm_min * 60 + bdt.tm_sec;

  bool time_permit = false;
  if (begin_time_second_of_day < end_time_second_of_day) {
    if (now_time >= begin_time_second_of_day && now_time < end_time_second_of_day) {
      time_permit = true;
    }
  } else {
    if (now_time >= begin_time_second_of_day || now_time < end_time_second_of_day) {
...
static int time_str_to_int(const char* str) {
  struct tm bdt;
  char *ptr = strptime(str, "%H:%M:%S", &bdt);
  if (ptr == nullptr || *ptr != '\0') {
    return -1;
  }
  return bdt.tm_hour*3600 + bdt.tm_min*60 + bdt.tm_sec;
}
...
{
  dout(10) << __func__ << " op " << *(op->get_req()) << " merged=" << op->is_merged() << dendl;
  send_error_op_reply(op, err, v, uv);
  if(op->is_merged()) {
    for(auto& m_op : op->merged_reqs) {
      if (m_op->origin_may_write()) {
        send_error_op_reply(m_op, err, eversion_t(), 0);
      } else {
        // some error occurred, try reading again
        send_error_op_reply(m_op, -EAGAIN, eversion_t(), 0);
      }
    }
  }
}

void OSDService::send_error_op_reply(OpRequestRef op, int err, eversion_t v, version_t uv)
{
  dout(5) << __func__ << " op " << *(op->get_req()) << " send error reply err="
          << err << " version=" << v
          << " user_version=" << uv << dendl;
...
float OSD::get_osd_recovery_sleep_from_config()
{
...
{
  float osd_delete_sleep = cct->_conf->osd_delete_sleep;
  if (osd_delete_sleep > 0)
    return osd_delete_sleep;
  if (!store_is_rotational && !journal_is_rotational)
    return cct->_conf->osd_delete_sleep_ssd;
  if (store_is_rotational && !journal_is_rotational)
    return cct->_conf->osd_delete_sleep_hybrid;
  return cct->_conf->osd_delete_sleep_hdd;
}

int OSD::init(uuid_d& cluster_fsid)
{
  ...
  store->set_cache_shards(cct->_conf->objectstore_cache_shard_num);
...
#ifdef WITH_LICENSE
  {
    // get store status
    struct store_statfs_t stbuf;
    bufferlist license_bl;
    string public_addr_ip = client_messenger->get_myaddr().ip_only_to_str();

    r = store->statfs(&stbuf);
    if (r < 0) {
      derr << __func__ << " failed get fs stat " << cpp_strerror(r) << dendl;
      return r;
    }

    if (cluster_fsid.is_zero()) {
      derr << __func__ << " cluster fsid is empty" << dendl;
      return -EINVAL;
    }
    // get my address
    osd_license = new License(cct,
                              "osd_license",
                              stbuf.total,
                              whoami,
                              public_addr_ip,
                              ...
                              cct->_conf->license_addr_ip,
                              cct->_conf->license_addr_port,
                              0, 0, 0, 0, 0, 0);
    r = read_local_object(osd_license->get_name(), license_bl);
    if (r < 0 && r != -ENOENT) {
      derr << __func__ << " failed to get license from store ret=" << r << dendl;
      return r;
    } else if (r >= 0) {
      auto p = license_bl.begin();
      r = osd_license->decode_and_check(p);
      if (r < 0) {
        derr << __func__ << " failed to decode license ret=" << r << dendl;
        return -EINVAL;
      }
    }
  }
...
#ifdef WITH_LICENSE
  // check license
  r = license_verify(true, 0);
  if (r < 0) {
    goto out;
  }
  dout(0) << "success to check license" << dendl;
#endif
...
#ifdef WITH_LICENSE
  {
    license_thread.create("osd_license");
  }
#endif
  ...
  {
    // init slow disk check
    map<string, bool> devices;
    _collect_metadata(&osd_metadata);
    r = store->get_devices_and_types(devices);
    if (r < 0) {
      derr << __func__ << " ERROR: failed to get devices of store: "
           << cpp_strerror(r) << dendl;
    } else {
      dout(1) << __func__ << " devices are " << devices << dendl;
      r = slow_disk_check.init(devices);
      if (r < 0) {
        derr << __func__ << " ERROR: failed to init slow disk check from metadata " << osd_metadata
             << " ret=" << r << dendl;
      }
    }

    // init the config of auto-adjust recovery
    pair<string, bool> data_device;
    r = store->get_data_device_and_type(data_device);
    if (r < 0) {
      derr << __func__ << " ERROR: failed to get the type of data device from store: "
           << cpp_strerror(r) << dendl;
    }
    service.init_disk_info(data_device);
    service.init_recovery_auto_config();
    service.handle_recovery_auto_adjust_config_change();
  }
...
void OSD::create_logger()
{
  ...
  osd_plb.add_u64_counter(
      l_osd_merged_op_before, "merged_ops_num_before",
      "the number of ops before merge ops");
  osd_plb.add_u64_counter(
      l_osd_merged_op_after, "merged_ops_num_after",
      "the number of ops after merge ops");
...
int OSD::write_local_object(const string &oid, const bufferlist &bl)
{
  ObjectStore::Transaction t;
  ghobject_t goid(hobject_t(sobject_t(object_t(oid), 0)));
  t.write(coll_t::meta(), goid, 0, bl.length(), bl);
  return store->queue_transaction(service.meta_ch, std::move(t), nullptr);
}

int OSD::read_local_object(const string &oid, bufferlist &bl)
{
  ghobject_t goid(hobject_t(sobject_t(object_t(oid), 0)));
  return store->read(service.meta_ch, goid, 0, 0, bl);
}

void OSD::license_entry() {
  dout(1) << "license entry start" << dendl;
  if (osd_license == nullptr) {
    dout(1) << "license is turn off" << dendl;
    return;
  }
  while(!license_stop) {
    double next_period = OSD_LICENSE_TICK_INTERVAL * ceph::util::generate_random_number(0.5, 1.5);
    utime_t t;
    t.set_from_double(next_period);

    Mutex::Locker l(license_lock);
    license_cond.WaitInterval(license_lock, t);
    if (!license_stop) {
      license_verify(false, next_period);
    }
  }
  dout(1) << "license entry stop" << dendl;
}

// check license
int OSD::license_verify(bool is_init, double duration_sec) {
  const int ONE_DAY = 86400;
  unsigned new_state = 0;
  utime_t now = ceph_clock_now();
  last_license_verify_time = ceph_mono_clock_now();

  int r = osd_license->check_auth(is_init, duration_sec);
  if (r == LICENSE_STATE_OK_OFFLINE) {
    // license is offline
    if (now - osd_license->get_last_check_succ_time() >
        cct->_conf->license_offline_error*ONE_DAY) {
      new_state |= CEPH_OSD_LICENSE_OFFLINE_LONG;
    } else {
      new_state |= CEPH_OSD_LICENSE_OFFLINE;
    }
    dout(0) << __func__ << " license is offline last check is "
            << osd_license->get_last_check_succ_time() << dendl;
    r = 0;
  } else if (r==LICENSE_STATE_REJECT_OFFLINE) {
    dout(0) << "osd license is offline for too long, last check is "
            << osd_license->get_last_check_succ_time() << dendl;
    r = -EACCES;
  } else if (r != LICENSE_STATE_REGED) {
    derr << "failed to check license ret=" << r << dendl;
    r = -EACCES;
  } else {
    r = 0;
  }
  ...
  {
    bufferlist bl;
    osd_license->encode(bl);
    int ret = write_local_object(osd_license->get_name(), bl);
    if (ret < 0) {
      derr << __func__ << " failed to store license info ret=" << r << dendl;
    }
  }

  // set expire status
  if (r == 0) {
    uint64_t expire = osd_license->get_expire_date();
    if (expire > 0) {
      // license has expire date
      if (now > expire) {
        r = -EACCES;
      } else if (expire - now <= cct->_conf->license_expire_error*ONE_DAY) {
        new_state |= CEPH_OSD_LICENSE_SOON_EXPIRE;
      } else if (expire - now <= cct->_conf->license_expire_warn*ONE_DAY) {
        new_state |= CEPH_OSD_LICENSE_NEAR_EXPIRE;
      }
    }
    license_state.store(new_state);
  }
  ...
  if (r < 0) {
    dout(0) << __func__ << " error: license is invalid, shutdown OSD" << dendl;
    queue_async_signal(SIGINT);
  }
  return r;
}

void OSD::send_license_status_update()
{
  const unsigned mask = CEPH_OSD_LICENSE_OFFLINE |
                        CEPH_OSD_LICENSE_OFFLINE_LONG |
                        CEPH_OSD_LICENSE_NEAR_EXPIRE |
                        CEPH_OSD_LICENSE_SOON_EXPIRE;

  unsigned cur = 0;
  if (osdmap->exists(whoami)) {
    cur = osdmap->get_state(whoami) & mask;
  }
  unsigned want = license_state.load();
  if (want == cur)
    return;

  set<string> s;
  OSDMap::calc_state_set(want, s);
  dout(5) << __func__ << " send license state " << s << dendl;
  monc->send_mon_message(new MOSDLicense(osdmap->get_epoch(), want));
}

void OSD::send_slow_disk_status_update()
{
  if (!cct->_conf->osd_slow_disk_check_enable) {
    return;
  }

  if (!osdmap->is_up(whoami)) {
    dout(1) << __func__ << " osd is not up" << dendl;
    last_slow_disk_state = 0;
    return;
  }

  utime_t now = ceph_clock_now();
  uint64_t new_state = slow_disk_check.get_disk_status();
  // add 30s, avoid osd reporting status to mon frequently
  if (last_slow_disk_state == new_state &&
      now - last_slow_disk_report_mon_time < cct->_conf->osd_slow_disk_check_warn_interval + 10) {
    dout(20) << __func__ << " disk state don't change, state=" << new_state
             << " last update " << last_slow_disk_report_mon_time
             << " warn interval " << cct->_conf->osd_slow_disk_check_warn_interval
             << dendl;
    return;
  }

  last_slow_disk_state = new_state;
  last_slow_disk_report_mon_time = now;

  map<uint64_t, string> s;
  slow_disk_check.get_disk_status_str(new_state, s, osd_metadata);
  dout(10) << __func__ << " send disk state " << s << dendl;
  monc->send_mon_message(new MOSDSlowDisk(osdmap->get_epoch(), monc->get_fsid(), osd_metadata));
}
...
void OSD::tick_without_osd_lock()
{
  ...
  if (is_active() || is_waiting_for_healthy()) {
    ...
    send_license_status_update();
    send_slow_disk_status_update();
    map_lock.put_read();
  ...
  service.auto_adjust_recover_speed_process();
  service.kick_recovery_queue();

#ifdef WITH_LICENSE
  utime_t now = ceph_mono_clock_now();
  if (now - last_license_verify_time > OSD_LICENSE_TICK_TIMEOUT) {
    dout(0) << __func__ << " license is invalid, shutdown OSD" << dendl;
    queue_async_signal(SIGINT);
  }
#endif
...
COMMAND("config unset " \
	...
	"osd", "rw")
COMMAND("config dump_recover_config",
	"dump configure of recovery",
	"osd", "r")
COMMAND("cluster_log " \
  ...
  bufferlist& odata, stringstream& ss, stringstream& ds)
{
  ...
  else if (prefix == "config dump_recover_config") {
    JSONFormatter *f = new JSONFormatter();
    f->open_object_section("recover_config");
    f->dump_string("osd_backfill_begin_time", cct->_conf->osd_backfill_begin_time);
    f->dump_string("osd_backfill_end_time", cct->_conf->osd_backfill_end_time);
    f->dump_int("osd_backfill_begin_weekday", cct->_conf->osd_backfill_begin_weekday);
    f->dump_int("osd_backfill_end_weekday", cct->_conf->osd_backfill_end_weekday);

    f->dump_string("osd_recovery_begin_time", cct->_conf->osd_recovery_begin_time);
    f->dump_string("osd_recovery_end_time", cct->_conf->osd_recovery_end_time);
    f->dump_int("osd_recovery_begin_weekday", cct->_conf->osd_recovery_begin_weekday);
    f->dump_int("osd_recovery_end_weekday", cct->_conf->osd_recovery_end_weekday);

    f->dump_bool("osd_recovery_speed_self_adaption", cct->_conf->osd_recovery_speed_self_adaption);
    f->dump_int("osd_max_backfills", cct->_conf->osd_max_backfills);
    f->dump_int("osd_recovery_max_active", cct->_conf->osd_recovery_max_active);
    f->dump_float("osd_recovery_sleep", cct->_conf->osd_recovery_sleep);
    f->close_section();
...
{
  bool permit = utime_permit(now,
                             cct->_conf->osd_scrub_begin_week_day,
                             cct->_conf->osd_scrub_end_week_day,
                             cct->_conf->osd_scrub_begin_hour * 3600,
                             cct->_conf->osd_scrub_end_hour * 3600);

  if (!permit) {
    dout(20) << __func__ << " should run between week day [" << cct->_conf->osd_scrub_begin_week_day
            << ", " << cct->_conf->osd_scrub_end_week_day << "), "
            << " hour [" << cct->_conf->osd_scrub_begin_hour << ", "
            << cct->_conf->osd_scrub_end_hour << "), now " << now << dendl;
  }
  return permit;
}
...
void OSD::_committed_osd_maps(epoch_t first, epoch_t last, MOSDMap *m)
{
  ...
  if (osdmap->is_up(whoami) &&
    ...
    if (is_booting()) {
      ...
      do_restart = false;
      // reset the state to 0, because we need to send the state to mon
      last_slow_disk_state = 0;
  ...
  if (osdmap->get_epoch() > 0 && is_active()) {
    if (!osdmap->exists(whoami)) {
      ...
      do_shutdown = true;   // don't call shutdown() while we have
			    // everything paused
    } else if (!osdmap->is_up(whoami) && (osdmap->get_state(whoami) & CEPH_OSD_SLOW_DISK_DROP)) {
      dout(0) << __func__ << " ERROR: mon mark me down, because I have slow disks, shutting down." << dendl;
      do_shutdown = true;  // osd mark me down, because me have slow disk
...
static int parse_reocver_time_str(const string& time_str, int default_val) {
  int val = time_str_to_int(time_str.c_str());
  if (val < 0) {
    val = default_val;
  }
  return val;
...
void OSDService::init_recovery_time_interval() {
  Mutex::Locker l(recovery_lock);
  recovery_start_second_of_day = parse_reocver_time_str(cct->_conf->osd_recovery_begin_time, 0);
  recovery_end_second_of_day = parse_reocver_time_str(cct->_conf->osd_recovery_end_time, 86400);
  backfill_start_second_of_day = parse_reocver_time_str(cct->_conf->osd_backfill_begin_time, 0);
  backfill_end_second_of_day = parse_reocver_time_str(cct->_conf->osd_backfill_end_time, 86400);
  dout(10) << __func__ << " recovery at ["
           << recovery_start_second_of_day << ", "
           << recovery_end_second_of_day << "), backfill at ["
           << backfill_start_second_of_day << ", "
           << backfill_end_second_of_day << ")" << dendl;
}
...
void OSDService::_maybe_queue_recovery() {
  ...
  // reovery with high priority
  while (!awaiting_throttle_high_priority.empty() && _recover_now(&available_pushes, true)) {
    uint64_t to_start = std::min(available_pushes, cct->_conf->osd_recovery_max_single_start);
    _queue_for_recovery(awaiting_throttle_high_priority.front(), to_start);
    awaiting_throttle_high_priority.pop_front();
    dout(10) << __func__ << " high priority recovery starting " << to_start
             << ", recovery_ops_reserved " << recovery_ops_reserved
             << " -> " << (recovery_ops_reserved + to_start) << dendl;
    recovery_ops_reserved += to_start;
  }

  // reovery with normal priority
  while (!awaiting_throttle.empty() && _recover_now(&available_pushes, false)) {
    uint64_t to_start = std::min(available_pushes, cct->_conf->osd_recovery_max_single_start);
    _queue_for_recovery(awaiting_throttle.front(), to_start);
    awaiting_throttle.pop_front();
    dout(10) << __func__ << " recovery starting " << to_start
             << ", recovery_ops_reserved " << recovery_ops_reserved
             << " -> " << (recovery_ops_reserved + to_start) << dendl;
    recovery_ops_reserved += to_start;
  }
  ...
  while (!backfill_awaiting_throttle_high_priority.empty() && _backfill_now(&available_pushes, true)) {
    uint64_t to_start = std::min(available_pushes, cct->_conf->osd_recovery_max_single_start);
    _queue_for_recovery(backfill_awaiting_throttle_high_priority.front(), to_start);
    backfill_awaiting_throttle_high_priority.pop_front();
    dout(10) << __func__ << " high priority backfill starting " << to_start
             << ", recovery_ops_reserved " << recovery_ops_reserved
             << " -> " << (recovery_ops_reserved + to_start) << dendl;
    recovery_ops_reserved += to_start;
  }

  // backfill with normal priority
  while (!backfill_awaiting_throttle.empty() && _backfill_now(&available_pushes, false)) {
    uint64_t to_start = std::min(available_pushes, cct->_conf->osd_recovery_max_single_start);
    _queue_for_recovery(backfill_awaiting_throttle.front(), to_start);
    backfill_awaiting_throttle.pop_front();
    dout(10) << __func__ << " backfill starting " << to_start
             << ", recovery_ops_reserved " << recovery_ops_reserved
             << " -> " << (recovery_ops_reserved + to_start) << dendl;
    recovery_ops_reserved += to_start;
  }
...
bool OSDService::_can_recover_or_backfill_now(uint64_t *available_pushes,
                                              bool high_priority,
                                              bool skip_time_limit_when_degraded,
                                              int begin_weekday,
                                              int end_weekday,
                                              int begin_time,
                                              int end_time)
{
  ...
  utime_t now = ceph_clock_now();
  if (now < defer_recovery_until) {
    dout(15) << __func__ << " defer until " << defer_recovery_until << dendl;
    return false;
  }

  if (recovery_paused || recovery_paused_by_client_op_inflight) {
    dout(15) << __func__ << " paused" << dendl;
    return false;
  }

  uint64_t max = get_osd_recovery_max_active();
  ...
  if ((!high_priority || !skip_time_limit_when_degraded) &&
      !utime_permit(now, begin_weekday, end_weekday, begin_time, end_time)) {
    dout(15) << __func__ << " should run between week day [" << begin_weekday
             << ", " << end_weekday << "), " << " time [" << begin_time
             << ", " << end_time << "), now " << now << dendl;
    return false;
  }
 ...
 bool OSDService::_recover_now(uint64_t *available_pushes, bool high_priority) {
  return _can_recover_or_backfill_now(available_pushes, high_priority,
      cct->_conf->osd_recovery_skip_time_limit_when_degraded, cct->_conf->osd_recovery_begin_weekday,
      cct->_conf->osd_recovery_end_weekday, recovery_start_second_of_day, recovery_end_second_of_day);
  return true;
}

bool OSDService::_backfill_now(uint64_t *available_pushes, bool high_priority) {
  return _can_recover_or_backfill_now(available_pushes, high_priority,
      cct->_conf->osd_backfill_skip_time_limit_when_degraded, cct->_conf->osd_backfill_begin_weekday,
      cct->_conf->osd_backfill_end_weekday, backfill_start_second_of_day, backfill_end_second_of_day);
}
  ...
  ThreadPool::TPHandle &handle)
{
   ...
   */
  float recovery_sleep = service.get_osd_recovery_sleep();
  {
...
void OSDService::start_recovery_op(PG *pg, const hobject_t& soid)
{
  ...
  dout(10) << "start_recovery_op " << *pg << " " << soid
           << " (" << recovery_ops_active << "/"
           << get_osd_recovery_max_active() << " rops)"
           << dendl;
  recovery_ops_active++;
...
void OSDService::finish_recovery_op(PG *pg, const hobject_t& soid, bool dequeue)
{
  ...
  dout(10) << "finish_recovery_op " << *pg << " " << soid
           << " dequeue=" << dequeue
           << " (" << recovery_ops_active << "/" << get_osd_recovery_max_active() << " rops)"
           << dendl;
...
// -- auto-adjust recover speed --

void OSDService::init_disk_info(pair<string, bool>& disk_info) {
  // init the type of data disk
  if (!disk_info.first.empty()) {
    if (disk_info.second) {
      data_disk_type = TYPE_SATA_HDD;
    } else if (disk_info.first.find("nvme") != string::npos) {
      data_disk_type = TYPE_NVME_SSD;
    } else {
      data_disk_type = TYPE_SATA_SSD;
    }
  }
  if (data_disk_type == TYPE_SATA_NONE) {
    derr << __func__ << " ERROR: failed to get the type of data disk, set the type of "
         << "data disk as HDD" << dendl;
    data_disk_type = TYPE_SATA_HDD;
  }
  ldout(cct, 0) << __func__ << " data device type: " << get_disk_type_name(data_disk_type) << dendl;
}

void OSDService::init_recovery_auto_config() {
  osd_max_backfills_temp = cct->_conf->osd_max_backfills;
  osd_recovery_max_active_temp = cct->_conf->osd_recovery_max_active;
  osd_recovery_sleep_temp = osd->get_osd_recovery_sleep_from_config();
}

#define HANDLE_RECOVERY_CONFIG(type)                                                                         \
  {                                                                                                          \
    recovery_speed_increase_r_op_lat_max = 1000000 * cct->_conf->osd_recovery_speed_increase_read_##type;    \
    recovery_speed_increase_w_op_lat_max = 1000000 * cct->_conf->osd_recovery_speed_increase_write_##type;   \
    recovery_speed_increase_rw_op_lat_max = 1000000 * cct->_conf->osd_recovery_speed_increase_rwrite_##type; \
    recovery_speed_decrease_r_op_lat_max = 1000000 * cct->_conf->osd_recovery_speed_decrease_read_##type;    \
    recovery_speed_decrease_w_op_lat_max = 1000000 * cct->_conf->osd_recovery_speed_decrease_write_##type;   \
    recovery_speed_decrease_rw_op_lat_max = 1000000 * cct->_conf->osd_recovery_speed_decrease_rwrite_##type; \
  }

void OSDService::handle_recovery_auto_adjust_config_change() {
  if (data_disk_type == TYPE_SATA_SSD) {
    HANDLE_RECOVERY_CONFIG(sata_ssd);
  } else if (data_disk_type == TYPE_NVME_SSD) {
    HANDLE_RECOVERY_CONFIG(nvme);
  } else {
    HANDLE_RECOVERY_CONFIG(hdd);
  }

  dout(1) << __func__ << " set recovery_speed_increase_r_op_lat_max=" << recovery_speed_increase_r_op_lat_max
          << " recovery_speed_increase_w_op_lat_max=" << recovery_speed_increase_w_op_lat_max
          << " recovery_speed_increase_rw_op_lat_max=" << recovery_speed_increase_rw_op_lat_max
          << " recovery_speed_decrease_r_op_lat_max=" << recovery_speed_decrease_r_op_lat_max
          << " recovery_speed_decrease_w_op_lat_max=" << recovery_speed_decrease_w_op_lat_max
          << " recovery_speed_decrease_rw_op_lat_max=" << recovery_speed_decrease_rw_op_lat_max
          << dendl;
}

float OSDService::get_osd_recovery_sleep() {
  return cct->_conf->osd_recovery_speed_self_adaption ? osd_recovery_sleep_temp :
         osd->get_osd_recovery_sleep_from_config();
}

bool OSDService::adjust_recover_speed(recover_adjust_t _type) {
  assert(_type == RECOVERY_SPEED_INC || _type == RECOVERY_SPEED_DEC);

  uint64_t new_max_backfills = osd_max_backfills_temp;
  uint64_t new_max_active = osd_recovery_max_active_temp;
  float new_op_sleep = osd_recovery_sleep_temp;

  if (_type == RECOVERY_SPEED_INC) { // increase
    // calc new recovery config
    if (new_op_sleep >= 0.0001) {
      float sleep_gap = cct->_conf->osd_recovery_gap_sleep;
      if (new_op_sleep > sleep_gap) {
        new_op_sleep -= sleep_gap;
      } else {
        new_op_sleep = 0;
      }
    } else if (new_max_active < cct->_conf->osd_recovery_adaption_max_active) {
      new_max_active += cct->_conf->osd_recovery_max_active_gap;
    } else if (new_max_backfills < cct->_conf->osd_recovery_adaption_max_backfills) {
      new_max_backfills += cct->_conf->osd_max_backfills_gap;
    }

    // limit the maximum speed of recovery
    if (new_max_backfills > cct->_conf->osd_recovery_adaption_max_backfills) {
      new_max_backfills = cct->_conf->osd_recovery_adaption_max_backfills;
    }
    if (new_max_active > cct->_conf->osd_recovery_adaption_max_active) {
      new_max_active = cct->_conf->osd_recovery_adaption_max_active;
    }
    if (new_max_backfills == cct->_conf->osd_recovery_adaption_max_backfills &&
        new_max_active == cct->_conf->osd_recovery_adaption_max_active &&
        new_op_sleep < cct->_conf->osd_recovery_adaption_lowest_sleep) {
      new_op_sleep = cct->_conf->osd_recovery_adaption_lowest_sleep;
    }
  } else {//decrease, quickly reduce the recovery speed to avoid affecting the client reqeusts
    // calc new recovery config
    if (new_op_sleep + 0.000001 < cct->_conf->osd_recovery_adaption_max_sleep) {
      new_op_sleep = cct->_conf->osd_recovery_adaption_max_sleep;
    } else if (new_max_active > cct->_conf->osd_recovery_adaption_min_active) {
      new_max_active /= 2;
    } else if (new_max_backfills > cct->_conf->osd_recovery_adaption_min_backfills) {
      new_max_backfills /= 2;
    }

    // limit the minimum speed of recovery
    if (new_max_active < cct->_conf->osd_recovery_adaption_min_active) {
      new_max_active = cct->_conf->osd_recovery_adaption_min_active;
    }
    if (new_max_backfills < cct->_conf->osd_recovery_adaption_min_backfills) {
      new_max_backfills = cct->_conf->osd_recovery_adaption_min_backfills;
    }
    if (new_max_backfills == cct->_conf->osd_recovery_adaption_min_backfills &&
        new_max_active == cct->_conf->osd_recovery_adaption_min_active &&
        new_op_sleep > cct->_conf->osd_recovery_adaption_max_sleep) {
  ...
  if (!cct->_conf->osd_recovery_speed_self_adaption) {
    return false;
  }

  if (osd_max_backfills_temp != new_max_backfills) {
    local_reserver.set_max(new_max_backfills);
    remote_reserver.set_max(new_max_backfills);
    osd_max_backfills_temp = new_max_backfills;
  }
  osd_recovery_max_active_temp = new_max_active;
  osd_recovery_sleep_temp = new_op_sleep;

  return true;
}

#define CALC_OP_LAT(op_perf, last_op_perf)                                       \
  ((op_perf.first - last_op_perf.first == 0) ? 0 :                               \
  (op_perf.second - last_op_perf.second)/(op_perf.first - last_op_perf.first))

// return:
// 0 hold
// 1 need increase
// 2 need decrease
OSDService::recover_adjust_t OSDService::calc_recover_adjust_type(pair<uint64_t, uint64_t> op_perf,
                                                                  pair<uint64_t, uint64_t> r_op_perf,
                                                                  pair<uint64_t, uint64_t> w_op_perf,
                                                                  pair<uint64_t, uint64_t> rw_op_perf) {
  uint64_t r_op_lat = CALC_OP_LAT(r_op_perf, last_osd_r_op_perf); // ns
  uint64_t w_op_lat = CALC_OP_LAT(w_op_perf, last_osd_w_op_perf);
  uint64_t rw_op_lat = CALC_OP_LAT(rw_op_perf, last_osd_rw_op_perf);

  dout(25) << __func__ << " r_op_lat=" << r_op_lat
           << " w_op_lat=" << w_op_lat
           << " rw_op_lat=" << rw_op_lat
           << dendl;

  if (r_op_lat >= recovery_speed_decrease_r_op_lat_max ||
      w_op_lat >= recovery_speed_decrease_w_op_lat_max ||
      rw_op_lat >= recovery_speed_decrease_rw_op_lat_max) {
    return RECOVERY_SPEED_DEC;
  } else if (r_op_lat < recovery_speed_increase_r_op_lat_max &&
      w_op_lat < recovery_speed_increase_w_op_lat_max &&
      rw_op_lat < recovery_speed_increase_rw_op_lat_max) {
    return RECOVERY_SPEED_INC;
  }

  return RECOVERY_SPEED_HOLD;
}

void OSDService::auto_adjust_recover_speed_process() {
  if (!cct->_conf->osd_recovery_speed_self_adaption &&
      !cct->_conf->osd_recovery_pause_when_client_op_inflight) {
    // may osd_recovery_pause_when_client_op_inflight is changed before uppause
    if (recovery_is_paused_with_client_op_inflight()) {
      unpause_recovery_with_client_op_inflight();
    }
    dout(30) << __func__ << " auto-adjust recover is not enabled" << dendl;
    return;
  }

  if (!cct->_conf->perf) {
    dout(30) << __func__ << " perf is not enabled" << dendl;
    return;
  }

  utime_t now = ceph_mono_clock_now();
  if (now - last_recovery_speed_check < ((double) cct->_conf->osd_recovery_speed_adaption_period * 0.94)) {
    return;
  }

  auto op_perf = logger->get_tavg_ns(l_osd_op_lat);
  auto r_op_perf = logger->get_tavg_ns(l_osd_op_r_lat);
  auto w_op_perf = logger->get_tavg_ns(l_osd_op_w_lat);
  auto rw_op_perf = logger->get_tavg_ns(l_osd_op_rw_lat);

  do {
    if (last_recovery_speed_check.is_zero() || // maybe the first time
        now < last_recovery_speed_check) { // maybe clock drift
      dout(10) << __func__ << " first clock or clock drift, last check is "
               << last_recovery_speed_check << " now is " << now
               << dendl;
      break;
    }

    // maybe the perf is reseted
    if (op_perf.first < last_osd_op_perf.first ||
        r_op_perf.first < last_osd_r_op_perf.first ||
        w_op_perf.first < last_osd_w_op_perf.first ||
        rw_op_perf.first < last_osd_rw_op_perf.first) {
      break;
    }

    dout(25) << __func__ << " new, op perf " << op_perf << " r_op_perf " << r_op_perf
             << " w_op_perf " << w_op_perf << " rw_op_perf " << rw_op_perf << dendl;
    dout(25) << __func__ << " old, op perf " << last_osd_op_perf << " r_op_perf " << last_osd_r_op_perf
             << " w_op_perf " << last_osd_w_op_perf << " rw_op_perf " << last_osd_rw_op_perf << dendl;

    uint64_t gap_ms = (now - last_recovery_speed_check).to_msec(); // ms

    // may need to pause recover
    if (cct->_conf->osd_recovery_pause_when_client_op_inflight) {
      if ((1000 * (op_perf.first - last_osd_op_perf.first)) >
          (gap_ms * cct->_conf->osd_recovery_pause_max_iops)) {
        pause_recovery_with_client_op_inflight();
        dout(5) << __func__ << " time gap " << gap_ms << " ms ops "
                << (op_perf.first - last_osd_op_perf.first)
                << " pause recovery when there have client requests inflight" << dendl;
        break;
      } else if (recovery_is_paused_with_client_op_inflight()) {
        unpause_recovery_with_client_op_inflight();
        dout(20) << __func__ << " time gap " << gap_ms << " ms ops "
                 << (op_perf.first - last_osd_op_perf.first)
                 << " unpause recovery" << dendl;
      }
    } else if (recovery_is_paused_with_client_op_inflight()) {
      unpause_recovery_with_client_op_inflight();
    }

    // may need to adjust the speed of recover
    recover_adjust_t ret = calc_recover_adjust_type(op_perf, r_op_perf, w_op_perf, rw_op_perf);

    dout(25) << __func__ << " recover adjust ret=" << get_recover_adjust_type(ret) << dendl;

    if (ret != RECOVERY_SPEED_HOLD) {
      adjust_recover_speed(ret);
      dout(25) << __func__ << " new osd_max_backfills_temp=" << osd_max_backfills_temp
               << "osd_recovery_max_active_temp=" <<  osd_recovery_max_active_temp
               << "osd_recovery_sleep_temp=" <<  osd_recovery_sleep_temp
               << dendl;
    }
  } while (0);

  last_recovery_speed_check = now;
  last_osd_op_perf = op_perf;
  last_osd_r_op_perf = r_op_perf;
  last_osd_w_op_perf = w_op_perf;
  last_osd_rw_op_perf = rw_op_perf;
}
...
const char** OSD::get_tracked_conf_keys() const
{
  static const char* KEYS[] = {
    ...
    // slow disk
    "osd_slow_disk_check_level1_hdd",
    "osd_slow_disk_check_level1_ssd",
    "osd_slow_disk_check_level1_nvme",
    "osd_slow_disk_check_level2_hdd",
    "osd_slow_disk_check_level2_ssd",
    "osd_slow_disk_check_level2_nvme",
    "osd_slow_disk_check_min_period",
    "osd_slow_disk_check_warn_interval",
    "osd_slow_disk_check_warn_long_interval",
    "osd_slow_disk_check_drop_interval",
    "osd_slow_disk_warn_percent",
    "osd_slow_disk_warn_long_percent",
    "osd_slow_disk_drop_percent",
    // recovery
    "osd_recovery_begin_time",
    "osd_recovery_end_time",
    "osd_recovery_begin_weekday",
    "osd_recovery_end_weekday",
    // backfill
    "osd_backfill_begin_time",
    "osd_backfill_end_time",
    "osd_backfill_begin_weekday",
    "osd_backfill_end_weekday",
    // auto-adjust recovery
    "osd_recovery_speed_self_adaption",
    "osd_recovery_speed_increase_read_hdd",
    "osd_recovery_speed_increase_read_sata_ssd",
    "osd_recovery_speed_increase_read_nvme",
    "osd_recovery_speed_increase_write_hdd",
    "osd_recovery_speed_increase_write_sata_ssd",
    "osd_recovery_speed_increase_write_nvme",
    "osd_recovery_speed_increase_rwrite_hdd",
    "osd_recovery_speed_increase_rwrite_sata_ssd",
    "osd_recovery_speed_increase_rwrite_nvme",
    "osd_recovery_speed_decrease_read_hdd",
    "osd_recovery_speed_decrease_read_sata_ssd",
    "osd_recovery_speed_decrease_read_nvme",
    "osd_recovery_speed_decrease_write_hdd",
    "osd_recovery_speed_decrease_write_sata_ssd",
    "osd_recovery_speed_decrease_write_nvme",
    "osd_recovery_speed_decrease_rwrite_hdd",
    "osd_recovery_speed_decrease_rwrite_sata_ssd",
    "osd_recovery_speed_decrease_rwrite_nvme",
    NULL
  };
  return KEYS;
}
...
{
  if (changed.count("osd_max_backfills") && !cct->_conf->osd_recovery_speed_self_adaption) {
  ...
  if (changed.count("osd_slow_disk_check_level1_hdd") ||
      changed.count("osd_slow_disk_check_level1_ssd") ||
      changed.count("osd_slow_disk_check_level1_nvme") ||
      changed.count("osd_slow_disk_check_level2_hdd") ||
      changed.count("osd_slow_disk_check_level2_ssd") ||
      changed.count("osd_slow_disk_check_level2_nvme") ||
      changed.count("osd_slow_disk_check_min_period") ||
      changed.count("osd_slow_disk_check_warn_interval") ||
      changed.count("osd_slow_disk_check_warn_long_interval") ||
      changed.count("osd_slow_disk_check_drop_interval") ||
      changed.count("osd_slow_disk_warn_percent") ||
      changed.count("osd_slow_disk_warn_long_percent") ||
      changed.count("osd_slow_disk_drop_percent")) {
    slow_disk_check.set_reset();
  }

  if (changed.count("osd_recovery_begin_time") ||
      changed.count("osd_recovery_end_time") ||
      changed.count("osd_backfill_end_time") ||
      changed.count("osd_backfill_end_time")) {
    service.init_recovery_time_interval();
  }

  if (changed.count("osd_recovery_begin_time") ||
      changed.count("osd_recovery_end_time") ||
      changed.count("osd_recovery_begin_weekday") ||
      changed.count("osd_recovery_end_weekday") ||
      changed.count("osd_backfill_begin_time") ||
      changed.count("osd_backfill_end_time") ||
      changed.count("osd_backfill_begin_weekday") ||
      changed.count("osd_backfill_end_weekday")) {
    service.kick_recovery_queue();
  }

  if (changed.count("osd_recovery_speed_self_adaption")) {
    service.init_recovery_auto_config();
  }

  if (changed.count("osd_recovery_speed_increase_read_hdd") ||
      changed.count("osd_recovery_speed_increase_read_sata_ssd") ||
      changed.count("osd_recovery_speed_increase_read_nvme") ||
      changed.count("osd_recovery_speed_increase_write_hdd") ||
      changed.count("osd_recovery_speed_increase_write_sata_ssd") ||
      changed.count("osd_recovery_speed_increase_write_nvme") ||
      changed.count("osd_recovery_speed_increase_rwrite_hdd") ||
      changed.count("osd_recovery_speed_increase_rwrite_sata_ssd") ||
      changed.count("osd_recovery_speed_increase_rwrite_nvme") ||
      changed.count("osd_recovery_speed_decrease_read_hdd") ||
      changed.count("osd_recovery_speed_decrease_read_sata_ssd") ||
      changed.count("osd_recovery_speed_decrease_read_nvme") ||
      changed.count("osd_recovery_speed_decrease_write_hdd") ||
      changed.count("osd_recovery_speed_decrease_write_sata_ssd") ||
      changed.count("osd_recovery_speed_decrease_write_nvme") ||
      changed.count("osd_recovery_speed_decrease_rwrite_hdd") ||
      changed.count("osd_recovery_speed_decrease_rwrite_sata_ssd") ||
      changed.count("osd_recovery_speed_decrease_rwrite_nvme")) {
    service.handle_recovery_auto_adjust_config_change();
  }
...
// --------------------------------
bool merge_cls_op_check_flag(int flag, int merge_type) {
  switch (merge_type) {
    case CEPH_OSD_RMW_FLAG_MERGE_RGW_TYPE:
      return !(flag & (~(CEPH_OSD_FLAG_ONDISK |
                         CEPH_OSD_FLAG_READ |
                         CEPH_OSD_FLAG_WRITE |
...
int OSD::init_op_flags(OpRequestRef& op)
{
  ...
  uint32_t can_merged_cls_op_num = 0;
  int merge_cls_op_type = 0;
  ...
  for (iter = m->ops.begin(); iter != m->ops.end(); ++iter) {
	...
	int is_write, is_read, merge_flag;
	string cname, mname;
	...
	is_write = flags & CLS_METHOD_WR;
        ...
        merge_flag = flags & CLS_MERGE_OP_TYPE_RGW;

	dout(10) << "class " << cname << " method " << mname << " "
                 << "merge type: " << merge_flag << " "
		 << "flags=" << (is_read ? "r" : "")
                             << (is_write ? "w" : "")
                             << (is_promote ? "p" : "")
                 << dendl;
	...
	if (is_write)
	  op->set_class_write();
        if (is_promote)
          op->set_promote();
        // Check if the request can be merged
        if (merge_flag && (can_merged_cls_op_num == 0 || merge_cls_op_type == merge_flag)) {
            ++can_merged_cls_op_num;
            merge_cls_op_type = merge_flag;
        }
  ...
  // ops can be merged only when:
  // 1. all sub ops have the same MERGE_OP_TYPE flag
  // 2. pool is not tier or ec
  // 3. only contains flags: CEPH_OSD_FLAG_ONDISK CEPH_OSD_FLAG_READ CEPH_OSD_FLAG_WRITE
  //    and CEPH_OSD_FLAG_KNOWN_REDIR
  // 4. don't hava snap
  if (can_merged_cls_op_num > 0 &&
      can_merged_cls_op_num == m->ops.size() &&
      pool &&
      !(pool->is_tier() || pool->require_rollback()) &&
      !pool->is_pool_snaps_mode() &&
      m->get_snap_seq() == 0 &&
      m->get_snaps().empty() &&
      merge_cls_op_check_flag(m->get_flags(), merge_cls_op_type)) {
    op->set_merge_op_type(merge_cls_op_type);
  }
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb)
{
 ...
 retry_pg:
  ...
  dout(20) << __func__ << " " << token
	   << " to_process " << slot->to_process
	   << " to_process len " << slot->to_process.size()
	   << " waiting " << slot->waiting
	   << " waiting_peering " << slot->waiting_peering << dendl;
