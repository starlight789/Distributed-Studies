...
public:
  ...
  bool recover_with_high_priority() {
    return (state & (PG_STATE_UNDERSIZED |
                     PG_STATE_DEGRADED)) ||
           acting.size() < pool.info.min_size;
  }
  ...
  std::pair<ghobject_t, bool> _delete_some(ObjectStore::Transaction *t, const ghobject_t& _next);
...
protected:
  ...
  void queue_recovery(bool is_backfill = false);
  bool queue_scrub();
