...
string osd_slow_disk_alarm_warn_str(slow_disk_warn_t _t) {
    switch(_t) {
        case OSD_SLOW_DISK_WARN_TYPE_WARN_DROP: 
            return "slow disk need to be dropped";
        case OSD_SLOW_DISK_WARN_TYPE_WARN_LONG:
            return "stuck in slow disk for too long";
        case OSD_SLOW_DISK_WARN_TYPE_WARN:
            return "slow disk";
        case OSD_SLOW_DISK_WARN_TYPE_NONE:
            return "none";
    }
    return "";
}

string osd_slow_disk_alarm_warn_str(const string &_t) {
    slow_disk_warn_t warn_t = atoi(_t.c_str());
    return osd_slow_disk_alarm_warn_str(warn_t);
}

bool osd_metadata_have_slow_disk(const map<string, string> &mp) {
  for (auto iter = mp.lower_bound(slow_disk_state_key); iter != mp.end(); ++iter) {
    if (!boost::algorithm::starts_with(iter->first, slow_disk_state_key)) {
      break;
    }
    string state_str = iter->first.substr(slow_disk_state_key.length());
    slow_disk_warn_t state = atoi(state_str.c_str());
    if (state > OSD_SLOW_DISK_WARN_TYPE_NONE && state < OSD_SLOW_DISK_WARN_TYPE_LAST) {
      return true;
    }
  }
  return false;
...
bool osd_metadata_have_slow_disk_need_drop(const map<string, string> &mp) {
  string state_key = slow_disk_state_key + to_string(OSD_SLOW_DISK_WARN_TYPE_WARN_DROP);
  if (mp.find(state_key) != mp.end()) {
    return true;
  }
  return false;
}

string osd_get_slow_disks_from_metadata(const map<string, string> &mp, slow_disk_warn_t _t) {
  string state_key = slow_disk_state_key + to_string((uint64_t) _t);
  auto iter = mp.find(state_key);
  if (iter != mp.end()) {
    return iter->second;
  }
  return "";
...
void pg_pool_t::decode(bufferlist::iterator& bl)
{
  ...
  if (struct_v >= 26) {
    ...
    if (application_metadata.size() == 1 &&
        application_metadata.find(APPLICATION_NAME_RGW) != application_metadata.end()) {
      only_used_by_rgw = true;
    }
  }
  if (struct_v >= 27) {
