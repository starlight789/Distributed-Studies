...
struct OpRequest : public TrackedOp {
  ...
  void set_merge_op_type(int flag);
  int get_merge_op_type();
  bool origin_may_write();
  bool origin_may_cache();
  bool origin_may_read();
...
private:
  ...
  static const uint8_t flag_op_merged   = 1 << 6;
...
public:
  ...
  const char *state_string() const override {
    ...
    case flag_op_merged: return "op merged";
    default: break;
    }
  ...
  void mark_op_merged(const string& s) {
    mark_flag_point_string(flag_op_merged, s);
  }
  ...
  inline bool is_merged() { return is_merged_op; }

  typedef boost::intrusive_ptr<OpRequest> Ref;

  void merge_op(OpRequest::Ref& op);
  int merged_ops_num() { return merged_reqs.size() + 1; }
...
public:
  std::list<OpRequest::Ref> merged_reqs;
};
