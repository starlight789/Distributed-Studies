...
public:
  ...
  /**
   * get the root of crush rule
   * @return the id of root bucket
   */
  int find_root_of_rule(unsigned ruleno, int &root) const;
   ...
   *
   * @param ruleno, the id of crush rule
   * @param id item id to check
   * @return weight of item
   */
  int get_item_weight(unsigned ruleno, int id) const;
...
public:
   ...
   * calculate a map of osds to weights for a given starting root
   *
   * Generate a map of which OSDs get how much crush weight for a
   * given starting root
   *
   * @param root node
   * @param pmap [out] map of osd to crush weight
   * @return sum of the crush weights for success, or negative error code
   */
   int get_osds_weight_of_root(int root, map<int,int> &pmap) const;
...
public:
  ...
  void do_rule(int rule, int x, vector<int>& out, int maxout,
    ...
    crush_choose_arg_map arg_map = choose_args_get_with_fallback(
      choose_args_index);
    int numrep = crush_do_rule(crush, rule, x, rawout, maxout, std::data(weight),
                               std::size(weight), work, arg_map.args);
    if (numrep < 0)
      numrep = 0;
    out.resize(numrep);
