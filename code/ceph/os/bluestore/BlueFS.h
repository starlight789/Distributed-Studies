...
private:
  ...
  ceph::coarse_real_time next_flush_compact;     ///< last compact time when flush
...
public:
  ...
  int get_devices_and_types(map<string, bool> &ls);
  int fsck();
  ...
  inline uint8_t choose_perfer_dev(uint8_t wanted_dev);
  ...
  void _maybe_compact_log(std::unique_lock<std::mutex>& l);
