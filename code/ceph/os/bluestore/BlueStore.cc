...
{
  std::lock_guard<std::recursive_mutex> l(cache->lock);
  wrapper_ghobject_t woid(&(o->oid));
  auto p = onode_map.find(woid);
  if (p != onode_map.end()) {
...
BlueStore::OnodeRef BlueStore::OnodeSpace::lookup(const ghobject_t& oid)
{
  ...
  OnodeRef o;
  bool hit = false;
  wrapper_ghobject_t woid(&oid);

  {
    std::lock_guard<std::recursive_mutex> l(cache->lock);
    ceph::unordered_map<wrapper_ghobject_t, OnodeRef>::iterator p = onode_map.find(woid);
    if (p == onode_map.end()) {
  ...
  const mempool::bluestore_cache_other::string& new_okey)
{
  ...
  ldout(cache->cct, 30) << __func__ << " " << old_oid << " -> " << new_oid
			<< dendl;
  ceph::unordered_map<wrapper_ghobject_t, OnodeRef>::iterator po, pn;
  wrapper_ghobject_t wold_oid(&old_oid);
  wrapper_ghobject_t wnew_oid(&new_oid);

  po = onode_map.find(wold_oid);
  pn = onode_map.find(wnew_oid);
  ceph_assert(po != pn);
  ...
  onode_map.erase(wold_oid);
  onode_map.insert(make_pair(wrapper_ghobject_t(&(oldo->oid)), oldo));
  cache->_add_onode(oldo, 1);

  // add at new position and fix oid, key
  o->oid = new_oid;
  o->key = new_okey;
  onode_map.insert(make_pair(wrapper_ghobject_t(&(o->oid)), o));
...
{
  for (auto& i : onode_map) {
    ldout(cct, LogLevelV) << *(i.first.obj) << " : " << i.second << dendl;
  }
}
  ...
  bool create)
{
  ...
  if (v.length() == 0) {
    ...
    for (auto& i : on->onode.attrs) {
      i.second.reassign_to_mempool(mempool::mempool_bluestore_onode_attrs);
    }
    ...
    if (on->onode.extent_map_shards.empty()) {
      ...
      on->extent_map.inline_bl.reassign_to_mempool(
        mempool::mempool_bluestore_inline_bl);
    } else {
      on->extent_map.init_shards(false, false);
    }
  }

  ldout(store->cct, 30) << " after on.key '" << on->key <<  "' len " << on->key.length()
                       << " capacity " << on->key.capacity()
                       << " on.oid.hobj.oid.name " << on->oid.hobj.oid.name
                       << " len " << on->oid.hobj.oid.name.length()
                       << " capacity " << on->oid.hobj.oid.name.capacity()
                       << dendl;
  o.reset(on);
  ...
  Collection *dest)
{
  ...
  while (p != onode_map.onode_map.end()) {
    if (!p->second->oid.match(destbits, destpg.pgid.ps())) {
      ...
      dest->onode_map.onode_map[wrapper_ghobject_t(&(o->oid))] = o;
...
void BlueStore::MempoolThread::_trim_shards(bool interval_stats)
{
  ...
  ldout(cct, 30) << __func__ << " max_shard_onodes: " << max_shard_onodes
                 << " max_shard_buffer: " << max_shard_buffer
                 << " onode alloc : " << meta_used
                 << " onode_num " << mempool::bluestore_cache_onode::allocated_items()
                 << " meta_alloc " << meta_alloc
                 << " max_shard_onodes " << max_shard_onodes
                 << " num_shards " <<  num_shards
                 << " bytes_per_onode " << meta_cache.get_bytes_per_onode()
                 << dendl;
  ...
  : c(c), o(o), it(it)
{
  ...
  if (o->onode.has_omap()) {
    ...
    it->lower_bound(head);
    ldout(c->store->cct, 20) << __func__ << " " << o->oid
                             << " omap set lower bound to " << pretty_binary_string(head)
                             << " tail is " << pretty_binary_string(tail) << dendl;
  }
}
...
int BlueStore::OmapIteratorImpl::upper_bound(const string& after)
{
  ...
  if (o->onode.has_omap()) {
    ...
    ldout(c->store->cct,20) << __func__ << " after " << after << " key "
			    << pretty_binary_string(key) << dendl;
    it->upper_bound(key);
    ldout(c->store->cct,20) << __func__ << " upper bound is key "
                            << pretty_binary_string(key) << dendl;
  } else {
    it = KeyValueDB::Iterator();
  }
...
int BlueStore::OmapIteratorImpl::lower_bound(const string& to)
{
  ...
  if (o->onode.has_omap()) {
    ...
    ldout(c->store->cct,20) << __func__ << " to " << to << " key "
			    << pretty_binary_string(key) << dendl;
    it->lower_bound(key);
    ldout(c->store->cct,20) << __func__ << " lower bound is key "
                            << pretty_binary_string(key) << dendl;
  } else {
    it = KeyValueDB::Iterator();
  }
...
{
  if (cct->_conf->bluestore_shard_finishers > 0) {
      m_finisher_num = cct->_conf->bluestore_shard_finishers;
  }
...
int BlueStore::get_devices_and_types(map<string, bool> &ls)
{
  bdev->get_device_and_type(ls);
  if (bluefs) {
    bluefs->get_devices_and_types(ls);
  }
  return 0;
}

int BlueStore::get_data_device_and_type(pair<string, bool>& device) {
  return bdev->get_device_and_type(device);
}
  ...
  )
{
  ...
  const string& prefix = o->onode.is_pgmeta_omap() ? PREFIX_PGMETA_OMAP : PREFIX_OMAP;
  auto bounds = KeyValueDB::IteratorBounds();
  if (o->onode.has_omap()) {
    std::string lower_bound, upper_bound;
    get_omap_key(o->onode.nid, string(), &lower_bound);
    get_omap_tail(o->onode.nid, &upper_bound);
    dout(20) << __func__ << " set db iterator lower_bound " << pretty_binary_string(lower_bound)
             << " upper_bound " << pretty_binary_string(upper_bound) << dendl;
  ...
  dout(20) << __func__ << " got db iterator " << dendl;
  ...
  PerfCounters &logger)
{
  ...
  // This is just used to show the read latency in platform.
...
void BlueStore::_kv_sync_thread()
{
  ...
  // if poweroff is set as false, we force BlueStore to periodically flush disks
  bool last_period_is_not_empty = true;
  bool need_force_flush = true;
  ...
  while (true) {
	...
	(bluefs_do_check_balance == false)) {
      if (kv_stop) {
        _no_poweroff_force_flush_all();
        break;
      }
      dout(20) << __func__ << " sleep" << dendl;
      uint64_t sleep_time_ms = cct->_conf->bluestore_bluefs_balance_interval * 1000;
      if (!cct->_conf->bdev_poweroff_safe && last_period_is_not_empty) {
        // need to flush the previously written data to disk
        sleep_time_ms = std::min(sleep_time_ms, cct->_conf->bdev_poweroff_force_flush_interval);
      }
      std::cv_status status = kv_cond.wait_for(l, std::chrono::milliseconds(sleep_time_ms));
      ...
      kv_ios = 0;
      kv_throttle_costs = 0;
      utime_t start = ceph_mono_clock_now();
      l.unlock();
	...
	if (aios) {
      ...
      if (likely(cct->_conf->bdev_poweroff_safe)) {
        need_force_flush = true;
      } else {
        if (bluefs_do_check_balance ||
            start.to_msec() - last_flush_time >= cct->_conf->bdev_poweroff_force_flush_interval) {
          dout(20) << __func__ << " bluefs_do_check_balance=" << bluefs_do_check_balance
                   << " now=" << start.to_msec()
                   << " last_flush_time=" << last_flush_time
                   << " bdev_poweroff_force_flush_interval="
                   << cct->_conf->bdev_poweroff_force_flush_interval << dendl;
          need_force_flush = true;
          force_flush = true;
          last_flush_time = start.to_msec();
        } else {
          need_force_flush = false;
        }
      }
	...
	deferred_done.clear();
      ...
      utime_t after_flush = ceph_mono_clock_now();
	...
	for (auto& txc : b->txcs) {
      ...
      int r = 0;
      if (need_force_flush) {
        dout(20) << __func__ << " submit transaction sync " << dendl;
        r = cct->_conf->bluestore_debug_omit_kv_commit ? 0 : db->submit_transaction_sync(synct);
        last_period_is_not_empty = false;
      } else {
        dout(20) << __func__ << " submit transaction async" << dendl;
        r = cct->_conf->bluestore_debug_omit_kv_commit ? 0 : db->submit_transaction(synct);
        last_period_is_not_empty = true;
      }
	...
	dout(10) << __func__ << " blobid_max now " << blobid_max << dendl;
      ...
      {
	utime_t finish = ceph_mono_clock_now();
	...
	if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) &&
	  ...
	  if (!g_conf->bluestore_debug_omit_block_device_write) {
	    if (b_len <= prefer_deferred_size &&
                prefer_deferred_size * 100  >= total_length * cct->_conf->bluestore_deferred_write_min_percentage) {
  ...
  for (auto& wi : wctx->writes) {
    ...
    if (!g_conf->bluestore_debug_omit_block_device_write) {
      if (l->length() <= prefer_deferred_size &&
          prefer_deferred_size * 100  >= need * cct->_conf->bluestore_deferred_write_min_percentage) {
	dout(20) << __func__ << " deferring small 0x" << std::hex
	...
	b->get_blob().map_bl(
	  b_off, *l,
	  [&](uint64_t offset, bufferlist& t) {
	    bdev->aio_write(offset, t, &txc->ioc, cct->_conf->bluestore_force_use_buffer_io);
	  });
      }
    }
  }
  ...
  WriteContext *wctx)
{
  ...
  if (offset / min_alloc_size == (end - 1) / min_alloc_size &&
    ...
    _do_write_small(txc, c, o, offset, length, p, wctx, length);
  } else {
    ...
    if (head_length) {
      _do_write_small(txc, c, o, head_offset, head_length, p, wctx, length);
    }
    ...
    if (tail_length) {
      _do_write_small(txc, c, o, tail_offset, tail_length, p, wctx, length);
    }
  }
}
   ...
   WriteContext *wctx)
{
  if (fadvise_flags & CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) {
    ...
    wctx->buffered = true;
        ...
        cct->_conf->bluestore_force_use_buffer_io) &&
			...
			bufferptr& val)
{
  ...
  if (val.is_partial()) {
    auto& b = o->onode.attrs[name.c_str()] = bufferptr(val.c_str(),
						       val.length());
    b.reassign_to_mempool(mempool::mempool_bluestore_onode_attrs);
  } else {
