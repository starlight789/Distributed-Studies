                      ...
                      struct spdk_nvme_ctrlr *ctrlr, const struct spdk_nvme_ctrlr_opts *opts)
{
  ...
  dout(0) << __func__ << " found device at bus: " << spdk_pci_device_get_bus(pci_dev)
          ...
          << dendl;
  char serial_number[128];
  int result = spdk_pci_device_get_serial_number(pci_dev, serial_number, 128);
  if (result < 0) {
    dout(10) << __func__ << " failed to get serial number from %p" << pci_dev << dendl;
...
int NVMEDevice::open(const string& p, bool is_data_dev)
{
  int r = 0;
...
int NVMEDevice::flush(bool force_flush)
{
  return 0;
}
