...
class Cond_monotonic {
  pthread_cond_t _c;
  pthread_condattr_t attr;
  Mutex *waiter_mutex;

  // don't allow copying.
  void operator=(Cond_monotonic &C);
  Cond_monotonic(const Cond_monotonic &C);

 public:
  Cond_monotonic() : waiter_mutex(NULL) {
    assert(!pthread_condattr_init(&attr));
    pthread_condattr_setclock(&attr, CLOCK_MONOTONIC);
    assert(!pthread_cond_init(&_c, &attr));
    pthread_condattr_destroy(&attr);
  }

  virtual ~Cond_monotonic() {
    pthread_cond_destroy(&_c);
  }
  ...
  int WaitInterval(Mutex &mutex, utime_t interval) {
    utime_t when = ceph_mono_clock_now();
    when += interval;
    return WaitUntil(mutex, when);
  }
  ...
  template<typename Duration>
  int WaitInterval(Mutex &mutex, Duration interval) {
    ceph::mono_clock::time_point when(ceph::mono_clock::now());
    when += interval;
    struct timespec ts = ceph::mono_clock::to_timespec(when);
