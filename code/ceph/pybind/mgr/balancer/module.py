...
default_max_degraded = .05    # max ratio of pgs degraded at a time
...
class Module(MgrModule):
    OPTIONS = [
            ...
            {'name': 'end_time'},
            {'name': 'begin_weekday'}, #'0 or 7 = Sunday, 1 = Monday, etc.'
            {'name': 'end_weekday'}, #'0 or 7 = Sunday, 1 = Monday, etc.'
            {'name': 'max_misplaced'},
            {'name': 'max_degraded'},
            {'name': 'min_score'},
            {'name': 'mode'},
    ...
    def handle_command(self, inbuf, command):
        ...
        if command['prefix'] == 'balancer status':
            ...
            pool_ids = self.get_config('pool_ids', '')
            if pool_ids == '':
                return (0, '', '')
            ...
            for p in pool_ids:
                if p in pool_name_by_id:
                    final_ids.append(str(p))
                    final_names.append(pool_name_by_id[p])
                else:
                    should_prune = True
            if should_prune: # some pools were gone, prune
                self.set_config('pool_ids', ','.join(final_ids))
            return (0, json.dumps(sorted(final_names), indent=4), '')
            ...
            existing = self.get_config('pool_ids', '')
            final = to_add
            if existing != '':
                existing = existing.split(',')
                final = set(to_add) | set(existing)
            self.set_config('pool_ids', ','.join(final))
            return (0, '', '')
    ...
    def time_permit(self):
        ...
        weekday = (local_time.tm_wday + 1) % 7
        permit = False
    ...
    def pg_status_permit(self, info):
        """
        check pg status
        """
        ...
        max_misplaced = float(self.get_config('max_misplaced', default_max_misplaced))
        max_degraded = float(self.get_config('max_degraded', default_max_degraded))
        self.log.debug('unknown %f degraded %f inactive %f misplaced %f max_misplaced %f max_degraded %f',
                       unknown, degraded, inactive, misplaced, max_misplaced, max_degraded)
        ...
        if unknown > 0.0:
            ...
            detail = 'Too many objects (%f > %f) are degraded; try again ' \
                     'later' % (degraded, max_degraded)
            self.log.info(detail)
            return -errno.EAGAIN, detail
        elif inactive > 0.0:
    ...
    def serve(self):
        ...
        while self.run:
            self.active = self.get_config('active', '') != ''
            sleep_interval = float(self.get_config('sleep_interval', default_sleep_interval))
            ...
            if self.active and self.time_permit():
                ...
                osdmap = self.get_osdmap()
                allow = self.get_config('pool_ids', '')
                final = []
                if allow != '':
                    allow = allow.split(',')
    ...
    def optimize(self, plan):
        ...
        self.log.info('Optimize plan %s mode %s', plan.name, plan.mode)

        info = self.get('pg_status')
        r, detail = self.pg_status_permit(info)
        if r == 0:
            if plan.mode == 'upmap':
    ...
    def do_upmap(self, plan):
        ...
        num_pool = len(pools)
        for pool in pools:
            left = (max_iterations - total_did) / num_pool
        ...
        if total_did == 0:
            return -errno.EALREADY, 'Unable to find further optimization,' \
                                    'or distribution is already perfect, or there is a fault'
        return 0, ''
