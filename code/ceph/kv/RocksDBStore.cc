...
int RocksDBStore::load_rocksdb_options(bool create_if_missing, rocksdb::Options& opt)
{
  ...
  if (g_conf->rocksdb_cache_type == "binned_lru") {
    bbt_opts.block_cache = rocksdb_cache::NewBinnedLRUCache(
      ...
      g_conf->rocksdb_cache_strict_capacity_limit,
      g_conf->rocksdb_high_pri_pool_ratio);
  } else if (g_conf->rocksdb_cache_type == "lru") {
    // we need to set the index and filter with high priority
...
bool RocksDBStore::RocksDBWholeSpaceIteratorImpl::valid()
{
  ...
  if (!r) {
    if (!dbiter->status().ok()) {
       lgeneric_dout(db->cct, 20) << " iterator is invalid, status not ok: "
                                  << dbiter->status().ToString() << dendl;
    } else {
       lgeneric_dout(db->cct, 20) << " iterator is invalid, status ok " << dendl;
    }
  } else {
    lgeneric_dout(db->cct, 20) << " iterator is valid r=" << r << dendl;
  }
  return r;
}
                                                ...
                                                IteratorBounds bounds)
{
  ...
  if (cf_handle) {
    dout(20) << __func__ << " with cf, prefix " << prefix << dendl;
    return std::make_shared<CFIteratorImpl>(
      prefix,
      db->NewIterator(rocksdb::ReadOptions(), cf_handle));
  } else {
    if (bounds.lower_bound) {
      bounds.lower_bound = combine_strings(prefix, *(bounds.lower_bound));
    }
    if (bounds.upper_bound) {
      bounds.upper_bound = combine_strings(prefix, *(bounds.upper_bound));
    }
    dout(20) << __func__ << " without cf, prefix " << prefix << dendl;
