...
TEST_F(PolicyTest, Eval3) {
  ...
  auto s3allow = (s3ListMultipartUploadParts | s3ListBucket |
                 ...
                 s3GetBucketObjectLockConfiguration | s3GetObjectRetention |
                 s3GetObjectLegalHold);
...
class RefererPolicyTest : public ::testing::Test {
protected:
  intrusive_ptr<CephContext> cct;
  static const string arbitrary_tenant;
  static string referer_allow_example;
  static string referer_deny_example;
  static string referer_full_example;
public:
  RefererPolicyTest() {
    cct = new CephContext(CEPH_ENTITY_TYPE_CLIENT);
  }
...
TEST_F(RefererPolicyTest, ReferEnvironment) {
  RGWEnv rgw_env;
  RGWUserInfo user;
  RGWRados rgw_rados;
  rgw_env.set("HTTP_REFERER", "http://www.example.com");
  req_state rgw_req_state(cct.get(), &rgw_env, &user);
  Environment iam_env = rgw_build_iam_environment(&rgw_rados, &rgw_req_state);
  auto ip = iam_env.find("aws:Referer");
  ASSERT_NE(ip, iam_env.end());
  EXPECT_EQ(ip->second, "http://www.example.com");
}

TEST_F(RefererPolicyTest, ParseReferer) {
  boost::optional<Policy> p;

  ASSERT_NO_THROW(p = Policy(cct.get(), arbitrary_tenant,
           bufferlist::static_from_string(referer_full_example)));
  ASSERT_TRUE(p);

  EXPECT_EQ(p->text, referer_full_example);
  EXPECT_EQ(p->version, Version::v2012_10_17);
  EXPECT_EQ(*p->id, "S3ReferPolicyTest");
  EXPECT_FALSE(p->statements.empty());
  EXPECT_EQ(p->statements.size(), 1U);
  EXPECT_EQ(*p->statements[0].sid, "ReferAllow");
  EXPECT_FALSE(p->statements[0].princ.empty());
  EXPECT_EQ(p->statements[0].princ.size(), 1U);
  EXPECT_EQ(*p->statements[0].princ.begin(),
      Principal::wildcard());
  ...
  ASSERT_FALSE(p->statements[0].conditions.empty());
  ASSERT_EQ(p->statements[0].conditions.size(), 2U);
  EXPECT_EQ(p->statements[0].conditions[0].op, TokenID::StringLike);
  EXPECT_EQ(p->statements[0].conditions[0].key, "aws:Referer");
  ASSERT_FALSE(p->statements[0].conditions[0].vals.empty());
  EXPECT_EQ(p->statements[0].conditions[0].vals.size(), 2U);
  EXPECT_EQ(p->statements[0].conditions[0].vals[0], "http://*.example.com");
  EXPECT_EQ(p->statements[0].conditions[0].vals[1], "http://www.baidu.com");

  EXPECT_EQ(p->statements[0].conditions[1].op, TokenID::StringNotLike);
  EXPECT_EQ(p->statements[0].conditions[1].key, "aws:Referer");
  ASSERT_FALSE(p->statements[0].conditions[1].vals.empty());
  EXPECT_EQ(p->statements[0].conditions[1].vals.size(), 2U);
  EXPECT_EQ(p->statements[0].conditions[1].vals[0], "http://www.sina.com");
  EXPECT_EQ(p->statements[0].conditions[1].vals[1], "http://www.tencent.com");
}

TEST_F(RefererPolicyTest, EvalReferer) {
  auto allowp  = Policy(cct.get(), arbitrary_tenant,
      bufferlist::static_from_string(referer_allow_example));
  auto denyp  = Policy(cct.get(), arbitrary_tenant,
           bufferlist::static_from_string(referer_deny_example));
  auto fullp  = Policy(cct.get(), arbitrary_tenant,
       bufferlist::static_from_string(referer_full_example));
  Environment e;  // Without any Referer
  Environment allowedReferer, blacklistedReferer;
  allowedReferer["aws:Referer"] = "http://www.example.com";
  blacklistedReferer["aws:Referer"] = "http://www.sina.com";

  auto trueacct = FakeIdentity(
    Principal::tenant("ACCOUNT-ID-WITHOUT-HYPHENS"));
  // Without an Referer in the environment then evaluation will always pass
  EXPECT_EQ(allowp.eval(e, trueacct, s3ListBucket,
      ARN(Partition::aws, Service::s3,
          "", arbitrary_tenant, "example_bucket")),
      Effect::Pass);
  ...
  EXPECT_EQ(allowp.eval(allowedReferer, trueacct, s3ListBucket,
      ARN(Partition::aws, Service::s3,
          "", arbitrary_tenant, "example_bucket")),
      Effect::Allow);


  EXPECT_EQ(denyp.eval(allowedReferer, trueacct, s3ListBucket,
           ARN(Partition::aws, Service::s3,
         "", arbitrary_tenant, "example_bucket")),
      Effect::Deny);
  EXPECT_EQ(denyp.eval(allowedReferer, trueacct, s3ListBucket,
  ...
  EXPECT_EQ(denyp.eval(blacklistedReferer, trueacct, s3ListBucket,
           ARN(Partition::aws, Service::s3,
         "", arbitrary_tenant, "example_bucket")),
      Effect::Pass);
  EXPECT_EQ(denyp.eval(blacklistedReferer, trueacct, s3ListBucket,
  ...
  EXPECT_EQ(fullp.eval(allowedReferer, trueacct, s3ListBucket,
           ARN(Partition::aws, Service::s3,
         "", arbitrary_tenant, "example_bucket")),
      Effect::Allow);
  EXPECT_EQ(fullp.eval(allowedReferer, trueacct, s3ListBucket,
  ...
  EXPECT_EQ(fullp.eval(blacklistedReferer, trueacct, s3ListBucket,
           ARN(Partition::aws, Service::s3,
         "", arbitrary_tenant, "example_bucket")),
      Effect::Pass);
  EXPECT_EQ(fullp.eval(blacklistedReferer, trueacct, s3ListBucket,
...
const string RefererPolicyTest::arbitrary_tenant = "arbitrary_tenant";
string RefererPolicyTest::referer_allow_example = R"(
{
  "Version": "2012-10-17",
  "Id": "S3SimpleReferPolicyTest",
  "Statement": [{
    "Sid": "1",
    "Effect": "Allow",
    "Principal": {"AWS": ["arn:aws:iam::ACCOUNT-ID-WITHOUT-HYPHENS:root"]},
...
string RefererPolicyTest::referer_deny_example = R"(
{
  "Version": "2012-10-17",
  "Id": "S3ReferPolicyTest",
  "Statement": {
    "Effect": "Deny",
    "Sid": "ReferDeny",
    "Action": "s3:ListBucket",
    ...
    "Condition": {
      "StringNotLike": {"aws:Referer": ["http://www.sina.com", "http://www.tencent.com"]}
    }
  }
}
)";

string RefererPolicyTest::referer_full_example = R"(
{
  "Version": "2012-10-17",
  "Id": "S3ReferPolicyTest",
  "Statement": {
    ...
    "Condition": {
      "StringLike": {"aws:Referer": ["http://*.example.com", "http://www.baidu.com"]},
      "StringNotLike": {"aws:Referer": ["http://www.sina.com", "http://www.tencent.com"]}
    }
  }
}
