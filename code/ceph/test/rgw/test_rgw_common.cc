...
void test_rgw_add_placement(RGWZoneGroup *zonegroup, RGWZoneParams *zone_params, const std::string& name, bool is_default)
{
  ...
  pinfo.storage_classes.set_storage_class(RGWStorageClass::STANDARD, &data_pool, nullptr);
  pinfo.data_extra_pool = rgw_pool(name + ".extra").to_str();
  
  if (is_default) {
    zonegroup->default_placement = rgw_placement_rule(name, RGWStorageClass::STANDARD);
  }
}
