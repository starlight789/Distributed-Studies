...
struct Dencoder {
  ...
  virtual int decode_json(JSONObj *obj) {
    cerr << "decode json not supported" << std::endl;
    return 0;
  };
...
template<class T>
class DencoderImplWithDecodeJson : public DencoderImplNoFeature<T> {
public:
  DencoderImplWithDecodeJson(bool stray_ok, bool nondeterministic)
    : DencoderImplNoFeature<T>(stray_ok, nondeterministic) {}
  void encode(bufferlist& out, uint64_t features) override {
    out.clear();
    using ceph::encode;
    encode(*(this->m_object), out);
  }
  int decode_json(JSONObj *obj) override {
    try {
    this->m_object->decode_json(obj);
...
#define TYPE_WITH_DECODEJSON(t) dencoders[T_STRINGIFY(t)] = new DencoderImplWithDecodeJson<t>(false, false);
...
#undef T_STRINGIFY
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ++i) {
    ...
    } else if (*i == string("jsonparse")) {
      if (!den) {
        cerr << "must first select type with 'type <name>'" << std::endl;
        exit(1);
      }
      JSONParser parser;
      bool r = parser.parse(encbl.c_str(), encbl.length());
      if (!r) {
        cerr << "json parse error, content:" << encbl.c_str() << std::endl;
        exit(1);
      }
      int ret = den->decode_json(&parser);
      if (0 != ret) {
        cerr << "decode json error with content:"<< encbl.c_str() << std::endl;
        exit(1);
      }
