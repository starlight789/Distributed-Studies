...
TEST(osd_type, split_block_device)
{
  string device_name;
  device_name = split_block_device_name("");
  ASSERT_EQ(0, device_name.compare(""));

  device_name = split_block_device_name("sda");
  ASSERT_EQ(0, device_name.compare("sda"));

  device_name = split_block_device_name("sde");
  ASSERT_EQ(0, device_name.compare("sde"));

  device_name = split_block_device_name("sde1");
  ASSERT_EQ(0, device_name.compare("sde1"));

  device_name = split_block_device_name("nvme");
  ASSERT_EQ(0, device_name.compare("nvme"));

  device_name = split_block_device_name("nvme1n1");
  ASSERT_EQ(0, device_name.compare("nvme1n1"));

  device_name = split_block_device_name("dm-0");
  ASSERT_EQ(0, device_name.compare("dm-0"));

  device_name = split_block_device_name("/dev/sda");
  std::cerr << "device is " << device_name << std::endl;
  ASSERT_EQ(0, device_name.compare("sda"));

  device_name = split_block_device_name("/dev/sde");
  ASSERT_EQ(0, device_name.compare("sde"));

  device_name = split_block_device_name("/dev/sde1");
  ASSERT_EQ(0, device_name.compare("sde1"));

  device_name = split_block_device_name("/dev/nvme");
  ASSERT_EQ(0, device_name.compare("nvme"));

  device_name = split_block_device_name("/dev/nvme1n1");
  ASSERT_EQ(0, device_name.compare("nvme1n1"));

  device_name = split_block_device_name("/dev/dm-0");
  ASSERT_EQ(0, device_name.compare("dm-0"));
}

TEST(osd_type, have_slow_disk) {
  map<string, string> mp;
  ASSERT_EQ(false, osd_metadata_have_slow_disk(mp));
  mp = {
    {"key1", "val1"},
  };
  ASSERT_EQ(false, osd_metadata_have_slow_disk(mp));
  mp = {
    ...
    {"bluefs_db_model","ST6000NM0115-1YZ"},
    {"bluefs_db_partition_path","/dev/sdf"},
    {"bluefs_db_rotational","1"},
    {"bluefs_db_size","6001175126016"},
    {"bluefs_db_type","hdd"},
    {"bluefs_single_shared_device","0"},
    {"bluestore_bdev_access_mode","blk"},
    {"bluestore_bdev_block_size","4096"},
    {"bluestore_bdev_dev"," 8:64"},
    {"bluestore_bdev_dev_node","sde"},
    {"bluestore_bdev_driver","KernelDevice"},
    {"bluestore_bdev_model","ST6000NM0115-1YZ"},
    {"bluestore_bdev_partition_path","/dev/sde"},
    {"bluestore_bdev_rotational","1"},
    {"bluestore_bdev_size","6001175126016"},
    {"bluestore_bdev_type","hdd"},
    {"slow_disk_state_1","sdd,sdc,sdb"},
  };
  ASSERT_EQ(true, osd_metadata_have_slow_disk(mp));
  mp = {
    {"bluefs", "1" },
    {"bluefs_db_access_mode","blk"},
    {"bluefs_db_block_size"," 4096"},
  ...
  mp = {
    ...
    {"bluestore_bdev_size","6001175126016"},
    {"bluestore_bdev_type","hdd"},
    {"slow_disk_state_xxx","sdb"},
  };
  ASSERT_EQ(false, osd_metadata_have_slow_disk(mp));
}

TEST(osd_type, have_drop_slow_disk) {
  map<string, string> mp;
  ASSERT_EQ(false, osd_metadata_have_slow_disk_need_drop(mp));
  mp = {
    {"key1", "val1"},
  };
  ASSERT_EQ(false, osd_metadata_have_slow_disk_need_drop(mp));
  ...
  ASSERT_EQ(true, osd_metadata_have_slow_disk_need_drop(mp));
}

TEST(osd_type, get_slow_disk) {
  map<string, string> mp;
  string devices;

  devices = osd_get_slow_disks_from_metadata(mp, OSD_SLOW_DISK_WARN_TYPE_WARN);
  ASSERT_EQ(0, devices.compare(""));
  ...
  devices = osd_get_slow_disks_from_metadata(mp, OSD_SLOW_DISK_WARN_TYPE_WARN);
  ASSERT_EQ(0, devices.compare("sdd,sdc,sdb"));
}
