...
void cls_rgw_bucket_complete_op(ObjectWriteOperation& o, RGWModifyOp op, string& tag,
                                ...
                                bool bilog_write_enable, bool bilog_delete_enable)
{
  ...
  call.log_op = log_op;
  call.bilog_flags = bilog_flags;
  call.bilog_write_enable = bilog_write_enable;
  call.bilog_delete_enable = bilog_delete_enable;
  if (remove_objs)
...
void cls_rgw_namespace_prepare_op(ObjectWriteOperation& o, RGWModifyOp op, string& tag,
                                 const string& obj_name, const string& parent_oid,
                                 const string& child_name, bool exclude)
{
  bufferlist in;
  struct rgw_cls_namespace_prepare_op call;
  call.op = op;
  call.tag = tag;
  call.obj_name = obj_name;
  call.child_name = child_name;
  call.exclude = exclude;
  encode(call, in);
  o.exec(RGW_CLASS, RGW_NAMESPACE_PREPARE_OP, in);
}

void cls_rgw_namespace_dir_prepare_op(ObjectWriteOperation& o, RGWModifyOp op, const string& optag,
                                     const string& parent_name, const string& child_name,
                                     ceph::real_time& ut, bool exclude)
{
  bufferlist in;
  struct rgw_cls_namespace_dir_prepare_op call;
  call.op = op;
  call.tag = optag;
  call.child_name = child_name;
  call.mtime = ut;
  call.exclude = exclude;
  encode(call, in);
  o.exec(RGW_CLASS, RGW_NAMESPACE_DIR_PREPARE_OP, in);
}

void cls_rgw_namespace_dir_complete_op(ObjectWriteOperation& o, RGWModifyOp op,
                                       int64_t poolid, uint64_t epoch, const string& optag,
                                       const string& child_name, ceph::real_time ut, bool force_delete)
{
  struct rgw_cls_namespace_dir_complete_op call;
  call.op = op;
  call.ver.pool = poolid;
  call.ver.epoch = epoch;
  call.tag = optag;
  call.child_name = child_name;
  call.mtime = ut;
  call.force_delete = force_delete;

  bufferlist in;
  encode(call, in);
  o.exec(RGW_CLASS, RGW_NAMESPACE_DIR_COMPLETE_OP, in);
}

void cls_rgw_namespace_dir_make(ObjectWriteOperation& o, bool need_complete) {
  struct rgw_cls_namespace_dir_make_op call;

  call.need_complete = need_complete;

  bufferlist in;
  encode(call, in);

  o.exec(RGW_CLASS, RGW_NAMESPACE_DIR_MAKE_OP, in);
}

int cls_rgw_namespace_dir_del(librados::IoCtx& io_ctx, const string& dir_oid) {
  struct rgw_cls_namespace_dir_del_op call;

  bufferlist in, out;
  encode(call, in);

  return io_ctx.exec(dir_oid, RGW_CLASS, RGW_NAMESPACE_DIR_DEL_OP, in, out);
}

int cls_rgw_namespace_list(librados::IoCtx& io_ctx, const string dir_oid, const string& marker,
                               const string& end_marker, uint32_t max,
                               rgw_cls_namespace_list_ret& list_results) {

  struct rgw_cls_namespace_dir_list_op call;
  call.marker = marker;
  call.end_marker = end_marker; // not use
  call.num_entries = max;

  bufferlist in, out;
  encode(call, in);

  int ret = io_ctx.exec(dir_oid, RGW_CLASS, RGW_NAMESPACE_LIST_OP, in, out);
  if (ret < 0) {
    return ret;
  }
  ...
  try {
    decode(list_results.dirs, iter);
    decode(list_results.is_truncated, iter);
  } catch (buffer::error& err) {
    return -EIO;
  }
...
int cls_rgw_namespace_head(librados::IoCtx& io_ctx, const string& dir_oid, const string& child_name,
                           bufferlist& out)
{
  struct rgw_cls_namespace_head_op call;
  call.child_name = child_name;

  bufferlist in;
  encode(call, in);

  return io_ctx.exec(dir_oid, RGW_CLASS, RGW_NAMESPACE_HEAD_OP, in, out);
}

void cls_rgw_namespace_complete_op(ObjectWriteOperation& o, RGWModifyOp op,
                                  const string& file_name, int64_t poolid,
                                  uint64_t epoch, const string& tag,
                                  uint64_t size, ceph::real_time* ut, bool force_delete, bool appendable)

{
  bufferlist in;
  struct rgw_cls_namespace_complete_op call;
  call.op = op;
  call.tag = tag;
  call.child_name = file_name;
  call.ver.pool = poolid;
  call.ver.epoch = epoch;
  call.size = size;
  call.appendable = appendable;
  if (ut != nullptr) {
    call.mtime = *ut;
  }
  call.force_delete = force_delete;
  encode(call, in);
  o.exec(RGW_CLASS, RGW_NAMESPACE_COMPLETE_OP, in);
}
...
static bool issue_bucket_list_op(librados::IoCtx& io_ctx,
    ...
    uint32_t num_entries, bool list_versions, bool skip_ns, BucketIndexAioManager *manager,
    struct rgw_cls_list_ret *pdata) {
  librados::ObjectReadOperation op;
  cls_rgw_bucket_list_op(op, start_obj, filter_prefix,
                         num_entries, list_versions, skip_ns, pdata);
  return manager->aio_operate(io_ctx, oid, &op);
}

int CLSRGWIssueBucketList::issue_op(int shard_id, const string& oid)
{
  return issue_bucket_list_op(io_ctx, oid, start_obj, filter_prefix, num_entries,
                              list_versions, skip_ns, &manager, &result[shard_id]);
}
...
void cls_rgw_namespace_suggest_changes(ObjectWriteOperation& o, bufferlist& updates)
{
  o.exec(RGW_CLASS, RGW_NAMESPACE_SUGGEST_CHANGES, updates);
}

int CLSRGWIssueGetDirHeader::issue_op(int shard_id, const string& oid)
{
  cls_rgw_obj_key nokey;
  return issue_bucket_list_op(io_ctx, oid, nokey, "", 0, false, false, &manager, &result[shard_id]);
}
...
int cls_rgw_usage_log_read_current_readop(IoCtx& io_ctx, string& oid, string& user,
                             uint64_t end_epoch, map<rgw_user_bucket, rgw_usage_log_entry>& usage)
{
  bufferlist in, out;
  rgw_cls_usage_log_read_op call;
  call.end_epoch = end_epoch;
  call.owner = user;
  encode(call, in);
  int r = io_ctx.exec(oid, RGW_CLASS, RGW_USER_USAGE_LOG_READ_CURRENT_READOP, in, out);
  if (r < 0)
    return r;
...
int cls_rgw_usage_log_read_readop(IoCtx& io_ctx, string& oid, string& user, uint64_t start_epoch,
                               uint64_t end_epoch, uint32_t max_entries, string& read_iter,
                               map<rgw_user_bucket, vector<rgw_usage_log_entry> >& usage, bool *is_truncated)
{
  if (is_truncated)
    *is_truncated = false;
  ...
  call.iter = read_iter;
  encode(call, in);
  int r = io_ctx.exec(oid, RGW_CLASS, RGW_USER_USAGE_LOG_READ_READOP, in, out);
  if (r < 0)
    return r;

  try {
    rgw_cls_usage_log_read_readop_ret result;
    bufferlist::iterator iter = out.begin();
    decode(result, iter);
    read_iter = result.next_iter;
    if (is_truncated)
...
int cls_rgw_usage_log_trim_total(IoCtx& io_ctx, const string& oid, string& user, string& bucket)
{
  bufferlist in;
  rgw_cls_usage_log_trim_op call;
  call.start_epoch = 0;
  ...
  ObjectWriteOperation op;
  op.exec(RGW_CLASS, RGW_USER_USAGE_LOG_TRIM_TOTAL, in);
  io_ctx.operate(oid, &op); // ignore the return code and just return ok
  return 0;
}

int cls_rgw_usage_log_trim_readop(IoCtx& io_ctx, const string& oid, string& user,
                                  uint64_t start_epoch, uint64_t end_epoch)
{
  bufferlist in;
  rgw_cls_usage_log_trim_op call;
  ...
  do {
    ObjectWriteOperation op;
    op.exec(RGW_CLASS, RGW_USER_USAGE_LOG_TRIM_READOP, in);
    int r = io_ctx.operate(oid, &op);
    if (r == -ENODATA)
      done = true;
    else if (r < 0)
...
int cls_rgw_lc_rm_entries(IoCtx& io_ctx, string& oid, vector<cls_rgw_lc_entry>& entries)
{
  bufferlist in, out;
  cls_rgw_lc_rm_entries_op call;
  call.entries = &entries;
  encode(call, in);
  int r = io_ctx.exec(oid, RGW_CLASS, RGW_LC_RM_ENTRIES, in, out);
  return r;
}

int cls_rgw_lc_set_entries(IoCtx& io_ctx, string& oid, vector<cls_rgw_lc_entry>& entries)
{
  bufferlist in, out;
  cls_rgw_lc_set_entries_op call;
  call.entries = &entries;
  encode(call, in);
  int r = io_ctx.exec(oid, RGW_CLASS, RGW_LC_SET_ENTRIES, in, out);
  return r;
}

int cls_rgw_lc_get_entry(IoCtx& io_ctx, const string& oid,
                         std::string& marker, cls_rgw_lc_entry& entry)
{
  bufferlist in, out;
