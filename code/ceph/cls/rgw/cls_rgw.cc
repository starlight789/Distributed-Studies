...
const string RGW_NAMESPACE_DIR_COMPLETE_ATTR = "completed";
const char OBJ_NAMESPACE_DELIMITER = '_'; // ASCII 95
...
static inline int get_ns_of_list_index_key(const string& key, string& ns) {
  // for namespace, min size would be 3: _x_
  if (key[0] != OBJ_NAMESPACE_DELIMITER || key.size() < 3 || key[1] == OBJ_NAMESPACE_DELIMITER) {
    return -EINVAL;
  }
  size_t pos = key.find(OBJ_NAMESPACE_DELIMITER, 2); // oid must match ^_[^_].+$
  if (pos == string::npos) {
    return -EINVAL;
  }
  ns = key.substr(0, pos+1); // keep prefix '_' and suffix '_'
  return 0;
}
...
static int encode_list_index_key(cls_method_context_t hctx, const cls_rgw_obj_key& key, string *index_key)
{
  ...
  encode_obj_index_key(tmp_key, &obj_index_key);
...
int rgw_bucket_list(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (!op.filter_prefix.empty() && op.skip_ns) {
    CLS_LOG(0, "ERROR: rgw_bucket_list(): prefix '%s' is not empty when skip_ns\n",
            op.filter_prefix.c_str());
    op.skip_ns = false;
  }
  ...
  do {
    -- retry_time;
    // skip the objects with namepsace, but only retry retry_time times,
    // in order to prevent holding pg_lock too long
    if (op.skip_ns && !last_ns.empty()) {
      if (last_ns > start_key) {
        start_key.swap(last_ns);
      }
      last_ns.clear();
    }

    rc = get_obj_vals(hctx, start_key, op.filter_prefix, left_to_read, &keys, &more);
    CLS_LOG(20, "rgw_bucket_list(): get keys.size()=%zu\n", keys.size());
    if (rc < 0)
      return rc;
    ...
    while (kiter != keys.end()) {
      ...
      // try to skip the object with namespace
      if (op.skip_ns && retry_time > 0) {
        if (get_ns_of_list_index_key(kiter->first, last_ns) == 0) {
          CLS_LOG(20, "rgw_bucket_list(): get ns %s from key=%s\n", last_ns.c_str(), kiter->first.c_str());
          start_key = kiter->first;
          cls_rgw_append_max_charater(last_ns);
          kiter = keys.lower_bound(last_ns);
          continue;
        } else {
          CLS_LOG(25, "rgw_bucket_list(): clear ns %s when get key=%s\n", last_ns.c_str(), kiter->first.c_str());
          last_ns.clear();
        }
      }
...
#define CHECK_CHUNK_SIZE 1000
  ...
  do {
    ...
    for (; kiter != keys.end(); ++kiter) {
      ...
      stats.num_entries++;
      if (stats.total_size >> 63 != 0 ||
          entry.meta.accounted_size >> 63 != 0) {
        CLS_LOG(0, "ERROR: %s(): bucket stats got wrong number, "
                   "stats.total_size:%lu, entry.accounted_size:%lu\n",
                    __func__, stats.total_size, entry.meta.accounted_size);
      }
...
static void unaccount_entry(struct rgw_bucket_dir_header& header, struct rgw_bucket_dir_entry& entry)
{
  ...
  struct rgw_bucket_category_stats& storageclass_stats = header.storageclass_stats[entry.meta.storage_class];
  if ((entry.meta.accounted_entry && stats.num_entries == 0) ||
      stats.num_entries >> 63 != 0 ||
      stats.total_size < entry.meta.accounted_size ||
      stats.total_size >> 63 != 0 ||
      entry.meta.accounted_size >> 63 != 0) {
    CLS_LOG(0, "ERROR: %s(): bucket stats got wrong number, stats.num_entries:%lu,"
               "stats.total_size:%lu, entry.accounted_size:%lu\n",
               __func__, stats.num_entries,
               stats.total_size, entry.meta.accounted_size);
  }
  if (entry.meta.accounted_entry) {
    stats.num_entries--;
    if (entry.meta.category == RGW_OBJ_CATEGORY_MAIN) {
      storageclass_stats.num_entries--;
    }
  }
  stats.total_size -= entry.meta.accounted_size;
  stats.total_size_rounded -= cls_rgw_get_rounded_size(entry.meta.accounted_size);
  ...
  if (entry.meta.category == RGW_OBJ_CATEGORY_MAIN) {
    storageclass_stats.total_size -= entry.meta.accounted_size;
    storageclass_stats.total_size_rounded -= cls_rgw_get_rounded_size(entry.meta.accounted_size);
    storageclass_stats.actual_size -= entry.meta.size;
  }
}

static void log_entry(const char *func, const char *str, const string& key,
                      struct rgw_bucket_dir_entry *entry)
{
...
static void log_entry(const char *func, const char *str, const string& key,
                      struct rgw_bucket_namespace_file *entry)
{
  CLS_LOG(1, "%s(): %s: epoch=%llu file=%s exists=%d magic=0x%016lx\n", func, str,
          (unsigned long long)entry->ver.epoch, key.c_str(), entry->exist(), entry->magic);
}

static void log_entry(const char *func, const char *str, const string& key,
                      struct rgw_bucket_namespace_dir *entry)
{
  CLS_LOG(1, "%s(): %s: dir entry %s magic=0x%016lx\n", func, str, key.c_str(), entry->magic);
}
                           ...
                           T* entry)
{
  ...
  auto cur_iter = current_entry.begin();
  try {
...
static int read_index_entry(cls_method_context_t hctx, string& name, T *entry)
{
  ...
  try {
    ...
    CLS_LOG(1, "ERROR: read_index_entry(): failed to decode entry of %s\n", name.c_str());
    return -EIO;
  }

  log_entry(__func__, "existing entry", name, entry);
  return 0;
}
...
int rgw_bucket_complete_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  /* resetting entry flags, entry might have been previously a delete marker */
  ...
  case CLS_RGW_OP_ADD:
    {
      ...
      struct rgw_bucket_category_stats& storageclass_stats = header.storageclass_stats[meta.storage_class];
      if (stats.total_size >> 63 != 0 ||
          meta.accounted_size >> 63 != 0) {
        CLS_LOG(0, "ERROR: %s(): bucket stats got wrong number, "
                 "stats.total_size:%lu, meta.accounted_size:%lu\n",
                 __func__, stats.total_size, entry.meta.accounted_size);
      }
      entry.meta = meta;
      entry.key = op.key;
      ...
      if (entry.meta.accounted_entry) {
        stats.num_entries++;
        if (meta.category == RGW_OBJ_CATEGORY_MAIN) {
          storageclass_stats.num_entries++;
        }
      }
      ...
      //only objects belong to main category need to be counted
      if (meta.category == RGW_OBJ_CATEGORY_MAIN) {
        storageclass_stats.total_size += meta.accounted_size;
        storageclass_stats.total_size_rounded += cls_rgw_get_rounded_size(meta.accounted_size);
        storageclass_stats.actual_size += meta.size;
      }
      bufferlist new_key_bl;
      encode(entry, new_key_bl);
      int ret = cls_cxx_map_set_val(hctx, idx, &new_key_bl);
      if (ret < 0)
        return ret;
      if (entry.versioned_epoch != 0) {
        bufferlist temp_key_bl;
        string instance_list_idx;
        struct rgw_bucket_dir_entry plain_entry;
        get_list_index_key(entry, &instance_list_idx);

        ret = read_index_entry(hctx, instance_list_idx, &plain_entry);
        if (ret < 0) {
          CLS_LOG(0, "rgw_bucket_complete_op(): read plain entry error %d\n", ret);
          if (ret != -ENOENT) {
            return ret;
          }
        }

        uint16_t origin_flags = entry.flags;
        entry.flags = plain_entry.flags;
        encode(entry, temp_key_bl);

        entry.flags = origin_flags;

        ret = cls_cxx_map_set_val(hctx, instance_list_idx, &temp_key_bl);
        if (ret < 0)
          return ret;
      }
    }
  ...
  if (op.log_op && !header.syncstopped && op.bilog_write_enable) {
  ...
  for (remove_iter = op.remove_objs.begin(); remove_iter != op.remove_objs.end(); ++remove_iter) {
    ...
    CLS_LOG(1, "rgw_bucket_complete_op(): entry.name=%s entry.instance=%s entry.meta.category=%d\n",
            remove_entry.key.name.c_str(), remove_entry.key.instance.c_str(), remove_entry.meta.category);
    if (remove_entry.exists) {
    ...
    if (ret < 0) {
      CLS_LOG(1, "rgw_bucket_complete_op(): cls_cxx_map_remove_key, failed to remove entry, "
...
int rgw_namespace_set_dir_complete(cls_method_context_t hctx)
{
  bufferlist in;
  encode(true, in);
  return cls_cxx_setxattr(hctx, RGW_NAMESPACE_DIR_COMPLETE_ATTR.c_str(), &in);
}

int rgw_namespace_check_dir_complete(cls_method_context_t hctx, bool& is_completed)
{
  if (!cls_check_oid_exist(hctx)) {
    is_completed = false;
    return 0;
  }

  bufferlist out;
  int r =  cls_cxx_getxattr(hctx, RGW_NAMESPACE_DIR_COMPLETE_ATTR.c_str(), &out);
  if (r == -ENODATA) {
    is_completed = false;
    return 0;
  } else if (r<0) {
  ...
  try {
    ...
    decode(is_completed, it);
  } catch (const buffer::error &err) {
    CLS_LOG(0, "ERROR: rgw_namespace_check_dir_complete() failed to decode\n");
    return -EIO;
  }
...
template <class T>
static int read_namespace_entry(cls_method_context_t hctx, const string& name, T& entry)
{
  bufferlist current_entry;
  int rc = cls_cxx_map_get_val(hctx, name, &current_entry);
  if (rc == -ENOENT) {
    ...
    CLS_LOG(0, "ERROR: read_namespace_entry() failed read child %s ret=%d\n",
            name.c_str(), rc);
    return rc;
  }

  // decode entry and check whether it is dir
  bufferlist::iterator p = current_entry.begin();
  try {
    decode(entry.magic, p);
  } catch (buffer::error &e) {
    CLS_LOG(0, "ERROR: read_namespace_entry() failed to decode magic of entry\n");
    return -EIO;
  }

  // must be expected_type
  if (!entry.is_corrent_type()) {
    CLS_LOG(0, "ERROR: read_namespace_entry() not expected type\n");
    return -EEXIST;
  }
  ...
  try {
    ...
    CLS_LOG(0, "ERROR: read_namespace_entry() failed to decode entry of %s\n",
            name.c_str());
    return -EIO;
  }
...
int rgw_namespace_remove_dir(cls_method_context_t hctx, const string* e_key)
{
  // check whether this dir are empty
  constexpr uint MAX_CHECK_NUM = 2u;
  set<string> keys;
  bool more = false;

  int r = cls_cxx_map_get_keys(hctx, "", MAX_CHECK_NUM, &keys, &more);
  if (r < 0) {
    CLS_LOG(0, "ERROR: rgw_namespace_remove_dir() failed to list ret=%d\n", r);
    return r;
  }

  // this oid probably only have one omap and the key of this omap is e_key
  if ((r >= 2) || (r == 1 && (e_key == nullptr || keys.count(*e_key) == 0))) {
    return -ENOTEMPTY;
  }
  ...
  if (r < 0) {
    CLS_LOG(0, "ERROR: rgw_namespace_remove_dir() failed to delete dir ret=%d\n", r);
  }
  return r;
}

int rgw_namespace_prepare_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  rgw_cls_namespace_prepare_op op;
  bufferlist::iterator iter = in->begin();
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_prepare_op() failed to decode request\n");
    return -EINVAL;
  }

  if (op.tag.empty()) {
    CLS_LOG(0, "ERROR: tag is empty\n");
    return -EINVAL;
  }

  CLS_LOG(20, "rgw_namespace_prepare_op(): request: op=%d obj_name=%s file_name=%s \
          tag=%s\n", op.op, op.obj_name.c_str(), op.child_name.c_str(), op.tag.c_str());

  struct rgw_bucket_namespace_file entry;
  bool noent = true;
  bool is_completed = false;
  string& key = op.child_name;

  // check whether parent exist
  bool is_exist= cls_check_oid_exist(hctx);

  // get namespace omap
  if (is_exist) {
    int r = rgw_namespace_check_dir_complete(hctx, is_completed);
    if (r < 0) {
      CLS_LOG(0, "ERROR: rgw_namespace_prepare_op failed to check whether dir is \
                 completed ret=%d\n", r);
      return r;
    }

    int rc = read_namespace_entry(hctx, key, entry);
    if (rc == -ENOENT) {
      noent = true;
    } else if (rc < 0) {
      CLS_LOG(0, "ERROR: rgw_namespace_prepare_op failed to read entry %s ret=%d\n",
              key.c_str(), rc);
      return rc;
    } else {
      noent = false;
  ...
  bufferlist info_bl;
  encode(entry, info_bl);
  int rc = cls_cxx_map_set_val(hctx, key, &info_bl);
  if (rc < 0) {
    CLS_LOG(0, "ERROR: rgw_namespace_prepare_op: failed insert child %s ret=%d\n",
            key.c_str(), rc);
    return rc;
  }
...
int rgw_namespace_complete_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  bool is_completed = false;
  rgw_cls_namespace_complete_op op;
  bufferlist::iterator iter = in->begin();
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_complete_op: failed to decode request\n");
    return -EINVAL;
  }

  CLS_LOG(20, "rgw_namespace_complete_op(): request: op=%d file_name=%s ver=%lu:%llu \
          tag=%s\n", op.op, op.child_name.c_str(), (unsigned long)op.ver.pool,
          (unsigned long long)op.ver.epoch, op.tag.c_str());

  // set dir is completed
  int r = rgw_namespace_check_dir_complete(hctx, is_completed);
  if (r < 0) {
     CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed to check whether dir is \
                completed ret=%d\n", r);
    return r;
  }
  ...
  if (rc == -ENOENT) {
    ...
    CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed read entry %s ret=%d\n",
            key.c_str(), rc);
    return rc;
  }
  ...
  if (op.tag.size()) {
    ...
    if (pinter == entry.pending_map.end()) {
      CLS_LOG(0, "ERROR: couldn't find tag for pending operation\n");
      return -EINVAL;
    }
    entry.pending_map.erase(pinter);
  ...
  if ((op.tag.size() && op.op == CLS_RGW_OP_CANCEL) ||
      (op.ver.pool == entry.ver.pool && op.ver.epoch && op.ver.epoch <= entry.ver.epoch)) {
    cancel = true;
  }

  if (cancel) {
    if (op.tag.size()) {
      if (op.force_delete && ondisk && entry.pending_map.empty() && !entry.exist()) {
        int ret = cls_cxx_map_remove_key(hctx, key);
        if (ret < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed remove key %s when cancel \
                     ret=%d\n", key.c_str(), ret);
          return ret;
        }
        // try to delete this oid (the child of this oid must be empty)
        if (!is_completed) {
          ret = rgw_namespace_remove_dir(hctx, &key);
          if (ret < 0 && ret != -ENOTEMPTY) {
            CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed to remove this oid %s \
                       when cancel ret=%d\n", key.c_str(), ret);
            return ret;
          }
        }
        ...
        bufferlist new_key_bl;
        encode(entry, new_key_bl);
        rc = cls_cxx_map_set_val(hctx, key, &new_key_bl);
        if (rc < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed update key %s when cancel \
                     ret=%d\n", key.c_str(), rc);
          return rc;
        }
      }
  ...
  case CLS_RGW_OP_DEL:
    if (ondisk) {
      if (entry.pending_map.empty()) {
        int ret = cls_cxx_map_remove_key(hctx, key);
        if (ret < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed to remove file %s ret=%d\n",
                  key.c_str(), ret);
          return ret;
        }
      } else {
        ...
        if (ret < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed to update file %s ret=%d\n",
                  key.c_str(), ret);
          return ret;
        }
      }
  ...
  case CLS_RGW_OP_ADD:
    {
      // set this oid is completed
      // NOTICE: we should make sure the parent directory of this oid is completed
      if (!is_completed) {
          r = rgw_namespace_set_dir_complete(hctx);
          if (r < 0) {
            CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed to set dir completed \
                       ret=%d\n", r);
            return r;
          }
      }
      ...
      entry.size = op.size;
      entry.set_mtime(op.mtime);
      entry.set_exist();
      entry.set_appendable(op.appendable);
      bufferlist new_key_bl;
      encode(entry, new_key_bl);

      CLS_LOG(20, "rgw_namespace_complete_op() %s magic=0x%016lx\n", key.c_str(),
              entry.magic);

      int ret = cls_cxx_map_set_val(hctx, key, &new_key_bl);
      if (ret < 0) {
        CLS_LOG(0, "ERROR: rgw_namespace_complete_op failed to add file %s ret=%d\n",
                key.c_str(), ret);
        return ret;
      }
    }
...
int rgw_namespace_dir_prepare_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  rgw_cls_namespace_dir_prepare_op op;
  bufferlist::iterator iter = in->begin();
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_dir_prepare_op(): failed to decode request\n");
    return -EINVAL;
  }
  ...
  // get namespace omap
  if (is_exist) {
    // check whether parent complete
    int r = rgw_namespace_check_dir_complete(hctx, is_completed);
    if (r < 0) {
      CLS_LOG(0, "ERROR: rgw_namespace_dir_prepare_op failed to chekc dir complete \
                 ret=%d\n", r);
      return r;
    }
    ...
    if (rc == -ENOENT) {
      ...
      CLS_LOG(0, "ERROR: rgw_namespace_dir_prepare_op failed to read entry %s \
                 ret=%d\n", key.c_str(), rc);
      return rc;
    } else {
      noent = false;
  ...
  CLS_LOG(20, "rgw_namespace_prepare_op(): exclude %d is_completed %d\n", op.exclude,
          is_completed);
  if (op.exclude && !is_completed) {
    return -ENOENT;
  }
  ...
  if (noent) { // no entry, initialize fields
    if (op.op == CLS_RGW_OP_DEL) {
      // if this entry not exist and op is Delete, return -ENOENT
      return -ENOENT;
    } else {
      entry.ver = rgw_bucket_entry_ver();
    }
  ...
  bufferlist info_bl;
  encode(entry, info_bl);
  CLS_LOG(20, "rgw_namespace_dir_prepare_op() %s magic=0x%016lx\n", op.child_name.c_str(),
          entry.magic);

  int r = cls_cxx_map_set_val(hctx, key, &info_bl);
  if (r < 0) {
    CLS_LOG(0, "ERROR: rgw_namespace_dir_prepare_op failed to prepare entry %s \
               ret=%d\n", key.c_str(), r);
    return r;
  }
...
int rgw_namespace_dir_complete_op(cls_method_context_t hctx, bufferlist *in,
                                  bufferlist *out)
{
  // decode request
  rgw_cls_namespace_dir_complete_op op;
  bufferlist::iterator iter = in->begin();
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op(): failed to decode request\n");
    return -EINVAL;
  }

  CLS_LOG(20, "rgw_namespace_dir_complete_op(): request: name=%s\n", op.child_name.c_str());

  // check whether parent complete
  bool is_completed = false;
  int r = rgw_namespace_check_dir_complete(hctx, is_completed);
  if (r < 0) {
    CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to check whether dir is \
               completed ret=%d\n", r);
    return r;
  }
  ...
  if (rc == -ENOENT) {
    ...
    CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to read entry %s ret=%d\n",
            key.c_str(), rc);
    return rc;
  }
  ...
  if (op.tag.size()) {
    ...
    if (pinter == entry.pending_map.end()) {
      CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op couldn't find tag for \
                 pending operation\n");
      return -EINVAL;
    }
    entry.pending_map.erase(pinter);
  ...
  if (cancel) {
    if (op.tag.size()) {
      if (op.force_delete && ondisk && entry.pending_map.empty() && entry.not_exist()) {
        int ret = cls_cxx_map_remove_key(hctx, key);
        if (ret < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to remove key %s when \
                     cancel ret=%d\n", key.c_str(), ret);
          return ret;
        }
        // try to delete this oid (the child of this oid must be empty)
        if (!is_completed) {
          ret = rgw_namespace_remove_dir(hctx, &key);
          if (ret < 0 && ret != -ENOTEMPTY) {
            CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to remove this oid %s \
                       when cancel ret=%d\n", key.c_str(), ret);
            return ret;
          }
        }
        ...
        rc = cls_cxx_map_set_val(hctx, key, &new_key_bl);
        if (rc < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to update key %s when \
                     cancel ret=%d\n", key.c_str(), rc);
          return rc;
        }
      }
  ...
  case CLS_RGW_OP_DEL:
    if (ondisk) {
      if (entry.pending_map.empty()) {
        ...
        int ret = cls_cxx_map_set_val(hctx, key, &new_key_bl);
        if (ret < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to update key %s when DEL "
                 "ret=%d\n", key.c_str(), ret);
          return ret;
        }
      }
  ...
  case CLS_RGW_OP_ADD:
    {
      ...
      if (!is_completed) {
        r = rgw_namespace_set_dir_complete(hctx);
        if (r < 0) {
          CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed complete ret=%d", r);
          return r;
        }
      }
      ...
      int ret = cls_cxx_map_set_val(hctx, key, &new_key_bl);
      if (ret < 0) {
        CLS_LOG(0, "ERROR: rgw_namespace_dir_complete_op failed to add key %s when ADD "
                "ret=%d\n", key.c_str(), ret);
        return ret;
      }
    }
...
int rgw_namespace_dir_make_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  rgw_cls_namespace_dir_make_op op;
  bufferlist::iterator iter = in->begin();
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_dir_make_op(): failed to decode request\n");
    return -EINVAL;
  }
  ...
  if (is_exist) {
    int r = rgw_namespace_check_dir_complete(hctx, is_completed);
    if (r < 0) {
      CLS_LOG(0, "ERROR: rgw_namespace_dir_make_op failed to chekc dir complete ret=%d\n", r);
      return r;
    }
    if (is_completed) {
      return -EEXIST;
  ...
  if (op.need_complete) {
    int r = rgw_namespace_set_dir_complete(hctx);
    if (r < 0) {
      CLS_LOG(0, "ERROR: rgw_namespace_dir_make_op failed complete ret=%d", r);
      return r;
    }
  }
...
int rgw_namespace_dir_del_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  // decode request
  rgw_cls_namespace_dir_del_op op;
  bufferlist::iterator iter = in->begin();
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_dir_del_op(): failed to decode request\n");
    return -EINVAL;
  }

  return rgw_namespace_remove_dir(hctx, nullptr);
}

int rgw_namespace_list_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  bufferlist::iterator iter = in->begin();
  struct rgw_cls_namespace_dir_list_op op;
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: cls_namespace_dir_list_op(): failed to decode request\n");
    return -EINVAL;
  }

  int r = cls_cxx_map_get_vals_v1(hctx, op.marker, "", op.num_entries, out);
  if (r < 0) {
    CLS_LOG(1, "WARNING: cls_namespace_dir_list_op got ret=%d\n", r);
    return r;
  }
  return 0;
}

int rgw_namespace_head_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  bufferlist::iterator iter = in->begin();

  struct rgw_cls_namespace_head_op op;
  try {
    decode(op, iter);
  } catch (buffer::error& err) {
    CLS_LOG(0, "ERROR: rgw_namespace_head_op(): failed to decode request\n");
    return -EINVAL;
  }

  CLS_LOG(20, "rgw_namespace_head_op(): child name %s\n", op.child_name.c_str());

  bufferlist current_entry;
  return cls_cxx_map_get_val(hctx, op.child_name, out);
}

template <class T>
int rgw_namespace_suggest_changes_node(cls_method_context_t hctx, timespan& tag_timeout,
                                       __u8 op, const string& child_name, T& cur_change,
                                       T& cur_disk)
{
  int ret = read_namespace_entry(hctx, child_name, cur_disk);
  if (ret == -ENOENT) {
    return -ENOENT;
  } else if (ret < 0) {
    return -EINVAL;
  ...
  // erase pending log which is out of date
  if (!cur_disk.pending_map.empty()) {
    real_time cur_time = real_clock::now();
    map<string, struct rgw_bucket_pending_info>::iterator iter = cur_disk.pending_map.begin();
    ...
    if (cur_disk.pending_map.empty()) {
      cur_disk.clear_pending();
    }
  }

  CLS_LOG(20, "rgw_namespace_suggest_changes_node(): cur_disk.pending_map.empty()=%d \
          op=%d cur_disk.exists=%d cur_change. pending_map.size()=%d \
          cur_change.exists=%d\n", cur_disk.pending_map.empty(),
          (int)op, cur_disk.exist(), (int)cur_change.pending_map.size(), cur_change.exist());

  if (cur_disk.pending_map.empty()) {
    // skip old version
    if (cur_change.ver.pool == cur_disk.ver.pool &&
        cur_change.ver.epoch < cur_disk.ver.epoch) {
      CLS_LOG(20, "rgw_namespace_suggest_changes_node() skip with a smaller version %llu \
              compare to %llu \n", (unsigned long long) cur_change.ver.epoch,
              (unsigned long long) cur_disk.ver.epoch);
      return 0;
    }

    switch (op) {
    case CEPH_RGW_REMOVE:
      ret = cls_cxx_map_remove_key(hctx, child_name);
      if (ret < 0) {
        return ret;
      }
      break;
    case CEPH_RGW_UPDATE:
      ...
      ret = cls_cxx_map_set_val(hctx, child_name, &cur_state_bl);
      if (ret < 0) {
        return ret;
      }
      break;
...
int rgw_namespace_suggest_changes_op(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  timespan tag_timeout(std::chrono::seconds(CEPH_RGW_TAG_TIMEOUT));
  ...
  while (!in_iter.end()) {
    ...
    try {
      decode(op, in_iter);
      decode(child_name, in_iter);
      decode(magic, in_iter);
    } catch (buffer::error& err) {
      CLS_LOG(0, "ERROR: rgw_namespace_suggest_changes_op(): failed to decode \
                 request\n");
      return -EINVAL;
    }

    CLS_LOG(20, "rgw_namespace_suggest_changes_op(): child name %s magic=0x%016lx",
            child_name.c_str(), magic);

    if (namespace_entry_is_file(magic)) {
      rgw_bucket_namespace_file cur_change(magic);
      rgw_bucket_namespace_file cur_disk;
      try {
      ...
      int r= rgw_namespace_suggest_changes_node(hctx, tag_timeout, op, child_name,
                                                cur_change, cur_disk);
      if (r == -ENOENT) {
        continue;
      } else if (r < 0) {
      ...
      rgw_bucket_namespace_dir cur_change(magic);
      rgw_bucket_namespace_dir cur_disk;
      try {
        decode(cur_change, in_iter);
      } catch (buffer::error& err) {
        CLS_LOG(0, "ERROR: rgw_namespace_suggest_changes_op(): failed to decode \
...
static int rgw_bucket_link_olh(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (existed && !real_clock::is_zero(op.unmod_since)) {
    ...
    /* maybe delete op, so when unmod >= mtime (translate op_ctx.unmod_time >= obj.mtime), donot skip insert index link */
    if (mtime > unmod) {
      CLS_LOG(5, "NOTICE: mtime > unmod, so skip this link index op");
  ...
  if (!olh.start_modify(op.olh_epoch) && !(olh.get_entry().key == op.key)) {
  ...
  while (!in_iter.end()) {
    ...
    if (cur_disk.pending_map.empty()) {
      if (cur_disk.exists) {
        ...
        if (old_stats.num_entries == 0 ||
            old_stats.total_size < cur_disk.meta.accounted_size ||
            old_stats.total_size >> 63 != 0 ||
            cur_disk.meta.accounted_size >> 63 != 0) {
          CLS_LOG(0, "ERROR: %s(): bucket stats got wrong number, stats.num_entries:%lu,"
                     "stats.total_size:%lu, entry.accounted_size:%lu\n",
                     __func__, old_stats.num_entries, old_stats.total_size,
                     cur_disk.meta.accounted_size);
        }
	...
	if (ret < 0)
      ...
      case CEPH_RGW_UPDATE:
        ...
        if (stats.total_size >> 63 != 0 ||
            cur_change.meta.accounted_size >> 63 != 0) {
          CLS_LOG(0, "ERROR: %s(): bucket stats got wrong number, "
                   "stats.total_size:%lu, meta.accounted_size:%lu\n",
                   __func__, stats.total_size, cur_change.meta.accounted_size);
        }
  ...
  for (iter = keys.begin(); iter != keys.end(); ++iter) {
	...
	*pmore = false;
    ...
    try {
      ...
      CLS_LOG(0, "ERROR: %s(): failed to decode buffer key: %s", __func__, iter->first.c_str());
      return -EIO;
    }
...
#define MAX_BI_LIST_ENTRIES 1000
  ...
  if (ret < 0) {
    CLS_LOG(0, "ERROR: %s(): list_plain_entries name=%s marker=%s returned ret=%d", __func__,
            op.name.c_str(), op.marker.c_str(), ret);
    return ret;
  }
  int count = ret;
...
static void usage_record_prefix_current_readop(uint64_t epoch, string& user, string& key)
{
  char buf[64];
  snprintf(buf, sizeof(buf), "WRop_%011llu_%s", (long long unsigned)epoch, user.c_str());
  key = buf;
}

static void usage_record_prefix_readop(uint64_t epoch, string& key)
{
  char buf[64];
  snprintf(buf, sizeof(buf), "WRop_%011llu_", (long long unsigned)epoch);
  key = buf;
}
...
static void usage_record_name_readop(const string& user, uint64_t epoch, string& key)
{
  char buf[32 + user.size()];
  snprintf(buf, sizeof(buf), "WRop_%011llu_%s", (long long unsigned)epoch, user.c_str());
  key = buf;
}
...
int rgw_user_usage_log_add(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  for (iter = info.entries.begin(); iter != info.entries.end(); ++iter) {
    ...
    // Add ReadOp and WriteOp log for rgw user.
    rgw_usage_log_entry read_and_write_entry;
    read_and_write_entry.aggregate_readop(entry);

    string key_by_readop;
    usage_record_name_readop(puser->to_str(), entry.epoch, key_by_readop);
    bufferlist record_readop_bl;
    int ret = cls_cxx_map_get_val(hctx, key_by_readop, &record_readop_bl);
    if (ret < 0 && ret != -ENOENT) {
    ...
    if (ret >= 0) {
      ...
      ret = usage_record_decode(record_readop_bl, e);
      if (ret < 0) {
        return ret;
      }
      read_and_write_entry.aggregate_readop(e);
    }
    bufferlist new_readop_bl;
    encode(read_and_write_entry, new_readop_bl);
    ret = cls_cxx_map_set_val(hctx, key_by_readop, &new_readop_bl);
    if (ret < 0) {
      return ret;
    }
...
static int usage_iterate_readop_range(cls_method_context_t hctx, uint64_t start, uint64_t end,
                            string& user, string& key_iter, uint32_t max_entries, bool *truncated,
                            int (*cb)(cls_method_context_t, const string&, rgw_usage_log_entry&, void *),
                            void *param)
{
  CLS_LOG(10, "usage_iterate_readop_range");
  ...
  assert(truncated != nullptr);
  usage_record_prefix_current_readop(end, user, end_key);

  if (key_iter.empty()) {
    usage_record_prefix_current_readop(start, user, start_key);
  } else {
    start_key = key_iter;
  }

  CLS_LOG(20, "usage_iterate_readop_range start_key=%s", start_key.c_str());
  int ret = cls_cxx_map_get_vals(hctx, start_key, filter_prefix, max_entries, &keys, &truncated_status);
  if (ret < 0)
    return ret;
  ...
  *truncated = truncated_status;
  ...
  for (; iter != keys.end(); ++iter) {
    ...
    if (key.compare(end_key) >= 0) {
      CLS_LOG(20, "usage_iterate_readop_range reached key=%s, done", key.c_str());
      *truncated = false;
      key_iter = key;
      return 0;
    }
...
static int usage_log_read_current_readop_cb(cls_method_context_t hctx, const string& key,
                                rgw_usage_log_entry& entry, void *param)
{
  ...
  rgw_user_bucket ub(puser->to_str(), entry.bucket);
  rgw_usage_log_entry& le = (*usage)[ub];
  le.aggregate_readop(entry);

  return 0;
}

static int usage_log_read_readop_cb(cls_method_context_t hctx, const string& key,
                                rgw_usage_log_entry& entry, void *param)
{
  map<rgw_user_bucket, vector<rgw_usage_log_entry> > *usage
                                = (map<rgw_user_bucket, vector<rgw_usage_log_entry> > *)param;
  rgw_user *puser;
  if (!entry.payer.empty()) {
    puser = &entry.payer;
  } else {
  ...
  rgw_user_bucket ub(puser->to_str(), entry.bucket);
  rgw_usage_log_entry le;
  le.aggregate_readop(entry);
  (*usage)[ub].push_back(le);
...
int rgw_user_usage_log_read_total(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  usage_record_prefix_total(op.owner, filter_prefix);

  if (!iter.empty()) {
    start_key = iter;
  } else {
    start_key = filter_prefix;
  }
...
int rgw_user_usage_log_read_current_readop(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "rgw_user_usage_log_read_current_readop()");
  bufferlist::iterator in_iter = in->begin();
  rgw_cls_usage_log_read_op op;
  try {
    decode(op, in_iter);
  } catch (buffer::error& err) {
    CLS_LOG(1, "ERROR: rgw_user_usage_log_read_current(): failed to decode request\n");
    return -EINVAL;
  }
  rgw_cls_usage_log_read_ret ret_info;
  map<rgw_user_bucket, rgw_usage_log_entry> *usage = &ret_info.usage;
  uint64_t end_epoch = op.end_epoch;
  string user = op.owner;

  string key_by_readop;
  usage_record_prefix_current_readop(end_epoch, user, key_by_readop);
  bufferlist record_current_readop_bl;
  int ret = cls_cxx_map_get_val(hctx, key_by_readop, &record_current_readop_bl);
  if (ret < 0 && ret != -ENOENT) {
    CLS_LOG(1, "ERROR: rgw_user_usage_log_add(): cls_cxx_map_read_key returned %d\n", ret);
    return ret;
  }
  rgw_usage_log_entry e;
  if (ret >= 0) {
    ret = usage_record_decode(record_current_readop_bl, e);
    if (ret < 0) {
      return ret;
    }
  }
  if (ret == -ENOENT) {
    ...
    e.epoch = end_epoch;
    e.read_ops.ops = 0;
    e.read_ops.successful_ops = 0;
    e.write_ops.ops = 0;
    e.write_ops.successful_ops = 0;
    e.write_ops.bytes_received = 0;
    e.read_ops.bytes_sent = 0;
  }
  ret = usage_log_read_current_readop_cb(hctx, key_by_readop, e, (void *)usage);
  if (ret < 0) {
    return ret;
  }
  encode(ret_info, *out);
...
int rgw_user_usage_log_read_readop(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "rgw_user_usage_log_read_readop()");
  bufferlist::iterator in_iter = in->begin();
  rgw_cls_usage_log_read_op op;
  try {
    decode(op, in_iter);
  } catch (buffer::error& err) {
    CLS_LOG(1, "ERROR: rgw_user_usage_log_read_read(): failed to decode request\n");
    return -EINVAL;
  }
  rgw_cls_usage_log_read_readop_ret ret_info;
  map<rgw_user_bucket, vector<rgw_usage_log_entry> > *usage = &ret_info.usage;
  string iter = op.iter;
  uint32_t max_entries = op.max_entries;
  if (ret_info.truncated) {
    ret_info.truncated = false;
  ...
  map<string, bufferlist> keys;
  string start_key;
  string filter_prefix = "WRop_";
  string end_key;

  usage_record_prefix_readop(op.end_epoch, end_key);

  if (!iter.empty()) {
    start_key = iter;
  } else {
    usage_record_prefix_readop(op.start_epoch, start_key);
  }
  ...
  for (map<string, bufferlist>::iterator i = keys.begin(); i != keys.end(); ++i, ++current_index) {
    const string& key = i->first;
    if (key.compare(end_key) > 0) {
      CLS_LOG(20, "rgw_user_usage_log_read_readop reached key=%s, done", key.c_str());
      ret_info.truncated = false;
      iter = key;
      break;
    }
    ...
    ret = usage_log_read_readop_cb(hctx, key, e, (void *)usage);
    if (ret < 0) {
      return ret;
    }
    if (current_index == num_keys - 1) {
...
static int usage_log_trim_readop_cb(cls_method_context_t hctx,const string& key,
                        rgw_usage_log_entry& entry, void *param)
{
  bool *found = (bool *)param;
  if (found) {
  ...
  string key_readop_usage;

  string o = entry.owner.to_str();
  usage_record_name_readop(o, entry.epoch, key_readop_usage);

  return cls_cxx_map_remove_key(hctx, key_readop_usage);
}
...
int rgw_user_usage_log_trim_total(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "rgw_user_usage_log_trim_total()");
  /* only continue if object exists! */
  int ret = cls_cxx_stat(hctx, NULL, NULL);
  if (ret < 0)
    return ret;
  ...
  // just remove the total usage of the given user and bucket, no need to check return code
  string key_total;
  usage_record_name_total(op.user, op.bucket, key_total);
  cls_cxx_map_remove_key(hctx, key_total);
  return 0;
}

int rgw_user_usage_log_trim_readop(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  CLS_LOG(10, "rgw_user_usage_log_trim_readop()");
  ...
  bool more = false;
  bool found = false;
#define MAX_READOP_USAGE_TRIM_ENTRIES 128
  ret = usage_iterate_readop_range(
          hctx, op.start_epoch, op.end_epoch, op.user, iter,
          MAX_READOP_USAGE_TRIM_ENTRIES, &more, usage_log_trim_readop_cb, (void *)&found);
  if (ret < 0)
    return ret;
...
static int gc_iterate_entries(cls_method_context_t hctx, const string& marker, bool expired_only,
                              string& out_marker, uint32_t max_entries, bool *truncated,
                              ...
                              void *param)
{
  ...
  if (iter == keys.end()) {
    if (truncated && *truncated)
      CLS_LOG(0, "ERROR: gc_iterate_entries() omap is empty while truncated is true");
    return 0;
  }

  const string* last_key = nullptr;
  for (; iter != keys.end(); ++iter) {
    const string& key = iter->first;
  ...
  if (!truncated || *truncated) {
    if (!last_key) {
      CLS_LOG(0, "ERROR: gc_iterate_entries() last_key shouldn't be nullptr");
      return 0;
    }
    out_marker = *last_key;
  }
...
static int rgw_cls_lc_set_entries(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  bufferlist::iterator in_iter = in->begin();

  vector<cls_rgw_lc_entry> entries;
  cls_rgw_lc_set_entries_op op(&entries);
  try {
    decode(op, in_iter);
  } catch (buffer::error& err) {
    CLS_LOG(1, "ERROR: rgw_cls_lc_set_entries(): failed to decode vector entry\n");
    return -EINVAL;
  }
  ...
  for (uint32_t i = 0; i < entries.size(); ++i) {
    bufferlist bl;
    encode(entries[i], bl);
    vals[entries[i].bucket] = bl;
  }
  int ret = cls_cxx_map_set_vals(hctx, &vals);
  return ret;
}

static int rgw_cls_lc_rm_entries(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  bufferlist::iterator in_iter = in->begin();
  vector<cls_rgw_lc_entry> entries;
  cls_rgw_lc_rm_entries_op op(&entries);
  try {
    decode(op, in_iter);
  } catch (buffer::error& err) {
    CLS_LOG(1, "ERROR: rgw_cls_lc_rm_entries(): failed to decode vector entry\n");
    return -EINVAL;
  }
  vector<string> keys;
  for (uint64_t i = 0; i < entries.size(); ++i) {
    keys.push_back(entries[i].bucket);
  }

  int ret = cls_cxx_map_remove_keys(hctx, keys);
  return ret;
}
...
static int rgw_cls_lc_set_entry(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  cls_rgw_lc_set_entry_op op(&entry);
  try {
    decode(op, in_iter);
  } catch (buffer::error& err) {
...
static int rgw_cls_lc_rm_entry(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  bufferlist::iterator in_iter = in->begin();
  cls_rgw_lc_entry entry;
  cls_rgw_lc_rm_entry_op op(&entry);
  try {
    decode(op, in_iter);
  } catch (buffer::error& err) {
...
CLS_INIT(rgw)
{
  ...
  cls_method_handle_t h_rgw_namespace_prepare_op;
  cls_method_handle_t h_rgw_namespace_complete_op;
  cls_method_handle_t h_rgw_namespace_dir_prepare_op;
  cls_method_handle_t h_rgw_namespace_dir_complete_op;
  cls_method_handle_t h_rgw_namespace_dir_make_op;
  cls_method_handle_t h_rgw_namespace_dir_del_op;
  cls_method_handle_t h_rgw_namespace_list_op;
  cls_method_handle_t h_rgw_namespace_head_op;
  cls_method_handle_t h_rgw_namespace_suggest_changes_op;
  ...
  cls_method_handle_t h_rgw_user_usage_log_read_current_readop;
  cls_method_handle_t h_rgw_user_usage_log_read_readop;
  cls_method_handle_t h_rgw_user_usage_log_trim;
  cls_method_handle_t h_rgw_user_usage_log_trim_total;
  cls_method_handle_t h_rgw_user_usage_log_trim_readop;
  ...
  cls_method_handle_t h_rgw_lc_set_entries;
  cls_method_handle_t h_rgw_lc_rm_entries;
  ...
  cls_register_cxx_method(h_class, RGW_BUCKET_PREPARE_OP, CLS_METHOD_RD | CLS_METHOD_WR | CLS_MERGE_OP_TYPE_RGW, rgw_bucket_prepare_op, &h_rgw_bucket_prepare_op);
  cls_register_cxx_method(h_class, RGW_BUCKET_COMPLETE_OP, CLS_METHOD_RD | CLS_METHOD_WR | CLS_MERGE_OP_TYPE_RGW, rgw_bucket_complete_op, &h_rgw_bucket_complete_op);

  // namespace
  cls_register_cxx_method(h_class, RGW_NAMESPACE_PREPARE_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_prepare_op, &h_rgw_namespace_prepare_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_COMPLETE_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_complete_op, &h_rgw_namespace_complete_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_DIR_PREPARE_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_dir_prepare_op, &h_rgw_namespace_dir_prepare_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_DIR_COMPLETE_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_dir_complete_op, &h_rgw_namespace_dir_complete_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_DIR_MAKE_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_dir_make_op, &h_rgw_namespace_dir_make_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_DIR_DEL_OP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_dir_del_op, &h_rgw_namespace_dir_del_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_LIST_OP, CLS_METHOD_RD, rgw_namespace_list_op, &h_rgw_namespace_list_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_HEAD_OP, CLS_METHOD_RD, rgw_namespace_head_op, &h_rgw_namespace_head_op);
  cls_register_cxx_method(h_class, RGW_NAMESPACE_SUGGEST_CHANGES, CLS_METHOD_RD | CLS_METHOD_WR, rgw_namespace_suggest_changes_op, &h_rgw_namespace_suggest_changes_op);
  ...
  cls_register_cxx_method(h_class, RGW_BI_LOG_LIST, CLS_METHOD_RD | CLS_MERGE_OP_TYPE_RGW, rgw_bi_log_list, &h_rgw_bi_log_list_op);
  ...
  cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_READ_CURRENT_READOP, CLS_METHOD_RD, rgw_user_usage_log_read_current_readop, &h_rgw_user_usage_log_read_current_readop);
  cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_READ_READOP, CLS_METHOD_RD, rgw_user_usage_log_read_readop, &h_rgw_user_usage_log_read_readop);
  ...
  cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_TRIM_TOTAL, CLS_METHOD_RD | CLS_METHOD_WR, rgw_user_usage_log_trim_total, &h_rgw_user_usage_log_trim_total);
  cls_register_cxx_method(h_class, RGW_USER_USAGE_LOG_TRIM_READOP, CLS_METHOD_RD | CLS_METHOD_WR, rgw_user_usage_log_trim_readop, &h_rgw_user_usage_log_trim_readop);
  ...
  cls_register_cxx_method(h_class, RGW_LC_SET_ENTRIES, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_set_entries, &h_rgw_lc_set_entries);
  cls_register_cxx_method(h_class, RGW_LC_RM_ENTRIES, CLS_METHOD_RD | CLS_METHOD_WR, rgw_cls_lc_rm_entries, &h_rgw_lc_rm_entries);
  ...
  cls_register_cxx_method(h_class, "guard_bucket_resharding", CLS_METHOD_RD | CLS_MERGE_OP_TYPE_RGW,
