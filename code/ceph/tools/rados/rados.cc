...
static string refcount = "refcount";
...
"   rmprefix --prefix <prefix> [--really-run]  remove object(s) with prefix (NOTICE: \n"
                                     "this command scans all files in the pool)\n"
"   rmbefore --file-lists <filename> --before <before timestamp seconds> [--really-run] [--skip-refcount] remove object\n"
...
"   setomapheader <obj-name> <val>   val is the bufferlist set to omapheader\n"
"   setomapheaderfromfile <obj-name> [file]\n"
"                                    read bufferlist from file\n"
...
"   --skip-refcount\n"
"        skip files weith attr refcount\n"
"\n"
"LOAD GEN OPTIONS:\n"
...
inline bool is_prefix_of_str(const string &prefix, const string &src) {
                             ...
                             std::vector<const char*> &nargs)
{
  ...
  i = opts.find("really-run");
  if (i != opts.end()) {
    dryrun = false;
  }
  i = opts.find("skip-refcount");
  if (i != opts.end()) {
    skip_refcount = true;
  }
  ...
  i = opts.find("before");
  if (i != opts.end()) {
    if (rados_sistrtoll(i, &before)) {
      return -EINVAL;
    }
  }
  i = opts.find("file-lists");
  if (i != opts.end()) {
    input_filename = std::string(i->second.c_str());
  }
  ...
  if (strcmp(nargs[0], "lspools") == 0) {
	...
	*outstream << std::endl;
  ...
  } else if (strcmp(nargs[0], "setomapheaderfromfile") == 0) {
    if (!pool_name || nargs.size() < 3)
      usage_exit();
    ...
    bufferlist bl;
    string err;
    ret = bl.read_file(file.c_str(), &err);
    if (ret < 0) {
      cerr << "error reading file: " << file << ", error:" << err << std::endl;
      return 1;
    }
    ...
    for (; iter != nargs.end(); ++iter) {
      ...
      if (skip_refcount) {
        if (oid.compare("--skip-refcount") == 0) {
          continue;
        }

        bufferlist bl;
        ret = detail::getxattr(io_ctx, oid, refcount, bl, use_striper);
        if (ret > 0) {
          cerr << "skip: have attr 'refcount'" << std::endl;
          goto out;
        } else if (ret == -ENOENT) {
          continue;
        } else if (ret < 0 && ret != -ENODATA) {
          cerr << "failed to get xattr refcount " << cpp_strerror(ret) << std::endl;
          goto out;
        }
      }
  ...
  } else if (strcmp(nargs[0], "rmbefore") == 0) {
    if (input_filename.empty()) {
      cerr << " error empty file list" << std::endl;
      goto out;
    } else if (before == 0) {
      cerr << " error bad before" << std::endl;
      goto out;
    } else if (!pool_name) {
      cerr << " error empty pool name" << std::endl;
      goto out;
    }
    string oid;
    ifstream files_io(input_filename);
    if(!files_io) {
      cerr << "err: failed to read file list:" << input_filename << std::endl;
      goto out;
    }
    uint64_t failed = 0;
    uint64_t deleted = 0;
    uint64_t skipped = 0;
    uint64_t dryrunned = 0;

    while(getline(files_io, oid)) {
      if (oid.empty()) {
        continue;
      }
      uint64_t size;
      ...
      ret = detail::stat2(io_ctx, oid, size, mtime, use_striper);
      if (ret == -ENOENT) {
        cout << " success: " << pool_name << "/" << oid << " is deleted" << std::endl;
        ++ deleted;
        continue;
      } else if (ret < 0) {
        cerr << " error: stat-ing " << pool_name << "/" << oid << ": "
      ...
      utime_t t(mtime);
      if (t.to_msec() >= (before*1000)) {
        cerr << " skip: " << pool_name << "/" << oid << " mtime " << t << std::endl;
        ++ skipped;
      } else {
        if (skip_refcount) {
          bufferlist bl;
          ret = detail::getxattr(io_ctx, oid, refcount, bl, use_striper);
          if (ret > 0) {
            cerr << "skip: " << pool_name << "/" << oid << " have attr 'refcount'" << std::endl;
            ++ skipped;
            continue;
          } else if (ret == -ENOENT) {
            cout << " success: " << pool_name << "/" << oid << " is deleted" << std::endl;
            ...
            cerr << "failed: " << pool_name << "/" << oid << " get xattr refcount " << cpp_strerror(ret) << std::endl;
            ++ failed;
            continue;
          }
        }
        if (dryrun) {
          cout << " dryrun: remove " << pool_name << "/" << oid << " mtime " << t << std::endl;
          ++ dryrunned;
        } else {
          ret = detail::remove(io_ctx, oid, use_striper);
          if (ret < 0 && ret != -ENOENT) {
            string name = (nspace.size() ? nspace + "/" : "" ) + oid;
            cerr << "failed: removing " << pool_name << ">" << name << ": " << cpp_strerror(ret) << std::endl;
            ++ failed;
          } else {
            cout << " success: remove " << pool_name << "/" << oid << " mtime " << t << std::endl;
            ++ deleted;
          }
        }
      }
    }
    cout << "deleted[" << deleted << "], failed[" << failed << "], skipped[" << skipped
         << "], dryrun[" << dryrunned << "]" << std::endl;
    goto out;
  }
  ...
  } else if (strcmp(nargs[0], "rmprefix") == 0) {
    if (!pool_name) {
      cerr << "pool name was not specified" << std::endl;
      ret = -1;
      goto out;
    ...
    if (prefix.empty()) {
      cerr << "prefix was not specified" << std::endl;
      ret = -1;
      goto out;
    }
    ...
    librados::NObjectIterator i_start = io_ctx.nobjects_begin();
    librados::NObjectIterator i_end = io_ctx.nobjects_end();

    for (int64_t n = 1; i_start != i_end; ++i_start, ++n) {
      if (n % 1000000 == 0) {
        cerr << "scaned " << n << " object" << std::endl;
      }
      string oid = i_start->get_oid();
      if (!is_prefix_of_str(prefix, oid)) {
        continue;
      }
      if (!i_start->get_nspace().empty()) {
        cout << "skip: ns[" << i_start->get_nspace() << "]" << " oid " << oid << std::endl;
        continue;
      }
      ret = detail::getxattr(io_ctx, oid, refcount, bl, use_striper);
      if (ret > 0) {
        cerr << "skip " << oid << " have attr 'refcount'" << std::endl;
        continue;
      } else if (ret == -ENOENT) {
        cout << "removed: " << oid << std::endl;
        continue;
      } else if (ret < 0 && ret != -ENODATA) {
        cerr << "error getting xattr refcount of " << oid << " " << cpp_strerror(ret) << std::endl;
        continue;
      }
      if (dryrun) {
        cout << "dry run remove: " << oid << std::endl;
      } else {
        ret = detail::remove(io_ctx, oid, use_striper);
        if (ret == -ENOENT) {
          cout << "removed: " << oid << std::endl;
        } else if (ret < 0) {
          cerr << "error removing " << oid << ": " << cpp_strerror(ret) << std::endl;
          break;
        } else {
          cout << "success remove: " << oid << std::endl;
        }
      }
    }
  } else {
...
int main(int argc, const char **argv)
{
  ...
  for (i = args.begin(); i != args.end(); ) {
    if (ceph_argparse_double_dash(args, i)) {
      ...
      opts["pool"] = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--before", (char*)NULL)) {
      opts["before"] = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--file-lists", (char*)NULL)) {
      opts["file-lists"] = val;
    ...
    } else if (ceph_argparse_flag(args, i, "--really-run", (char*)NULL)) {
      opts["really-run"] = "true";
    } else if (ceph_argparse_flag(args, i, "--skip-refcount", (char*)NULL)) {
      opts["skip-refcount"] = "true";
    } else {
      if (val[0] == '-')
        usage_exit();
      ++i;
