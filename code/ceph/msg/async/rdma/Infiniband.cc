...
int Infiniband::QueuePair::init()
{
  ...
  ldout(cct, 20) << __func__ << " qp" << qp->qp_num << " successfully change queue pair to INIT:"
                 << " qp=" << qp << dendl;
  return 0;
}
...
int Infiniband::QueuePair::to_dead()
{
  if (dead)
    return 0;
  ldout(cct, 20) << __func__ << " qp" << qp->qp_num << " to dead" << dendl;

  ibv_send_wr *bad_wr = NULL, beacon;
  ibv_qp_attr qpa;
  ...
  int ret = ibv_modify_qp(qp, &qpa, mask);
  if (ret) {
    lderr(cct) << __func__ << " qp" << qp->qp_num << " failed to transition to ERROR state: "
               << cpp_strerror(errno) << dendl;
    return -errno;
  }
  ...
  if (ret) {
    lderr(cct) << __func__ << " qp" << qp->qp_num << " failed to send a beacon: "
               << cpp_strerror(errno) << dendl;
    return -errno;
  }
...
void *Infiniband::MemoryManager::mem_pool::slow_malloc()
{
  ...
  ldout(ctx->manager->cct, 10) << __func__  << dendl;
...
{
  ldout(cct, 20) << __func__ << " start init Infiniband." << dendl;
  Mutex::Locker l(lock);
  ...
  rx_queue_len = device->device_attr->max_srq_wr - 1;
  ...
  ldout(cct, 20) << __func__ << " finish Infiniband." << dendl;
}
...
int Infiniband::post_chunks_to_srq(int num)
{
  ...
  assert(badworkrequest == nullptr && ret == 0);
  return i;
}
