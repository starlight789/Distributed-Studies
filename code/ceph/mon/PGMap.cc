...
int PGMapDigest::get_pool_used_percent(const OSDMap &osd_map, int64_t poolid,
  ...
  auto iter = pg_pool_sum.find(poolid);
  if (iter == pg_pool_sum.end()) {
    return -ENOENT;
  }
  const object_stat_sum_t &sum = iter->second.stats.sum;

  double raw_used_rate = ::pool_raw_used_rate(osd_map, poolid);
  double curr_object_copies_rate = 0.0;
  if (sum.num_object_copies > 0)
    curr_object_copies_rate = (double)(sum.num_object_copies - sum.num_objects_degraded) /
                              (double)sum.num_object_copies;

  // note avail passed in is raw_avail, calc raw_used here.
  if (avail) {
    used_percent = (double)(sum.num_bytes * raw_used_rate * curr_object_copies_rate);
    used_percent /= used_percent + avail;
  } else if (sum.num_bytes) {
    used_percent = 1.0;
  }
  return 0;
...
int64_t PGMapDigest::get_rule_used_space(const OSDMap &osd_map, int ruleno) const {
  int64_t used_bytes = 0;
  set<int64_t> pool_ids;
  osd_map.get_pool_ids_by_rule(ruleno, &pool_ids);

  for (int poolid : pool_ids) {
    auto iter = pg_pool_sum.find(poolid);
    if (iter == pg_pool_sum.end()) {
      return -ENOENT;
    }
    ...
    used_bytes += (int64_t) (sum.num_bytes * raw_used_rate * curr_object_copies_rate);
  }
