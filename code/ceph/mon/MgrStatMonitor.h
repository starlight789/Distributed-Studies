...
public:
  ...
  int get_active_clean_pgs_num() {
    const auto &found = digest.num_pg_by_state.find(PG_STATE_ACTIVE | PG_STATE_CLEAN);
    if (found != digest.num_pg_by_state.end()) {
      return found->second;
    }
    return 0;
  }
