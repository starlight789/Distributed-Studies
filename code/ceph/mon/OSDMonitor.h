...
public:
  ...
  struct CleanUpmapJob : public ParallelPGMapper::Job {
    ...
    const OSDMap& osdmap;
    OSDMap::Incremental& pending_inc;
    // lock to protect pending_inc from changing
...
private:
  ...
  void reset_osd_pending_state(const unsigned mask, unsigned new_state, const int from);
  ...
  bool preprocess_license(MonOpRequestRef op);
  bool prepare_license(MonOpRequestRef op);
public:
...
public:
  ...
  // slow disk check
public:
  map<int, map<string, string>> osd_slow_disk_metadatas;
  bool can_drop_osd_slow_disk(int from);
  bool have_enough_space_to_drop_osd(const PGMapDigest& digest, int from);
  bool preprocess_osd_slow_disk(MonOpRequestRef op);
  bool prepare_osd_slow_disk(MonOpRequestRef op);
  void osd_slow_disk_health_check(const map<int, map<string, string>> &metadatas,
