...
namespace librados
{
  ...
  enum ObjectOperationGlobalFlags {
    ...
    OPERATION_SKIP_CACHE         = LIBRADOS_OPERATION_SKIP_CACHE,
  };
  ...
  public:
    ...
    int operate(const std::string& oid, ObjectWriteOperation *op, bool skip_cache = false);
    int operate(const std::string& oid, ObjectReadOperation *op, bufferlist *pbl);
    int operate(const std::string& oid, ObjectReadOperation *op, int flags, bufferlist *pbl);
  ...
  public:
    ...
    virtual int mon_command(std::string cmd, const bufferlist& inbl,
    ...
    /// return the base data pool id
    int get_base_pool(int64_t pool_id, int64_t* base_tier);
