...
void RGWCivetWeb::get_throttle_token(int num, shared_ptr<TokenBucketThrottle> token_bucket) {
  if (!token_bucket) {
    return;
  }
  ThrottleRequest throttle_request;
  bool waited = token_bucket->get<RGWCivetWeb, ThrottleRequest,
       &RGWCivetWeb::get_throttle_token_callback>(num, this, &throttle_request);
  if (waited == true) {
    unique_lock<std::mutex> lock(throttle_request.mutex);
    while (throttle_request.ready == false) {
      throttle_request.ready_cond.wait(lock);
    }
  }
}
