...
struct es_index_obj_response {
  ...
  void decode_json(JSONObj *obj) {
    ...
    JSONDecoder::decode_json("tagging", tagging, obj);
  }
};
...
int RGWMetadataSearchOp::verify_permission()
{
  if (s->user->system || s->user->admin) {
    need_permission = false;
    return 0;
  }
  if (s->bucket_exists) {
    RGWAccessControlPolicy bucket_policy(s->cct);
    int ret = read_bucket_policy(store, s, s->bucket_info, s->bucket_attrs, &bucket_policy, s->bucket);
    if (ret < 0) {
      ldout(s->cct, 10) << __func__ << "(): read_bucket_policy ret:" << ret << dendl;
      return ret;
    }
    if (!bucket_policy.verify_permission(*s->auth.identity,
                                                        s->perm_mask,
                                                        RGW_PERM_READ)) {
      ldout(s->cct, 10) << __func__ << "(): EACCES" << dendl;
      return -EACCES;
    }
    need_permission = false;
  }
...
void RGWMetadataSearchOp::execute()
{
  ...
  static map<string, ESEntityTypeMap::EntityType> generic_map = { {"bucket", ESEntityTypeMap::ES_ENTITY_STR},
                                                           ...
                                                           {"tagging.key", ESEntityTypeMap::ES_ENTITY_STR},
                                                           {"tagging.value", ESEntityTypeMap::ES_ENTITY_STR},
                                                           };
...
#define MAX_KEYS_MAX 10000
  ...
  void send_response() override {
    ...
    for (auto& i : response.hits.hits) {
      ...
      s->formatter->close_section();
      s->formatter->open_array_section("TagSet");
      for (auto& m : e.tagging) {
        s->formatter->open_object_section("tagging");
        s->formatter->dump_string("Key", m.key);
        s->formatter->dump_string("Value", m.value);
        s->formatter->close_section();
      }
      s->formatter->close_section();
