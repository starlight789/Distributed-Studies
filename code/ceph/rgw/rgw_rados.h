...
#define RGW_OBJ_NS_MULTIPART_PREFIX "__multipart"
#define RGW_OBJ_NS_SHADOW_PREFIX    "__shadow"
...
#define S3_PATH_DELIMITER "/"
#define S3_PATH_DELIMITER_CHAR '/'

#define RGW_ADMIN_CLIENT_ID "radosgw-admin.client@baidu.com"
...
struct RGWUsageBatch {
  ...
  void insert(ceph::real_time& t, rgw_usage_log_entry& entry, bool *account, bool is_intranet_addr = false) {
    bool exists = m.find(t) != m.end();
    *account = !exists;
    if (is_intranet_addr)
      m[t].set_update_readop_usage();
    m[t].aggregate(entry);
  }
};
...
class RGWGetDataCB {
public:
  virtual bool is_crypt() { return false; }
...
protected:
  ...
  bool skip_cache{false};      /* tail object enable skip pool */
...
public:
  ...
  void get_implicit_location(uint64_t cur_part_id, uint64_t cur_stripe, uint64_t ofs,
                             string *override_prefix, rgw_obj_select *location);
  ...
  const map<uint64_t, RGWObjManifestRule>& get_rules() { return rules; }
  ...
  void set_head_placement_rule(const rgw_placement_rule& placement_rule) {
    head_placement_rule = placement_rule;
  }
  ...
  void set_skip_cache(bool _skip_cache) {
    skip_cache = _skip_cache;
  }

  bool is_enable_skip_cache() {
    return skip_cache;
  }
  ...
  public:
    ...
    void set_obj(const rgw_obj& obj) {
      location.set_obj(obj);
    }
  };
  ...
  public:
    ...
    int restore_env(RGWObjManifest *_m, const rgw_placement_rule& head_placement_rule);
...
public:
  RGWZoneStorageClasses() {
    standard_class = &m[RGWStorageClass::STANDARD];
  }
  ...
  void set_storage_class(const string& sc, const rgw_pool *data_pool, const string *compression_type) {
    RGWZoneStorageClass* storage_class;
    if (sc.empty()) {
      storage_class = &m[RGWStorageClass::STANDARD];
    } else {
      storage_class = &m[sc];
    }
    if (data_pool) {
  ...
  void unset_storage_class(const string& sc) {
    m.erase(sc);
  }
...
struct RGWZonePlacementInfo {
  ...
  RGWZoneStorageClasses tail_pools;
  ...
  void encode(bufferlist& bl) const {
    ...
    rgw_pool standard_data_pool = get_data_pool(RGWStorageClass::STANDARD);
    encode(standard_data_pool.to_str(), bl);
    encode(data_extra_pool.to_str(), bl);
    encode((uint32_t)index_type, bl);
    string standard_compression_type = get_compression_type(RGWStorageClass::STANDARD);
    encode(standard_compression_type, bl);
    encode(storage_classes, bl);
    encode(tail_pools, bl);
    ENCODE_FINISH(bl);
  }

  void decode(bufferlist::iterator& bl) {
    unsigned offset = bl.get_off();
    try {
      decode_new(bl);
  ...
  void decode_new(bufferlist::iterator& bl) {
    ...
    if (struct_v >= 7) {
      ...
      storage_classes.set_storage_class(RGWStorageClass::STANDARD, &standard_data_pool,
                                        (!standard_compression_type.empty() ?
                                         &standard_compression_type : nullptr));
    }
    if (struct_v >= 8) {
      decode(tail_pools, bl);
    }
    DECODE_FINISH(bl);
  }
  ...
  void decode_old(bufferlist::iterator& bl) {
    ...
    storage_classes.set_storage_class(RGWStorageClass::STANDARD, &standard_data_pool,
                                      (!standard_compression_type.empty() ?
                                       &standard_compression_type : nullptr));
    if (struct_v >= 7) {
      string data_ia_pool_str;
      decode(data_ia_pool_str, bl);
      if (!data_ia_pool_str.empty()) {
        rgw_pool data_ia_pool(data_ia_pool_str);
        storage_classes.set_storage_class(RGWStorageClass::STANDARD_IA, &data_ia_pool,
                                          (!standard_compression_type.empty() ?
                                           &standard_compression_type : nullptr));
      }
    }
  ...
  const rgw_pool& get_data_extra_pool() const {
    if (data_extra_pool.empty()) {
      return storage_classes.get_standard().data_pool;
    }
    return data_extra_pool;
  }

  bool tail_pool_exists(const string& sc) const {
    const RGWZoneStorageClass *storage_class;
    if (!tail_pools.find(sc, &storage_class)) {
      return false;
    }
    return true;
  }

  const rgw_pool get_tail_pool(const string& sc) const {
    if (sc.empty()) {
      return tail_pools.get_standard().data_pool;
    }
    const RGWZoneStorageClass *storage_class;
    if (!tail_pools.find(sc, &storage_class)) {
      return rgw_pool();
    }
    return storage_class->data_pool;
  }
  ...
  const rgw_pool& get_standard_data_pool() const {
    return get_data_pool(RGWStorageClass::STANDARD);
  }
  ...
  const string& get_compression_type(const string& sc) const {
    ...
    return storage_class->compression_type;
  }
...
struct RGWZoneParams : RGWSystemMetaObj {
  ...
  uint64_t start_sync_time = 0;
  uint64_t stop_sync_time = 0;
  ...
  void encode(bufferlist& bl) const override {
    ...
    encode(start_sync_time, bl);
    encode(stop_sync_time, bl);
    ENCODE_FINISH(bl);
  }
  ...
  void decode(bufferlist::iterator& bl) override {
    ...
    if (struct_v >= 13) {
      decode(start_sync_time, bl);
      decode(stop_sync_time, bl);
    } else {
      start_sync_time = 0;
      stop_sync_time = 0;
    }
  ...
  /*
   * return data pool of the object
   */
  bool get_data_pool(const rgw_placement_rule& placement_rule, const rgw_obj& obj, rgw_pool *pool) const {
    ...
    if (placement_rule.is_tail_rule) {
      const rgw_pool temp_pool = iter->second.get_tail_pool(placement_rule.storage_class);
      if (!temp_pool.empty()) {
        *pool  = temp_pool;
      }
      return !temp_pool.empty();
    }
  ...
  /*
   * return tail pool of the object
   */
  bool get_tail_pool(const rgw_placement_rule& placement_rule, rgw_pool *pool) const {
    if (placement_rule.empty()) {
      return false;
    }
    auto iter = placement_pools.find(placement_rule.name);
    ...
    const rgw_pool temp_pool = iter->second.get_tail_pool(placement_rule.storage_class);
    if (!temp_pool.empty()) {
      *pool  = iter->second.get_tail_pool(placement_rule.storage_class);
    }
    return !temp_pool.empty();
  }
  ...
  bool valid_placement(const rgw_placement_rule& rule) const {
    ...
    if (rule.storage_class != RGWStorageClass::STANDARD) {
      return iter->second.storage_class_exists(RGWStorageClass::STANDARD) &&
        iter->second.storage_class_exists(rule.storage_class);
    } else {
      return iter->second.storage_class_exists(RGWStorageClass::STANDARD);
    }
  }
...
struct RGWZoneGroupPlacementTarget {
  ...
  void decode(bufferlist::iterator& bl) {
    ...
    if (storage_classes.empty()) {
      storage_classes.insert(RGWStorageClass::STANDARD);
    }
    DECODE_FINISH(bl);
  }
  void dump(Formatter *f) const;
...
class Finisher;
template <typename RequestType> class RGWAsyncProcessor;
class RGWAsyncRadosRequest;
class RGWAsyncRequest;
using RGWAsyncRadosProcessor = RGWAsyncProcessor<RGWAsyncRadosRequest>;
using RGWAsyncRequestProcessor = RGWAsyncProcessor<RGWAsyncRequest>;
...
struct RGWRenameSrcInfo {
  std::string client_id;
  std::string src_idtag;
  std::string dst_object;
  bool _inner_mem = false;
  RGWObjManifest* manifest = nullptr;

  RGWRenameSrcInfo() {}
  RGWRenameSrcInfo(const std::string& client_id,
                   const std::string& src_idtag,
                   const std::string& dst_object,
                   RGWObjManifest* manifest)
    : client_id(client_id), src_idtag(src_idtag),
      dst_object(dst_object), manifest(manifest) {}

  virtual ~RGWRenameSrcInfo() {
    if (_inner_mem && !manifest) {
      delete manifest;
      manifest = nullptr;
    }
  };
  ...
  void decode(bufferlist::iterator& bl) {
    ...
    decode(client_id, bl);
    decode(src_idtag, bl);
    decode(dst_object, bl);
    if (!manifest) {
      manifest = new RGWObjManifest();
      _inner_mem = true;
    }
    decode(*manifest, bl);
    DECODE_FINISH(bl);
...
WRITE_CLASS_ENCODER(RGWRenameSrcInfo)
...
class RGWRados : public AdminSocketHook
{
  ...
  int open_pool_ctx(const rgw_pool& pool, librados::IoCtx&  io_ctx, librados::Rados* rados_handle = NULL);
  ...
  RGWAsyncRequestProcessor*  async_processor;

  RGWMetaNotifier *meta_notifier;
  RGWDataNotifier *data_notifier;
  RGWAwakeDataSync *awake_data_sync;
  ...
  uint32_t bucket_index_max_shards;
  // the defalut namespace type
  RGWBucketNamespaceType bucket_namespace_default_type;
...
public:
  RGWRados() : lock("rados_timer_lock"), watchers_lock("watchers_lock"), timer(NULL),
               ...
               run_sync_thread(false), run_reshard_thread(false), async_rados(nullptr), async_processor(nullptr), meta_notifier(NULL),
               data_notifier(NULL), awake_data_sync(NULL), meta_sync_processor_thread(NULL),
  ...
  RGWAsyncRequestProcessor* get_async_processor() const { return async_processor; };
  ...
  int get_raw_obj_ref(const rgw_raw_obj& obj, rgw_rados_ref *ref, librados::Rados* rados_handle = NULL);
  ...
  int list_todelete_bucket(list<string>& buckets);
  void tick();

  int clear_redundant_index(RGWObjState *dest_state, RGWBucketInfo& bucket_info, rgw_obj& obj);

  CephContext *ctx() { return cct; }
  /** do all necessary setup of the storage device */
  int initialize(CephContext *_cct, bool _use_gc_thread, bool _use_lc_thread, bool _quota_threads, bool _run_sync_thread, bool _run_reshard_thread, bool _admin_op) {
    set_context(_cct);
  ...
  int read_usage_readop(rgw_user& user, uint64_t usage_iter, uint64_t end_epoch, uint32_t max_entries,
                 bool *is_truncated, RGWUsageIter& read_iter, map<rgw_user_bucket, vector<rgw_usage_log_entry> >& usage);
  int read_usage_current_readop(set<string>& user_list, uint64_t end_epoch,
                 map<rgw_user_bucket, rgw_usage_log_entry>& usage);
  int trim_usage(rgw_user& user, uint64_t start_epoch, uint64_t end_epoch);
  int trim_total_usage(rgw_user& user, string bucket);
  int trim_readop_usage(rgw_user& user, uint64_t start_epoch, uint64_t end_epoch);
  int clear_usage();
  ...
  int select_bucket_placement(RGWUserInfo& user_info, const string& zonegroup_id, const rgw_placement_rule& rule,
  ...
  int select_new_bucket_location(RGWUserInfo& user_info, const string& zonegroup_id, const rgw_placement_rule& rule,
                                 rgw_placement_rule *pselected_rule_name, RGWZonePlacementInfo *rule_info);
  ...
  bool get_obj_tail_pool(const rgw_placement_rule& placement_rule, rgw_pool *pool);
  ...
  bool datasync_all_disabled(const RGWBucketInfo& info) const {
    for (auto& z : zonegroup.zones) {
      if (z.first == zone_public_config.id)
        continue;
      if (info.datasync_flag_enabled(z.second.name))
        return false;
    }
    return true;
  }
  ...
  public:
    ...
    class UpdateIndex {
      ...
      bool blind;
      bool prepared{false};
      bool bilog_write_enable = true;
      bool bilog_delete_enable = true;
    ...
    public:
      ...
      void set_bilog_write_enable(bool _bilog_write_enable) {
        bilog_write_enable = _bilog_write_enable;
      }

      void set_bilog_delete_enable(bool _bilog_delete_enable) {
        bilog_delete_enable = _bilog_delete_enable;
      }
    ...
    class Namespace {
      ...
      librados::IoCtx io_ctx;
      int level;
      int complete_top_level;
      int complete_bottom_level;
      int64_t namespace_poolid = 0;
      string optag;
      string obj_name; // the object name
      string parent_oid; // the oid name of parent
      string file_name; // file name
      vector<string_extent> parents;
      vector<uint64_t> parent_versions; // for recording parent directory versions
    ...
    public:
      ...
      Namespace(RGWRados::Bucket *_target, const rgw_obj& _obj) :
          target(_target),
          obj_name(_obj.key.name) {
        store = target->get_store();
        without_namespace = (
            target->get_bucket_info().namespace_type != BUCKET_NAMESPACE_ENABLE ||
            store->cct->_conf->rgw_bucket_namespace_disable_force ||
            !_obj.key.ns.empty());
      }

      Namespace(RGWRados::Bucket *_target, const string& name) :
          target(_target),
          obj_name(name) {
        store = target->get_store();
        without_namespace = (
      ...
      inline librados::IoCtx& get_ioctx() { return io_ctx; }
      int init();

      int prepare(const req_state* s, RGWModifyOp op, const string *write_tag,
                  ceph::real_time& ut);
      int complete(int64_t poolid, uint64_t epoch, uint64_t size, ceph::real_time& ut,
                   bool appendable = false);
      int complete_del(int64_t poolid, uint64_t epoch, ceph::real_time* remove_time);
      // force_delete: delete namesapce entry when cancel
      int cancel(bool force_delete = false);
      string& get_obj() { return obj_name; }

      // NOTICE: we can success mkdir only when the parent directory is completed
      static int make_dir(RGWRados *store, const req_state *s);

      static int delete_dir(RGWRados::Object *target, const string& obj_name);

      static int list_dir(req_state* s, RGWRados::Bucket *target, const string& prefix,
                          const string& marker, const string& end_marker, int64_t max,
                          vector<rgw_bucket_dir_entry>* objs, 
                          map<string, bool>* common_prefixes, bool* is_truncated,
                          rgw_obj_key* next_marker);

      static int head_obj(RGWRados* store, RGWBucketInfo& bucket_info,
      ...
      static bool is_make_dir_request(req_state* s) {
        if (s->bucket_info.namespace_type == BUCKET_NAMESPACE_ENABLE &&
            s->object.endwith_slash() &&
    ...
    public:
      ...
      inline bool list_need_skip_namespace(bool enforce_ns,
                                           const string& ns,
                                           const string& prefix) {
        if (ns.empty() && enforce_ns && prefix.empty()) {
          return true;
        } else {
          return false;
        }
      ...
      int list_objects(int64_t max,
        ...
        if (params.allow_unordered) {
          return list_objects_unordered(max, result, common_prefixes, is_truncated);
        } else {
          return list_objects_ordered(max, result, common_prefixes, is_truncated);
        }
      }
  ...
  protected:
    ...
    int complete_atomic_modification(bool delay_remove_head_obj = false);

    int operate_op(librados::ObjectWriteOperation *op) {
      return store->obj_operate(bucket_info, obj, op);
    }

    int operate_op(librados::ObjectReadOperation *op) {
      return store->obj_operate(bucket_info, obj, op);
    }

    RGWObjState** get_inner_state() { return &state; }
  ...
  public:
    ...
    bool get_bilog_write_enable() {
      return bilog_write_enable;
    }

    bool get_bilog_delete_enable() {
      return bilog_delete_enable;
    }
    ...
    struct Write {
      ...
      struct MetaParams {
        ...
        const ceph::real_time *unmod_time{nullptr};
        bool appendable;
        bufferlist* tags_bl{nullptr};
      ...
      explicit Write(RGWRados::Object *_target) : target(_target), skip_cache(false) {}

      int _do_write_meta(uint64_t size, uint64_t accounted_size,
                         map<std::string, bufferlist>& attrs,
                         bool modify_tail, bool assume_noent,
                         void *index_op, void *_namespace_op, off_t ofs = -1);
      int write_meta(uint64_t size, uint64_t accounted_size,
                     map<std::string, bufferlist>& attrs, off_t ofs = -1);
      int write_meta_without_namespace(uint64_t size, uint64_t accounted_size,
                                       map<string, bufferlist>& attrs, off_t ofs = -1);
    ...
    struct Delete {
      ...
      struct DeleteParams {
        ...
        DeleteParams() : versioning_status(0), olh_epoch(0), bilog_flags(0),
                         ...
                         zones_trace(nullptr), completeMultipart(false) {}
      } params;
      ...
      // don't need to update bucket namesapce when delete part obj or delete multipart meta
      bool without_bucket_namespace = false;

      explicit Delete(RGWRados::Object *_target) : target(_target) {}

      inline bool is_namespace_dir(CephContext* cct) {
        if (cct->_conf->rgw_bucket_namespace_disable_force) {
          return false;
        }
        if (target->get_bucket_info().namespace_type != BUCKET_NAMESPACE_ENABLE) {
          return false;
        }
        string& obj_name = target->get_obj().key.name;
        if (obj_name.size() > 0 && obj_name[obj_name.size()-1] == S3_PATH_DELIMITER_CHAR) {
          return true;
        };
        return false;
      }

      int delete_obj(bool delete_directly = false);

      int delete_namespace_dir();
    };
    ...
    struct Rename {
      ...
      explicit Rename(RGWRados* _store, RGWRados::Object* _source, RGWRados::Object* _target)
        : source(_source), target(_target), store(_store) {}

      int set_src_obj_rename_attr(const string& src_tag, const string& dst_tag);

      int clear_rename_attr(const std::string& dst_tag);

      int reset_manifest_by_rename(librados::ObjectWriteOperation& wop,
                                   bufferlist& manifest_bl);

      int store_rename_dst_obj(ceph::real_time& mtime,
                               const string& src_tag,
                               const string& dst_tag,
                               bool assume_noent = true);

      void rename_backward_operation(const string& dst_tag,
                                     RGWRados::Bucket::UpdateIndex* src_idx = nullptr,
                                     RGWRados::Bucket::UpdateIndex* dst_idx = nullptr);

      void set_renaming_progress(RenameProgressBar _progress) { progress = _progress; }

      void change_owner_in_acl(const req_state* s, map<std::string, bufferlist>& attrs);
      int rm_dst_rename_attr_and_keep_mtime(rgw_rados_ref& dref,
                                            timespec& mtime_ts,
    ...
    private:
      RGWRados *store;
      RenameProgressBar progress = DONE_SRC_IDX;
  ...
  int aio_put_obj_data(void *ctx, rgw_raw_obj& obj, bufferlist& bl,
                        off_t ofs, bool exclusive, void **handle, bool skip_cache = false);
  ...
  int fetch_remote_obj(RGWObjectCtx& obj_ctx,
                       ...
                       void *progress_data,
                       rgw_zone_set *zones_trace= nullptr,
                       bool *not_modified = nullptr);

  int async_fetch_mirror_obj(req_state* s,
                             MirroringConfiguration& config,
                             RGWGetDataCB* get_data_cb);

  int fetch_mirror_obj(req_state* s,
                       real_time* mtime,            // return to client
                       map<string, bufferlist>& attrs,
                       string *version_id, // return to client
                       RGWGetDataCB* get_data_cb);

  inline std::pair<int, string>
  get_renaming_info(map<string, bufferlist>& attrs,
                    const string& id_tag) {
    for (auto iter = attrs.lower_bound(RGW_ATTR_RENAME_SOURCE);
         iter != attrs.end(); ++iter) {
      if (!boost::algorithm::starts_with(iter->first, RGW_ATTR_RENAME_SOURCE))
        return std::make_pair(ENOENT, "");
      ...
      try {
        ...
        ldout(cct, 0) << "ERROR: couldn't decode rename info, key" << iter->first << dendl;
        return std::make_pair(-EIO, "");
      }

      ldout(cct, 20) << "find id_tag in rename xattr src_idtag:" << id_tag
                     << ", idtag in rename_info:" << rename_info.src_idtag << dendl;
      if (rename_info.src_idtag.compare(id_tag) == 0) {
        return std::make_pair(0, std::move(iter->first));
      }
    }
    return std::make_pair(ENOENT, "");
  }

  int rename_obj(RGWObjectCtx& obj_ctx,
                 const rgw_obj& src_obj,
                 const rgw_obj& dst_obj,
                 const RGWBucketInfo& bucket_info);

  int is_renaming_conflict(RGWObjState* store);

  int process_src_renaming_attr(RGWObjectCtx *obj_ctx,
                                RGWObjState* s,
                                const RGWBucketInfo& bucket_info,
                                map<string, bufferlist>::iterator& iter,
                                int& result);

  int process_dst_renaming_attr(RGWObjectCtx *obj_ctx,
                                RGWObjState* s,
                                const RGWBucketInfo& bucket_info,
                                map<string, bufferlist>::iterator& iter);
  ...
  int copy_obj(RGWObjectCtx& obj_ctx,
               ...
               RGWBucketInfo& dest_bucket_info,
               RGWBucketInfo& src_bucket_info,
               const rgw_placement_rule& dest_placement_rule,
               ceph::real_time *src_mtime,
               ceph::real_time *mtime,
               const ceph::real_time *mod_ptr,
               const ceph::real_time *unmod_ptr,
  ...
  int bucket_namespace_delete_root(RGWBucketInfo& bucket_info);
  ...
  int delete_obj(RGWObjectCtx& obj_ctx,
                         ...
                         rgw_zone_set *zones_trace = nullptr,
                         bool delete_directly = false,
                         ceph::real_time* unmod_since = nullptr,
                         bool bilog_write_enable = true);

  /** Truncate a raw object.*/
  int truncate_raw_obj(const rgw_raw_obj& obj, uint64_t offset, bool skip_cache = false);

  /** Delete a raw object.*/
  int delete_raw_obj(const rgw_raw_obj& obj, bool skip_cache = false);
   ...
   * Returns: 0 on success, -ERR# otherwise.
   */
  inline int set_attr(void *ctx, const RGWBucketInfo& bucket_info, rgw_obj& obj, const char *name, bufferlist& bl);

  inline int remove_attr(void *ctx, const RGWBucketInfo& bucket_info,
                         rgw_obj& obj, map<string, bufferlist>& attrs,
                         const string& name, bool ignore_rename);

  int set_attrs(void *ctx, const RGWBucketInfo& bucket_info, rgw_obj& obj,
                        map<string, bufferlist>& attrs,
                        map<string, bufferlist>* rmattrs,
                        map<string, bufferlist>* cmpattrs = NULL,
                        bool ignore_rename = false);

  int waiting_for_renaming(RGWObjectCtx* obj_ctx, RGWObjState** astate,
                           const RGWBucketInfo& bucket_info, rgw_obj& obj,
                           bool wait_short = false);

  int check_renaming_state(RGWObjectCtx *obj_ctx, RGWObjState* s, const RGWBucketInfo& bucket_info);
  ...
  virtual int raw_obj_stat(rgw_raw_obj& obj, uint64_t *psize, ceph::real_time *pmtime, uint64_t *epoch,
                       ...
                       RGWObjVersionTracker *objv_tracker, bool filter_system_attr = true);
  virtual int raw_obj_stat_with_ctx(RGWObjectCtx *ctx, rgw_raw_obj& obj, RGWObjState *s, RGWObjVersionTracker *objv_tracker);
  ...
  int set_olh(RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info, const rgw_obj& target_obj, bool delete_marker, rgw_bucket_dir_entry_meta *meta,
              ...
              rgw_zone_set *zones_trace = nullptr, bool log_data_change = false, bool is_transition = false);
  ...
  int get_bucket_stats(RGWBucketInfo& bucket_info, int shard_id, string *bucket_ver, string *master_ver,
      ...
      map<std::string, RGWStorageStats>* storageclass_stats = NULL);
...
public:
  ...
  // Returns 0 on successful refresh. Returns error code if there was an
  ...
  int cls_obj_complete_op(BucketShard& bs, const rgw_obj& obj, RGWModifyOp op, string& tag, int64_t pool, uint64_t epoch,
                          ...
                          bool bilog_write_enable = true, bool bilog_delete_enable = true);
  int cls_obj_complete_add(BucketShard& bs, const rgw_obj& obj, string& tag, int64_t pool, uint64_t epoch, rgw_bucket_dir_entry& ent,
                           ...
                           rgw_zone_set *zones_trace = nullptr, bool bilog_write_enable = true, bool bilog_delete_enable = true);
  int cls_obj_complete_del(BucketShard& bs, string& tag, int64_t pool, uint64_t epoch, rgw_obj& obj,
                           ...
                           rgw_zone_set *zones_trace = nullptr, bool bilog_write_enable = true);
  int cls_obj_complete_cancel(BucketShard& bs, string& tag, rgw_obj& obj, uint16_t bilog_flags, rgw_zone_set *zones_trace = nullptr);

  // namespace
  int cls_namespace_prepare_op(librados::IoCtx& io_ctx, RGWModifyOp op, string& optag,
                               const string& obj_name, const string& parent_oid,
                               const string &child_name);
  int cls_namespace_prepare_op_excl(librados::IoCtx& io_ctx, RGWModifyOp op, string& optag,
                                    const string& obj_name, const string& parent_oid,
                                    const string &child_name);

  int cls_namespace_dir_prepare_op(librados::IoCtx& io_ctx, RGWModifyOp op, const string& optag,
                                   const string& parent_oid, const string& child_name,
                                   ceph::real_time& ut);
  int cls_namespace_dir_prepare_op_excl(librados::IoCtx& io_ctx, RGWModifyOp op,
                                        const string& optag, const string& parent_oid,
                                        const string& child_name, ceph::real_time& ut);
  int cls_namespace_dir_complete_add(librados::IoCtx& io_ctx, int64_t poolid, uint64_t epoch,
                                     const string& optag, const string& parent_oid,
                                     const string& child_name, ceph::real_time& ut);
  int cls_namespace_dir_complete_del(librados::IoCtx& io_ctx, int64_t poolid, uint64_t epoch,
                                     const string& optag, const string& parent_oid,
                                     const string& child_name, ceph::real_time& ut);
  int cls_namespace_dir_complete_cancel(librados::IoCtx& io_ctx, const string& optag,
                                        const string& parent_oid, const string& child_name,
                                        ceph::real_time& ut, bool force_delete);
  int cls_namespace_dir_make_prepare(librados::IoCtx& io_ctx, const string& dir_oid);
  int cls_namespace_dir_make_complete(librados::IoCtx& io_ctx, const string& dir_oid);
  int cls_namespace_dir_del(librados::IoCtx& io_ctx, const string& dir_oid);

  int cls_bucket_namespace_list(req_state* s, const RGWBucketInfo& bucket_info,
                                const string& parent_oid,
                                string& parent_name, size_t level, const string& marker,
                                const string& end_marker, map<string, bool>* common_prefixes,
                                vector<rgw_bucket_dir_entry>* objs, uint32_t max,
                                string& last_entry,  bool* is_truncated);
  int cls_bucket_namespace_head_op(RGWBucketInfo& bucket_info, const string& parent_oid,
                                   const string& parent_name, size_t level,
                                   const string& child_name, uint64_t& obj_size,
                                   ceph::real_time& lastmod, bool& is_dir, bool& is_appendable);
  int cls_namespace_complete_add(librados::IoCtx& io_ctx, const string& optag,
  ...
  int cls_namespace_complete_cancel(librados::IoCtx& io_ctx, const string& optag,
                                    const string& parent_oid, const string &file_name,
                                    bool force_delete);
  int cls_namespace_complete_del(librados::IoCtx& io_ctx, const string& optag,
                                 const string& parent_oid, const string& file_name,
                                 int64_t poolid, uint64_t epoch,
                                 ceph::real_time* remove_time);
  int cls_namespace_complete_op(librados::IoCtx& io_ctx, RGWModifyOp op, const string& parent_oid,
                                const string& file_name, int64_t poolid, uint64_t epoch,
                                const string& tag, uint64_t size, ceph::real_time* ut,
                                bool force_delete, bool appendable = false);
  ...
  int cls_bucket_list_ordered(RGWBucketInfo& bucket_info, int shard_id,
                              ...
                              bool skip_ns, // skip object with namespace when list objects
  ...
  int cls_obj_usage_log_read_total(string& oid, string& user, uint32_t max_entries, string& read_iter,
                                   map<rgw_user_bucket, rgw_usage_log_entry>& usage, bool *is_truncated);
  int cls_obj_usage_log_read_current_readop(string& oid, string& user, uint64_t end_epoch, map<rgw_user_bucket, rgw_usage_log_entry>& usage);
  int cls_obj_usage_log_read_readop(string& oid, string& user, uint64_t start_epoch, uint64_t end_epoch, uint32_t max_entries,
  ...
  int cls_obj_usage_log_trim_total(string& oid, string& user, string& bucket);
  int cls_obj_usage_log_trim_readop(string& oid, string& user, uint64_t start_epoch, uint64_t end_epoch);
  ...
  void update_gc_chain(rgw_obj& head_obj, RGWObjManifest& manifest, cls_rgw_obj_chain *chain,  RGWObjState *s = nullptr, bool delay_remove_head_obj = false);
  ...
  int process_lc();
  int list_lc_progress(string& marker, uint32_t max_entries, vector<cls_rgw_lc_entry>& progress_map, int& index);
  ...
  virtual librados::Rados* get_rados_handle();
 ...
 private:
  ...
  /**
   * Check the actual on-disk state of namespace file
   */
  int check_bucket_namespace_file_state(librados::IoCtx io_ctx,
                                        const RGWBucketInfo& bucket_info,
                                        const string& child_name,
  ...
  /**
   * Check the actual on-disk state of namespace dir
   */
  int check_bucket_namespace_dir_state(librados::IoCtx io_ctx, const string& child_name,
                                     const string& dir_oid, rgw_bucket_namespace_dir& dir_ent,
                                     bufferlist& suggested_updates);
   ...
   */
  uint32_t calc_ordered_bucket_list_per_shard(const uint32_t expansion_factor,
...
public:
  ...
  static RGWRados *get_storage(CephContext *cct, bool use_gc_thread, bool use_lc_thread, bool quota_threads,
			       bool run_sync_thread, bool run_reshard_thread, bool use_cache = true, bool admin_op = false) {
    RGWRados *store = init_storage_provider(cct, use_gc_thread, use_lc_thread, quota_threads, run_sync_thread,
					    run_reshard_thread, use_cache, admin_op);
    return store;
  }
  ...
  static RGWRados *init_storage_provider(CephContext *cct, bool use_gc_thread, bool use_lc_thread, bool quota_threads, bool run_sync_thread, bool run_reshard_thread, bool use_metadata_cache, bool admin_op);
...
public:
  ...
  virtual int pre_handle_data(bufferlist& bl, bool *has_tail) {
    return 0;
  };
  virtual bool sent_first_chunk() {
    return false;
  };
  virtual void set_crypt(std::unique_ptr<BlockCrypt>* block_crypt) {
    crypt = block_crypt;
  }
  std::unique_ptr<BlockCrypt>* crypt = nullptr;
}; /* RGWPutObjDataProcessor */
...
protected:
  uint64_t first_obj_offset{0};
  uint64_t obj_len{0};

  set<rgw_raw_obj> written_objs;
  rgw_obj head_obj;
  bool enable_skip_cache{false};
  uint64_t file_shuntflow_size{0};
...
public:
  ...
  void set_skip_cache_flag(bool flag) { enable_skip_cache = flag; }
  virtual bool get_skip_cache_flag() { return enable_skip_cache; }
  void set_file_shuntflow_size(uint64_t size) { file_shuntflow_size = size; }
...
protected:
  ...
  string version_id;
  bool is_transition{false};
  const real_time *unmod_time{nullptr};

  uint64_t content_length;
  bool unknown_actual_size{false};
...
protected:
  ...
  int complete_parts();
  int complete_writing_data(bool need_append_head_obj = false);

  int prepare_init(RGWRados *store, string *oid_rand);

  virtual void try_file_shuntflow();
...
public:
  ...
  bool sent_first_chunk() override {
  ...
  void set_placement_rule(const rgw_placement_rule& rule) {
    tail_placement_rule = rule;
  }

  void set_is_transition(bool val) {
    is_transition = val;
  }

  void set_unmod_time(const real_time *_unmod_time) {
    unmod_time = _unmod_time;
  }

  void set_content_length(uint64_t _content_length) {
    content_length = _content_length;
  }

  void set_unknown_actual_size(bool _unknown_actual_size) {
    unknown_actual_size = _unknown_actual_size;
  }
...
{
  string supplied_request_id;
  string cur_etag;
  bufferlist cur_idtag;
  bool supplied_request_id_exist;
  bool head_obj_exist;
  bool cover_obj;
  bool need_append_head_obj;
  bool head_obj_updated;
  bool need_cmpxattr;
  uint64_t supplied_offset;
  uint64_t content_length;
  uint64_t append_times;

  /* first_chunk_ofs
   * -2, default, no need append data to head_obj
   * -1, append data to new head_obj
   * >=0, append data to existed head_obj
   * */
  off_t first_chunk_ofs;
...
protected:
  ...
  int do_complete(size_t accounted_size, const string& etag,
                  ...
                  rgw_zone_set *zones_trace);
public:
  RGWPutObjProcessor_Append(RGWObjectCtx& obj_ctx, RGWBucketInfo& bucket_info,
      uint64_t _p, req_state *_s, uint64_t offset, uint64_t *cur_accounted_size) :
                   RGWPutObjProcessor_Atomic(obj_ctx, bucket_info, _s->bucket,
                       _s->object.name, _p, _s->req_id, false),
                   supplied_request_id(_s->info.env->get("HTTP_X_BCE_REQUEST_ID", "")),
                   supplied_request_id_exist(_s->info.env->exists("HTTP_X_BCE_REQUEST_ID")),
                   head_obj_exist(true),
                   cover_obj(false),
                   need_append_head_obj(false),
                   head_obj_updated(false),
                   need_cmpxattr(true),
                   supplied_offset(offset),
                   content_length(_s->content_length),
                   //pre_data_ofs(0),
                   append_times(0),
                   first_chunk_ofs(-2),
                   cur_accounted_size(cur_accounted_size) {}
  int pre_handle_data(bufferlist& bl, bool *has_tail) override;
  ~RGWPutObjProcessor_Append() override;
  uint64_t get_offset() {
    return supplied_offset;
  }
}; /* RGWPutObjProcessor_Append */
...
class RGWPutObjProcessor_Multipart : public RGWPutObjProcessor_Atomic
{
  ...
  req_state *s;
  string upload_id;
  bool repeat_part{false};

protected:
  int rewrite_repeated_part_omap_key(RGWRados *store, string p, rgw_raw_obj raw_meta_obj,  bufferlist & bl);
...
struct RGWClients {
  // rgw_client_id: service_name + RGW_CLIENT_ID_SEPARATOR + start_epoch
  std::string rgw_client_id = "";
  std::unordered_set<std::string> rgws;
  RWLock lock = RWLock("rgwclients");
  ceph::mono_time update_time;

  RGWClients() {}
  virtual ~RGWClients() {}
};

extern RGWClients rgw_client;

inline int try_refresh_client_id() {
  ThrottleManager::instance().get_rgw_nums();
  while (rgw_client.rgw_client_id.empty()) {
    std::this_thread::sleep_for(std::chrono::milliseconds(300));
    int ret = ThrottleManager::instance().get_rgw_nums();
    if (ret < 0) {
      dout(0) << __func__ << "() ERROR: get_rgw_nums error:" << ret << dendl;
      return ret;
    }
  }
  return 0;
