...
static int parse_grantee_str_for_bos(RGWRados *store, string& grantee_str,
        const struct s3_acl_header *perm, ACLGrant& grant)
{
  ...
  if (id_val == "*") {
    id_type = "uri";
    id_val = RGW_URI_ALL_USERS;
  }

  if (strcasecmp(id_type.c_str(), "id") == 0) {
    rgw_user user(id_val);

    grant.set_canon(user, "", rgw_perm);
...
#ifdef WITH_BCEBOS
static int parse_acl_header_for_bos(RGWRados *store, const RGWEnv *env,
         const struct s3_acl_header *perm, std::list<ACLGrant>& _grants)
{
  ...
  ACLGrant grant;
  int ret = parse_grantee_str_for_bos(store, grantee, perm, grant);
  if (ret < 0)
    return ret;
...
static const s3_acl_header acl_header_perms[] = {
  {RGW_PERM_READ, "HTTP_X_BCE_GRANT_READ"},
  {RGW_PERM_WRITE, "HTTP_X_BCE_GRANT_WRITE"},
  {RGW_PERM_FULL_CONTROL, "HTTP_X_BCE_GRANT_FULL_CONTROL"},
...
int RGWAccessControlPolicy_S3::create_from_headers(RGWRados *store, req_state *s, ACLOwner& _owner)
{
  ...
  for (const struct s3_acl_header *p = acl_header_perms; p->rgw_perm; p++) {
    ...
    if (s->prot_flags & RGW_REST_BOS) {
      r = parse_acl_header_for_bos(store, s->info.env, p, grants);
    } else
#endif
    {
      r = parse_acl_header(store, s->info.env, p, grants);
    }
...
int RGWAccessControlPolicy_S3::rebuild_with_bos(RGWRados *store, ACLOwner *owner, RGWAccessControlPolicy& dest)
{
  if (owner) {
        ...
        if (type.get_type() == ACL_TYPE_CANON_USER) {
          if (!src_grant.get_id(uid)) {
            if (owner) {
              ...
              uid = dest.get_owner().get_id();
            }
          }
        }
    
        ACLPermission& perm = src_grant.get_permission();
        new_grant.set_canon(uid, "", perm.get_permissions());
        grant_ok = true;
        rgw_user new_id;
        new_grant.get_id(new_id);
        ldout(cct, 10) << "new grant: " << new_id << dendl;
      }
      break;
    case ACL_TYPE_GROUP:
      {
