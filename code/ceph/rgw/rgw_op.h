...
public:
  ...
  virtual int serve_retarget(RGWOp *op) { return 0; }
};
...
public:
  ...
  void set_ret(int ret) { op_ret = ret; }
  ...
  bool should_retarget() { return website_retarget; }

  virtual int verify_params() { return 0; }
  virtual bool prefetch_data() { return false; }
  virtual bool is_head_obj() { return false; }
  ...
  virtual int verify_op_ban_stat();
  ...
  int process_callback(const string& etag, const string& contentType);
  void generate_callback_body(string& request_body, const string& vars,
      const string& etag, const string& contentType);
  void generate_notification_body(string& request_body, const string& vars, const string& etag);
  int send_notification(rgw_notification_entry& entry, const string& etag);
  int handle_notification(RGWNotification& n, const string& etag,
        string event, const string& obj_name, const string& bucket_name);
  int get_bucket_notification(bufferlist& notification_bl);
  int get_object_lock_configure(req_state* s,
                                RGWObjectRetention** retention,
                                RGWObjectLegalHold** legal_hold);
  virtual int serve_errordoc(RGWHandler * const handler) { return 0; }
};
...
private:
  ...
  int convert_to_timg_format(const std::string& origin_request, std::string& result);
  int forward_to_timg(const std::string& origin_request);

enum image_error_code {
  IMAGE_OK = 0,
  IMAGE_INTERNAL_ERROR = 1000,
  IMAGE_INVALID_PARAMS = 1003,
  IMAGE_TOO_LARGE = 2500,
  IMAGE_INVALID,
};
...
protected:
  ...
  ceph::real_time symlink_lastmod;
  ceph::real_time unmod_time;
  ceph::real_time *mod_ptr;
  ceph::real_time *unmod_ptr;
  map<string, bufferlist> attrs;
  map<string, bufferlist> symlink_attrs;
  bool get_data;
  ...
  bool exists = true;
  bool retarget_op{false};

  int init_common();
  string generate_timg_req(string image_process_req);
  int verify_image_protection();
  bool is_chunk{false};
public:
  RGWGetObj() {
    range_str = NULL;
  ...
  // when namespace is enable, we can head dirctory
  // we don't need to read object policy
  // because dirctory don't have object policy
  bool is_head_obj() override { return !get_data; }

  void head_namespce_obj(bufferlist& bl);
  ...
  int verify_target_object_permission(std::string& target_object_name, std::string& target_bucket_name);
  ...
  void set_retarget(bool retarget) {
    retarget_op = retarget;
  }
};
...
public:
  ...
  void set_content_length(uint64_t len) {
    op->set_content_length(len);
  }

  void set_attr(const string& key, const string& value) {
    bufferlist bl;
    bl.append(value.c_str(), value.length());
    op->get_attrs()[key] = std::move(bl);
  }
  ...
  void send_response_error(int ret) {
    op->set_ret(ret);
    op->send_response_data_error();
  }
...
class RGWListRgw : public RGWOp {
protected:
  int epoch;
  string last_modified;
  vector<std::string> rgw_ip;
  vector<std::string> rgw_port;
...
public:
  ...
  void send_response() override = 0;
  const std::string name() override { return "list_rgw"; }
  RGWOpType get_type() override { return RGW_OP_LIST_RGW; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};
...
protected:
  ...
  bool is_bucket_namespace_list = false;
...
protected:
  ...
  vector<string> support_suffixs{".jpg", ".html", ".png", ".bmp", ".webp"};
public:
  RGWSetBucketWebsite() {}
  ...
  bool check_suffix(const string& path) {
    for (auto suffix : support_suffixs) {
      if (suffix.length() > path.length()) {
        continue;
      }
      std::size_t index = path.find(suffix, path.length() - suffix.length());
...
class RGWGetBucketNamespace : public RGWOp {
public:
  RGWGetBucketNamespace() {}
  ...
  void send_response() override = 0;
  const string name() override { return "get_bucket_namespace"; }
  RGWOpType get_type() override { return RGW_OP_GET_BUCKET_NAMESPACE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWSetBucketNamespace : public RGWOp {
protected:
  bufferlist in_data;
public:
  RGWSetBucketNamespace() {}
  ...
  void send_response() override = 0;
  const string name() override { return "set_bucket_namespace"; }
  RGWOpType get_type() override { return RGW_OP_SET_BUCKET_NAMESPACE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWDeleteBucketNamespace : public RGWOp {
public:
  RGWDeleteBucketNamespace() {}
  ...
  void send_response() override = 0;
  const string name() override { return "delete_bucket_namespace"; }
  RGWOpType get_type() override { return RGW_OP_SET_BUCKET_NAMESPACE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWPutImageStyle : public RGWOp {
protected:
  bufferlist in_data;
public:
  RGWPutImageStyle() {}
  ...
  void send_response() override = 0;
  const string name() override { return "put_image_style"; }
  RGWOpType get_type() override { return RGW_OP_PUT_IMAGE_STYLE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWDeleteImageStyle : public RGWOp {
public:
  RGWDeleteImageStyle() {}
  ...
  void send_response() override = 0;
  const string name() override { return "delete_image_style"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_IMAGE_STYLE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWImageStyle;

class RGWGetImageStyle : public RGWOp {
protected:
  RGWImageStyle style;
public:
  RGWGetImageStyle() {}
  ...
  void send_response() override = 0;
  const string name() override { return "get_image_style"; }
  RGWOpType get_type() override { return RGW_OP_GET_IMAGE_STYLE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWListImageStyle : public RGWOp {
protected:
  vector<RGWImageStyle> styles;
public:
  RGWListImageStyle() {}
  ...
  void send_response() override = 0;
  const string name() override { return "list_image_style"; }
  RGWOpType get_type() override { return RGW_OP_LIST_IMAGE_STYLE; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWPutImageProtection : public RGWOp {
protected:
  bufferlist in_data;
public:
  RGWPutImageProtection() {}
  ...
  void send_response() override = 0;
  const string name() override { return "put_image_protection"; }
  RGWOpType get_type() override { return RGW_OP_PUT_IMAGE_PROTECTION; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWGetImageProtection : public RGWOp {
protected:
  vector<string> resources;
public:
  RGWGetImageProtection() {}
  ...
  void send_response() override = 0;
  const string name() override { return "get_image_protection"; }
  RGWOpType get_type() override { return RGW_OP_GET_IMAGE_PROTECTION; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWDeleteImageProtection : public RGWOp {
public:
  RGWDeleteImageProtection() {}
  ...
  void send_response() override = 0;
  const string name() override { return "delete_image_protection"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_IMAGE_PROTECTION; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};
...
public:
  RGWCreateBucket() : has_cors(false), obj_lock_enabled(false) {}
...
class RGWGetBucketStorageClass : public RGWOp {
public:
  RGWGetBucketStorageClass() {}
  ...
  void send_response() override = 0;
  const string name() override { return "get_bucket_storage_class"; }
  RGWOpType get_type() override { return RGW_OP_GET_BUCKET_STORAGE_CLASS; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWPutBucketStorageClass : public RGWOp {
public:
  RGWPutBucketStorageClass() {}
 ...
 virtual int get_params() { return 0; }
  ...
  void send_response() override = 0;
  const string name() override { return "put_bucket_storage_class"; }
  RGWOpType get_type() override { return RGW_OP_PUT_BUCKET_STORAGE_CLASS; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};
...
class RGWPutBucketLogging : public RGWOp {
protected:
  string target_bucket;
  string target_prefix;
...
class RGWDeleteBucketLogging : public RGWOp {
public:
  RGWDeleteBucketLogging() {}
  ...
  void send_response() override = 0;
  const string name() override { return "delete_bucket_logging"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_BUCKET_LOGGING; }
  uint32_t op_mask() override { return RGW_OP_TYPE_DELETE; }
};
...
public:
  ...
  void send_response() override = 0;
  const string name() override { return "get_bucket_encryption"; }
...
public:
  ...
  void send_response() override = 0;
  const string name() override { return "put_bucket_encryption"; }
...
public:
  ...
  void send_response() override = 0;
  const string name() override { return "delete_bucket_encryption"; }
...
public:
  ...
  virtual RGWPutObjProcessor *select_processor(RGWObjectCtx& obj_ctx, int* const op_ret = nullptr);
...
public:
  ...
  const string name() override { return "rename_obj"; }
  RGWOpType get_type() override { return RGW_OP_RENAME_OBJ; }
  uint32_t op_mask() override {return RGW_OP_TYPE_WRITE; }

  bool prefetch_data() override { return true; }
};

class RGWGetBucketTrash : public RGWOp {
public:
  RGWGetBucketTrash() {}
  ...
  void send_response() override = 0;
  const string name() override { return "get_bucket_trash"; }
  RGWOpType get_type() override { return RGW_OP_GET_BUCKET_TRASH; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWPutBucketTrash : public RGWOp {
protected:
  string trash_dir;
...
public:
  ...
  void send_response() override = 0;
  const string name() override { return "put_bucket_trash"; }
  RGWOpType get_type() override { return RGW_OP_PUT_BUCKET_TRASH; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWDeleteBucketTrash : public RGWOp {
public:
  RGWDeleteBucketTrash() {}
  ...
  void send_response() override = 0;
  const string name() override { return "delete_bucket_trash"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_BUCKET_TRASH; }
  uint32_t op_mask() override { return RGW_OP_TYPE_DELETE; }
};
...
protected:
  string acls;
  RGWAccessControlPolicy *bos_acl;
...
class RGWDeleteACLs : public RGWOp {
protected:
  ACLOwner owner;
public:
  RGWDeleteACLs() {}
  ...
  void send_response() override = 0;
  const string name() override { return "delete_acls"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_ACLS; }
  uint32_t op_mask() override { return RGW_OP_TYPE_DELETE; }
};
...
class RGWPutBucketMirroring : public RGWOp {
protected:
  int len;
  char *data;

public:
  RGWPutBucketMirroring() {
    len = 0;
    data = NULL;
  }
  ~RGWPutBucketMirroring() override {
    if (data) {
      free(data);
    }
    data = NULL;
  ...
  virtual int get_params() = 0;
  void send_response() override = 0;
  const string name() override { return "put_mirroring"; }
  RGWOpType get_type() override { return RGW_OP_PUT_BUCKET_MIRRORING; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWGetBucketMirroring : public RGWOp {
protected:
  RGWMirroringConfiguration config;

public:
  RGWGetBucketMirroring() {}
  ~RGWGetBucketMirroring() override {}
  ...
  void send_response() override = 0;
  const string name() override { return "get_mirroring"; }
  RGWOpType get_type() override { return RGW_OP_GET_BUCKET_MIRRORING; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWDeleteBucketMirroring : public RGWOp {
public:
  RGWDeleteBucketMirroring() {};
  ~RGWDeleteBucketMirroring() override {}
  ...
  void execute() override;
  const std::string name() override {
    return "delete_mirroring";
  }
  RGWOpType get_type() override {
    return RGW_OP_PUT_BUCKET_MIRRORING;
  }
};
...
//  virtual int get_policy_from_state(RGWRados *store, struct req_state *s, stringstream& ss) { return 0; }
  ...
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
  int valid_lifecycle_placement(RGWLifecycleConfiguration& config);
};
...
public:
  RGWInitMultipart() : obj_retention(nullptr),
                       obj_legal_hold(nullptr) {}
...
protected:
  friend class RGWMultiJSONParser;
  string upload_id;
  string etag;
  string version_id;
  char *data;
...
extern int rgw_build_object_policies(RGWRados *store, struct req_state *s,
                                     bool prefetch_data, bool is_head_op=false);
extern rgw::IAM::Environment rgw_build_iam_environment(RGWRados* store,
						       struct req_state* s);

extern int read_bucket_policy(RGWRados *store,
                       struct req_state *s,
                                        ...
                                        bool& need_to_wait)
{
  bool again = false;
  bool has_tail = true;
  int ret = processor->pre_handle_data(data, &has_tail);
  if (ret < 0) {
    return ret;
  }
  if (!has_tail) {
  ...
  do {
    ...
    if (handle != nullptr) {
      ...
      // set need_to_wait false after sent first chunk
      if (need_to_wait && processor->sent_first_chunk()) {
        need_to_wait = false;
      }
      if (ret < 0) {
        return ret;
...
class RGWInitBucketObjectLock : public RGWOp {
protected:
...
public:
  ...
  virtual void send_response() = 0;
  virtual int get_params() = 0;
  const std::string name() override {return "init_bucket_object_lock"; }
  RGWOpType get_type() override { return RGW_OP_INIT_BUCKET_OBJ_LOCK; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};
...
public:
  ...
  virtual void send_response() = 0;
  virtual int get_params() = 0;
  const std::string name() override { return "put_bucket_object_lock"; }
...
public:
  ...
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override {return "get_bucket_object_lock"; }
  RGWOpType get_type() override { return RGW_OP_GET_BUCKET_OBJ_LOCK; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ; }
};

class RGWDeleteBucketObjectLock : public RGWOp {
public:
  int verify_permission() override;
  void pre_exec() override;
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override {return "delete_bucket_object_lock"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_BUCKET_OBJ_LOCK; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWCompleteBucketObjectLock : public RGWOp {
public:
  int verify_permission() override;
  void pre_exec() override;
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override {return "complete_bucket_object_lock"; }
  RGWOpType get_type() override { return RGW_OP_COMPLETE_BUCKET_OBJ_LOCK; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWExtendBucketObjectLock : public RGWOp {
protected:
  int64_t extend_retention_days;
public:
  int verify_permission() override;
  void pre_exec() override;
  void execute() override;
  virtual int get_params() = 0;
  virtual void send_response() = 0;
  const std::string name() override {return "extend_bucket_object_lock"; }
  RGWOpType get_type() override { return RGW_OP_EXTEND_BUCKET_OBJ_LOCK; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};
...
public:
  ...
  virtual int get_params() = 0;
  const std::string name() override { return "put_obj_retention"; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
...
public:
  ...
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override {return "get_obj_retention"; }
...
public:
  ...
  virtual void send_response() override = 0;
  virtual int get_params() = 0;
  const std::string name() override { return "put_obj_legal_hold"; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
...
public:
  ...
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override {return "get_obj_legal_hold"; }
...
class RGWBanControl : public RGWControl {
public:
  RGWBanControl() = default;
  void execute() override;
  int get_params();
  const string name() override { return "ban_control"; }
  RGWOpType get_type() override { return RGW_OP_BAN_CONTROL; }
};

class RGWUnBanControl : public RGWControl {
public:
  RGWUnBanControl() = default;
  void execute() override;
  int get_params();
  const string name() override { return "unban_control"; }
  RGWOpType get_type() override { return RGW_OP_UNBAN_CONTROL; }
};
...
public:
  RGWPutBucketNotification() = default;
  ...
  virtual int get_params() = 0;
  const std::string name() override {
    return "put_bucket_notification";
  }
  RGWOpType get_type() override {
    return RGW_OP_PUT_BUCKET_NOTIFICATION;
  }
};
...
protected:
  bufferlist notification_bl;
public:
  RGWGetBucketNotification() = default;
  void send_response() override = 0;
  int verify_permission() override;
  uint32_t op_mask() override {
    return RGW_OP_TYPE_READ;
  ...
  void execute() override;
  const std::string name() override {
    return "get_bucket_notification";
  }
  RGWOpType get_type() override {
    return RGW_OP_GET_BUCKET_NOTIFICATION;
  }
};

class RGWDeleteBucketNotification : public RGWOp {
public:
  RGWDeleteBucketNotification() = default;
  void send_response() override = 0;
  int verify_permission() override;
  uint32_t op_mask() override {
    return RGW_OP_TYPE_WRITE;
  ...
  void execute() override;
  const std::string name() override {
    return "delete_bucket_notification";
  }
  RGWOpType get_type() override {
    return RGW_OP_DELETE_BUCKET_NOTIFICATION;
  }
};

class RGWPutBucketQuota : public RGWOp {
protected:
  int64_t max_size_mb;
  int64_t max_size_kb;
  int64_t max_objects;
  ...
  bufferlist in_data;
public:
  RGWPutBucketQuota() = default;
  ~RGWPutBucketQuota() {}

  virtual int verify_permission() = 0;
  virtual int get_params() = 0;
  virtual int check_quota_params() = 0;
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override { return "put_bucket_quata"; }
  RGWOpType get_type() override { return RGW_OP_PUT_BUCKET_QUOTA; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE;}
};

class RGWGetBucketQuota : public RGWOp {
public:
  RGWGetBucketQuota() = default;
  ~RGWGetBucketQuota() {}

  virtual int verify_permission() = 0;
  void execute() override{};
  virtual void send_response() = 0;
  const std::string name() override { return "get_bucket_quata"; }
  RGWOpType get_type() override { return RGW_OP_GET_BUCKET_QUOTA; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ;}
};

class RGWDeleteBucketQuota : public RGWOp {
public:
  RGWDeleteBucketQuota() = default;
  ~RGWDeleteBucketQuota() {}

  virtual int verify_permission() = 0;
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override { return "delete_bucket_quata"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_BUCKET_QUOTA; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE;}
};

class RGWPutUserQuota : public RGWOp {
protected:
  std::string uid_str;
  int64_t max_size_mb;
  int64_t max_size_kb;
  int64_t max_objects;
  int32_t max_bucket_count;
  RGWUserInfo user_info;
public:
  RGWPutUserQuota() = default;
  ~RGWPutUserQuota() {}
  ...
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override { return "put_user_quata"; }
  RGWOpType get_type() override { return RGW_OP_PUT_USER_QUOTA; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWGetUserQuota : public RGWOp {
protected:
  RGWUserInfo user_info;
public:
  RGWGetUserQuota() = default;
  ~RGWGetUserQuota() {}

  virtual int verify_permission() = 0;
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override { return "get_user_quata"; }
  RGWOpType get_type() override { return RGW_OP_GET_USER_QUOTA; }
  uint32_t op_mask() override { return RGW_OP_TYPE_READ;}
};

class RGWDeleteUserQuota : public RGWOp {
public:
  RGWDeleteUserQuota() = default;
  ~RGWDeleteUserQuota() {}

  virtual int verify_permission() = 0;
  void execute() override;
  virtual void send_response() = 0;
  const std::string name() override { return "delete_user_quata"; }
  RGWOpType get_type() override { return RGW_OP_DELETE_USER_QUOTA; }
  uint32_t op_mask() override { return RGW_OP_TYPE_WRITE; }
};

class RGWGetSymlink : public RGWOp {
 protected:
  std::string target_object;
  std::string target_bucket;
  std::string target_bucket_tenant;
  RGWObjState *state{nullptr};
 public:
  RGWGetSymlink() = default;
  ~RGWGetSymlink() {}
  void init(RGWRados *store, struct req_state *s, RGWHandler *h) override {
    RGWOp::init(store, s, h);
  }
  ...
  virtual void send_response() = 0;
  const string name() { return "get_symlink"; }
  uint32_t op_mask() { return RGW_OP_TYPE_READ; }
  RGWOpType get_type() { return RGW_OP_GET_SYMLINK_OBJ; }
};

class RGWPutSymlink : public RGWOp {
protected:
  std::string target_bucket_name;
  std::string target_bucket_tenant;
  std::string target_object_name;
  std::string storage_class;
  std::string target_etag;
  map<string, bufferlist> attrs;
  RGWAccessControlPolicy policy;
  rgw_obj_key target_object;
  rgw_bucket target_bucket;
  RGWBucketInfo target_bucket_info;
  std::map<std::string, ceph::bufferlist> target_bucket_attrs;
  bool enable_overwrite{true};
  void emplace_attr(std::string&& key, buffer::list&& bl) {
    attrs.emplace(std::move(key), std::move(bl)); /* key and bl are r-value refs */
  }
public:
  RGWPutSymlink() = default;
  ~RGWPutSymlink() {}
  ...
  virtual void send_response() {}
  virtual int get_params() = 0;
  const string name() { return "put_symlink"; }
  uint32_t op_mask() { return RGW_OP_TYPE_WRITE; }
  RGWOpType get_type() { return RGW_OP_PUT_SYMLINK_OBJ; }
};

int notify_bcm_resource(const req_state* s, bool is_create);

extern int verify_object_permission(req_state* s,
                                    RGWRados* store,
                                    string& bucket_name,
                                    string& object_name,
                                    std::uint64_t action,
