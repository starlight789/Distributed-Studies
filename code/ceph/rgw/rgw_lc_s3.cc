...
void LCFilter_S3::decode_xml(XMLObj *obj)
{
  ...
  RGWXMLDecoder::decode_xml("Suffix", suffix, o);
  if (!prefix.empty())
    num_conditions++;
...
int RGWLifecycleConfiguration_S3::fill_suffix(req_state* s)
{
  if (!s->is_from_platform_frontend()) {
    return 0;
  }

  const static string lc_suffix = "HTTP_X_AMZ_LC_SUFFIX_";
  const std::map<string, string, ltstr_nocase>& env_map = s->info.env->get_map();
  auto iter = env_map.lower_bound(lc_suffix);

  while (iter != env_map.end()) {
    if (!boost::algorithm::starts_with(iter->first, lc_suffix)) {
      break;
    }

    string rule_id;
    std::transform(iter->first.begin() + lc_suffix.length(), iter->first.end(),
                   std::back_inserter(rule_id), [](const int c) {
                     return c == '-' ? '_' : (c == '_' ? '-' : std::tolower(c));
                   });
    multimap<string, LCRule>::iterator rule_iter;
    bool found = false;
    for (rule_iter = rule_map.begin(); rule_iter != rule_map.end(); ++rule_iter) {
      if (rule_iter->second.get_id().compare(rule_id) == 0) {
        rule_iter->second.get_non_const_filter().set_suffix(iter->second);
        found = true;
        break;
      }
    }
...
int RGWLifecycleConfiguration_S3::rebuild(RGWRados *store, RGWLifecycleConfiguration& dest)
{
  ...
  for (iter = rule_map.begin(); iter != rule_map.end(); ++iter) {
    ...
    if (ret < 0) {
      dout(10) << __func__ << " check rule err:" << ret << dendl;
      return ret;
    }
  }
  /* no need to valid conflic
  if (!dest.valid()) {
