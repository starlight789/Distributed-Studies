...
class RGWReadSyncStatusMarkerCR : public RGWCoroutine {
  RGWMetaSyncEnv *sync_env;
  int shard_id;
  rgw_meta_sync_marker& marker;

 public:
  RGWReadSyncStatusMarkerCR(RGWMetaSyncEnv *_sync_env, int _shard_id,
                            rgw_meta_sync_marker& _marker)
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env),
      shard_id(_shard_id), marker(_marker)
  {}
  int operate() override;
};

int RGWReadSyncStatusMarkerCR::operate() {
  reenter(this) {
    using CR = RGWSimpleRadosReadCR<rgw_meta_sync_marker>;
    yield {
      rgw_raw_obj obj{sync_env->store->get_zone_params().log_pool,
                      sync_env->shard_obj_name(shard_id)};
      call(new CR(sync_env->async_rados, sync_env->store, obj, &marker, false));
    }
    if (retcode < 0) {
...
class RGWMetaSyncShardCR : public RGWCoroutine {
  ...
  bool need_refresh_meta_sync_marker = false;
...
#define INCREMENTAL_MAX_ENTRIES 100
        /* when need_refresh_meta_sync_marker = true, we just seized lease from
         * another RGW, and local markers are behind
         */
        if (need_refresh_meta_sync_marker) {
          need_refresh_meta_sync_marker = false;
          temp_marker = sync_marker;
          yield call(new RGWReadSyncStatusMarkerCR(sync_env, shard_id, *temp_marker));
          if (retcode < 0) {
            tn->log(10, SSTR(*this << ": failed to refresh sync status marker, retcode=" << retcode));
            yield lease_cr->go_down();
            drain_all();
          ...
          // FIXME: should we need more check here!
          sync_marker = std::move(*temp_marker);
          temp_marker = boost::none;
          if (sync_marker.state != rgw_meta_sync_marker::IncrementalSync ||
              sync_marker.realm_epoch != realm_epoch) {
            tn->log(10, SSTR(*this << ": conflict sync state"));
            yield lease_cr->go_down();
            drain_all();
            *reset_backoff = true;
            return -EAGAIN;
	...
	if (mdlog_marker > max_marker) {
          ...
          if (log_entries.empty() && !truncated) {
            tn->log(10, SSTR(*this << ": empty mdlog entries, sync marker maybe stale, need refresh"));
            need_refresh_meta_sync_marker = true;
            continue;
          }
