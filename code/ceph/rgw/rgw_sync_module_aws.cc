...
struct AWSSyncConfig_S3 {
  ...
  int init(CephContext *cct, const JSONFormattable& config) {
    ...
    if (r < 0) {
      dout(0) << "ERROR: multipart_sync_threshold " << multipart_sync_threshold
              << " conf_to_uint64 error:" << r << dendl;
      return r;
    }

    r = conf_to_uint64(cct, config, "multipart_min_part_size", &multipart_min_part_size);
    if (r < 0) {
      dout(0) << "ERROR: multipart_min_part_size " << multipart_min_part_size
              << " conf_to_uint64 error:" << r << dendl;
      return r;
    }
...
struct AWSSyncConfig {
  ...
  int init_target(CephContext *cct, const JSONFormattable& profile_conf, std::shared_ptr<AWSSyncConfig_Profile> *ptarget) {
    ...
    if (ret < 0) {
      ldout(cct, 0) << "ERROR: init_profile error:" << ret << dendl;
      return ret;
    }
  ...
  void init_conns(RGWDataSyncEnv *sync_env, const string& id) {
    ...
    if (root_conf->endpoint.find("bos") != string::npos) {
      dout(0) << "NOTICE: consider endpoint as bos public cloud:" << root_conf->endpoint << dendl;
      bos_cloud = true;
    }
    ...
    for (auto i : explicit_profiles) {
      auto& c = i.second;
      if (c->conn_conf->endpoint.find("bos") != string::npos) {
        dout(0) << "NOTICE: consider endpoint as bos public cloud" << c->conn_conf->endpoint << dendl;
        bos_cloud = true;
      }
...
static int do_decode_rest_obj(CephContext *cct, map<string, bufferlist>& attrs, map<string, string>& headers, rgw_rest_obj *info)
{
  ...
  if (aiter != attrs.end()) {
    ...
    ldout(cct, 10) << "WARNING: acl attrs not provided" << dendl;
  }
  if (bos_cloud) {
    aiter = attrs.find(RGW_ATTR_STORAGE_CLASS);
    if (aiter != attrs.end()) {
      string storage_class = rgw_bl_to_str(aiter->second);
      if (storage_class.compare(RGWStorageClass::STANDARD_HP) == 0) {
...
public:
  ...
  int init() override {
    ...
    /*
     * Sync to public cloud with decrypted data in KMS and RGW-AUTO mode.
     * Only client put object with individual explicit encrypt key, we couldn't
     *  decrypte data by self. Against this mode, we store encrypted data to public cloud.
     * */
    ...
    if (range.is_set) {
      ...
      if (range.size == 0) {
        req_params.range_end = range.ofs;
      } else {
...
public:
  ...
  static void init_send_attrs(CephContext *cct,
    ...
    for (auto& hi : rest_obj.attrs) {
      ldout(cct, 20) << "rest_obj attrs: " << hi.first << "=" << hi.second << dendl;
...
public:
  ...
  int operate() {
    reenter(this) {
      ...
      if (retcode < 0) {
        if (bos_cloud && retcode == -ERR_INVALID_STORAGE_CLASS) {
          ldout(cct, 20) << "WARNING: sync to bos cloud not support STANDARD_HP storage_class:" << src_obj << dendl;
          return set_cr_done();
        }
        ldout(cct, 20) << "WARNING: sync small file[" << src_obj << "] to cloud failed:" << retcode << dendl;
...
public:
  ...
  int operate() {
    reenter(this) {
      ...
      yield call(new RGWStreamSpliceCR(cct, sync_env->http_manager, in_crf, out_crf));
      if (retcode < 0) {
        ldout(sync_env->cct, 0) << "ERROR: failed to sync obj " << dest_obj
          << " via multipart ret:" << retcode << dendl;
        return set_cr_error(retcode);
      }
...
public:
  ...
  int operate() {
    reenter(this) {
      ...
      if (retcode >= 0) {
        ldout(sync_env->cct, 0) << "NOTICE: continue left upload parts, upload_id:"
                                << status.upload_id
                                << " part_size:" << status.part_size
                                << " num_parts:" << status.num_parts
                                << " cur_part:" << status.cur_part
                                << " cur_ofs:" << status.cur_ofs
                                << dendl;
        for (auto p : status.parts) {
          ldout(sync_env->cct, 0) << "NOTICE: has successfully uploaded parts:"
                                  << " part num:" << p.second.part_num
                                  << " part ofs:" << p.second.ofs
                                  << " part size:" << p.second.size
                                  << dendl;
        }
      ...
      if (retcode == -ENOENT) {
        ...
        if (retcode < 0) {
          ldout(sync_env->cct, 0) << "ERROR: failed to init multipart " << src_obj << " retcode=" << retcode << dendl;
          return set_cr_error(retcode);
        }
...
#define MULTIPART_MAX_PARTS 10000
      ...
      for (; (uint32_t)status.cur_part <= status.num_parts; ++status.cur_part) {
        ...
        if (retcode < 0) {
          ...
          if (retcode == -ENOENT) {
            ldout(sync_env->cct, 0) << "ERROR: failed to sync obj=" << src_obj << ", sync via multipart upload, upload_id=" << status.upload_id << " has been abort unexpected" << dendl;
            ret_err = -EAGAIN;
            goto clear;
          }
          return set_cr_error(retcode);
    ...
    reenter(this) {
      ...
      if (bucket_created.find(target_bucket_name) == bucket_created.end()){
        ...
        if (retcode < 0 ) {
          ...
          // BOS ErrCode (S3 http code: 200)
          if (result.code != "BucketAlreadyExists") {
            string str(out_bl.c_str(), out_bl.length());
            ldout(sync_env->cct, 5) << "ERROR: create bucket error response body: " << str << dendl;
            return set_cr_error(retcode);
          }
        }
      ...
      if (retcode < 0) {
        ldout(sync_env->cct, 0) << "ERROR: failed to sync obj to cloud:" << retcode << dendl;
        return set_cr_error(retcode);
      }
