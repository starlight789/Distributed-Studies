			      ...
			      RGWReshard* reshard_log)
{
  ...
  aiter = bucket_attrs.find(RGW_ATTR_LC);
  if (aiter != bucket_attrs.end()) {
    // reshard bucket lc entry
    if (bucket_info.num_shards < uint32_t(num_shards)) {
      ret = store->get_lc()->set_lc_entries(bucket_info.bucket, bucket_info.num_shards, uint32_t(num_shards));
      if (ret != 0) {
        ldout(store->ctx(), 0) << __func__ << "(): set_lc_entries err"
      ...
      ret = store->get_lc()->remove_lc_entries(bucket_info.bucket,
                                               num_shards,
                                               bucket_info.num_shards);
      if (ret != 0) {
        ldout(store->ctx(), 0) << __func__ << "(): remove_lc_entries err"
                                  << ", bucket:" << bucket_info.bucket.name
                                  << ", ret:" << ret
                                  << dendl;
        return ret;
