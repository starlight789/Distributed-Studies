...
void dump_access_control(req_state *s, RGWOp *op, const string& custom_meta)
{
  string origin;
  string method;
  string header;
  ...
  if (s->is_from_platform_frontend()) {
    ldout(s->cct, 10) << "HTTP_X_AMZ_USER_AGENT header match success" << dendl;
    const char *auth_hdr = s->info.env->get("HTTP_AUTHORIZATION");
    if (auth_hdr) {
      const char *origin_hdr = s->info.env->get("HTTP_ORIGIN");
      const char *method = s->info.method;
      dump_access_control(s, origin_hdr, method, nullptr, 
          ("Etag,Content-Disposition" + custom_meta).c_str(), CORS_MAX_AGE_INVALID);
      return;
    } else {
      ldout(s->cct, 10) << "no authorization, deny console request" << dendl;
    }
  }

  if (!op || !op->generate_cors_headers(origin, method, header, exp_header, &max_age))
    return;
...
#ifdef WITH_BCEBOS
    if (s->prot_flags & RGW_REST_BOS) {
      dump_header(s, "x-bce-request-id", s->trans_id);
    } else
#endif
    {
...
void end_header(struct req_state* s, RGWOp* op, const char *content_type,
                ...
                bool force_no_error, const string& custom_meta, bool force_err_msg)
{
  string ctype;
  ...
  dump_access_control(s, op, custom_meta);
#ifdef WITH_BCEBOS
  if (s->prot_flags & RGW_REST_BOS) {
    dump_header(s, "Server", "BceBos");
  }
#endif
  ...
  if ( (!force_no_error && s->is_err()) || force_err_msg) {
    dump_start(s);
    dump(s);
  ...
  if (err_no) {
    ...
    if (!error_content.empty()) {
      ...
      try {
        ...
        ldout(s->cct, 0) << __func__ << "(): send_body err=" << e.what() << dendl;
      }
    } else {
      end_header(s, op);
    }
              ...
              const size_t len)
{
  ...
  RESTFUL_IO(s)->get_throttle_token(len,
      ThrottleManager::instance().get_node_bandwidth_throttle());

  // bucket bandwidth limit
  RESTFUL_IO(s)->get_throttle_token(len,
      ThrottleManager::instance().get_bucket_bandwidth_throttle(s));

  // user bandwidth limit
  RESTFUL_IO(s)->get_throttle_token(len,
      ThrottleManager::instance().get_user_bandwidth_throttle(s));
  try {
...
{
  // node bandwidth limit
  RESTFUL_IO(s)->get_throttle_token(max,
      ThrottleManager::instance().get_node_bandwidth_throttle());
  ...
  try {
    ...
    return -e.code().value() == -EIO ? -ERR_INVALID_BODY : -e.code().value();
  }
}
...
int RGWPutImageStyle_ObjStore::get_params()
{
  char* data = nullptr;
  int len = 0;
  const auto max_size = s->cct->_conf->rgw_max_put_param_size;
  op_ret = rgw_rest_read_all_input(s, &data, &len, max_size, false);
  if (op_ret < 0) {
    if (op_ret == -ERANGE) op_ret = -ERR_MAX_MESSAGE_LENGTH_EXCEEDED;
    return op_ret;
  }
...
int RGWPutImageProtection_ObjStore::get_params()
{
  char* data = nullptr;
  int len = 0;
  const auto max_size = s->cct->_conf->rgw_max_put_param_size;
			    ...
			    const uint64_t max_len, const bool allow_chunked)
{
  ...
  if (s->length)
    cl = atoll(s->length);
#ifdef WITH_BCEBOS
  else if (!allow_chunked && !(s->prot_flags & RGW_REST_BOS))
#else
...
#ifdef WITH_BCEBOS
      if (s->prot_flags & RGW_REST_BOS)
        return -ERR_MAX_MESSAGE_LENGTH_EXCEEDED;
#endif
      return -ERANGE;
    }
...
#ifdef WITH_BCEBOS
  if (s->prot_flags & RGW_REST_BOS) {
    marker_str = s->info.args.get("partNumberMarker");
  } else
#endif
  {
...
#ifdef WITH_BCEBOS
  if (s->prot_flags & RGW_REST_BOS) {
    str = s->info.args.get("maxUploads");
  } else
#endif
  {
    str = s->info.args.get("max-uploads");
  }
...
#ifdef WITH_BCEBOS
  if (s->prot_flags & RGW_REST_BOS) {
    key_marker = s->info.args.get("keyMarker");
  } else
#endif
  {
    key_marker = s->info.args.get("key-marker");
  }
...
int RGWHandler_REST::init_permissions(RGWOp* op)
{
  ...
  if (s->cct->_conf->rgw_abcstore_multi_region &&
      (op->get_type() == RGW_OP_GET_BUCKET_LOCATION ||
       op->get_type() == RGW_OP_STAT_BUCKET))
    return 0;
...
int RGWHandler_REST::read_permissions(RGWOp* op_obj)
{
  ...
  case OP_COPY:
    ...
    break;
  case OP_DELETE:
    if (!s->info.args.exists("tagging") && !s->info.args.exists("acl")){
      only_bucket = true;
    }
    break;
  case OP_OPTIONS:
...
#ifdef WITH_BCEBOS
  if (s->prot_flags & RGW_REST_BOS && resource_mgrs["/bos"]) {
    bool admin_req = false;
    if (uri.compare(1, g_conf->rgw_admin_entry.length(), g_conf->rgw_admin_entry) == 0) {
      RESTArgs::get_bool(s, g_conf->rgw_admin_entry, false, &admin_req);
    }
    if (!admin_req) {
      return resource_mgrs["/" + s->cct->_conf->rgw_bos_url_prefix]->get_resource_mgr(s, uri, out_uri);
    }
  }
#endif
...
#ifdef WITH_BCEBOS
bool RGWREST::is_bos_used(struct req_state* const s) {
  // classify client request to s3 or bos
  std::string authorization;
  if (s->info.env->exists("HTTP_AUTHORIZATION")) {
    authorization = s->info.env->get("HTTP_AUTHORIZATION");
  } else if (s->info.args.exists("authorization")) {
    authorization = s->info.args.get("authorization");
  }
  ...
  if (pos != std::string::npos) {
    std::string subr = authorization.substr(0, pos);
    if (subr.compare("bce-auth-v1") == 0)
      return true;
  }
...
bool RGWREST::is_s3_used(struct req_state* const s) {
  ...
  if (http_auth && http_auth[0]) {
    if (!strncmp(http_auth, AWS4_HMAC_SHA256_STR, strlen(AWS4_HMAC_SHA256_STR)) ||
        !strncmp(http_auth, "AWS ", 4)) {
      return true;
    }
  } else {
    if (s->info.args.get("X-Amz-Algorithm") == AWS4_HMAC_SHA256_STR ||
        !s->info.args.get("AWSAccessKeyId").empty()) {
      return true;
    }
  }
...
RGWHandler_REST* RGWREST::get_handler(
  ...
  *init_error = preprocess(s, rio);
  ...
  if (is_s3_used(s)) {
    ...
    ldout(s->cct, 20) << "consider it as s3 api" << dendl;
  } else if (is_bos_used(s) || (is_anonymous(s) && s->cct->_conf->rgw_default_api_bos)) {
    s->prot_flags |= RGW_REST_BOS;
    ldout(s->cct, 20) << "consider it as bos api" << dendl;
  }
#endif
  RGWRESTMgr *m = mgr.get_manager(s, frontend_prefix, s->decoded_uri,
                                  &s->relative_uri);
  if (! m) {
    *init_error = -ERR_NOT_IMPLEMENTED;
    return nullptr;
  }
...
int RGWGetObj_ObjStore::serve_errordoc(RGWHandler * const handler) {
  return handler->serve_retarget(this);
}

int RGWListBucket_ObjStore::serve_errordoc(RGWHandler * const handler) {
  return handler->serve_retarget(this);
}

int RGWHandler_REST::serve_retarget(RGWOp *op) {
  // get retarget object name
  rgw_obj_key new_obj;
  s->bucket_info.website_conf.get_retarget_key(s->object.name, &new_obj.name);
  ldout(s->cct, 5) << "retarget get_effective_key " << s->object << " -> "
		    << new_obj << dendl;

  // check object exist
  RGWObjState *astate;
  bool force_not_found = false;
  rgw_obj retarget_obj(s->bucket, new_obj);
  int ret = store->get_obj_state(static_cast<RGWObjectCtx *>(s->obj_ctx),
                        s->bucket_info, retarget_obj, &astate);
  // astate will not be empty
  if (ret < 0 || !astate->exists) {
    if (s->bucket_info.website_conf.get_error_doc().empty()) {
      // 404.html donot exist, return NoSuchKey
      op->set_ret(-ENOENT);
      return -ENOENT;
    }
    retarget_obj.key = s->bucket_info.website_conf.get_error_doc();
  }

  s->formatter->reset(); /* Try to throw it all away */

  if (retarget_obj.key.name == s->bucket_info.website_conf.get_error_doc()) {
    force_not_found = true;
  }

  // donot explicit bosapi or s3api, because static website is anonymous user, result will display on browser
  std::shared_ptr<RGWGetObj_ObjStore_S3> getop(new RGWGetObj_ObjStore_S3);
  getop->set_get_data(true);
  getop->init(store, s, this);

  s->object = retarget_obj.key;
  //int ret = 0;
  ret = this->init_permissions(getop.get());
  if (ret < 0) {
    ldout(s->cct, 20) << "serve_errordoc failed, init_permissions ret=" << ret << dendl;
    return ret; // Trigger double error handler
  }

  ret = this->read_permissions(getop.get());
  if (ret < 0) {
  ...
  // No going back now
  getop->set_retarget(force_not_found);
  getop->pre_exec();
  getop->execute();
  getop->complete();

  op->set_ret(getop->get_ret());
  return 0;
}
