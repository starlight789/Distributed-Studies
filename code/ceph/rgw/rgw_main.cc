...
class RGWProcess;
extern ceph::logging::Log*  g_access_log;
...
static void init_rgw_access_log() {
  g_access_log = new ceph::logging::Log(&g_conf->subsys);
  g_access_log->start();
  g_access_log->set_log_file(g_conf->rgw_access_log_file);
  g_access_log->reopen_log_file();
  g_access_log->set_max_new(g_conf->log_max_new);
  g_access_log->set_max_recent(g_conf->log_max_recent);
  if (g_conf->log_flush_on_exit) {
    g_access_log->set_flush_on_exit();
  }
  g_access_log->set_coarse_timestamps(g_conf->get_val<bool>("log_coarse_timestamps"));
}
...
int main(int argc, const char **argv)
{
  ...
  register_async_signal_handler(SIGHUP, handle_sighup);
...
#ifdef WITH_BCEBOS
  if (apis_map.count("bos") > 0) {
    RGWRESTMgr_BOS *bos_resource = new RGWRESTMgr_BOS;
    rest.register_resource(g_conf->rgw_bos_url_prefix, bos_resource);
  }
#endif
  ...
  // init QoS Manager
  ThrottleManager::instance().init(g_ceph_context, store);
...
#ifdef WITH_BTHREAD
    else if (framework == "bhttp") {
      int port;
      config->get_val("port", 80, &port);
      std::string uri_prefix;
      config->get_val("prefix", "", &uri_prefix);
      RGWProcessEnv env{ store, &rest, olog, port, uri_prefix, auth_registry };
      fe = new RGWBhttpFrontend(env, config);
    }
#endif
    else {
      derr << "ERROR: dont support frontend " << framework << dendl;
      return -1;
    }
  ...
  if (!g_conf->rgw_access_log_file.empty()) {
    init_rgw_access_log();
  }
  ...
  unregister_async_signal_handler(SIGHUP, handle_sighup);
  ...
  ThrottleManager::instance().shutdown();
