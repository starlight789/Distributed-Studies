...
template class RGWAsyncProcessor<RGWAsyncRadosRequest>;
...
public:
  RGWAsyncLockSystemObj(RGWCoroutine *caller, RGWAioCompletionNotifier *cn, RGWRados *_store,
                        ...
                        librados::Rados* rados_handle = NULL);
};
...
public:
  RGWSimpleRadosLockCR(RGWAsyncRadosProcessor *_async_rados, RGWRados *_store,
                       ...
                       uint32_t _duration,
                       librados::Rados* _rados_handle = NULL);
...
public:
  ...
  int send_request() override {
    req = new RGWAsyncFetchRemoteObj(this, stack->create_completion_notifier(), store, source_zone, bucket_info,
                                     dest_placement_rule, key, versioned_epoch, copy_if_newer, umod_time, zones_trace);
    async_rados->queue(req);
    return 0;
  }
...
public:
  ...
  void set_rados_handle(librados::Rados* _handle) {
    rados_handle = _handle;
  }
};
