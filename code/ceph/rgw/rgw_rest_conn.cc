...
RGWRESTConn::RGWRESTConn(CephContext *_cct, RGWRados *_store,
  ...
  : cct(_cct),
    ...
    mirror_used(_mirror_used)
{
  if (store && !mirror_used) {
...
int RGWRESTConn::get_obj(const rgw_user& uid, req_info *info /* optional */, const rgw_obj& obj,
                         ...
                         map<string, string>* explicit_headers, map<string, string>* explicit_params)
{
  ...
  params.cb = cb;
  params.explicit_headers = explicit_headers;
  params.explicit_params = explicit_params;
...
int RGWRESTConn::get_obj(const rgw_obj& obj, const get_obj_params& in_params, bool send, RGWRESTStreamRWRequest **req)
{
  ...
  if (in_params.explicit_params) {
    for (auto iter = in_params.explicit_params->begin(); iter != in_params.explicit_params->end(); ++iter) {
      params.push_back(param_pair_t(iter->first, iter->second));
    }
  }

  if (mirror_used) {
    if (range_explicited) {
      *req = new RGWRESTStreamAsyncMirrorReadRequest(store, cct, url, in_params.cb, NULL, &params, host_style);
    } else {
      *req = new RGWRESTStreamMirrorReadRequest(cct, url, in_params.cb, NULL, &params, host_style);
    }
  } else if (in_params.get_op) {
  ...
  if (in_params.explicit_headers) {
    for (auto iter = in_params.explicit_headers->begin(); iter != in_params.explicit_headers->end(); ++iter) {
      extra_headers[iter->first] = iter->second;
    }
  }
