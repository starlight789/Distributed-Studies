...
void usage()
{
  ...
  cout << "  bucket sync status         get sync status of the specified bucket\n";
  cout << "                             support option --source-zone\n";
  cout << "  bucket sync markers        get each shard sync marker and status of the specified bucket\n";
  cout << "                             require option --source-zone\n";
  cout << "  bucket sync time           set bucket sync time, with --start-sync-time, --stop-sync-time\n";
  cout << "  bucket sync run            start sync specified bucket from specified source zone\n";
  cout << "  bucket sync init           init sync for the specified bucket\n";
  cout << "  bucket namespace set       enable bucket namespace\n";
  cout << "                             addd option --force to force set bucket namespace\n";
  cout << "                             even if this bucket is not emtpy (can use start-marker)\n";
  cout << "  bucket namespace get       get bucket namespace status\n";
  cout << "  bucket namespace rm        disable bucket namespace\n";
  cout << "                             addd option --force to force delete bucket namespace\n";
  cout << "                             even if this bucket is not emtpy\n";
  cout << "  bi get                     retrieve bucket index object entries\n";
  cout << "  bi put                     store bucket index object entries\n";
  cout << "  bi list                    list raw bucket index entries\n";
  cout << "  bi purge                   purge bucket index entries\n";
  cout << "  olh get                    get olh info for specified object\n";
  cout << "                             require option --bucket --object\n";
  cout << "  olh readlog                get olh log for specified object\n";
  cout << "                             require option --bucket --object\n";
  cout << "  object rm                  remove object\n";
  cout << "  object stat                stat an object for its metadata\n";
  cout << "  object unlink              unlink object from bucket index\n";
  cout << "  object rewrite             rewrite the specified object\n";
  cout << "  objects expire             run expired objects cleanup (for swift)\n";
  ...
  cout << "  sync status                get zone sync status, support option --source-zone\n";
  ...
  cout << "  gcobj clear                remove still existed tail obj, which should have been removed by gc\n";
  cout << "                             requires: --infile --before-day\n";
  cout << "                             supports: --yes-i-really-mean-it, if not specified, will only output need removed objs to log\n";
  ...
  cout << "                             support option --start-date, end-date, marker\n";
  cout << "  mdlog trim                 trim metadata log\n";
  cout << "                             require option --shard-id, --period\n";
  cout << "                             support option --start-date, end-date or start-marker, end-marker\n";
  ...
  cout << "                             require option --bucket\n";
  cout << "                             support option --shard-id, --marker\n";
  cout << "  bilog trim                 trim bucket index log\n";
  cout << "                             require option --bucket\n";
  cout << "                             support option --start-marker, --end-marker, --shard-id\n";
  ...
  cout << "  multipart orphans help     show manpage for how to use multipart orphans find command\n";
  cout << "  multipart orphans find     run search for repetitive upload part (use --infile to specify repetitive part list)\n";
  ...
  cout << "   --dest-zone               specify the dest zone name (for configure bucket sync)\n";
  ...
  cout << "   --force                   force set or delete bucket namespace\n";
  cout << "   --before-day              an integer, specify how much days, raw obj\n";
  cout << "                             exists more than these days can be clear\n";
  cout << "\n";
  ...
  cout << "   --hostnames               vector of hostname in RGWZoneGroup, same work with rgw_dns_name\n";
  cout << "\n";
...
enum {
  ...
  OPT_BUCKET_REWRITE,
  OPT_BUCKET_RESHARD,
  OPT_BUCKET_NAMESPACE_SET,
  OPT_BUCKET_NAMESPACE_GET,
  OPT_BUCKET_NAMESPACE_DEL,
  OPT_POLICY,
  OPT_POOL_ADD,
  OPT_POOL_RM,
  OPT_POOLS_LIST,
  ...
  OPT_LOG_SHOW,
  OPT_LOG_RM,
  OPT_USAGE_READOP_SHOW,
  OPT_USAGE_READOP_TRIM,
  OPT_USAGE_SHOW,
  OPT_USAGE_TRIM,
  OPT_USAGE_CLEAR,
  OPT_OBJECT_RM,
  ...
  OPT_MULTIPART_ORPHANS_HELP,
  OPT_MULTIPART_ORPHANS_FIND,
  OPT_ZONEGROUP_ADD,
...
static int get_cmd(const char *cmd, const char *prev_cmd, const char *prev_prev_cmd, bool *need_more)
{
  ...
  *need_more = false;
  ...
  if (strcmp(prev_cmd, "user") == 0) {
    ...
    } else if (strcmp(prev_cmd, "namespace") == 0) {
      if (strcmp(cmd, "set") == 0) {
        return OPT_BUCKET_NAMESPACE_SET;
      }
      if (strcmp(cmd, "get") == 0) {
        return OPT_BUCKET_NAMESPACE_GET;
      }
      if (strcmp(cmd, "rm") == 0) {
        return OPT_BUCKET_NAMESPACE_DEL;
      }
    }
    ...
    if (strcmp(cmd, "rm") == 0)
      return OPT_LOG_RM;
  } else if ((prev_prev_cmd && strcmp(prev_prev_cmd, "usage") == 0) &&
            (strcmp(prev_cmd, "readop") == 0)) {
    if (strcmp(cmd, "show") == 0)
      return OPT_USAGE_READOP_SHOW;
    if (strcmp(cmd, "trim") == 0)
      return OPT_USAGE_READOP_TRIM;
  ...
  } else if (strcmp(prev_cmd, "gcobj") == 0) {
    if (strcmp(cmd, "clear") == 0)
      return OPT_GCOBJ_CLEAR;
  } else if (strcmp(prev_cmd, "lc") == 0) {
    if (strcmp(cmd, "list") == 0)
      return OPT_LC_LIST;
    if (strcmp(cmd, "process") == 0)
      return OPT_LC_PROCESS;
  } else if (prev_prev_cmd && strcmp(prev_prev_cmd, "multipart") == 0) {
    if (strcmp(prev_cmd, "orphans") == 0) {
      if (strcmp(cmd, "help") == 0)
        return OPT_MULTIPART_ORPHANS_HELP;
      if (strcmp(cmd, "find") == 0)
        return OPT_MULTIPART_ORPHANS_FIND;
    }
...
int set_bucket_sync_time(RGWRados *store, const string& tenant_name, const string& bucket_name, const string& zone, int start, int stop) {
  RGWBucketInfo bucket_info;
  map<string, bufferlist> attrs;
  RGWObjectCtx obj_ctx(store);
  ...
  if (start >= 0) bucket_info.sync_config[zone].start_sync_time = start;
  if (stop >= 0) bucket_info.sync_config[zone].stop_sync_time = stop;
...
int set_bucket_sync_enabled(RGWRados *store, int opt_cmd, const string& tenant_name, const string& bucket_name, const string& zone)
{
  RGWBucketInfo bucket_info;
  map<string, bufferlist> attrs;
  RGWObjectCtx obj_ctx(store);
  ...
  if (opt_cmd == OPT_BUCKET_SYNC_ENABLE) {
    bucket_info.disable_sync[zone] = false;
  } else if (opt_cmd == OPT_BUCKET_SYNC_DISABLE) {
    bucket_info.disable_sync[zone] = true;
  }

  int shards_num = bucket_info.num_shards? bucket_info.num_shards : 1;
  int shard_id = bucket_info.num_shards? 0 : -1;
/* In multi slave zone situation, each slave zone has different sync config.
 * Thus, master zone can stop bi log only if every slave zone has disabled sync.
 * Meanwhile, master zone can resync bi log when any one slave zone enable sync.
 * You can also stop sync of only master zone.
 */
  bool all_slave_disabled = true;
  string& master_zone = store->get_zonegroup().master_zone;

  for (auto ziter : store->get_zonegroup().zones) {
    RGWZone& z = ziter.second;
    if (z.id == master_zone)
      continue;
    if (!bucket_info.disable_sync[z.name]) {
      all_slave_disabled = false;
      break;
    }
  }

  string zone_id;
  if (!store->find_zone_id_by_name(zone, &zone_id)) {
    cerr << "WARNING: cannot find zone id for name=" << zone << std::endl;
    return EINVAL;
  }

  if (opt_cmd == OPT_BUCKET_SYNC_DISABLE) {
    if (all_slave_disabled || zone_id == master_zone) {
    ...
    if (!all_slave_disabled && !bucket_info.disable_sync[store->get_zone().name]) {
      r = store->resync_bi_log_entries(bucket_info, -1);
      if (r < 0) {
        lderr(store->ctx()) << "ERROR: failed writing resync bilog" << dendl;
        return r;
      }
      bucket_info.bilog_flags[store->get_zone().name] = true;
    }
  }
...
int update_bucket_namespace(RGWRados *store, int opt_cmd, RGWBucketInfo& bucket_info,
                            map<string, bufferlist> attrs)
{
  if (opt_cmd == OPT_BUCKET_NAMESPACE_SET) {
    if (store->ctx()->_conf->rgw_bucket_namespace_disable_force) {
      cerr << "couldn't enable bucket namespace, because it is force disable" << std::endl;
      return -EINVAL;
    }
    bucket_info.namespace_type = BUCKET_NAMESPACE_ENABLE;
  } else if (opt_cmd == OPT_BUCKET_NAMESPACE_DEL) {
    bucket_info.namespace_type = BUCKET_NAMESPACE_DISABLE;
  }

  int r = store->put_bucket_instance_info(bucket_info, false, real_time(), &attrs);
  if (r < 0) {
    cerr << "ERROR: failed to write bucket instance info: " << cpp_strerror(-r) << std::endl;
    return -r;
  }
  return 0;
}

int create_object_namespace(RGWRados *store, RGWBucketInfo& bucket_info, rgw_bucket_dir_entry& entry) {

  RGWRados::Bucket bop(store, bucket_info);
  const string& obj_name = entry.key.name;

  // skip dir
  if (obj_name.empty() || obj_name[obj_name.size() - 1] == '/') {
    return -EISDIR;
  }

  RGWRados::Bucket::Namespace namespace_op(&bop, obj_name);

  // prepare
  int r = namespace_op.prepare(nullptr, CLS_RGW_OP_ADD, nullptr, entry.meta.mtime);
  if (r < 0) {
    return r;
  }

  // complete
  r = namespace_op.complete(entry.ver.pool, entry.ver.epoch, entry.meta.size, entry.meta.mtime,
    entry.meta.appendable);
  if (r >= 0) {
    return r;
  }

  cerr << " ERROR: failed to namespace complete for " << namespace_op.get_obj()
       << " ret=" << r << std::endl;

  // cancel
  int ret = namespace_op.cancel();
  if (ret < 0) {
     cerr << "ERROR: failed to namespace cancel "
          << namespace_op.get_obj() << " ret=" << ret << std::endl;
  }
  return r;
}

int create_bucket_namespace(RGWRados *store, RGWBucketInfo& bucket_info, const string& marker)
{
  bool truncated = false;
  int echo_loop_entries = 1000;
  int ret = 0;
  ...
  do {
    ret = list_op.list_objects(echo_loop_entries, &result, &common_prefixes, &truncated);
    if (ret < 0) {
      cerr << "ERROR: failed to list_objects: " << cpp_strerror(-ret) << std::endl;
      return -ret;
    }
    for (vector<rgw_bucket_dir_entry>::iterator iter = result.begin(); iter != result.end(); ++iter) {
      rgw_bucket_dir_entry& entry = *iter;
      ret =  create_object_namespace(store, bucket_info, entry);
      if (ret == -EISDIR) {
        cerr << "WARNING skip: " << entry.key.name
             << " [it is directory]" << std::endl;
      } else if (ret == -EEXIST) {
        cerr << "WARNING skip: " << entry.key.name
             << " [file name conflict with directory]" << std::endl;
      } else if (ret < 0) {
        cerr << "FAILED  create namespace for : " << entry.key.name << " "
             << cpp_strerror(-ret) << std::endl;
        return ret;
      } else {
        std::cout << "SUCCESS create namespace for : " << entry.key.name << std::endl;
      }
    }
  } while (truncated);
...
static void sync_status(Formatter *formatter, const string& source_zone)
{
  ...
  for (auto iter : store->zone_conn_map) {
    ...
    if (!source_zone.empty() && source_zone.compare(source_id) != 0) {
      continue;
    }
  ...
  struct ZoneStatus {
    ...
    struct FullSync {
      ...
      void dump(Formatter *f) const {
        encode_json("num_full", num_full, f);
        encode_json("total_shards", total_shards, f);
        encode_json("complete_objs", complete_objs, f);
      }
    };
    struct IncSync {
      uint32_t num_inc = 0;
    ...
    FullSync full_sync;
    IncSync inc_sync;
    BehindShards behind_shards;
    uint64_t gap_entries = 0;
    bool caught = false;
/*   
    class IncSync {
      uint32_t num_inc;
    ...
    */
    void dump(Formatter *f) const {
      ...
      encode_json("source_zone_id", sz_id, f);
      encode_json("source_zone_name", sz_name, f);
      f->dump_bool("enabled", enabled);
      encode_json("full_sync", full_sync, f);
      encode_json("inc_sync", inc_sync, f);
      encode_json("behind_shards", behind_shards, f);
      f->dump_bool("caught", caught);
      encode_json("behind_entries", gap_entries, f);
      //f->close_section();  // state
    }
  };
  ...
  string bucket;
  bool enabled = true;
  vector<ZoneStatus> zone_status;

  void dump(Formatter *f) const {
    f->open_object_section("syncstatus");
    encode_json("realm", realm, f);
    encode_json("zonegroup_id", zg_id, f);
    encode_json("zonegroup_name", zg_name, f);
    encode_json("zone_id", z_id, f);
    encode_json("zone_name", z_name, f);
    encode_json("bucket", bucket, f);
    f->dump_bool("sync_enabled", enabled);
    encode_json("status", zone_status, f);
    f->close_section();  // syncstatus
  }
};
...
static int bucket_source_sync_status(RGWRados *store, const RGWZone& zone,
                                     ...
                                     BucketSyncStatus& sync_status, JSONFormatter* f)
{
  BucketSyncStatus::ZoneStatus& state = sync_status.zone_status[sync_status.zone_status.size() - 1];
  if (f) {
    state.sz_id = source.id;
    state.sz_name = source.name;
  } else {
  ...
  uint64_t full_complete = 0;
  uint64_t gap_entries = 0;
  size_t total_shards = status.size();
  ...
  if (f) {
    state.full_sync.num_full = num_full;
    state.full_sync.total_shards = total_shards;
    state.full_sync.complete_objs = full_complete;
    state.inc_sync.num_inc = num_inc;
    state.inc_sync.total_shards = total_shards;
  } else {
  ...
  r = remote_bilog_markers(store, conn, bucket_info, &remote_markers);
  if (r < 0) {
  ...
  for (auto& r : remote_markers.get()) {
    ...
    if (m.state != BucketSyncState::StateIncrementalSync || pos != r.second) {
      ...
      gap_entries += sync_marker_gap(pos, r.second, &out);
    }
  }
  if (f) {
    if (!shards_behind.empty()) {
      state.behind_shards.num = shards_behind.size();
      state.behind_shards.shards = std::move(shards_behind);
      state.gap_entries = gap_entries;
    } else if (!num_full) {
      state.caught = true;
    }
  } else {
    if (!shards_behind.empty()) {
      ...
      out << indented{width} << "behind entries: [" << gap_entries << "]\n" ;
    } else if (!num_full) {
...
static int bucket_sync_status(RGWRados *store, const RGWBucketInfo& info,
                              ...
                              std::ostream& out, JSONFormatter *f)
{
  RGWRealm& realm = store->realm;
  RGWZoneGroup& zonegroup = store->get_zonegroup();
  RGWZone& zone = store->get_zone();
  ...
  BucketSyncStatus status;

  if (f) {
    status.realm = realm;
    status.zg_id = zonegroup.get_id();
    status.zg_name = zonegroup.get_name();
    status.z_id = zone.id;
    status.z_name = zone.name;
    status.bucket = info.bucket.name;
  } else {
  ...
  if (!info.datasync_flag_enabled(zone.name)) {
    if (f) {
  ...
  if (!source_zone_id.empty()) {
    ...
    if (f) {
      BucketSyncStatus::ZoneStatus state;
      status.zone_status.push_back(std::move(state));
    }
    ret = bucket_source_sync_status(store, zone, z->second, c->second,
                                     info, width, out, status, f);
  } else {
    for (const auto& z : zonegroup.zones) {
...
static bool check_pool_is_empty(const string& poolname)
{
  std::list<std::string> pools {poolname};
  std::map<std::string,librados::pool_stat_t> stats;
  int ret = store->get_rados_handle()->get_pool_stats(pools, stats);
  if (ret < 0) {
    return false;
  }
...
int main(int argc, const char **argv)
{
  ...
  boost::optional<string> tail_pool;
  ...
  int totp_window = 0;
  int trim_delay_ms = 0;
  int start_sync_time = 0;
  int stop_sync_time = 0;
  ...
  for (std::vector<const char*>::iterator i = args.begin(); i != args.end(); ) {
    if (ceph_argparse_double_dash(args, i)) {
      ...
      // do nothing
    } else if (ceph_argparse_binary_flag(args, i, &show_log_all, NULL, "--show-log-all", (char*)NULL)) {
      // do nothing
    ...
    } else if (ceph_argparse_witharg(args, i, &val, "--before-day", (char*)NULL)) {
      before_day = (int)strict_strtol(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse before day: " << err << std::endl;
        return EINVAL;
      }
    ...
    } else if (ceph_argparse_witharg(args, i, &val, "--loops", (char*)NULL)) {
      loops_num = (int)strict_strtol(val.c_str(), 10, &err);
      if (!err.empty()) {
        cerr << "ERROR: failed to parse loops: " << err << std::endl;
        return EINVAL;
      }
	...
	categories[*iter] = true;
     ...
     // do nothing
    ...
    } else if (ceph_argparse_binary_flag(args, i, &force_int, NULL, "--force", (char*)NULL)) {
    ...
    } else if (ceph_argparse_witharg(args, i, &val, "--hostnames", (char*)NULL)) {
      get_str_list(val, hostnames);
      ...
      data_extra_pool = val;
    } else if (ceph_argparse_witharg(args, i, &val, "--tail-pool", (char*)NULL)) {
      tail_pool = val;
    ...
    } else if (ceph_argparse_witharg(args, i, &val, "--start-sync-time", (char*)NULL)) {
      start_sync_time = atoi(val.c_str());
    } else if (ceph_argparse_witharg(args, i, &val, "--stop-sync-time", (char*)NULL)) {
      stop_sync_time = atoi(val.c_str());
  ...
  if (raw_storage_op) {
    ...
    store = RGWStoreManager::get_storage(g_ceph_context, false, false, false, false, false,
      need_cache && g_conf->rgw_cache_enabled, true);
  }
  if (!store) {
    ...
    case OPT_ZONEGROUP_PLACEMENT_DEFAULT:
      {
        ...
        if (!rule.storage_class.empty() &&
            rule.storage_class != RGWStorageClass::STANDARD &&
            rule.storage_class != RGWStorageClass::STANDARD_HP &&
            rule.storage_class != RGWStorageClass::STANDARD_IA &&
            rule.storage_class != RGWStorageClass::ARCHIVE) {
          cerr << "ERROR: invalid storage class" << std::endl;
          return EINVAL;
        }
	...
	if (ret < 0) {
	  ...
	  return -ret;
        ...
        if (opt_cmd == OPT_ZONEGROUP_PLACEMENT_ADD ||
          ...
          RGWZoneGroupPlacementTarget& target = zonegroup.placement_targets[rule.name];
          if (!tags.empty()) {
            target.tags.clear();
            for (auto& t : tags) {
          ...
          zonegroup.default_placement.name = rule.name;
        }
    ...
    case OPT_ZONE_MODIFY:
      {
        ...
        if (start_sync_time > 0) {
          zone.start_sync_time = start_sync_time;
          need_zone_update = true;
        }

        if (stop_sync_time > 0) {
          zone.stop_sync_time = stop_sync_time;
          need_zone_update = true;
        }
	...
	int ret = zone.init(g_ceph_context, store);
	  ...
	  return -ret;
        ...
        if (opt_cmd == OPT_ZONE_PLACEMENT_ADD ||
          ...
          string opt_tail_pool = tail_pool.value_or(string());
          ...
          if (!opt_tail_pool.empty()) {
            const RGWZoneStorageClass *origin_tail{nullptr};
            if (info.tail_pools.find(storage_class, &origin_tail) 
              && !check_pool_is_empty(origin_tail->data_pool.name)) {
              // if tail_pool is not empty, refuse this operation
              cerr << "ERROR: Cannot modify tail pool when origin tail pool is not empty" << std::endl;
              return -EINVAL;
            }
            rgw_pool tp = opt_tail_pool;
            info.tail_pools.set_storage_class(storage_class, &tp, compression_type.get_ptr());
          }
          ...
          } else if (zone.placement_pools.find(placement_id) != zone.placement_pools.end()) {
            RGWZonePlacementInfo& info = zone.placement_pools[placement_id];
            info.storage_classes.unset_storage_class(storage_class);
            if (info.storage_classes.empty()) {
              zone.placement_pools.erase(placement_id);
            }
          }
        }
...
next:
  ...
  if (opt_cmd == OPT_USAGE_READOP_SHOW) {
    ...
    adjust_timezone(store, start_epoch);
    adjust_timezone(store, end_epoch);
    ret = RGWReadUsage::show(store, user_id, start_epoch, end_epoch, show_log_all, f);
    if (ret < 0) {
      cerr << "ERROR: failed to show readop usage" << std::endl;
      return -ret;
    }
  }
  ...
  if (opt_cmd == OPT_USAGE_READOP_TRIM) {
    if (!yes_i_really_mean_it) {
      cerr << "readop usage trim will remove *all* users data" << std::endl;
    ...
    ret = RGWReadUsage::trim(store, user_id, start_epoch, end_epoch);
    if (ret < 0) {
  ...
  if (opt_cmd == OPT_BUCKET_REWRITE) {
    ...
    while (is_truncated) {
      ...
      int r = store->cls_bucket_list_ordered(bucket_info, RGW_NO_SHARD, marker,
                                 prefix, NUM_ENTRIES, true, false, expansion_factor,
...
#define DEFAULT_RESHARD_MAX_ENTRIES 1000
  ...
  if (opt_cmd == OPT_GC_LIST) {
	...
	formatter->dump_int("survive_time", info.survive_time);
	...
	formatter->dump_bool("skip_cache", chain.skip_cache);
	formatter->dump_unsigned("enable_delay_remove_head_obj", chain.enable_delay_remove_head_obj);
  ...
  if (opt_cmd == OPT_GCOBJ_CLEAR) {
    if (before_day < 1 || before_day > 3650) {
      cerr << "ERROR: before_day must specified with positive number" << std::endl;
      return 1;
    }
    RGWGCLeftObjClear gc_c(store, infile, before_day, yes_i_really_mean_it);
...
#define MAX_LC_LIST_ENTRIES 100
    ...
    do {
      int ret = store->list_lc_progress(marker, max_entries,
                                        bucket_lc_map, index);
      if (ret < 0) {
        cerr << "ERROR: failed to list objs: " << cpp_strerror(-ret) << std::endl;
        return 1;
      ...
      for (const auto& entry : bucket_lc_map) {
        ...
        formatter->dump_string("bucket", entry.bucket);
        formatter->dump_int("shard_id", entry.shard_id);
        char exp_buf[100];
  ...
  if (opt_cmd == OPT_MULTIPART_ORPHANS_HELP) {
    cout << "   --path-prefix             path prefix for filtering roles\n";
    cout << "run these commands to find repetitive upload parts:\n";

    cout << "1. copy these in shell bash, specify data pool name, and run the bash file\n";
    cout << "#!/bin/bash\n";
    cout << "DATA_POOL=\"default.rgw.buckets.data\"\n";
    cout << "concurrency=30\n";
    cout << "files=\"\"\n";
    cout << "rados ls -p $DATA_POOL | grep -E '__multipart_|__shadow_' > datapool.list\n";
    cout << "egrep '.*\\..*\\..*__multipart_|.*\\..*\\..*__shadow_' datapool.list > part.list\n";
    cout << "egrep '.*\\..*\\..*__multipart_' datapool.list > multipart.list\n";
    cout << "rm -f statis.list\n";
    cout << "\n";

    cout << "sed_upload_id() {\n";
    cout << "  begin=$1\n";
    cout << "  end=$2\n";
    cout << "  sed -n \"${begin},${end}p\" multipart.list | while read line\n";
    cout << "  do\n";
    cout << "    random_str=$(echo $line | awk -F '.' '{print $(NF-1)}')\n";
    cout << "    echo $line | sed  \"s/$random_str//\" >> statis.list.$begin\n";
    cout << "  done\n";
    cout << "}\n";
    cout << "\n";

    cout << "num=`wc -l multipart.list | awk '{print $1}'`\n";
    cout << "gap=`expr $num/$concurrency`\n";
    cout << "b=1\n";
    cout << "e=$((1+$gap))\n";
    cout << "\n";
    cout << "for ((i=0;i<=${concurrency};i++));do\n";
    cout << "{\n";
    cout << "  files=\"$files statis.list.$b\"\n";
    cout << "  sed_upload_id $b $e &\n";
    cout << "  b=$(($e+1))\n";
    cout << "  e=$(($b+$gap))\n";
    cout << "}\n";
    cout << "done\n";
    cout << "wait\n";
    cout << "for i in ${files};do\n";
    cout << "  [[ -f $i ]] && cat $i >> statis.list\n";
    cout << "done\n";
    cout << "\n";

    cout << "cat statis.list| sort|uniq -c | sort -r -n -k 1 | sed  '/ 1 */d'  | awk '{for (i=2;i<NF;i++)printf(\"%s \",$i);printf(\"%s\\n\",$NF)}' | sed 's/\\.\\.[0-9]*$//' | sort | uniq > statis.num2.list\n";
    cout << "2. radosgw-admin multipart orphans find --infile [statis.num2.list file path],[part.list file path]\n";
    cout << "3. all raw objs in useless_part_list file can be deleted\n";
    cout << "4. all objs prefix with bucket marker in invalid_bucket_list file can be deleted, which means its bucket has been deleted (need check refcount)\n";
    cout << "5. all raw objs in invalid_object_list means its s3 object has been deleted , so these raw obj can be removed with check refcount\n";

    return 0;
  }

  if (opt_cmd == OPT_MULTIPART_ORPHANS_FIND) {
    // list buckets ,get whole bucket id and its bucket name
    RGWMultipartOrphanSearch search(store, infile);
  ...
  if (opt_cmd == OPT_SYNC_STATUS) {
    sync_status(formatter, source_zone);
  }
  ...
  if ((opt_cmd == OPT_BUCKET_SYNC_DISABLE) || (opt_cmd == OPT_BUCKET_SYNC_ENABLE)) {
    ...
    if (dest_zone_name.empty()) {
      cerr << "ERROR: dest zone not specified" << std::endl;
      return EINVAL;
    }
    ...
    rgw_obj obj(bucket, object);
    ret = set_bucket_sync_enabled(store, opt_cmd, tenant, bucket_name, dest_zone_name);
    if (ret < 0)
      return -ret;
  }

  if (opt_cmd == OPT_BUCKET_SYNC_TIME) {
    if (bucket_name.empty()) {
      cerr << "ERROR: bucket not specified" << std::endl;
      return EINVAL;
    }
    ...
    ret = set_bucket_sync_time(store, tenant, bucket_name, dest_zone_name, start_sync_time, stop_sync_time);
    if (ret < 0)
      return -ret;
  }
  ...
  if (opt_cmd == OPT_BUCKET_SYNC_STATUS) {
    ...
    if (pretty_format) {
      bucket_sync_status(store, bucket_info, source_zone, std::cout, nullptr);
    } else {
      if (format.compare("json") != 0) {
        cerr << "ERROR: only support json format"  << std::endl;
        return EINVAL;
      }
      bucket_sync_status(store, bucket_info, source_zone, std::cout, (JSONFormatter *)formatter);
    }
  }
 ...
 if (opt_cmd == OPT_MFA_CHECK) {
  ...
  if (opt_cmd == OPT_BUCKET_NAMESPACE_SET) {
    ...
    // don't allow to enable bucket namespace when versioning is enabled
    if (bucket_info.versioned()) {
      cerr << "ERROR: could enable bucket namespace when bucekt versioning is enabled" << std::endl;
      return -r;
    }

    if (!store->is_meta_master()) {
      cerr << "WRANING: you should enable bucket namespace on master" << std::endl;
      if (!force) {
        return -EINVAL;
      }
    }
    ...
    if (bucket_info.namespace_type == BUCKET_NAMESPACE_ENABLE) {
      cerr << "WRANING: bucket namespace already is enabled" << std::endl;
      if (!force) {
        return 0;
      }
    }

    // whether check buccket is empty
    if (!force) {
      ret = store->check_bucket_empty(bucket_info);
      if (ret == -ENOTEMPTY) {
        cerr << "ERROR: bucket is not empty!" << std::endl;
        return ENOTEMPTY;
      } else if (ret < 0) {
        cerr << "ERROR: failed to check whether bucket is empty : " << cpp_strerror(ret) <<  std::endl;
        return -ret;
      }
    }

    ret = update_bucket_namespace(store, opt_cmd, bucket_info, attrs);
    if (ret < 0)
      return -ret;

    if (force) {
      ret = create_bucket_namespace(store, bucket_info, start_marker);
      if (ret < 0) {
        cerr << "ERROR: failed to create bucket namespace ret=" << ret << std::endl;
        return -ret;
      }
    }
  }

  if (opt_cmd == OPT_BUCKET_NAMESPACE_GET) {
    if (bucket_name.empty()) {
      cerr << "ERROR: bucket not specified" << std::endl;
      return EINVAL;
    }
    ...
    if (bucket_info.namespace_type == BUCKET_NAMESPACE_ENABLE) {
      cout << "enable" << std::endl;
    } else {
      cout << "disable" << std::endl;
    }
  }

  if (opt_cmd == OPT_BUCKET_NAMESPACE_DEL) {
    if (bucket_name.empty()) {
      cerr << "ERROR: bucket not specified" << std::endl;
      return EINVAL;
    }
    ...
    if (!store->is_meta_master()) {
      cerr << "WRANING: you should also disable bucket namespace on master" << std::endl;
    }

    if (bucket_info.namespace_type == BUCKET_NAMESPACE_DISABLE) {
      cerr << "WRANING: bucket namespace already is disabled" << std::endl;
      return 0;
    }

    if (!force) {
      ret = store->bucket_namespace_delete_root(bucket_info);
      if (ret == -ENOTEMPTY) {
        cerr << "ERROR: bucket is not empty!" << std::endl;
        return -ret;
      } if (ret < 0 && ret != -ENOENT) {
