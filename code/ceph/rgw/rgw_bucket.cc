...
int rgw_remove_object(RGWRados *store, RGWBucketInfo& bucket_info, rgw_bucket& bucket, rgw_obj_key& key)
{
  ...
  return store->delete_obj(rctx, bucket_info, obj, bucket_info.versioning_status(), false);
}
                                ...
                                int concurrent_max, bool keep_index_consistent)
{
  ...
  while (is_truncated) {
    ...
    for (; it != objs.end(); ++it) {
      ...
      ret = store->get_obj_state(&obj_ctx, info, obj, &astate, false, false, true);
      if (ret == -ENOENT) {
      ...
      if (astate->has_manifest) {
        ...
        store->obj_to_raw(info.head_placement_rule, head_obj, &raw_head_obj);
...
int rgw_remove_todelete_bucket_data(RGWRados *store, RGWBucketInfo& info)
{
  int ret = 0;
  map<RGWObjCategory, RGWStorageStats> stats;
  std::vector<rgw_bucket_dir_entry> objs;
  ...
  ret = store->get_bucket_stats(info, RGW_NO_SHARD, &bucket_ver, &master_ver, stats, NULL);
  if (ret < 0) {
    dout(0) << __func__ << " EORROR, failed to get bucket stats, bucket: " << info.bucket.name
                        << ", ret: " << ret << dendl;
    return ret;
  }
  ...
  do {
    ...
    ret = list_op.list_objects(max, &objs, &common_prefixes, &is_truncated);
    if (ret < 0) {
      dout(0) << __func__ << " EORROR, failed to list bucket objects, bucket: " << info.bucket.name
                          << ", ret: " << ret << dendl;
      return ret;
    }
    ...
    for (const auto& obj : objs) {
      ...
      if (ret < 0 && ret != -ENOENT) {
        dout(0) << __func__ << " EORROR, failed to remove object: " << obj.key << ", ret: " << ret << dendl;
        return ret;
      }
    }
  } while(is_truncated);
  ...
  if (ret < 0) {
    dout(0) << __func__ << " EORROR, failed to abort_bucket_multiparts, bucket: " << info.bucket.name
                        << ", ret: " << ret << dendl;
    return ret;
  }
...
int rgw_remove_todelete_bucket_data_bypass_gc(RGWRados *store, RGWBucketInfo& info,
                                              int concurrent_max, bool keep_index_consistent)
{
  int ret = 0;
  std::vector<rgw_bucket_dir_entry> objs;
  ...
  while (is_truncated) {
    ...
    for (; it != objs.end(); ++it) {
      RGWObjState *astate = NULL;
      rgw_obj obj(info.bucket, (*it).key);

      ret = store->get_obj_state(&obj_ctx, info, obj, &astate, false);
      if (ret == -ENOENT) {
        dout(0) << __func__ << " WARNING: cannot find obj state for obj " << obj.get_oid() << dendl;
        continue;
      }
      if (ret < 0) {
        lderr(store->ctx()) << __func__ << " ERROR: get obj state returned with error. obj: "
                                        << obj.get_oid() << ", ret: " << ret << dendl;
        return ret;
      }
      ...
      if (astate->has_manifest) {
        ...
        for (; miter != manifest.obj_end() && max_aio--; ++miter) {
          if (!max_aio) {
            ret = drain_handles(handles);
            if (ret < 0) {
              lderr(store->ctx()) << __func__ << " ERROR: could not drain handles as aio completion returned with "
                                              << ret << dendl;
              return ret;
            }
            max_aio = concurrent_max;
          ...
          ret = store->delete_raw_obj_aio(last_obj, handles);
          if (ret < 0) {
            lderr(store->ctx()) << __func__ << " ERROR: delete obj aio failed with " << ret << dendl;
            return ret;
          }
        } // for all shadow objs
      ...
      if (!max_aio) {
        ret = drain_handles(handles);
        if (ret < 0) {
          lderr(store->ctx()) << __func__ << " ERROR: could not drain handles as aio completion returned with " << ret << dendl;
          return ret;
        }
        max_aio = concurrent_max;
      }
...
int rgw_is_todelete_bucket_empty(RGWRados *store, RGWBucketInfo& info, rgw_bucket& bucket, bool& is_empty)
{
  int ret = 0;
  std::vector<rgw_bucket_dir_entry> objs;
  map<string, bool> common_prefixes;
  ...
  is_empty = objs.size() == 0 ? true : false;

  return ret;
}

string todelete_bucket_oid(string bucket_instance_id) {
  return todelete_bucket_instance_oid_prefix + bucket_instance_id;
}

int rgw_store_todelete_bucket_info(RGWRados *store,
                        string bucket_instance,
                        RGWBucketInfo& info)
{
  int ret = 0;

  string todelete_bucket_id = todelete_bucket_oid(bucket_instance);
  bufferlist bl;
  using ceph::encode;
  encode(info, bl);

  ret = rgw_put_system_obj(store, store->get_zone_params().log_pool, todelete_bucket_id,
                           bl, true, NULL, real_time());
  if (ret < 0) {
    dout(0) << __func__ << " ERROR, failed to put object: " << todelete_bucket_id << ", ret: " << ret << dendl;
    return ret;
  }
...
int rgw_get_todelete_bucket_info(RGWRados *store,
                        string bucket_instance,
                        RGWBucketInfo& info)
{
  int ret = 0;
  ...
  bufferlist bl;
  RGWObjectCtx obj_ctx(store);
  ret = rgw_get_system_obj(store, obj_ctx, store->get_zone_params().log_pool, todelete_bucket_id, bl, NULL, NULL);
  if (ret < 0) {
    ldout(store->ctx(), 0) << __func__ << " ERROR: failed to get todelete_bucket object, ret: " << ret << dendl;
    return ret;
  }
  ...
  try {
    ...
    ldout(store->ctx(), 0) << "error decoding data from: " << todelete_bucket_id << dendl;
    return -EIO;
  }
...
int rgw_delete_todelete_bucket(RGWRados *store, string bucket_instance)
{
  string todelete_bucket_id = todelete_bucket_oid(bucket_instance);

  rgw_raw_obj obj(store->get_zone_params().log_pool, todelete_bucket_id);
  int ret = store->delete_system_obj(obj);
  if (ret < 0) {
    dout(0) << __func__ << " ERROR, failed to delete object: " << todelete_bucket_id << ", ret: " << ret << dendl;
    return ret;
  }
...
// only used by todelete_bucket obj
int set_todelete_bucket_stat_resharding(RGWRados *store, RGWBucketInfo& info)
{
  map<string, bufferlist> attrs;

  RGWBucketReshard br(store, info, attrs, nullptr);
  return br.set_resharding_status(info.bucket.bucket_id, info.num_shards, CLS_RGW_RESHARD_IN_PROGRESS);
}
...
void rgw_clear_index_and_todelete_bucket(RGWRados *store, const string& bucket_name, RGWBucketInfo& info)
{
  int ret = 0;
  ret = rgw_get_todelete_bucket_info(store, bucket_name, info);
  if (ret == 0) {
    ret = store->clean_bucket_index(info, info.num_shards);
    if (ret < 0) {
      dout(0) << __func__ << " failed to clean bucket index, bucket: "
                           << bucket_name << ", ret: " << ret << dendl;
    }
  }
  ret = rgw_delete_todelete_bucket(store, bucket_name);
  if (ret < 0) {
    dout(0) <<  __func__ << " failed to remove todelete_bucket obj: " << bucket_name
                         << ", ret: " << ret << dendl;
  }
}
...
int RGWBucket::set_qos(RGWBucketAdminOpState& op_state, std::string *err_msg)
{
  rgw_bucket bucket = op_state.get_bucket();
  RGWBucketInfo bucket_info;
  map<string, bufferlist> attrs;
  ...
  bufferlist qps_bl;
  encode(op_state.qps_limit, qps_bl);
  attrs[RGW_ATTR_QPS_LIMIT] = qps_bl;

  bufferlist bandwidth_bl;
  encode(op_state.bandwidth_limit, bandwidth_bl);
  attrs[RGW_ATTR_BANDWIDTH_LIMIT] = bandwidth_bl;
...
int RGWBucket::set_affiliation(RGWBucketAdminOpState& op_state, std::string *err_msg)
{
  int ret = 0;
  rgw_bucket bucket = op_state.get_bucket();
  RGWBucketInfo bucket_info;
  map<string, bufferlist> attrs;
  RGWObjectCtx obj_ctx(store);
  ret = store->get_bucket_info(obj_ctx, bucket.tenant, bucket.name, bucket_info, NULL, &attrs);
  if (ret < 0) {
    set_err_msg(err_msg, "could not get bucket info for bucket=" + bucket.name + ": " + cpp_strerror(-ret));
    return ret;
  }

  attrs[RGW_ATTR_AFFILIATION].clear();
  attrs[RGW_ATTR_AFFILIATION].append(op_state.get_organization());
  attrs[RGW_ATTR_AFFILIATION].append("/");
  attrs[RGW_ATTR_AFFILIATION].append(op_state.get_project());
  ret = rgw_bucket_set_attrs(store, bucket_info, attrs,
              &bucket_info.objv_tracker);
  if (ret < 0) {
    set_err_msg(err_msg, "ERROR: failed setting bucket instance attr: " + cpp_strerror(-ret));
    return ret;
  }
  return ret;
}
                              ...
                              map<std::string, RGWStorageStats>* storageclass_stats = NULL)
{
  ...
  for (iter = stats.begin(); iter != stats.end(); ++iter) {
    ...
    s.dump(formatter);
    if (storageclass_stats && iter->first == RGW_OBJ_CATEGORY_MAIN) {
      map<std::string, RGWStorageStats>::iterator citer;

      formatter->open_object_section("usage_by_storageclass");
      for (citer = storageclass_stats->begin(); citer != storageclass_stats->end(); ++citer) {
        RGWStorageStats& cs = citer->second;
        const char* storage_class = (citer->first).c_str();
                                  ...
                                  std::string *err_msg)
{
  ...
  while (is_truncated) {
    ...
    int r = store->cls_bucket_list_ordered(bucket_info, RGW_NO_SHARD,
                                           marker, prefix, listing_max_entries,
                                           true, false,
                                           expansion_factor,
...
static int bucket_stats(RGWRados *store, const std::string& tenant_name, std::string&  bucket_name, Formatter *formatter,
                        bool with_put_perm = false, RGWBucketInfo* bucket_info_t = nullptr)
{
  RGWBucketInfo bucket_info;
  map<RGWObjCategory, RGWStorageStats> stats;
  ...
  string bucket_ver, master_ver;
  string max_marker;
  int ret = store->get_bucket_stats(bucket_info, RGW_NO_SHARD, &bucket_ver, &master_ver, stats, &max_marker, NULL, &storageclass_stats);
  if (ret < 0) {
  ...
  aiter = attrs.find(RGW_ATTR_QPS_LIMIT);
  if (aiter != attrs.end()) {
    bufferlist::iterator liter = aiter->second.begin();
    decode(qps_limit, liter);
  }
  aiter = attrs.find(RGW_ATTR_BANDWIDTH_LIMIT);
  if (aiter != attrs.end()) {
    bufferlist::iterator liter = aiter->second.begin();
  ...
  std::string project = "";
  aiter = attrs.find(RGW_ATTR_AFFILIATION);
  if (aiter != attrs.end()) {
    std::string affiliation = aiter->second.to_str();
  ...
  if (with_put_perm) {
    ...
    dump_bucket_usage(stats, formatter, &storageclass_stats);
  ...
  formatter->dump_string("placement_rule", bucket_info.head_placement_rule.to_str());
  ...
  formatter->dump_stream("namespace_type") << bucket_info.namespace_type;
  ::encode_json("owner", bucket_info.owner, formatter);
  ...
  dump_bucket_usage(stats, formatter, &storageclass_stats);
  encode_json("bucket_quota", bucket_info.quota, formatter);
  formatter->dump_int("qps_limit", qps_limit);
  formatter->dump_int("bandwidth_limit", bandwidth_limit);
  formatter->dump_int("version_flag", bucket_info.flags);
  formatter->open_array_section("disable_sync_flags");
  for (auto& s : bucket_info.disable_sync) {
    formatter->open_object_section("zone");
    formatter->dump_string("zone", s.first);
    formatter->dump_bool("disable_sync", s.second);
    formatter->close_section();
  }
  formatter->close_section();
  formatter->open_array_section("bilog_start");
  for (auto& f : bucket_info.bilog_flags) {
    if (store->get_zone().name.compare(f.first) == 0) {
      formatter->open_object_section("zone");
      formatter->dump_string("zone", f.first);
      formatter->dump_bool("bilog_start", f.second);
      formatter->close_section();
    }
  }
  formatter->close_section();
  formatter->open_array_section("sync_config");
  for (auto& config : bucket_info.sync_config) {
    formatter->open_object_section("zone");
    formatter->dump_string("zone", config.first);
    formatter->dump_int("start_sync_time", config.second.start_sync_time);
    formatter->dump_int("stop_sync_time", config.second.stop_sync_time);
    formatter->dump_bool("is_delay", config.second.is_delay);
    formatter->close_section();
  }
  formatter->close_section();
  formatter->dump_string("organization", organization);
  formatter->dump_string("project", project);
  formatter->dump_bool("is_banned", is_ban);
  formatter->dump_string("storage_class", bucket_info.storage_class);
  formatter->close_section();
...
struct pool_t {
  std::string name;
  int64_t id;
  size_t free_capacity; // kb

  void decode_json(JSONObj *obj) {
    JSONDecoder::decode_json("name", name, obj);
    JSONDecoder::decode_json("id", id, obj);
    JSONObjIter iter = obj->find_first("stats");
    if (iter.end()) {
      return;
    }
    JSONDecoder::decode_json("max_avail", free_capacity, *iter);
    free_capacity = free_capacity / 1024;
  }
};

int RGWBucketAdminOp::bucket_pool_capacity(RGWRados *store, RGWBucketInfo& bucket_info, int64_t *avail_size)
{
  string zone_id,zone_name;
  RGWZoneParams zone(zone_id, zone_name);
  int ret = zone.init(store->ctx(), store);
  if (ret < 0) {
    ldout(store->ctx(), 0) << __func__ << " ERROR: unable to initialize zone: ret=" << ret << dendl;
    return ret;
  }

  auto it = zone.placement_pools.find(bucket_info.head_placement_rule.name);
  if (it == zone.placement_pools.end()) {
    ldout(store->ctx(), 0) << __func__ << " ERROR: no such placement: placement_rule_name=" << bucket_info.head_placement_rule.name << dendl;
    return STATUS_NO_CONTENT;
  }
  rgw_pool data_pool = it->second.get_standard_data_pool();
  if (data_pool.empty()) {
    ldout(store->ctx(), 0) << __func__ << " ERROR: failed to get data pool" << dendl;
    return STATUS_NO_CONTENT;
  }

  librados::Rados* rados = store->get_rados_handle();
  bufferlist inbl, outbl;
  int op_ret = rados->mon_command("{\"prefix\": \"df\" ,\"format\": \"json\"}", inbl, &outbl, NULL);
  if (op_ret < 0) {
    ldout(store->ctx(), 0) << __func__ << " ERROR: failed to get pool info from mon. outbl: " << outbl.c_str() << dendl;
    return op_ret;
  }
  JSONParser parser;
  op_ret = parser.parse(outbl.c_str(), outbl.length());
  if (op_ret < 0) {
    ldout(store->ctx(), 0) << __func__ << " ERROR: failed to parse pool info. outbl: " << outbl.c_str() << dendl;
    return op_ret;
  }
  std::vector<pool_t> pools;
  JSONObjIter iter = parser.find_first("pools");
  decode_json_obj(pools, *iter);
  auto p_it = pools.begin();
  for (; p_it != pools.end(); p_it++) {
    if (p_it->name == data_pool.name) {
      // get base data pool id
      int64_t base_tier_id = 0;
      int r = rados->get_base_pool(p_it->id, &base_tier_id);
      if (r < 0) {
        ldout(store->ctx(), 0) << __func__ << " ERROR: failed to get base data pool id. pool_id=" << p_it->id << dendl;
        return r;
      }
      if (base_tier_id == p_it->id) {
        *avail_size = p_it->free_capacity;
      } else {
        auto iter = pools.begin();
        for (; iter != pools.end(); iter++) {
          if (iter->id == base_tier_id) {
            *avail_size = iter->free_capacity;
          }
        }
      }
    }
...
int RGWBucketAdminOp::info(RGWRados *store, RGWBucketAdminOpState& op_state,
                  RGWFormatterFlusher& flusher, bool with_put_perm)
{
  int ret = 0;
  ...
  if (op_state.is_user_op()) {
    ...
    RGWBucketInfo bucket_info;
    ret = bucket_stats(store, user_id.tenant, bucket_name, formatter, with_put_perm, &bucket_info);
    if (ret < 0) {
      return ret;
    }
    ...
    if (with_put_perm) {
      int64_t pool_size = 0;
      int r = bucket_pool_capacity(store, bucket_info, &pool_size);
      encode_json("bucket_pool_free_size", pool_size, formatter);
      formatter->close_section();
      if (r < 0) {
        ldout(cct, 0) << __func__ << " ERROR: failed to get pool capacity. r=" << r << dendl;
        return r;
      }
    }
  } else {
...
int RGWBucketAdminOp::sync_status(RGWRados *store, RGWBucketAdminOpState& op_state,
                                  RGWFormatterFlusher& flusher)
{
  int ret = 0;
  string& bucket_name = op_state.get_bucket_name();
  ...
  if (bucket_name.empty()) {
    formatter->dump_string("errmsg", "bucket is empty!");
    formatter->close_section(); // sync_status
    flusher.flush();
    return -EINVAL;
  }
  ...
  ret = store->get_bucket_info(obj_ctx, "", bucket_name, bucket_info, nullptr, nullptr);
  if (ret < 0) {
    stringstream ss;
    ss << "get bucket info error:";
    ss << ret;
    formatter->dump_string("errmsg", ss.str());
    formatter->close_section(); // sync_status
    flusher.flush();
    return ret;
  }
  ...
  formatter->dump_string("zone_name", zone.name);
  if (!bucket_info.datasync_flag_enabled(zone.name)) {
    formatter->dump_bool("sync_enabled", false);
  } else {
    formatter->dump_bool("sync_enabled", true);
  }

  RGWZoneGroup& zonegroup = store->get_zonegroup();


  formatter->open_array_section("zones");
  for (const auto& z : zonegroup.zones) {
    if (!z.second.tier_type.empty()) {
      continue;
    }
    ...
    if (c != store->zone_conn_map.end()) {
      ...
      formatter->dump_string("source_zone_name", z.second.name);

      if (!zone.syncs_from(z.second.id)) {
        formatter->dump_bool("sync_enabled", false);
      } else {
        formatter->dump_bool("sync_enabled", true);
      }
      ret = rgw_bucket_sync_status(store, z.second.id, bucket_info, &status);
      if (ret < 0) {
        lderr(store->ctx()) << "failed to read bucket sync status: " << cpp_strerror(ret) << dendl;
        return ret;
      }

      using BucketSyncState = rgw_bucket_shard_sync_info::SyncState;
      for (size_t shard_id = 0; shard_id < status.size(); shard_id++) {
      ...
      ret = remote_bilog_markers(store, c->second, bucket_info, &remote_markers);
      if (ret < 0) {
        lderr(store->ctx()) << "failed to read remote log: " << cpp_strerror(ret) << dendl;
        return ret;
      }
      ...
      for (auto& r : remote_markers.get()) {
        ...
        if (m.state != BucketSyncState::StateIncrementalSync || pos != r.second) {
          ...
          ceph::logging::Entry *_dout_e = store->ctx()->_log->create_entry(10, dout_subsys);
          std::ostream* _dout = &_dout_e->get_ostream();
          gap_entries += sync_marker_gap(pos, r.second, _dout);
        }
      }

      if (shards_behind.empty() && !num_full) {
        formatter->dump_bool("caught", true);
      }
      formatter->dump_unsigned("behind_entries", gap_entries);
      formatter->close_section(); // zone
    }
  }
  formatter->close_section(); // zones
  formatter->close_section(); // sync_status
...
int RGWBucketAdminOp::set_qos(RGWRados *store, RGWBucketAdminOpState& op_state)
{
  RGWBucket bucket;
  ...
  if (ret < 0)
    return ret;
  return bucket.set_qos(op_state);
}

int RGWBucketAdminOp::set_affiliation(RGWRados *store, RGWBucketAdminOpState& op_state)
{
  RGWBucket bucket;
  int ret = bucket.init(store, op_state);
  if (ret < 0)
    return ret;
  return bucket.set_affiliation(op_state);
}
...
int RGWDataChangesLog::add_entry(rgw_bucket& bucket, int shard_id) {
  ...
  // still add change log to data_log[i], just don't notify unneccessary zone
  // bilog is added in cls_rgw
  RGWObjectCtx obj_ctx(store);
  RGWBucketInfo bucket_info;
  int r = store->get_bucket_info(obj_ctx, bucket.tenant, bucket.name, bucket_info, NULL);
  if (r < 0) {
    ldout(store->ctx(), 0) << __func__ << "() get bucket("<< bucket.name << ") info error:" << r << dendl;
    return r;
  }

  int index = choose_oid(bs);

  auto iter = bucket_info.sync_config.find(store->get_zone().name);
  if ((iter != bucket_info.sync_config.end() && iter->second.is_delay) ||
      !bucket_info.datasync_flag_enabled(store->get_zone().name)) {
    ldout(store->ctx(), 10) << __func__ << "() local zone is delay or sync disabled, jump notify others zone" << dendl;
    return 0;
  }

  for (auto iter = store->zone_data_notify_to_map.begin(); iter != store->zone_data_notify_to_map.end(); ++iter) {
    auto conf_iter = bucket_info.sync_config.find(iter->first);
    if (conf_iter != bucket_info.sync_config.end()) {
      if (conf_iter->second.is_delay) {
        ldout(store->ctx(), 10) << __func__ << "() zone is delay, jump notify:"<< iter->first << dendl;
        continue;
      }
    }
    auto iter_z = store->zone_by_id.find(iter->first);
    if (iter_z == store->zone_by_id.end()) {
      ldout(store->ctx(), 0) << __func__ << "() ERROR can't find zone in zone_by_id:"<< iter->first << dendl;
      continue;
    }
    if (!bucket_info.datasync_flag_enabled(iter_z->second.name)) {
      ldout(store->ctx(), 10) << __func__ << "() zone is sync disabled, jump notify:"<< iter_z->second.name << dendl;
      continue;
    }

    ldout(store->ctx(), 20) << __func__ << "() notify to zone:"<< iter->first << dendl;
    mark_modified(iter->first, index, bs);
  }
  ...
  do {
    ...
    ldout(cct, 10) << "RGWDataChangesLog::add_entry() sending update with now=" << now << " cur_expiration=" << expiration << dendl;
...
void RGWDataChangesLog::mark_modified(string zone_id, int shard_id, const rgw_bucket_shard& bs)
{
  auto key = bs.get_key();
  modified_lock.get_read();

  // one bs may be writed many times in a interval，so judge once before getting write lock
  auto iter_f = modified_shards.find(zone_id);
  if (iter_f != modified_shards.end()) {
    map<int, set<string> >& temp_map = iter_f->second;
    map<int, set<string> >::iterator iter_s = temp_map.find(shard_id);
    if (iter_s != temp_map.end()) {
      set<string>& keys = iter_s->second;
  ...
  iter_f = modified_shards.find(zone_id);
  if (iter_f == modified_shards.end()) {
    map<int, set<string> > temp_map;
    temp_map[shard_id].insert(key);
    modified_shards[zone_id] = temp_map;
  } else {
    // no need to judge whether the key is already stored, because stl::set can insert
    // repeatly and mostly not in set at this place
    iter_f->second[shard_id].insert(key);
  }
}

void RGWDataChangesLog::read_clear_modified(map<string, map<int, set<string> > > &modified)
{
  RWLock::WLocker wl(modified_lock);
...
public:
  ...
  int put(RGWRados *store, string& entry, RGWObjVersionTracker& objv_tracker,
    ...
    if (be.linked) {
      ...
      if (old_be.linked && be.owner != old_be.owner) {
        ret = rgw_unlink_bucket(store, old_be.owner, be.bucket.tenant,
  ...
  int remove(RGWRados *store, string& entry, RGWObjVersionTracker& objv_tracker) override {
    ...
    RGWBucketInfo bucket_info;
    ret = store->get_bucket_info(obj_ctx, tenant_name, bucket_name, bucket_info, NULL);
    if (ret < 0) {
      ldout(store->ctx(), 0) << "ERROR: could not fetch bucket info: ret=" << ret << dendl;
      return ret;
    }

    ret = store->get_lc()->remove_lc_entries(bucket_info.bucket, 0, bucket_info.num_shards);
    if (ret < 0) {
      ldout(store->ctx(), 0) << "WARNING: failed to delete lifecyle entry"
...
public:
  ...
  int put(RGWRados *store, string& entry, RGWObjVersionTracker& objv_tracker,
    ...
    if (!exists || old_bci.info.bucket.bucket_id != bci.info.bucket.bucket_id) {
      ...
      ret = store->select_bucket_location_by_rule(bci.info.head_placement_rule.name, &rule_info);
      if (ret < 0) {
      ...
      bci.info.head_placement_rule = old_bci.info.head_placement_rule;
    }

    string zone = store->get_zone().name;

    if (exists && (old_bci.info.datasync_flag_enabled(zone) != bci.info.datasync_flag_enabled(zone)       // local zone start/stop sync
          || store->datasync_all_disabled(bci.info)                                                       // all others zone stop sync
          || (store->datasync_all_disabled(old_bci.info) && !store->datasync_all_disabled(bci.info)))) {  // all others zone stop -> start
      int shards_num = bci.info.num_shards? bci.info.num_shards : 1;
      int shard_id = bci.info.num_shards? 0 : -1;

      bool bucket_initial = true;
      #ifdef WITH_UNAUTO_SYNC
        bucket_initial = bci.info.disable_sync.size() == 0 && bci.info.bilog_flags.size() == 0;
      #endif
      if (!bci.info.datasync_flag_enabled(zone) || store->datasync_all_disabled(bci.info) ||
           bucket_initial) { // disable bucket sync at initial status
        ret = store->stop_bi_log_entries(bci.info, -1);
        if (ret < 0) {
          lderr(store->ctx()) << "ERROR: failed writing bilog" << dendl;
          return ret;
        }
        bci.info.bilog_flags[zone] = false;
      } else {
        ...
        bci.info.bilog_flags[zone] = true;
      }
    ...
    // multisite sync lifecyle entry
    auto old_aiter = old_bci.attrs.find(RGW_ATTR_LC);
    auto aiter = bci.attrs.find(RGW_ATTR_LC);
    if (old_aiter == old_bci.attrs.end() && aiter != bci.attrs.end()) {
      ldout(store->ctx(), 20) << "multisite sync lifecyle entry" << dendl;
      ret = store->get_lc()->set_lc_entries(bci.info.bucket, 0, bci.info.num_shards);
      if (ret < 0) {
        ldout(store->ctx(), 0) << "ERROR: multisite sync lifecyle set_lc_entry"
                              << ", bucket:" << bci.info.bucket.name
                              << ", ret:" << ret
                              << dendl;
        return ret;
      }
    } else if (old_aiter != old_bci.attrs.end() && aiter == bci.attrs.end()) {
      ldout(store->ctx(), 20) << "multisite clear lifecyle entry" << dendl;
      ret = store->get_lc()->remove_lc_entries(old_bci.info.bucket, 0, old_bci.info.num_shards);
      if (ret < 0) {
        ldout(store->ctx(), 0) << "WARNING: failed to delete lifecyle entry"
                               << ", bucket_name:" << old_bci.info.bucket.name
                               << ", ret=" << ret << dendl;
        return ret;
      }
    }

    if (bci.info.namespace_type != old_bci.info.namespace_type) {
      // TODO: need to consider the following case:
      // 1. old namepsace type is enable, new type is disable
      // 2. old namespace type is disable, new type is enable, and the bucket is not empty
    }
  ...
  int remove(RGWRados *store, string& entry,
    ...
    return rgw_store_todelete_bucket_info(store, entry, info);
  }
