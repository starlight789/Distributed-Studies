...
namespace rgw {
  ...
  public:
    ...
    static constexpr uint32_t FLAG_TRUNCATE = 0x2000;
...
#define CREATE_FLAGS(x) \
  ...
  public:
    ...
    bool is_truncate() const { return flags & FLAG_TRUNCATE; }
    ...
    int open(uint32_t gsh_flags, uint32_t posix_flags) {
...
public:
  ...
  void send_response() override {
	...
	sref.remove_prefix(last_del+1);
      ...
      this->operator()(sref, next_marker, cnow, 0, RGW_FS_TYPE_DIRECTORY);
      ++ix;
    }
  }
...
public:
  ...
  RGWPutObjProcessor *select_processor(RGWObjectCtx& obj_ctx, int* const op_ret = nullptr) override {
    struct req_state* s = get_state();
