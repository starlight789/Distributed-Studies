...
private:
  bool with_put_perm = false;
};

int RGWOp_Bucket_Info::verify_permission()
{
  if (s->prot_flags & RGW_REST_BOS) {
    bool admin_req;
    RESTArgs::get_bool(s, "admin", false, &admin_req);
    if (admin_req && verify_bucket_permission(s, rgw::IAM::s3PutObject)) {
      with_put_perm = true;
      return 0;
    }
  }

  return s->user->caps.check_cap("buckets", RGW_CAP_READ);
}
...
void RGWOp_Bucket_Info::execute()
{
  ...
  http_ret = RGWBucketAdminOp::info(store, op_state, flusher, with_put_perm);
}
...
void RGWOp_Set_Bucket_Quota::execute()
{
  ...
  if (use_http_params) {
    ...
    int64_t old_max_size_kb;
    if (old_quota->max_size > 0) {
      old_max_size_kb = rgw_rounded_kb(old_quota->max_size);
    } else {
      old_max_size_kb = -rgw_rounded_kb(abs(old_quota->max_size));
    }
    int64_t max_size_kb;
...
#ifdef WITH_BCEIAM
class RGWOp_Set_Bucket_Affiliation : public RGWRESTOp {

public:
  RGWOp_Set_Bucket_Affiliation() {}
  ...
  virtual const string name() { return "set_bucket_affiliation"; }
};

void RGWOp_Set_Bucket_Affiliation::execute()
{
  bool uid_arg_existed = false;
  std::string uid_str;
  RESTArgs::get_string(s, "uid", uid_str, &uid_str, &uid_arg_existed);
  ...
  bool organization_arg_existed = false;
  std::string organization;
  RESTArgs::get_string(s, "organization", organization, &organization, &organization_arg_existed);
  if (! organization_arg_existed) {
    http_ret = -EINVAL;
    return;
  }

  bool project_arg_existed = false;
  std::string project;
  RESTArgs::get_string(s, "project", project, &project, &project_arg_existed);
  if (! project_arg_existed) {
    http_ret = -EINVAL;
    return;
  }
  ...
  op_state.set_user_id(uid);
  op_state.set_bucket_name(bucket);
  op_state.set_project(project);
  op_state.set_organization(organization);

  http_ret = RGWBucketAdminOp::set_affiliation(store, op_state);
}
#endif
...
class RGWOp_Set_Bucket_Qos : public RGWRESTOp {

public:
  RGWOp_Set_Bucket_Qos() {}
  ...
  virtual const string name() { return "set_bucket_qos"; }
};

void RGWOp_Set_Bucket_Qos::execute()
{
  bool uid_arg_existed = false;
  std::string uid_str;
  RESTArgs::get_string(s, "uid", uid_str, &uid_str, &uid_arg_existed);
  ...
  if (!bucket_arg_existed) {
    http_ret = -EINVAL;
    s->err.message = "the bucket name is null";
    return;
  }

  int64_t qps_limit = -1;
  int64_t bandwidth_limit = -1;
  bool qps_arg_existed = false;
  bool bandwidth_arg_existed = false;
  RESTArgs::get_int64(s, "qps-limit", qps_limit, &qps_limit, &qps_arg_existed);
  RESTArgs::get_int64(s, "bandwidth-limit", bandwidth_limit, &bandwidth_limit, &bandwidth_arg_existed);
  if (!qps_arg_existed && !bandwidth_arg_existed) {
    http_ret = -EINVAL;
    s->err.message = "the qos params is null";
    return;
  }
  ...
  op_state.set_user_id(uid);
  op_state.set_bucket_name(bucket);
  op_state.set_qps_limit(qps_limit);
  op_state.set_bandwidth_limit(bandwidth_limit);

  http_ret = RGWBucketAdminOp::set_qos(store, op_state);
}

class RGWOp_Get_Bucket_Syncstatus : public RGWRESTOp {

public:
  RGWOp_Get_Bucket_Syncstatus() {}
  ...
  const string name() override { return "get_bucket_sync_status"; }
};

void RGWOp_Get_Bucket_Syncstatus::execute()
{
  RGWBucketAdminOpState op_state;

  std::string bucket, source_zone;

  RESTArgs::get_string(s, "bucket", bucket, &bucket);
  RESTArgs::get_string(s, "source_zone", source_zone, &source_zone);

  op_state.set_bucket_name(bucket);

  http_ret = RGWBucketAdminOp::sync_status(store, op_state, flusher);
}
...
RGWOp *RGWHandler_Bucket::op_get()
{
  ...
  if (s->info.args.sub_resource_exists("syncstatus"))
    return new RGWOp_Get_Bucket_Syncstatus;
...
RGWOp *RGWHandler_Bucket::op_put()
{
  ...
  if (s->info.args.sub_resource_exists("qos"))
    return new RGWOp_Set_Bucket_Qos;
#ifdef WITH_BCEIAM
  if (s->info.args.exists("organization"))
    return new RGWOp_Set_Bucket_Affiliation;
#endif
