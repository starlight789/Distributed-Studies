...
protected:
  ...
  ACLOwner owner;
  bool obj_same_with_bucket_acl = true;
...
public:
  ...
  void encode(bufferlist& bl) const {
    ...
    encode(owner, bl);
    encode(acl, bl);
    encode(obj_same_with_bucket_acl, bl);
    ENCODE_FINISH(bl);
  }
  void decode(bufferlist::iterator& bl) {
    DECODE_START_LEGACY_COMPAT_LEN(3, 2, 2, bl);
    ...
    decode(acl, bl);
    if (struct_v >= 3)
      decode(obj_same_with_bucket_acl, bl);
    DECODE_FINISH(bl);
  }
  void dump(Formatter *f) const;
  ...
  // BOS: if don't set object acl, obj acl is same with bucket acl,
  // if set object acl is private, just bucket have full_control permission
  void set_obj_same_with_bucket_acl(bool _obj_same_with_bucket_acl) {
    obj_same_with_bucket_acl = _obj_same_with_bucket_acl;
  }

  bool is_obj_same_with_bucket_acl() {
    return obj_same_with_bucket_acl;
  }
