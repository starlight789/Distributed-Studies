...
public:
  ...
  void set_storage_class(const string& _storage_class) {
...
class LCFilter
{
 protected:
  std::string prefix;
  std::string suffix;  // suffix need specified in Filter struct
  RGWObjTags obj_tags;
 ...
 public:
  ...
  bool empty() const {
    return !(has_prefix() || has_tags() || has_suffix());
  }
...
protected:
  string id;
  string prefix; // don't support suffix specified in Rule struct
  string status;
...
public:
  ...
  void set_filter(const LCFilter& _filter) {
    filter = _filter;
  }
...
protected:
  ...
  bool has_overlap_suffix(const lc_op& first, const lc_op& second);
public:
...
public:
  ...
  public:
    LCWorker(const DoutPrefixProvider* _dpp, CephContext *_cct, RGWLC *_lc, int _ix);
  ...
  int bucket_lc_process(string& bucket, int shard_id, LCWorker* worker, time_t stop_at, bool once);
  int bucket_lc_post(int index, int max_lock_sec, cls_rgw_lc_entry& entry, int& result, LCWorker* worker);
  bool going_down();
  void start_processor();
  void stop_processor();
  int set_lc_entries(rgw_bucket& bucket, int begin_shard_id, int end_shard_id);
  int remove_lc_entries(rgw_bucket& bucket, int begin_shard_id, int end_shard_id);
  ...
  int reshard_bucket_lc_entry(const string& bucket_name);
  int handle_multipart_expiration(RGWRados::Bucket *target, const multimap<string, lc_op>& prefix_map,
