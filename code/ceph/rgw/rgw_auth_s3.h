...
#ifdef WITH_BCEIAM
      ,iam_engine(cct, store, *ver_abstractor,
                  static_cast<rgw::auth::RemoteApplier::Factory*>(this))
#endif
  {
    ...
    if (cct->_conf->rgw_s3_auth_use_iam) {
      add_engine(Control::SUFFICIENT, iam_engine);
    }
#endif
  }
...
#ifdef WITH_BCEBOS
void generate_bce_default_header(const req_info& info,
                                boost::string_view& signed_hdrs);

int parse_bos_authorization(const req_info& info,                   /* in  */
                            boost::string_view& access_key_id,      /* out */
                            boost::string_view& signed_hdrs,        /* out */
                            boost::string_view& bce_auth_version,   /* out */
                            boost::string_view& date,               /* out */
                            boost::string_view& expiration_periond, /* out */
                            boost::string_view& client_signature,   /* out */
                            const bool using_auth_qs);              /* in  */
#endif
...
static inline std::string aws4_uri_encode(const std::string& src, bool encode_slash = true)
{
  std::string result;
...
static inline std::string aws4_uri_recode(const boost::string_view& src, bool encode_slash = true)
{
...
static inline const char* get_v4_exp_payload_hash(const req_info& info, int prot_flag = 0)
{
  const char *expected_request_payload_hash = nullptr;
#ifdef WITH_BCEBOS
  if (prot_flag & RGW_REST_BOS) {
    // if here we come, we must have at least one expected_request_payload_hash 
    expected_request_payload_hash = info.env->get("HTTP_X_BCE_CONTENT_SHA256");
    if (!expected_request_payload_hash) {
      expected_request_payload_hash = info.args.get("x-bce-content-sha256").c_str();
    }
  } else
#endif
  {
...
#ifdef WITH_BCEBOS
std::string
get_bos_canonical_qs(const req_info& info, const bool using_auth_qs);

std::string
get_bos_canonical_headers(const req_info& info,
                          const string& signedheaders,
                          const bool using_auth_qs,
                          const bool force_boto2_compat);

std::string
get_bos_canonical_request(const req_info& info,
                          const boost::string_view& canonical_qs,
                          const boost::string_view& canonical_hdrs);

AWSEngine::VersionAbstractor::string_to_sign_t
get_bos_string_to_sign(CephContext* const cct,
                       const boost::string_view& bce_auth_version,
                       const boost::string_view& access_key_id,
                       const boost::string_view& date,
                       const boost::string_view& expiration_periond);
#endif
...
#ifdef WITH_BCEBOS
extern AWSEngine::VersionAbstractor::server_signature_t
get_bos_signature(const boost::string_view& canonical_request,
                 CephContext* const cct,
                 const boost::string_view& secret_key,
                 const AWSEngine::VersionAbstractor::string_to_sign_t& string_to_sign);
#endif
