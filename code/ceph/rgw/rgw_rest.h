...
public:
  ...
  virtual int serve_errordoc(RGWHandler * const handler);
};
...
public:
  ...
  ~RGWRenameObj_ObjStore() override {}
};
...
class RGWPutImageStyle_ObjStore : public RGWPutImageStyle {
public:
  RGWPutImageStyle_ObjStore() {}
  ~RGWPutImageStyle_ObjStore() override {}

  int get_params() override;
};

class RGWPutImageProtection_ObjStore : public RGWPutImageProtection {
public:
  RGWPutImageProtection_ObjStore() {}
  ~RGWPutImageProtection_ObjStore() override {}
...
class RGWPutBucketNotification_ObjStore : public RGWPutBucketNotification {
public:
  RGWPutBucketNotification_ObjStore() {}
  ~RGWPutBucketNotification_ObjStore() override {}
};

class RGWGetBucketNotification_ObjStore : public RGWGetBucketNotification {
public:
  RGWGetBucketNotification_ObjStore() {}
  ~RGWGetBucketNotification_ObjStore() override {}
};

class RGWDeleteBucketNotification_ObjStore : public RGWDeleteBucketNotification {
public:
  RGWDeleteBucketNotification_ObjStore() {}
  ~RGWDeleteBucketNotification_ObjStore() override {}
};

class RGWPutBucketQuota_ObjStore : public RGWPutBucketQuota {
public:
  RGWPutBucketQuota_ObjStore() {}
  ~RGWPutBucketQuota_ObjStore() override {}
};

class RGWGetBucketQuota_ObjStore : public RGWGetBucketQuota {
public:
  RGWGetBucketQuota_ObjStore() {}
  ~RGWGetBucketQuota_ObjStore() override {}
};

class RGWDeleteBucketQuota_ObjStore : public RGWDeleteBucketQuota {
public:
  RGWDeleteBucketQuota_ObjStore() {}
  ~RGWDeleteBucketQuota_ObjStore() override {}
};

class RGWPutUserQuota_ObjStore : public RGWPutUserQuota {
public:
  RGWPutUserQuota_ObjStore() {}
  ~RGWPutUserQuota_ObjStore() override {}
};

class RGWGetUserQuota_ObjStore : public RGWGetUserQuota {
public:
  RGWGetUserQuota_ObjStore() {}
  ~RGWGetUserQuota_ObjStore() override {}
};

class RGWDeleteUserQuota_ObjStore : public RGWDeleteUserQuota {
public:
  RGWDeleteUserQuota_ObjStore() {}
  ~RGWDeleteUserQuota_ObjStore() override {}
};
...
class RGWGetSymlink_ObjStore : public RGWGetSymlink {
public:
  RGWGetSymlink_ObjStore() {}
  ~RGWGetSymlink_ObjStore() {}
  void send_response() {}
};

class RGWPutSymlink_ObjStore : public RGWPutSymlink {
public:
  RGWPutSymlink_ObjStore() {}
  ~RGWPutSymlink_ObjStore() {}
};
...
public:
  ...
  virtual int serve_retarget(RGWOp *op);
};
...
#ifdef WITH_BCEBOS
  bool is_bos_used(struct req_state *s);
  bool is_s3_used(struct req_state *s);
#endif
};
...
extern void end_header(struct req_state *s,
                       ...
                       const string& custom_meta = "",
                       bool force_err_msg = false);
...
extern void dump_access_control(struct req_state *s, const char *origin,
				...
				uint32_t max_age);
extern void dump_access_control(req_state *s, RGWOp *op, const string& custom_meta);
...
inline bool is_anonymous(const struct req_state* const s) {
  return !s->info.env->exists("HTTP_AUTHORIZATION") && !s->info.args.exists("authorization");
}

inline bool is_browser_client(const struct req_state* const s) {
  if (s->info.env->exists("HTTP_X_REQUEST_BY")) {
    std::string request_by(s->info.env->get("HTTP_X_REQUEST_BY"));
    if (request_by.compare("ERApplication") == 0) {
      return false;
    }
  }

  if (s->info.env->exists("HTTP_USER_AGENT")) {
    std::string user_agent(s->info.env->get("HTTP_USER_AGENT"));
    if (user_agent.find("Mozilla") != std::string::npos) {
      return true;
    }
    if (user_agent.find("Presto") != std::string::npos) {
      return true;
    }
    if (user_agent.find("UCWEB") != std::string::npos) {
      return true;
    }
  }
  return false;
