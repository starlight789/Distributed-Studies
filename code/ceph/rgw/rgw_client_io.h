...
public:
  ...
  /* throttle tranffic on rgw */
  virtual void get_throttle_token(int num, shared_ptr<TokenBucketThrottle> token_bucket) {
    return;
  }
...
public:
  ...
  void get_throttle_token(int num, shared_ptr<TokenBucketThrottle> token_bucket) override {
    get_decoratee().get_throttle_token(num, token_bucket);
    return;
  }
