...
//128 + strlen(S3_USER_STR)
#define JSON_MAX_VALUE_LEN 147
...
{{ "s3:AbortMultipartUpload", s3AbortMultipartUpload },
 ...
 { "s3:DeleteBucketNamespace", s3DeleteBucketNamespace },
 ...
 { "s3:DeleteBucketLogging", s3DeleteBucketLogging },
 { "s3:DeleteBucketEncryption", s3DeleteBucketEncryption },
 ...
 { "s3:GetBucketMirroring", s3GetBucketMirroring },
 { "s3:GetBucketWebsite", s3GetBucketWebsite },
 { "s3:GetBucketNamespace", s3GetBucketNamespace },
 ...
 { "s3:GetBucketStorageClass", s3GetBucketStorageClass },
 ...
 { "s3:ListMultiObjects", s3ListMultiObjects },
 { "s3:ListBucket", s3ListBucket },
 { "s3:ListObjects", s3ListObjects },
 ...
 { "s3:PutBucketMirroring", s3PutBucketMirroring },
 { "s3:PutBucketWebsite", s3PutBucketWebsite },
 { "s3:PutBucketNamespace", s3PutBucketNamespace },
 ...
 { "s3:ListParts", s3ListParts },
 { "s3:PutBucketCors", s3PutBucketCors },
 { "s3:GetBucketCors", s3GetBucketCors },
 { "s3:GetBucket", s3GetBucket },
 { "s3:HeadBucket", s3HeadBucket },
 { "s3:GetObjectMeta", s3GetObjectMeta },
 { "s3:PutBucketStyle", s3PutImageStyle },
 { "s3:GetBucketStyle", s3GetImageStyle },
 { "s3:PutCopyRightProtection", s3PutImageProtection },
 { "s3:GetCopyRightProtection", s3GetImageProtection },
 { "s3:InitBucketObjectLockConfiguration", s3InitBucketObjectLockConfiguration },
 { "s3:DeleteBucketObjectLockConfiguration", s3DeleteBucketObjectLockConfiguration },
 { "s3:CompleteBucketObjectLockConfiguration", s3CompleteBucketObjectLockConfiguration },
 { "s3:ExtendBucketObjectLockConfiguration", s3ExtendBucketObjectLockConfiguration },
 { "READ", bosRead },
 { "LIST", bosList },
 { "WRITE", bosWrite },
 { "FULL_CONTROL", bosFullControl },
 { "MODIFY", bosModify }};

static const actpair bos_bucket_op[] =
{{ "s3:GetBucket", s3GetBucket },
 { "s3:HeadBucket", s3HeadBucket },
 { "s3:ListObjects", s3ListObjects },
 { "s3:GetBucketLocation", s3GetBucketLocation },
...
static const actpair bos_read_op[] =
{{ "READ", bosRead },
 { "s3:GetObject", s3GetObject },
 { "s3:GetObjectMeta", s3GetObjectMeta },
 { "s3:HeadBucket", s3HeadBucket },
...
static const actpair bos_list_op[] =
{{ "LIST", bosList },
 { "s3:ListObjects", s3ListObjects },
 { "s3:ListMultiObjects", s3ListMultiObjects }};

static const actpair bos_write_op[] =
{{ "WRITE", bosWrite },
 { "s3:PutObject", s3PutObject },
 { "s3:AbortMultipartUpload", s3AbortMultipartUpload },
 { "s3:DeleteObject", s3DeleteObject }};
...
struct PolicyParser : public BaseReaderHandler<UTF8<>, PolicyParser> {
  ...
  bool Key(const char* str, SizeType length, bool copy) {
    if (s.empty() || length > JSON_MAX_VALUE_LEN) {
      return false;
    }
  ...
  bool String(const char* str, SizeType length, bool copy) {
    ...
    string bos_str(str);
    //bos acl support without resource or resource is bucket name
    //arn:aws:s3:nil : without resource
    //arn:aws:s3:nme : resource is bucket name
    if (strcmp(str, "arn:aws:s3:::nil") == 0 || strcmp(str, "arn:aws:s3:::nme") == 0) {
      string all_control = "*";
      return s.back().do_string(cct, all_control.c_str(), all_control.size());
    }
...
bool ParseState::do_string(CephContext* cct, const char* s, size_t l) {
  ...
  if ((w->id == TokenID::Version) && k &&
    ...
    // Statement
  } else if (w->id == TokenID::HaveEffect) {
    t->bos_policy = true;
    ...
    for (auto& p : actpairs) {
      if (match_policy({s, l}, p.name, MATCH_POLICY_ACTION)) {
        ...
        if (strcmp(p.name, "READ") == 0) {
          for (auto bos_op : bos_read_op) {
            (w->id == TokenID::Action ? t->action[bos_op.bit] = 1 : t->notaction[bos_op.bit] = 1);
          }
        } else if (strcmp(p.name, "LIST") == 0) {
          for (auto bos_op : bos_list_op) {
            (w->id == TokenID::Action ? t->action[bos_op.bit] = 1 : t->notaction[bos_op.bit] = 1);
          }
        } else if (strcmp(p.name, "WRITE") == 0) {
          for (auto bos_op : bos_write_op) {
            (w->id == TokenID::Action ? t->action[bos_op.bit] = 1 : t->notaction[bos_op.bit] = 1);
          }
        } else if (strcmp(p.name, "s3:GetBucket") == 0) {
          for (auto bos_op : bos_bucket_op) {
            (w->id == TokenID::Action ? t->action[bos_op.bit] = 1 : t->notaction[bos_op.bit] = 1);
          }
        } else if (strcmp(p.name, "FULL_CONTROL") == 0) {
          (w->id == TokenID::Action ? t->action.set() : t->notaction.set());
        } else {
...
boost::optional<MaskedIP> Condition::as_network(const string& s) {
  ...
  if (s.find(".*") == s.size() - 2) {
    t.assign(s, 0, s.find(".*"));
    int split_count = count(t.begin(), t.end(), '.');
    int split = split_count;
    if (split_count < 3) {
...
Effect Statement::eval(const Environment& e,
  ...
  std::vector<Condition> allow_referer;
  std::vector<Condition> without_referer;
  for (auto cond : conditions) {
    if (cond.op == TokenID::StringEquals || cond.op == TokenID::StringLike) {
      allow_referer.push_back(cond);
    } else {
      without_referer.push_back(cond);
    }
  }

  if (bos_policy) {
    if (!allow_referer.empty() && !std::any_of(allow_referer.begin(), allow_referer.end(), [&e](const Condition& c) { return c.eval(e);})) {
      return Effect::Pass;
    }

    if (std::all_of(without_referer.begin(), without_referer.end(), [&e](const Condition& c) { return c.eval(e);})) {
      return effect;
    }
  } else {
...
const char* action_bit_string(uint64_t action) {
  ...
  case s3GetBucketNamespace:
    return "s3:GetBucketNamespace";

  case s3PutBucketNamespace:
    return "s3:PutBucketNamespace";

  case s3DeleteBucketNamespace:
    return "s3:DeleteBucketNamespace";
  ...
  case s3PutImageStyle:
    return "s3:PutImageStyle";

  case s3GetImageStyle:
    return "s3:GetImageStyle";

  case s3PutImageProtection:
    return "s3:PutCopyRightProtection";

  case s3GetImageProtection:
    return "s3:GetCopyRightProtection";

  case s3PutBucketStorageClass:
    return "s3:PutBucketStorageClass";

  case s3GetBucketStorageClass:
    return "s3:GetBucketStorageClass";

  case s3DeleteBucketLogging:
    return "s3:DeleteBucketLogging";
  ...
  case s3DeleteBucketEncryption:
    return "s3:DeleteBucketEncryption";
  ...
  case s3InitBucketObjectLockConfiguration:
    return "s3:InitBucketObjectLockConfiguration";

  case s3DeleteBucketObjectLockConfiguration:
    return "s3:DeleteBucketObjectLockConfiguration";

  case s3CompleteBucketObjectLockConfiguration:
    return "s3:CompleteBucketObjectLockConfiguration";

  case s3ExtendBucketObjectLockConfiguration:
    return "s3:ExtendBucketObjectLockConfiguration";
