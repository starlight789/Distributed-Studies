...
struct ElasticConfig {
  ...
  string get_obj_path(const RGWBucketInfo& bucket_info, const rgw_obj_key& key) {
    std::string es_id = url_encode(bucket_info.bucket.bucket_id + ":" + key.name + ":" + (key.instance.empty() ? "null" : key.instance));
    boost::string_view es_id_sv(es_id);
    const auto es_id_sha256 = calc_hash_sha256(es_id_sv);
    return index_path +  "/object/" + buf_to_hex(es_id_sha256).data();
  }
  ...
  void dump(Formatter *f) const {
    ...
    for (auto i : attrs) {
      ...
      if (boost::algorithm::starts_with(attr_name, RGW_ATTR_META_PREFIX)) {
        ...
        custom_meta[name] = rgw_bl_to_str(val);
        continue;
      }
      ...
      if (name == "acl") {
        ...
        if (name != "pg_ver" &&
            name != "source_zone" &&
            name != "idtag") {
          out_attrs[name] = rgw_bl_to_str(val);
        }
      }
    }
