                              ...
                              const bool skip_retarget)
{
  ...
  if (ret < 0) {
    if (ret == -ENOENT && op->get_type() == RGW_OP_GET_OBJ) {
      RGWGetObj* get_op = static_cast<RGWGetObj *>(op);
      if (get_op->is_head_obj()) {
        return ret;
      }
      get_op->set_exists(false);
    } else {
      return ret;
    }
  }
  ...
  if (op->should_retarget()) {
    req->log(s, "serve_errordoc");
    ret = op->serve_errordoc(handler);
    if (ret < 0) {
      return ret;
    }
  }
                    ...
                    void* asio_ctx)
{
  ...
  s->req = (RGWRequest *) req->s;

  s->req_id = store->unique_id(req->id);
  s->host_id = store->host_id;

  if (store->ctx()->_conf->rgw_encrypt_ip_into_request_id) {
    s->trans_id = store->unique_trans_id();
  } else {
  ...
  dout(10) << "req=" << s->trans_id << " " << "handler=" << typeid(*handler).name() << dendl;
  ...
  req->op = op;
  dout(10) << "req=" << s->trans_id << " " << "op=" << typeid(*op).name() << dendl;
  ...
  if (ret < 0) {
    req->log(s, "failed to authorize request");
  ...
  if (s->user->suspended) {
    dout(2) << "req=" << s->trans_id << " " << "user is suspended, uid=" << s->user->user_id << dendl;
