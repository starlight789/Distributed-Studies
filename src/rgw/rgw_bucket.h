...
extern int rgw_remove_todelete_bucket_data(RGWRados *store, RGWBucketInfo& info);
extern int rgw_remove_todelete_bucket_data_bypass_gc(RGWRados *store, RGWBucketInfo& info,
                                                     int concurrent_max, bool keep_index_consistent);
extern int rgw_is_todelete_bucket_empty(RGWRados *store, RGWBucketInfo& info, rgw_bucket& bucket, bool& is_empty);
extern int rgw_store_todelete_bucket_info(RGWRados *store, string bucket_instance, RGWBucketInfo& info);
extern int rgw_get_todelete_bucket_info(RGWRados *store, string bucket_instance, RGWBucketInfo& info);
extern int rgw_delete_todelete_bucket(RGWRados *store, string bucket_instance);
extern int set_todelete_bucket_stat_resharding(RGWRados *store, RGWBucketInfo& info);
extern void rgw_clear_index_and_todelete_bucket(RGWRados *store, const string& bucket_name, RGWBucketInfo& info);
...
struct RGWBucketAdminOpState {
  ...
  void set_project(const std::string& project) {
    project_id = project;
  }

  void set_qps_limit(int64_t qps) {
    qps_limit = qps;
  }
  void set_bandwidth_limit(int64_t bandwidth) {
  ...
  const string& get_organization() { return organization_id; }
  const string& get_project() { return project_id; }
...
public:
  ...
  int set_qos(RGWBucketAdminOpState& op_state, std::string *err_msg = NULL);
  int set_affiliation(RGWBucketAdminOpState& op_state, std::string *err_msg = NULL);
...
public:
  ...
  static int info(RGWRados *store, RGWBucketAdminOpState& op_state, RGWFormatterFlusher& flusher, bool with_put_perm = false);
  static int bucket_pool_capacity(RGWRados *store, RGWBucketInfo& bucket_info, int64_t *avail_size);
  ...
  static int set_affiliation(RGWRados *store, RGWBucketAdminOpState& op_state);
  ...
  static int clear_stale_instances(RGWRados *store, RGWBucketAdminOpState& op_state,
				   RGWFormatterFlusher& flusher);
  static int set_qos(RGWRados *store, RGWBucketAdminOpState& op_state);
  static int sync_status(RGWRados *store, RGWBucketAdminOpState& op_state,
                         RGWFormatterFlusher& flusher);
};
...
class RGWDataChangesLog {
   ...
   * string: zone_id
   * int: shard_id
   * set<string>: modified keys
   */ 
  map<string, map<int, set<string> > > modified_shards;
...
public:
  ...
  void mark_modified(string zone_id, int shard_id, const rgw_bucket_shard& bs);
  void read_clear_modified(map<string, map<int, set<string> > > &modified);
