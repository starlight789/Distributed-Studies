...
public:
  ...
  class ReceiveCB {
    ...
    public:
      ...
      virtual void set_attr(const string& key, const string& val) {}
      virtual void set_chunk() {}
      virtual void notify_async() {}
  };
...
class RGWRESTStreamMirrorReadRequest : public RGWRESTStreamRWRequest {
protected:
  int handle_header(const string& name, const string& val) override;
public:
  RGWRESTStreamMirrorReadRequest(CephContext *_cct, const string& _url, ReceiveCB *_cb, param_vec_t *_headers,
    param_vec_t *_params, HostStyle _host_style = PathStyle)
    : RGWRESTStreamRWRequest(_cct, "GET", _url, _cb, _headers, _params, _host_style) {}
  void unpause_receive() override;
};

class RGWRESTStreamAsyncMirrorReadRequest : public RGWRESTStreamRWRequest {
  RGWRados* store;
...
protected:
  ...
  int receive_data(void *ptr, size_t len, bool *pause) override;
public:
  RGWRESTStreamAsyncMirrorReadRequest(RGWRados *_store,
                                      CephContext *_cct,
                                      const string& _url,
                                      ReceiveCB *_cb,
    ...
    : RGWRESTStreamRWRequest(_cct, "GET", _url, _cb, _headers, _params, _host_style),
      store(_store), obj_ctx(_store) {}

  virtual ~RGWRESTStreamAsyncMirrorReadRequest() override {
    if (processor)
      free(processor);
    processor = nullptr;
  }

  void set_placement_rule(rgw_placement_rule& placement) { placement_rule = placement; }
  int prepare(RGWBucketInfo& bucket_info, rgw_bucket& bucket, const string& object_name);
