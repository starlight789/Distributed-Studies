...
int RGWStreamReadHTTPResourceCRF::init()
{
  ...
  if (r < 0) {
    dout(10) << "ERROR: add request to HTTPManager failed:" << r << dendl;
    return r;
  }
  ...
  reenter(this) {
    {
      int ret = in_crf->init();
      if (ret < 0) {
        ldout(cct, 10) << __func__ << ": in_crf[" << in_crf->get_rest_obj().key
                       << "] init() retcode=" << ret << dendl;
    ...
    do {
      ...
      do {
        ...
        if (retcode < 0) {
          ldout(cct, 10) << __func__ << ": in_crf[" << in_crf->get_rest_obj().key
                         << "] read() retcode=" << retcode << dendl;
          return set_cr_error(ret);
        }
      } while (need_retry);

      ldout(cct, 20) << "[" << in_crf->get_rest_obj().key << "] read " << bl.length() << " bytes" << dendl;
      ...
      if (!sent_attrs) {
        ...
        if (ret < 0) {
          ldout(cct, 10) << __func__ << ": out_crf[" << in_crf->get_rest_obj().key
                         << " init() ret=" << ret << dendl;
          return set_cr_error(ret);
        }
      ...
      do {
        yield {
          ldout(cct, 20) << "[" << in_crf->get_rest_obj().key << "] writing " << bl.length() << " bytes" << dendl;
