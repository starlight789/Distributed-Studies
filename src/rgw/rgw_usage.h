...
public:
  ...
  static int trim(RGWRados *store, rgw_user& uid, uint64_t start_epoch,
	          uint64_t end_epoch, std::string bucket="");
...
public:
  ...
  static int trim(RGWRados *store, rgw_user& uid, uint64_t start_epoch, uint64_t end_epoch);

  static int show_multisite_dataflow(RGWRados *store, rgw_user& uid, uint64_t start_epoch,
