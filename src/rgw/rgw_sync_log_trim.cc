...
class BucketClearCR : public RGWCoroutine {
  RGWRados *const store;
  ceph::mono_time start_time;

  static const std::string section; //< metadata section for bucket instances
public:
  BucketClearCR(RGWRados *store)
    : RGWCoroutine(store->ctx()), store(store)  {}

  int operate();
};

const std::string BucketClearCR::section{"bucket.instance"};

int BucketClearCR::operate()
{
  reenter(this) {
    start_time = ceph::mono_clock::now();
    ...
    int ret = 0;
    list<string> buckets;
    ret = store->list_todelete_bucket(buckets);
    if (ret < 0 && ret != -ENOENT) {
      ldout(cct, 0) <<  __func__ << " ERROR: failed to list todelete_bucket, ret: "
                                 << ret << " " << cpp_strerror(-ret) << dendl;
      return set_cr_error(ret);
    } else if (ret == -ENOENT || buckets.empty()) {
      ldout(cct, 5) <<  __func__ << " no todelete_bucket." << dendl;
      return set_cr_done();
    }

    for (list<string>::iterator iter = buckets.begin(); iter != buckets.end(); ++iter) {
      RGWBucketInfo info;
      RGWObjectCtx obj_ctx(store);

      ret = store->get_bucket_instance_info(obj_ctx, *iter, info, NULL, NULL);
      if (ret == -ENOENT) {
        ldout(cct, 5) <<  __func__ << " bucket not exist, bucket:  " << info.bucket.name << dendl;
        rgw_clear_index_and_todelete_bucket(store, *iter, info);
        continue;
      } else if (ret < 0) {
        ldout(cct, 0) <<  __func__ << " failed to get bucket instance info, bucket: "
                                   << *iter << ", ret: " << ret << dendl;
        continue;
      }

      if (store->ctx()->_conf->rgw_sync_bucket_clear_bypass_gc) {
        ret = rgw_remove_todelete_bucket_data_bypass_gc(store, info,
                                                        store->ctx()->_conf->rgw_bucket_clear_max_aio, true);
      } else {
        ret = rgw_remove_todelete_bucket_data(store, info);
      }
      if (ret == -EIO) {
        ldout(cct, 5) <<  __func__ << " bucket not exist, bucket:  " << info.bucket.name << dendl;
        rgw_clear_index_and_todelete_bucket(store, *iter, info);
      ...
      if (ret < 0) {
        ldout(cct, 0) <<  __func__ << " ERROR: unable to remove bucket: " << info.bucket.name
                                    << ", ret: " << ret << cpp_strerror(-ret) << dendl;
        continue;
      }

      // set bucket state CLS_RGW_RESHARD_IN_PROGRESS to prevent the operation of PutObj
      // it has nothing to do with Reshard
      ret = set_todelete_bucket_stat_resharding(store, info);
      if (ret < 0) {
        ldout(cct, 0) <<  __func__ << " ERROR: unable to set bucket: " << info.bucket.name
                                    << " CLS_RGW_RESHARD_IN_PROGRESS, ret: " << cpp_strerror(-ret) << dendl;
        continue;
      }

      bool is_empty;
      ret = rgw_is_todelete_bucket_empty(store, info, info.bucket, is_empty);
      if (ret < 0 || !is_empty)
        continue;

      if (store->ctx()->_conf->rgw_multisite_bucket_clear_flag) {
        RGWObjVersionTracker objv_tracker;
        ret = rgw_bucket_instance_remove_entry(store, info.bucket.get_key(), &objv_tracker);
        if (ret < 0) {
          continue;
        }
        ret = store->clean_bucket_index(info, info.num_shards);
        if (ret < 0) {
          ldout(cct, 0) << __func__ << " ERROR: failed to clean bucket index, bucket: "
                                    << *iter << ", ret: " << ret << dendl;
          continue;
        }
      }

      ret = rgw_delete_todelete_bucket(store, *iter);
      if (ret < 0) {
        ldout(cct, 0) << __func__ << " ERROR: failed to delete todelete_bucket obj: "
                                  << *iter << ", ret: " << ret << dendl;
        continue;
      }
    }

    ldout(cct, 5) << "bucket index log processing completed in "
        << ceph::mono_clock::now() - start_time << dendl;
    return set_cr_done();
  }
  return 0;
...
class BucketClearPollCR : public RGWCoroutine {
  RGWRados *const store;
  RGWHTTPManager *const http;
  const utime_t interval; //< polling interval
  const utime_t bc_interval;
  const rgw_raw_obj& obj;
  const std::string name{"bucket_clear"}; //< lock name
  const std::string cookie;

 public:
  BucketClearPollCR(RGWRados *store, RGWHTTPManager *http,
                   utime_t interval, utime_t bc_interval, const rgw_raw_obj& obj)
    : RGWCoroutine(store->ctx()), store(store), http(http),
      interval(interval), bc_interval(bc_interval), obj(obj),
      cookie(RGWSimpleRadosLockCR::gen_random_cookie(cct))
  {}
...
int BucketClearPollCR::operate()
{
  reenter(this) {
    for (;;) {
      ...
      if (retcode < 0) {
        ldout(cct, 0) << __func__ << "failed to lock: " << cpp_strerror(retcode) << dendl;
        continue;
      }

      set_status("trimming");
      yield call(new BucketClearCR(store));
      if (retcode < 0) {
        // on errors, unlock so other gateways can try
        set_status("unlocking");
        yield call(new RGWSimpleRadosUnlockCR(store->get_async_rados(), store,
...
RGWCoroutine* BucketTrimManager::create_bucket_clear_cr(RGWHTTPManager *http,
                                       utime_t trim_interval, utime_t interval)
{
  return new BucketClearPollCR(impl->store, http, trim_interval, interval, impl->status_obj);
}
