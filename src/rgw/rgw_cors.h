...
public:
  ...
  void set_allowed_methods(uint8_t m) { allowed_methods = m; }
  void set_max_age(uint32_t a) { max_age = a; }
  bool have_allowed_origins() {return !allowed_origins.empty();}
  bool have_allowed_hdrs() {return !allowed_hdrs.empty();}
  bool have_exposable_hdrs() {return !exposable_hdrs.empty();}
};
