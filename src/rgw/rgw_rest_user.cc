...
class RGWOp_Qos_Set : public RGWRESTOp {
...
public:
  ...
  const string name() override { return "set_qos_info"; }
};

void RGWOp_Qos_Set::execute()
{
  std::string uid_str;
  RESTArgs::get_string(s, "uid", uid_str, &uid_str);
  ...
  http_ret = RGWUserAdminOp_Qos::set(store, op_state, flusher);
}
