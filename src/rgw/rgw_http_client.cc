                                        ...
                                        void * const _info)
{
  ...
  dout(20) << __func__ << " receive data call handle_data, len:" << len - skip_bytes
           << " req_data->id=" << req_data->id << ", curl_handle=" << req_data->curl_handle
           << dendl;
  ...
  if (pause) {
    dout(20) << "RGWHTTPClient::receive_http_data(): pause read, req_data:" << req_data << dendl;
    skip_bytes = len;
    Mutex::Locker l(req_data->lock);
...
bool RGWHTTPClient::_set_read_paused(bool pause)
{
  assert(req_data->lock.is_locked());

  RGWHTTPManager *mgr = req_data->mgr;
  dout(30) << "call set_request_state pause:" << pause
           << "req_data->read_paused:" << req_data->read_paused
           << "req_data:"<< req_data
           << dendl;
...
int RGWHTTPClient::init_request(rgw_http_req_data *_req_data)
{
  ...
  // Change CURL_MAX_WRITE_SIZE in /usr/include/curl/curl.h also doesn't work.
  curl_easy_setopt(easy_handle, CURLOPT_BUFFERSIZE, 131072L); // 128KB
...
bool RGWHTTPClient::_is_done()
{
  return req_data->_is_done();
}
...
void RGWHTTPClient::inner_lock()
{
  req_data->lock.Lock();
}

void RGWHTTPClient::unlock()
{
  req_data->lock.Unlock();
}

void RGWHTTPClient::notify_data()
{
  req_data->cond.Signal();
}

void RGWHTTPClient::wait_for_data_or_done()
{
  if (!req_data->_is_done()) {
    req_data->cond.Wait(req_data->lock);
  }
}
