...
class StreamIO : public rgw::asio::ClientIO {
  ...
  void get_throttle_token_callback(int r, SyncPoint* sync) {
    sync->put(0);
  }
 ...
 public:
  ...
  size_t write_data(const char* buf, size_t len) override {
    ...
    if (ec) {
      dout(0) << "ERROR: write_data failed: " << ec.message() << dendl;
  ...
  size_t recv_body(char* buf, size_t max) override {
    ...
    while (body_remaining.size && !parser.is_done()) {
      ...
      if (ec) {
        dout(0) << "ERROR: failed to read body: " << ec.message() << dendl;
        throw rgw::io::Exception(ec.value(), std::system_category());
      }
    }
  ...
  void get_throttle_token(int num, shared_ptr<TokenBucketThrottle> token_bucket) override {
    ...
    SyncPoint sync(stream.lowest_layer().get_io_service(), yield);
    bool waited = token_bucket->get<StreamIO, SyncPoint,
         &StreamIO::get_throttle_token_callback>(num, this, &sync);
    if (waited == true) {
      sync.get();
    }
  }
                       ...
                       boost::asio::yield_context yield)
{
  ...
  for (;;) {
    ...
    void *asio_ctx[2];
    asio_ctx[0] = &socket.get_io_service();
    asio_ctx[1] = &yield;
    process_request(env.store, env.rest, &req, env.uri_prefix,
                    *env.auth_registry, &client, env.olog, nullptr, asio_ctx);
