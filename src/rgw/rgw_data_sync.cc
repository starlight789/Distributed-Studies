...
int RGWReadDataSyncStatusCoroutine::operate()
{
  reenter(this) {
    ...
    ldout(sync_env->cct, 30) << __func__ << "() RGWReadDataSyncStatusCoroutine call ReadInfoCR" 
                             << ", log_id=" << random_log_id << dendl;
    yield {
    ...
    if (retcode < 0) {
      ldout(sync_env->cct, 4) << "ERROR: failed to read sync status info with "
          << cpp_strerror(retcode) << ", log_id=" << random_log_id << dendl;
      return set_cr_error(retcode);
    }
    // read shard markers
    ldout(sync_env->cct, 30) << __func__ << "() RGWReadDataSyncStatusCoroutine call ReadMarkersCR"
                             << ", log_id=" << random_log_id << dendl;
    ...
    if (retcode < 0) {
      ldout(sync_env->cct, 4) << "ERROR: failed to read sync status markers with "
          << cpp_strerror(retcode) << ", log_id=" << random_log_id << dendl;
      return set_cr_error(retcode);
    }
    ldout(sync_env->cct, 15) << __func__ << "() RGWReadDataSyncStatusCoroutine "
                             << ", state:" << sync_status->sync_info.state
                             << ", log_id=" << random_log_id << dendl;
    return set_cr_done();
  }
  return 0;
...
public:
  RGWReadRemoteDataLogShardInfoCR(RGWDataSyncEnv *_sync_env,
                                  int _shard_id, RGWDataChangesLogInfo *_shard_info,
                                  string _random_log_id = "") : RGWCoroutine(_sync_env->cct),
                                sync_env(_sync_env),
                                http_op(NULL),
                                shard_id(_shard_id),
  ...
  int operate() override {
	...
	snprintf(buf, sizeof(buf), "%d", shard_id);
        ...
        if (ret < 0) {
          ldout(sync_env->cct, 0) << "ERROR: failed to read from " << p << ", log_id:" << random_log_id << dendl;
...
public:
  RGWReadRemoteDataLogShardCR(RGWDataSyncEnv *_sync_env,
                              int _shard_id, string *_pmarker, list<rgw_data_change_log_entry> *_entries, bool *_truncated,
                              string _random_log_id = "") : RGWCoroutine(_sync_env->cct),
                                                      sync_env(_sync_env),
                                                      http_op(NULL),
                                                      shard_id(_shard_id),
...
public:
  RGWInitDataSyncStatusCoroutine(RGWDataSyncEnv *_sync_env, uint32_t num_shards,
    ...
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), store(sync_env->store),
      ...
      num_shards(num_shards), status(status), random_log_id(_random_log_id) {
    lock_name = "sync_lock";
...
#define COOKIE_LEN 16
  ...
  int operate() override {
    int ret;
    librados::Rados* rados_handle = store->get_rados_handle();
    reenter(this) {
      using LockCR = RGWSimpleRadosLockCR;
      yield call(new LockCR(sync_env->async_rados, store,
                            rgw_raw_obj{pool, sync_status_oid},
                            lock_name, cookie, lock_duration, rados_handle));
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: failed to take a lock on " << sync_status_oid << ", log_id:" << random_log_id));
        return set_cr_error(retcode);
      }
      ...
      yield call(new WriteInfoCR(sync_env->async_rados, store,
                                 ...
                                 status->sync_info));
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: failed to write sync status info with "<< sync_status_oid << retcode << ", log_id:" << random_log_id));
        return set_cr_error(retcode);
      }
      ...
      yield call(new LockCR(sync_env->async_rados, store,
                            ...
                            lock_name, cookie, lock_duration, rados_handle));
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: lock again failed on " << sync_status_oid << ", log_id:" << random_log_id));
        return set_cr_error(retcode);
      }

      tn->log(10, SSTR("took lease" << ", log_id:" << random_log_id));

      /* fetch current position in logs */
      yield {
        RGWRESTConn *conn = store->get_zone_conn_by_id(sync_env->source_zone);
        if (!conn) {
          tn->log(0, SSTR("ERROR: connection to zone " << sync_env->source_zone << " does not exist!" << ", log_id:" << random_log_id));
          return set_cr_error(-EIO);
        }
        for (uint32_t i = 0; i < num_shards; i++) {
          spawn(new RGWReadRemoteDataLogShardInfoCR(sync_env, i, &shards_info[i], random_log_id), true);
        }
      }
      while (collect(&ret, NULL)) {
        if (ret < 0) {
          tn->log(0, SSTR("ERROR: failed to read remote data log shards" << ", log_id:" << random_log_id));
      ...
      while (collect(&ret, NULL)) {
        if (ret < 0) {
          tn->log(0, SSTR("ERROR: failed to write data sync status markers" << ", log_id:" << random_log_id));
          return set_state(RGWCoroutine_Error);
        }
        yield;
      }
...
public:
  RGWListBucketIndexesCR(RGWDataSyncEnv *_sync_env,
                         rgw_data_sync_status *_sync_status,
                         string& _random_log_id) : RGWCoroutine(_sync_env->cct), sync_env(_sync_env),
  ...
  int operate() override {
    reenter(this) {
      ...
      if (retcode < 0) {
        ldout(sync_env->cct, 0) << "ERROR: failed to fetch metadata for section bucket.instance" << ", log_id:" << random_log_id << dendl;
        return set_cr_error(retcode);
      }
      ...
      for (iter = result.begin(); iter != result.end(); ++iter) {
        ldout(sync_env->cct, 19) << "list metadata: section=bucket.instance key=" << *iter  << ", log_id:" << random_log_id << dendl;
      ...
      if (!failed) {
        for (map<uint32_t, rgw_data_sync_marker>::iterator iter = sync_status->sync_markers.begin(); iter != sync_status->sync_markers.end(); ++iter) {
          ...
          ldout(sync_env->cct, 10) << "ERROR: failed to build bucket instances map:"
            << meta_info.data.get_bucket_info().bucket 
            << ", log_id:" << random_log_id << dendl;
      ...
      while (collect(&ret, NULL)) {
        if (ret < 0) {
          ldout(sync_env->cct, 10) << "ERROR: failed to store sync status, log_id:" << random_log_id << dendl;
          yield call(sync_env->error_logger->log_error_cr(sync_env->conn->get_remote_id(), "data.init", "",
...
public:
  ...
  RGWCoroutine *store_marker(const string& new_marker, uint64_t index_pos, const real_time& timestamp) override {
    ...
    tn->log(19, SSTR("updating marker marker_oid=" << marker_oid << " marker=" << new_marker));
    RGWRados *store = sync_env->store;
...
class RGWRunBucketSyncCoroutine : public RGWCoroutine {
  ...
  string random_log_id;
public:
  RGWRunBucketSyncCoroutine(RGWDataSyncEnv *_sync_env, const rgw_bucket_shard& bs, const RGWSyncTraceNodeRef& _tn_parent, RGWDataSyncShardMarkerTrack *_marker_tracker = nullptr, string _entry_marker = "", string _random_log_id = "")
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
      ...
      marker_tracker(_marker_tracker), entry_marker(_entry_marker), 
      random_log_id(_random_log_id) {
  ...
  int operate() override {
    reenter(this) {
      do {
        {
          do {
            uint64_t start_sync_time = sync_env->store->get_zone_params().start_sync_time;
            uint64_t stop_sync_time = sync_env->store->get_zone_params().stop_sync_time;
            time_t now_t = time(nullptr);
            struct tm now_tm;
            localtime_r(&now_t, &now_tm);
            uint64_t now_minute = now_tm.tm_hour * 100 + now_tm.tm_min;
            if (stop_sync_time > start_sync_time ?
              now_minute < start_sync_time || now_minute > stop_sync_time :
              now_minute < start_sync_time && now_minute > stop_sync_time) {
              tn->log(19, SSTR("call sleep, now_minute: " << now_minute << ", log_id:" << random_log_id));
              std::this_thread::sleep_for(std::chrono::seconds(60));
              int ret = sync_env->store->get_zone_params().init(sync_env->cct, sync_env->store);
              if (ret < 0) {
                tn->log(0, SSTR("failed to init zone" << ret << ", log_id:" << random_log_id));
          ...
          tn->log(19, SSTR("skip sleep, start sync data, log_id:" << random_log_id));
        }
        yield {
          int ret = rgw_bucket_parse_bucket_key(sync_env->cct, raw_key,
                                                &bs.bucket, &bs.shard_id);
          if (ret < 0) {
            tn->log(0, SSTR("ERROR, rgw_bucket_parse_bucket_key error:"<< ret 
                  << ", bs:" << bs << ", log_id:" << random_log_id));
            return set_cr_error(-EIO);
          }
          if (marker_tracker) {
            marker_tracker->reset_need_retry(raw_key);
          }
          tn->log(10, SSTR("triggering sync of bucket/shard " << bucket_shard_str{bs}
                << ", entry_marker=" << entry_marker
                << ", log_id=" << random_log_id));
          call(new RGWRunBucketSyncCoroutine(sync_env, bs, tn, marker_tracker, entry_marker, random_log_id));
        }
      ...
      if (sync_status == -ENOENT) {
        ...
        // error_repo forever
        tn->log(0, SSTR("WARNING: skipping data log entry for missing bucket " << raw_key << ",bs:" << bs << ", log_id=" << random_log_id));
        sync_status = 0;
      }
      ...
      if (sync_status < 0) {
        ...
        if (sync_status != -EBUSY && sync_status != -EAGAIN) {
          ...
          if (retcode < 0) {
            tn->log(0, SSTR("ERROR: failed to log sync failure: retcode=" << retcode << ",bs:" << bs << ", log_id=" << random_log_id));
          }
        }
        if (error_repo && !error_repo->append(raw_key)) {
          tn->log(0, SSTR("ERROR: failed to log sync failure in error repo: retcode=" << retcode << ",bs:" << bs << ", log_id=" << random_log_id));
        }
        ...
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to remove omap key from error repo ("
             << error_repo->get_obj() << " retcode=" << retcode << ",bs:" << bs << ", log_id=" << random_log_id));
        }
      }
      /* FIXME: what do do in case of error */
      if (marker_tracker && !entry_marker.empty()) {
        // entry_marker: bucket shard
        tn->log(19, SSTR("marker_tracker finish entry_marker:" << entry_marker
                              << ",bs:" << bs
                              << ", log_id=" << random_log_id));
        /** update marker 
         * class RGWSyncShardMarkerTrack finish() -> insert into finish_markers and erase from pending
         * when finish this RGWCoroutinue -> call flush() -> update status_oid
         */
...
#define OMAP_GET_MAX_ENTRIES 100
    ...
    reenter(&full_cr) {
      ldout(sync_env->cct, 15) << __func__
                  << "start full sync marker state:" << sync_marker.state
                  << ", marker:" << sync_marker.marker
                  << ", next_step_marker:" << sync_marker.next_step_marker
                  << ", total_entries:" << sync_marker.total_entries
                  << ", pos:" << sync_marker.pos
                  << ", log_id:" << random_log_id  << dendl;
      yield init_lease_cr();
      while (!lease_cr->is_locked()) {
        if (lease_cr->is_done()) {
          tn->log(5, SSTR("ERROR: failed to take lease" << ", log_id:" << random_log_id));
      ...
      do {
        if (!lease_cr->is_locked()) {
          ...
          drain_all();
          tn->log(5, SSTR("ERROR: lease_cr not locked, log_id:" << random_log_id));
        ...
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: RGWRadosGetOmapKeysCR() returned ret=" << retcode << ", log_id:" << random_log_id));
          lease_cr->go_down();
          drain_all();
          return set_cr_error(retcode);
        }
        ...
        tn->log(19, SSTR("retrieved " << entries.size() << " entries to sync" << ", log_id:" << random_log_id));
        iter = entries.begin();
        for (; iter != entries.end(); ++iter) {
          tn->log(19, SSTR("full sync: " << *iter << ", log_id:" << random_log_id));
          total_entries++;
          if (!marker_tracker->start(*iter, total_entries, real_time())) {
            tn->log(0, SSTR("ERROR: cannot start syncing " << *iter << ". Duplicate entry?" << ", log_id:" << random_log_id));
          } else {
            // fetch remote and write locally
            yield spawn(new RGWDataSyncSingleEntryCR(sync_env, *iter, *iter, marker_tracker, error_repo, false, tn, random_log_id), false);
          }
          ...
          while ((int)num_spawned() > spawn_window) {
            ...
            while (collect(&ret, lease_stack.get())) {
              if (ret < 0) {
                tn->log(10, SSTR("a sync operation returned error" << ", log_id:" << random_log_id));
              }
            }
          }
        }
      ...
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: failed to set sync marker: retcode=" << retcode << ", log_id:" << random_log_id));
        lease_cr->go_down();
        drain_all();
        return set_cr_error(retcode);
      }
  ...
  int incremental_sync() {
    reenter(&incremental_cr) {
      ldout(sync_env->cct, 15) << __func__
                  << " start incremenatl sync. marker state:" << sync_marker.state
                  << ", marker:" << sync_marker.marker
                  << ", next_step_marker:" << sync_marker.next_step_marker
                  << ", total_entries:" << sync_marker.total_entries
                  << ", pos:" << sync_marker.pos
                  << ", log_id:" << random_log_id << dendl;
      if (lease_cr) {
        tn->log(10, SSTR("lease already held from full sync" << ", log_id:" << random_log_id));
      } else {
        yield init_lease_cr();
        while (!lease_cr->is_locked()) {
          if (lease_cr->is_done()) {
            tn->log(5, SSTR("failed to take lease" << ", log_id:" << random_log_id));
            set_status("lease lock failed, early abort");
            drain_all();
            return set_cr_error(lease_cr->get_ret_status());
          }
      ...
      do {
        if (!lease_cr->is_locked()) {
          stop_spawned_services();
          drain_all();
          tn->log(10, SSTR("ERROR: lease_cr is not locked, log_id:" << random_log_id));
          return set_cr_error(-ECANCELED);
        }
        ...
        for (modified_iter = current_modified.begin(); modified_iter != current_modified.end(); ++modified_iter) {
          yield {
            tn->log(19, SSTR("received async update notification: " << *modified_iter << ", log_id:" << random_log_id));
            spawn(new RGWDataSyncSingleEntryCR(sync_env, *modified_iter, string(), marker_tracker, error_repo, false, tn, random_log_id), false);
          }
        }
        ...
        if (error_retry_time <= ceph::coarse_real_clock::now()) {
          ...
          tn->log(19, SSTR("read error repo, got " << error_entries.size() << " entries"
                           << ", error_oid:" << error_oid
                           << ", log_id:" << random_log_id));
          iter = error_entries.begin();
          for (; iter != error_entries.end(); ++iter) {
            error_marker = *iter;
            tn->log(19, SSTR("handle error entry: " << error_marker
                             << ", error_oid:" << error_oid
                             << ", log_id:" << random_log_id));
            spawn(new RGWDataSyncSingleEntryCR(sync_env, error_marker, error_marker, nullptr /* no marker tracker */, error_repo, true, tn, random_log_id), false);
          }
          if (!omapkeys->more) {
...
#define INCREMENTAL_MAX_ENTRIES 100
	tn->log(19, SSTR("shard_id=" << shard_id << " sync_marker=" << sync_marker.marker << ", log_id:" << random_log_id));
	{
          spawned_keys.clear();
          yield call(new RGWReadRemoteDataLogShardCR(sync_env, shard_id, &sync_marker.marker, &log_entries, &truncated, random_log_id));
          ...
          for (log_iter = log_entries.begin(); log_iter != log_entries.end(); ++log_iter) {
            tn->log(19, SSTR("shard_id=" << shard_id << " log_entry: " << log_iter->log_id
                             << ":" << log_iter->log_timestamp << ":" << log_iter->entry.key
                             << ", log_id:" << random_log_id));
            if (!marker_tracker->index_key_to_marker(log_iter->entry.key, log_iter->log_id)) {
              tn->log(19, SSTR("skipping sync of entry: " << log_iter->log_id << ":" << log_iter->entry.key
                               << " sync already in progress for bucket shard"
                               << ", try_update_high_marker log_id:" << random_log_id));
            ...
            if (!marker_tracker->start(log_iter->log_id, 0, log_iter->log_timestamp)) {
              ...
              if (spawned_keys.find(log_iter->entry.key) == spawned_keys.end()) {
                tn->log(19, SSTR("call RGWDataSyncSingleEntryCR, shard_id=" << shard_id
                                 << " in spawned_keys: " << log_iter->log_id
                                 << ":" << log_iter->log_timestamp
                                 << ":" << log_iter->entry.key
                                 << ", log_id:" << random_log_id));

                spawned_keys.insert(log_iter->entry.key);
                spawn(new RGWDataSyncSingleEntryCR(sync_env, log_iter->entry.key, log_iter->log_id, marker_tracker, error_repo, false, tn, random_log_id), false);
                if (retcode < 0) {
                  tn->log(0, SSTR("ERROR: RGWDataSyncSingleEntryCR failed , ret=" << retcode
                                  << ", shard_id=" << shard_id
                                  << ", log_id:" <<log_iter->log_id
                                  << ":" << log_iter->log_timestamp
                                  << ":" << log_iter->entry.key
                                  << ", log_id:" << random_log_id));
                ...
                tn->log(19, SSTR("found shard_id=" << shard_id << " in spawned_keys : " << log_iter->log_id
                                 << ":" << log_iter->log_timestamp << ":" << log_iter->entry.key
                                 << ", jump RGWDataSyncSingleEntryCR, log_id:" << random_log_id));
              }
            }
          }
          while ((int)num_spawned() > spawn_window) {
        ...
        tn->log(19, SSTR("shard_id=" << shard_id << " sync_marker=" << sync_marker.marker << " truncated=" << truncated << ", log_id:" << random_log_id));
        if (!truncated) {
...
public:
  RGWDataSyncShardControlCR(RGWDataSyncEnv *_sync_env, rgw_pool& _pool,
                            uint32_t _shard_id, rgw_data_sync_marker& _marker,
                            RGWSyncTraceNodeRef& _tn_parent,
                            const string& _random_log_id) :
                          RGWBackoffControlCR(_sync_env->cct, false),
                          sync_env(_sync_env),
                          pool(_pool),
                          shard_id(_shard_id),
                          sync_marker(_marker),
  ...
  RGWCoroutine *alloc_cr() override {
    ...
    gen_rand_alphanumeric_lower(sync_env->cct, &random_suffix, MULTISITE_LOG_ID_LENGTH);
    if (random_log_id.length() > 2 * MULTISITE_LOG_ID_LENGTH) {
      ldout(sync_env->cct, 15) << __func__ << ", previous log_id:" << random_log_id << dendl;
      random_log_id.replace(random_log_id.length() - MULTISITE_LOG_ID_LENGTH, MULTISITE_LOG_ID_LENGTH, random_suffix);
      ldout(sync_env->cct, 15) << __func__ << ", new log_id:" << random_log_id << dendl;
    } else {
      random_log_id.append(":");
      random_log_id.append(random_suffix);
      ldout(sync_env->cct, 15) << __func__ << ", log_id:" << random_log_id << dendl;
    }
    return new RGWDataSyncShardCR(sync_env, pool, shard_id, sync_marker, tn, backoff_ptr(), random_log_id);
  }
...
public:
  ...
  gen_rand_alphanumeric_lower(_sync_env->cct, &random_log_id, MULTISITE_LOG_ID_LENGTH);
  }
  ...
  int operate() override {
    reenter(this) {
      ldout(sync_env->cct, 15) << __func__ << "() call RGWReadDataSyncStatusCoroutine" << ", log_id:" << random_log_id << dendl;
      /* read sync status */
      yield call(new RGWReadDataSyncStatusCoroutine(sync_env, &sync_status, random_log_id));

      data_sync_module = sync_env->sync_module->get_data_handler();

      if (retcode < 0 && retcode != -ENOENT) {
        tn->log(0, SSTR("ERROR: failed to fetch sync status, retcode=" << retcode << ", log_id:" << random_log_id));
        return set_cr_error(retcode);
      }
      ...
      if ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateInit) {
        ...
        yield call(new RGWInitDataSyncStatusCoroutine(sync_env, num_shards, instance_id, tn, &sync_status, random_log_id));
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to init sync, retcode=" << retcode << ", log_id:" << random_log_id));
          return set_cr_error(retcode);
        }
      ...
      if  ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateBuildingFullSyncMaps) {
        tn->log(10, SSTR("building full sync maps" << ", log_id:" << random_log_id));
        /* call sync module init here */
        sync_status.sync_info.num_shards = num_shards;
        yield call(data_sync_module->init_sync(sync_env));
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: sync module init_sync() failed, retcode=" << retcode << ", log_id:" << random_log_id));
          return set_cr_error(retcode);
        }
        /* state: building full sync maps */
        yield call(new RGWListBucketIndexesCR(sync_env, &sync_status, random_log_id));
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to build full sync maps, retcode=" << retcode << ", log_id:" << random_log_id));
          return set_cr_error(retcode);
        }
        ...
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: failed to write sync status, retcode=" << retcode << ", log_id:" << random_log_id));
          return set_cr_error(retcode);
        }
      ...
      yield {
        if  ((rgw_data_sync_info::SyncState)sync_status.sync_info.state == rgw_data_sync_info::StateSync) {
          tn->log(10, SSTR("spawning " << num_shards << " shards sync" << ", log_id:" << random_log_id));
          for (map<uint32_t, rgw_data_sync_marker>::iterator iter = sync_status.sync_markers.begin();
            ...
            RGWDataSyncShardControlCR *cr = new RGWDataSyncShardControlCR(sync_env, sync_env->store->get_zone_params().log_pool,
                                                                          iter->first, iter->second, tn, random_log_id + ":" + to_string(iter->first));
            cr->get();
            shard_crs_lock.Lock();
...
public:
  ...
  RGWCoroutine *sync_object(RGWDataSyncEnv *sync_env, RGWBucketInfo& bucket_info, rgw_obj_key& key, real_time& mtime, std::optional<uint64_t> versioned_epoch, rgw_zone_set *zones_trace) override;
...
RGWCoroutine *RGWDefaultDataSyncModule::sync_object(RGWDataSyncEnv *sync_env, RGWBucketInfo& bucket_info, rgw_obj_key& key,
    real_time& mtime, std::optional<uint64_t> versioned_epoch, rgw_zone_set *zones_trace)
{
  return new RGWFetchRemoteObjCR(sync_env->async_rados, sync_env->store, sync_env->source_zone, bucket_info,
                                 std::nullopt, key, versioned_epoch,
...
public:
  RGWInitBucketShardSyncStatusCoroutine(RGWDataSyncEnv *_sync_env,
    ...
    : RGWCoroutine(_sync_env->cct), sync_env(_sync_env), bs(bs),
      ...
      status(_status), random_log_id(_random_log_id)
  {}
  ...
  int operate() override {
    reenter(this) {
      ...
      if (retcode < 0 && retcode != -ENOENT) {
        ldout(cct, 0) << "ERROR: failed to fetch bucket index status, log_id=" << random_log_id << dendl;
        return set_cr_error(retcode);
      }
      ldout(cct, 19) << "bucket index marker info: " 
                     << "bucket_ver:" << info.bucket_ver 
                     << ", master_ver:" << info.master_ver
                     << ", max_marker:" << info.max_marker
                     << ", syncstopped:" << info.syncstopped
                     <<", log_id=" << random_log_id << dendl;
      yield {
        auto store = sync_env->store;
        rgw_raw_obj obj(store->get_zone_params().log_pool, sync_status_oid);

        /* cancel remove bucket sync status obj
        if (info.syncstopped) {
      ...
      /*
      if (info.syncstopped) {
        ldout(cct, 10) << "NOTE: remote bucket info is sync stopped, log_id=" << random_log_id << dendl;
        retcode = -ENOENT;
      }
      if (retcode < 0) {
        return set_cr_error(retcode);
...
public:
  ...
  int operate() override {
    reenter(this) {
      ...
      do {
        yield {
          ...
          if (error_injection &&
            ...
            logger.log("fetch");
            call(data_sync_module->sync_object(sync_env, *bucket_info, key, timestamp, versioned_epoch, &zones_trace));
        ...
        // precondition failed, continue as if it succeeded
        if (retcode == -ERR_PRECONDITION_FAILED) {
          tn->log(20, SSTR("Skipping object sync: precondition failed (object contains newer change or policy doesn't allow sync)"));
          retcode = 0;
        }
      ...
      if (retcode == -ERR_NOT_MODIFIED) {
        tn->log(5, SSTR("sync object: "
            << bucket_shard_str{bs} << "/" << key.name << "not modified !!!"));
        retcode = 0;
      }

      if (retcode < 0 && retcode != -ENOENT && retcode != -ERR_NOT_MODIFIED) {
      ...
      if (!error_ss.str().empty()) {
        tn->log(0, SSTR("call log error_cr"));
      ...
      if (sync_status == 0) {
        /* update marker */
        // entry_marker: object name
        tn->log(10, SSTR("calling marker_tracker->finish:"<< entry_marker ));
        set_status() << "calling marker_tracker->finish(" << entry_marker << ")";
        yield call(marker_tracker->finish(entry_marker));
        sync_status = retcode;
      }
      if (sync_status < 0) {
        tn->log(0, SSTR("set cr error"));
        return set_cr_error(sync_status);
      }
      tn->log(10, SSTR("call set cr done"));
      return set_cr_done();
    }
    return 0;
  }
...
int RGWBucketShardFullSyncCR::operate()
{
  ...
  reenter(this) {
    ...
    do {
      ...
      ldout(sync_env->cct, 30) << __func__
                  << "listing bucket for full sync "
                  << ", bs:" << bs
      ...
      yield call(new RGWListBucketShardCR(sync_env, bs, list_marker,
                                          &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
        ldout(sync_env->cct, 0) << __func__ << "() list bucket shard err:" << retcode << ", log_id:" << random_log_id << dendl;
      ...
      for (; entries_iter != list_result.entries.end(); ++entries_iter) {
        if (!lease_cr->is_locked()) {
          ldout(sync_env->cct, 0) << __func__ << "() lease cr is not locked, log_id:" << random_log_id << dendl;
          drain_all();
          return set_cr_error(-ECANCELED);
        }
        tn->log(19, SSTR("[full sync] syncing object: "
        ...
        if (!marker_tracker.start(entry->key, total_entries, real_time())) {
          tn->log(0, SSTR("ERROR: cannot start syncing " << entry->key << ". Duplicate entry?" << ", log_id:" << random_log_id));
        } else {
        ...
        while (num_spawned() > BUCKET_SYNC_SPAWN_WINDOW) {
          ...
          while (again) {
            ...
            if (ret < 0) {
              ldout(sync_env->cct, 10) << __func__ << "() a sync operation returned error, log_id:" << random_log_id << dendl;
              sync_status = ret;
              /* we have reported this error */
            }
          }
    ...
    if (sync_status == 0) {
      ...
      tn->log(10, SSTR("backing out with sync_status=" << sync_status << ", log_id:" << random_log_id));
    }
    if (retcode < 0 && sync_status == 0) { /* actually tried to set incremental state and failed */
      tn->log(0, SSTR("ERROR: failed to set sync state on bucket "
          << bucket_shard_str{bs} << " retcode=" << retcode << ", log_id:" << random_log_id));
      return set_cr_error(retcode);
    }
    if (sync_status < 0) {
      return set_cr_error(sync_status);
...
int RGWBucketShardIncrementalSyncCR::operate()
{
  ...
  reenter(this) {
    do {
      if (!lease_cr->is_locked()) {
        drain_all();
        tn->log(0, SSTR("ERROR: lease is not taken, cancel listing bilog for incremental sync, bs:" << bs << ",position:" << sync_info.inc_marker.position << ", log_id:" << random_log_id));
        return set_cr_error(-ECANCELED);
      }

      tn->log(15, SSTR("listing bilog for incremental sync, bs:" << bs << ", position:" << sync_info.inc_marker.position << ", log_id:" << random_log_id));
      ...
      yield call(new RGWListBucketIndexLogCR(sync_env, bs, sync_info.inc_marker.position,
                                             &list_result));
      if (retcode < 0 && retcode != -ENOENT) {
        tn->log(0, SSTR("ERROR listing bilog for incremental sync, bs:" << bs << ", error:"<< retcode << ", log_id:" << random_log_id));
        /* wait for all operations to complete */
        drain_all();
        return set_cr_error(retcode);
      }
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        ...
        if (e.op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP) {
          tn->log(19, SSTR("syncstop, bs:" << bs << ", on:"<< e.timestamp << ", log_id:" << random_log_id));
          syncstopped = true;
      ...
      for (; entries_iter != entries_end; ++entries_iter) {
        if (!lease_cr->is_locked()) {
          tn->log(0, SSTR("lease_cr is not locked, bs:" << bs << ", log_id:" << random_log_id));
          drain_all();
          return set_cr_error(-ECANCELED);
        }
        entry = &(*entries_iter);
        ldout(sync_env->cct, 19) << __func__ << "() list_result entry:"<< entry->object << ", op:" << entry->op << ", entry state:" << entry->state << ", log_id:" << random_log_id << dendl;
        {
        ...
        if (entry->op == RGWModifyOp::CLS_RGW_OP_SYNCSTOP || entry->op == RGWModifyOp::CLS_RGW_OP_RESYNC) {
          tn->log(19, SSTR("detected syncstop or resync on " << entries_iter->timestamp << ", skipping entry" << ", log_id:" << random_log_id));
        ...
        if (!key.set(rgw_obj_index_key{entry->object, entry->instance})) {
          ...
          tn->log(19, SSTR("parse_raw_oid() on " << entry->object << " returned false, skipping entry" << ", log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }

        tn->log(19, SSTR("parsed entry: id=" << cur_id << " iter->object=" << entry->object << " iter->instance=" << entry->instance << " name=" << key.name << " instance=" << key.instance << " ns=" << key.ns << ", log_id:" << random_log_id));

        if (!key.ns.empty()) {
          set_status() << "skipping entry in namespace: " << entry->object;
          tn->log(19, SSTR("skipping entry in namespace: " << entry->object << ", log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        ...
        if (entry->op == CLS_RGW_OP_CANCEL) {
          ...
          tn->log(19, SSTR("skipping object: "
              << bucket_shard_str{bs} << "/" << key << ": canceled operation" << ", log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        if (entry->state != CLS_RGW_STATE_COMPLETE) {
          ...
          tn->log(19, SSTR("skipping object: "
              << bucket_shard_str{bs} << "/" << key << ": non-complete operation" << ", log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        if (entry->zones_trace.find(zone_id) != entry->zones_trace.end()) {
          ...
          tn->log(19, SSTR("skipping object: "
              <<bucket_shard_str{bs} <<"/"<<key<<": redundant operation" << ", log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        if (make_pair<>(entry->timestamp, entry->op) != squash_map[make_pair(entry->object, entry->instance)]) {
          ...
          tn->log(19, SSTR("skipping object: "
              << bucket_shard_str{bs} << "/" << key << ": squashed operation" << ",log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        tn->set_flag(RGW_SNS_FLAG_ACTIVE);
        tn->log(19, SSTR("syncing object: "
            << bucket_shard_str{bs} << "/" << key << ", log_id:" << random_log_id));
        ...
        while (!marker_tracker.can_do_op(key, has_olh_epoch(entry->op))) {
          ...
          tn->log(5, SSTR("can't do op on key=" << key << " need to wait for conflicting operation to complete" << ", log_id:" << random_log_id));
          yield wait_for_child();
          bool again = true;
          while (again) {
            again = collect(&ret, nullptr);
            if (ret < 0) {
              tn->log(0, SSTR("ERROR: a child operation returned error (ret=" << ret << ")" << ", log_id:" << random_log_id));
              sync_status = ret;
              /* we have reported this error */
            }
          }
        ...
        if (!marker_tracker.index_key_to_marker(key, cur_id, has_olh_epoch(entry->op))) {
          ...
          tn->log(19, SSTR("skipping sync of entry: " << cur_id << ":" << key << " sync already in progress for object" << ", log_id:" << random_log_id));
          marker_tracker.try_update_high_marker(cur_id, 0, entry->timestamp);
          continue;
        }
        // yield {
          ...
          if (!marker_tracker.start(cur_id, 0, entry->timestamp)) {
            tn->log(0, SSTR("ERROR: cannot start syncing " << cur_id << ". Duplicate entry?" << ", log_id:" << random_log_id));
          } else {
            std::optional<uint64_t> versioned_epoch;
            ...
            tn->log(10, SSTR("sync object entry->timestamp=" << entry->timestamp << ", log_id:" << random_log_id));
    ...
    if (retcode < 0) {
      tn->log(0, SSTR("ERROR: marker_tracker.flush() returned retcode=" << retcode << ", log_id:" << random_log_id));
      return set_cr_error(retcode);
    }
    if (sync_status < 0) {
      tn->log(0, SSTR("backing out with sync_status=" << sync_status << ", log_id:" << random_log_id));
      return set_cr_error(sync_status);
    }
    return set_cr_done();
  }
...
{
  map<string, RGWBucketSyncConf>::iterator iter;
  map<string, RGWZone>::iterator iter_z;
  bool jump_list = false;
  reenter(this) {
    yield {
      set_status("acquiring sync lock");
      auto store = sync_env->store;
    ...
    while (!lease_cr->is_locked()) {
      if (lease_cr->is_done()) {
        tn->log(5, SSTR("failed to take lease for it is already done" << ", bs:" << bs << ", log_id:" << random_log_id));
        set_status("lease lock failed, early abort");
        drain_all();
        return set_cr_error(lease_cr->get_ret_status());
      }
      tn->log(19, SSTR("lease_cr is not done, call sleep wait for lock" << ", bs:" << bs << ", log_id:" << random_log_id));
      set_sleeping(true);
      yield;
    }

    tn->log(10, SSTR("took lease call RGWReadBucketSyncStatusCoroutine" << ", log_id:" << random_log_id));
    yield call(new RGWReadBucketSyncStatusCoroutine(sync_env, bs, &sync_status));
    if (retcode < 0 && retcode != -ENOENT) {
      tn->log(0, SSTR("ERROR: failed to read sync status for bucket" << ", bs:" << bs << ", log_id:" << random_log_id));
      lease_cr->go_down();
      drain_all();
      return set_cr_error(retcode);
    }

    tn->log(19, SSTR("sync status for bucket: " << sync_status.state << ", bs:" << bs << ", log_id:" << random_log_id));
    ...
    if (retcode == -ENOENT) {
      ...
      yield {
        tn->log(10, SSTR("no local info for bucket:" << ": fetching metadata" << ", log_id:" << random_log_id));
      ...
      if (retcode < 0) {
        tn->log(0, SSTR("ERROR: failed to fetch bucket instance info for " << bucket_str{bs.bucket} << ", log_id:" << random_log_id));
        lease_cr->go_down();
        drain_all();
        return set_cr_error(retcode);
      }
    ...
    if (retcode < 0) {
      tn->log(0, SSTR("ERROR: failed to retrieve bucket info for bucket=" << bucket_str{bs.bucket}<< ", bs:" << bs << ", log_id:" << random_log_id));
      lease_cr->go_down();
      drain_all();
      return set_cr_error(retcode);
    }

    iter_z = sync_env->store->zone_by_id.find(sync_env->conn->get_remote_id());
    if (iter_z == sync_env->store->zone_by_id.end()) {
      tn->log(0, SSTR("ERROR: can't find dest zone in zone_by_id, jump list bucket index log, bs:"<< bs << ", log_id=" << random_log_id));
      lease_cr->go_down();
      drain_all();
      return set_cr_error(-ENOENT);
    }

    iter = bucket_info.sync_config.find(iter_z->second.name);
    if (iter != bucket_info.sync_config.end() && iter->second.is_delay) {
      tn->log(10, SSTR("dest zone not time, jump list bucket index log, bs:"<< bs << ", log_id=" << random_log_id));
      jump_list = true;
    }

    iter = bucket_info.sync_config.find(sync_env->store->get_zone().name);
    if (iter != bucket_info.sync_config.end() && iter->second.is_delay) {
      tn->log(10, SSTR(" local zone not time, jump list bucket index log, bs:"<< bs << ", log_id=" << random_log_id));
      jump_list = true;
    }

    if (!bucket_info.datasync_flag_enabled(iter_z->second.name) ||
        !bucket_info.datasync_flag_enabled(sync_env->store->get_zone().name)) {
      tn->log(10, SSTR("sync disabled, jump list bucket index log, bs:"<< bs << ", log_id=" << random_log_id));
      jump_list = true;
    }
    ...
    if (jump_list) {
      ...
      /*
      if (marker_tracker && !entry_marker.empty()) {
        tn->log(19, SSTR("clear entry_marker:" << entry_marker << ", log_id=" << random_log_id));
        marker_tracker->finish(entry_marker);
      }
      */
      // call go_down -> trigger release lock
      lease_cr->go_down();
      drain_all();
      // do not call set_cr_done(), or next time will treat it as already synced
      // if not set done, this coroutine will never end
      return set_cr_done();
    }

    do {
      if (sync_status.state == rgw_bucket_shard_sync_info::StateInit) {
        tn->log(19, SSTR("call list bilog, bs:"<< bs << ", log_id=" << random_log_id));
        yield call(new RGWInitBucketShardSyncStatusCoroutine(sync_env, bs, sync_status, random_log_id));
        if (retcode == -ENOENT) {
          tn->log(0, SSTR("ERROR: init bucket shard sync status failed: ENOENT, log_id=" << random_log_id));
          lease_cr->abort(); // deleted lease object, abort instead of unlock
          drain_all();
          return set_cr_done();
        }
        if (retcode < 0) {
          tn->log(0, SSTR("ERROR: init sync on bucket failed, retcode=" << retcode << ",bs:" << bs << ", log_id:" << random_log_id));
          lease_cr->go_down();
          drain_all();
          return set_cr_error(retcode);
        }
      ...
      if (sync_status.state == rgw_bucket_shard_sync_info::StateFullSync) {
        tn->log(19, SSTR("call RGWBucketShardFullSyncCR, bs:"<< bs << ", log_id="<<random_log_id));
        yield call(new RGWBucketShardFullSyncCR(sync_env, bs, &bucket_info,
                                                status_oid, lease_cr.get(),
                                                sync_status, tn, random_log_id));
        if (retcode < 0) {
          tn->log(5, SSTR("ERROR: full sync on bucket failed, retcode=" << retcode << ", log_id:" << random_log_id));
          lease_cr->go_down();
          drain_all();
          return set_cr_error(retcode);
        }
      ...
      if (sync_status.state == rgw_bucket_shard_sync_info::StateIncrementalSync) {
        tn->log(19, SSTR("call RGWBucketShardIncrementalSyncCR, bs:" << bs << ", log_id:" << random_log_id));
        yield call(new RGWBucketShardIncrementalSyncCR(sync_env, bs, &bucket_info,
                                                       status_oid, lease_cr.get(),
                                                       sync_status, tn, random_log_id));
        if (retcode < 0) {
          tn->log(5, SSTR("ERROR: incremental sync on bucket failed, retcode=" << retcode << ", bs:"<< bs << ", log_id:" << random_log_id));
          lease_cr->go_down();
          drain_all();
          return set_cr_error(retcode);
        }
...
uint64_t sync_marker_gap(const string& pos, const string& dst, std::ostream* out) {
  auto period_pos = pos.find(".");
  if (period_pos == std::string::npos) {
    *out << "ERROR: invalid sync marker:" << pos << std::endl;
    return 0;
  }
  uint64_t position = 0;
  if (stringtoull(pos.substr(0, period_pos), &position) < 0) {
    *out << "ERROR: invalid sync marker:" << pos << std::endl;
    return 0;
  }
  period_pos = dst.find(".");
  if (period_pos == std::string::npos) {
    *out << "ERROR: invalid remote sync marker:" << dst << std::endl;
    return 0;
  }
  uint64_t remote_marker = 0;
  if (stringtoull(dst.substr(0, period_pos), &remote_marker) < 0) {
    *out << "ERROR: invalid remote sync marker:" << dst << std::endl;
    return 0;
  }
  if (remote_marker < position) {
    *out << "ERROR: invalid sync marker:" << pos << ", remote:" << dst << std::endl;
    return 0;
  }
  return remote_marker - position;
}

int remote_bilog_markers(RGWRados *store, RGWRESTConn *conn, const RGWBucketInfo& info,
                         BucketIndexShardsManager *markers)
{
  ...
  if (r < 0) {
    lderr(store->ctx()) << "failed to decode remote log markers:" << result.max_marker << dendl;
    return r;
  }
  return 0;
}
