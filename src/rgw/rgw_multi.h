...
extern int erase_orphan_part(RGWRados *store, RGWUploadPartInfo info, string tag, rgw_obj meta_obj);

extern int get_orphan_parts(RGWRados *store, RGWBucketInfo& bucket_info, CephContext *cct,
                                string& meta_oid, int num_parts,
                                map<string, RGWUploadPartInfo>& parts);
