...
namespace rgw {
  ...
  int RGWWriteRequest::exec_start() {
    ...
    auto compression_type =
      get_store()->get_zone_params().get_compression_type(s->dest_placement);
  ...
  int RGWWriteRequest::exec_finish()
  {
    ...
    if (!pinned_extent.empty()) {
      op_ret = EIO;
      ldout(s->cct, 0)
        << __func__ << " close with unflushed extent "
        << "(" << rgw_fh->bucket_name() << ","
        << rgw_fh->object_name() << ")"
        << dendl;
      goto done;
	     ...
	     struct rgw_file_handle *fh, uint32_t posix_flags, uint32_t flags)
{
  ...
  return rgw_fh->open(flags, posix_flags);
}
