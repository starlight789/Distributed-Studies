...
void get_user_list(RGWRados *store, set<string>& user_list) {
  void *handle = nullptr;
  string marker = "";
  int max = 1000;
  int ret = store->meta_mgr->list_keys_init("user", marker, &handle);
  if (ret < 0) {
    return;
  }
  ...
  do {
    ...
    if (ret < 0 && ret != -ENOENT) {
      ldout(store->ctx(), 0) << "get_user_list" << " ERROR: could not list_key_next, ret=" << ret << dendl;
      return;
    } if (ret != -ENOENT) {
      for (list<string>::iterator iter = keys.begin(); iter != keys.end(); ++iter) {
        user_list.insert(*iter);
      }
    }
  } while (truncated);
                           ...
                           RGWStorageStats *stats = NULL)
{
  ...
  f->dump_int("qps_limit", (int)info.qps);
  f->dump_int("bandwidth_limit", (int)info.bandwidth);
  info.caps.dump(f);
  if (stats) {
...
int RGWUser::init(RGWUserAdminOpState& op_state)
{
  ...
  if (!user_id.empty() && (user_id.compare(RGW_USER_ANON_ID) != 0)) {
    found = (rgw_get_user_info_by_uid(store, user_id, user_info, &op_state.objv, NULL, NULL) >= 0);
...
int RGWUser::list(RGWUserAdminOpState& op_state, RGWFormatterFlusher& flusher)
{
  ...
  do {
    ...
    ret = store->meta_mgr->list_keys_next(handle, left,
    keys, &truncated);
    if (ret < 0 && ret != -ENOENT) {
      return ret;
    } if (ret != -ENOENT) {
...
int RGWUserAdminOp_Qos::set(RGWRados *store, RGWUserAdminOpState& op_state,
                  RGWFormatterFlusher& flusher)
{
  RGWUserInfo info;
  RGWUser user;
  int ret = user.init(store, op_state);
  if (ret < 0) {
    ldout(store->ctx(), 0) << "init user failed" << dendl;
    return ret;
  }
  if (!op_state.has_existing_user())
    return -ERR_NO_SUCH_USER;
  ...
  map<string, bufferlist> attrs;
  ret = rgw_get_user_info_by_uid(store, user_id, info, &op_state.objv, NULL, NULL);
  if (ret < 0) {
    ldout(store->ctx(), 0) << "unable to get user info, uid:" <<  user_id << dendl;
    return ret;
  }

  info.qps = op_state.qps_limit;
  info.bandwidth = op_state.bandwidth_limit;

  ret = rgw_store_user_info(store, info, NULL, &op_state.objv, real_time(), false);
  if (ret < 0) {
    ldout(store->ctx(), 0) << "unable to store user info, uid:" <<  user_id << dendl;
    return ret;
  }
  return 0;
}
