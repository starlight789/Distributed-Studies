...
class ESQueryNode_Op_Wildcard : public ESQueryNode_Op {
public:
  ESQueryNode_Op_Wildcard (ESQueryCompiler *compiler) : ESQueryNode_Op(compiler) {}

  virtual void dump(Formatter *f) const {
    f->open_object_section("wildcard");
    f->open_object_section(field.c_str());
    val->encode_json("value", f);
    f->close_section();
    f->close_section();
  }
};
...
static bool alloc_node(ESQueryCompiler *compiler, ESQueryStack *s, ESQueryNode **pnode, string *perr)
{
  ...
  if (is_bool_op(op)) {
    ...
    node = new ESQueryNode_Op_Wildcard(compiler);
  } else {
