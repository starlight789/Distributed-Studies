...
void RGWCORSConfiguration_S3::to_json(ostream& out) {
  JSONFormatter jf;
  jf.open_object_section("");
  jf.open_array_section("corsConfiguration");
  for(list<RGWCORSRule>::iterator it = rules.begin();
      it != rules.end(); ++it) {
    (static_cast<RGWCORSRule_S3 &>(*it)).to_json(jf);
  }
  jf.close_section();
  jf.close_section();
  jf.flush(out);
...
void RGWCORSRule_S3::to_json(JSONFormatter& jf) {
  jf.open_object_section("");
  /*AllowedMethods*/
  if (allowed_methods != 0) {
    jf.open_array_section("allowedMethods");
    if (allowed_methods & RGW_CORS_GET) {
      jf.dump_string("", "GET");
    }
    if (allowed_methods & RGW_CORS_PUT)
  ...
  for(set<string>::iterator it = allowed_origins.begin();
      ...
      ++it) {
    if (!exist) {
      jf.open_array_section("allowedOrigins");
      exist = true;
    }
    jf.dump_string("", *it);
  }
  ...
  for(set<string>::iterator it = allowed_hdrs.begin();
      it != allowed_hdrs.end(); ++it) {
    if (!exist) {
      jf.open_array_section("allowedHeaders");
      exist = true;
    }
    jf.dump_string("", *it);
  }
  ...
  /*MaxAgeSeconds*/
  if (max_age != CORS_MAX_AGE_INVALID) {
    jf.dump_unsigned("maxAgeSeconds", max_age);
  }
  /*ExposeHeader*/
  exist = false;
  for(list<string>::iterator it = exposable_hdrs.begin();
      it != exposable_hdrs.end(); ++it) {
    if (!exist) {
      jf.open_array_section("allowedExposeHeaders");
      exist = true;
    }
    jf.dump_string("", *it);
  }
