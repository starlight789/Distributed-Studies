...
{
  rgw_rados_ref ref;
  int r = store->get_raw_obj_ref(obj, &ref, rados_handle);
  if (r < 0) {
...
RGWAsyncLockSystemObj::RGWAsyncLockSystemObj(RGWCoroutine *caller, RGWAioCompletionNotifier *cn, RGWRados *_store,
                      RGWObjVersionTracker *_objv_tracker, const rgw_raw_obj& _obj,
                       ...
                       librados::Rados* _rados_handle) : RGWAsyncRadosRequest(caller, cn), store(_store),
                                                              obj(_obj),
                                                              lock_name(_name),
                                                              cookie(_cookie),
...
RGWSimpleRadosLockCR::RGWSimpleRadosLockCR(RGWAsyncRadosProcessor *_async_rados, RGWRados *_store,
                      ...
                      uint32_t _duration, 
                      librados::Rados* _rados_handle) : RGWSimpleCoroutine(_store->ctx()),
...
int RGWSimpleRadosLockCR::send_request()
{
  ...
  req = new RGWAsyncLockSystemObj(this, stack->create_completion_notifier(),
                                 store, NULL, obj, lock_name, cookie, duration, rados_handle);
  async_rados->queue(req);
  return 0;
}
...
int RGWSimpleRadosUnlockCR::send_request()
{
  ...
  req = new RGWAsyncUnlockSystemObj(this, stack->create_completion_notifier(),
                                 store, NULL, obj, lock_name, cookie, rados_handle);
  async_rados->queue(req);
  return 0;
}
...
int RGWAsyncFetchRemoteObj::_send_request()
{
  ...
  int r = store->fetch_remote_obj(obj_ctx,
                       ...
                       &umod_time, /* const real_time* unmod_ptr, */
  ...
  if (not_modified) {
    ldout(store->ctx(), 10) << "fetch_remote_obj returned " << ERR_NOT_MODIFIED << dendl;
...
int RGWContinuousLeaseCR::operate()
{
  ...
  if (!rados_handle) {
    rados_handle = store->get_rados_handle();
  }
  reenter(this) {
    while (!going_down) {
      yield call(new RGWSimpleRadosLockCR(async_rados, store, obj, lock_name, cookie, interval, rados_handle));

      caller->set_sleeping(false); /* will only be relevant when we return, that's why we can do it early */
      if (retcode < 0) {
        set_locked(false);
        ldout(store->ctx(), 19) << *this << ": couldn't lock " << obj << ":" << lock_name << ": retcode=" << retcode << dendl;
    ...
    yield call(new RGWSimpleRadosUnlockCR(async_rados, store, obj, lock_name, cookie, rados_handle));
    return set_state(RGWCoroutine_Done);
  }
  return 0;
}
...
{
  rgw_raw_obj raw_obj;
  store->obj_to_raw(bucket_info.head_placement_rule, obj, &raw_obj);
