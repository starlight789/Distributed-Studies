                                    ...
                                    const RGWBucketWebsiteConf& ws_conf)
{
  ...
  if (s->object.empty()) {
    ...
    if (!s->bucket_info.head_placement_rule.name.empty()) {
      dump_header(s, "X-Storage-Policy", s->bucket_info.head_placement_rule.name);
    }
    dump_header(s, "X-Storage-Class", s->bucket_info.head_placement_rule.get_storage_class());
...
int RGWPutMetadataBucket_ObjStore_SWIFT::get_params()
{
  ...
  get_rmattrs_from_headers(s, CONT_PUT_ATTR_PREFIX, CONT_REMOVE_ATTR_PREFIX,
			   rmattr_names);
  placement_rule.name = s->info.env->get("HTTP_X_STORAGE_POLICY", "");
                                              ...
                                              RGWRados& store)
{
  ...
  formatter.dump_int("max_object_name_size", MAX_OBJ_NAME_LEN);
...
int RGWHandler_REST_SWIFT::postauth_init()
{
  ...
  dout(10) << "s->object=" <<
    (!s->object.empty() ? s->object : rgw_obj_key("<NULL>"sv))
           << " s->bucket="
