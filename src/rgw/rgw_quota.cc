...
{
  ldout(store->ctx(), 20) << "objs_delta = " << objs_delta << dendl;
                                                  ...
                                                  const uint64_t num_objs) const
{
  ...
  dout(20) << "cur.num_objects = " << stats.num_objects << dendl;
  ...
  int check_quota(const char * const entity,
    ...
    if (quota_applier.is_num_objs_exceeded(entity, quota, stats, num_objs)) {
      if (strcmp(entity, "user") == 0) {
        return -ERR_USER_OBJECT_NUM_QUOTA_EXCEEDED;
      }
      return -ERR_BUCKET_OBJECT_NUM_QUOTA_EXCEEDED;
    }

    if (quota_applier.is_size_exceeded(entity, quota, stats, size)) {
      if (strcmp(entity, "user") == 0) {
        return -ERR_USER_CAPACITY_QUOTA_EXCEEDED;
      }
      return -ERR_BUCKET_CAPACITY_QUOTA_EXCEEDED;
    }
