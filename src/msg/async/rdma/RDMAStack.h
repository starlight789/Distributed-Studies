...
class RDMADispatcher {
 ...
 public:
  ...
  Infiniband::QueuePair* get_qp_lockless(uint32_t qp);
  Infiniband::QueuePair* get_qp(uint32_t qp);
  void enqueue_dead_qp_lockless(uint32_t qpn);
  void enqueue_dead_qp(uint32_t qpn);
  void schedule_qp_destroy(uint32_t qpn);
...
class RDMAConnectedSocketImpl : public ConnectedSocketImpl {
 ...
 private:
  ...
  int notify_fd = -1;
  std::atomic<uint32_t> notify_marker = {0};
 ...
 public:
  ...
  std::ostream& gen_prefix(std::ostream& _dout) const;
