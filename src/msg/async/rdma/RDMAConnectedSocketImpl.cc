...
std::ostream& RDMAConnectedSocketImpl::gen_prefix(std::ostream& _dout) const
{
  _dout << " RDMAConnectedSocketImpl qp" << my_msg.qpn << " ";
  return _dout;
}
...
int RDMAConnectedSocketImpl::activate()
{
  ...
  qpa.min_rnr_timer = 0x12; // 5.12 ms
...
ssize_t RDMAConnectedSocketImpl::read(char* buf, size_t len)
{
  ...
  uint32_t need_read = notify_marker.load(std::memory_order_acquire);
  if (need_read > 0) {
    r = ::read(notify_fd, &i, sizeof(i));
    notify_marker.compare_exchange_strong(need_read, 0, std::memory_order_release,
        std::memory_order_acquire);
  }
  ldout(cct, 20) << __func__ << " notify_fd : " << i << " r = " << r
                 << " notify_marker=" << need_read << dendl;

  if (!active) {
    if (error) {
      ldout(cct, 1) << __func__ << " get an error when ib_not_active." << dendl;
      return -error;
    }
...
{
  ldout(cct, 20) << __func__ << " start read buffers " << dendl;
  size_t read = 0, tmp = 0;
...
{
  ldout(cct, 20) << __func__ << " start send " << dendl;
  if (error) {
    if (!active)
      return -EPIPE;
    return -error;
...
void RDMAConnectedSocketImpl::fin() {
  ldout(cct, 20) << __func__ << " start send fin " << dendl;
  ibv_send_wr wr;
...
void RDMAConnectedSocketImpl::cleanup() {
  ldout(cct, 20) << __func__ << " cleanup " << dendl;
...
void RDMAConnectedSocketImpl::notify()
{
  ...
  notify_marker.fetch_add(1, std::memory_order_release);
}

void RDMAConnectedSocketImpl::shutdown()
{

  ldout(cct, 20) << __func__ << " shutdown " << dendl;
  if (!error)
    fin();
  error = ECONNRESET;
  active = false;
...
{
  ldout(cct, 20) << __func__ << " close " << dendl;
  if (!error)
    fin();
  error = ECONNRESET;
  active = false;
