    ...
    switch (async_event.event_type) {
      case IBV_EVENT_CQ_ERR:
        lderr(cct) << __func__ << " Fatal Error, effect all QP bound with same CQ, "
                   << " CQ Overflow, dev = " << get_stack()->get_infiniband().get_device()->ctxt
      ...
      case IBV_EVENT_QP_LAST_WQE_REACHED:
        ...
        break;
      default:
        ldout(cct, 1) << __func__ << " ibv_get_async_event: dev=" 
                      << get_stack()->get_infiniband().get_device()->ctxt
    ...
    if (!tx_ret && !rx_ret) {
      ...
      if (!dead_queue_pairs.empty()) {
        ...
        for (auto& qp: dead_qps) {
          ...
          ldout(cct, 10) << __func__ << " qp" << qp->get_local_qp_number()
                         << " finally delete qp = " << qp << dendl;
          delete qp;
        }
      }
...
{
  ldout(cct, 20) << __func__ << " qp" << qpn << " enqueue to dead qp" << dendl;
...
void RDMADispatcher::schedule_qp_destroy(uint32_t qpn)
{
  ...
  if (qp->to_dead()) {
    ...
    // But only zero pointer in order to return null from
    // get_conn_lockless();
...
void RDMADispatcher::handle_tx_event(ibv_wc *cqe, int n)
{
  ...
  for (int i = 0; i < n; ++i) {
    ...
    ldout(cct, 25) << __func__ << " qp" << response->qp_num
                   ...
                   << " " << Infiniband::wc_status_to_string(response->status) 
                   << dendl;
    ...
    if (response->status != IBV_WC_SUCCESS) {
      ...
      switch (response->status) {
        case IBV_WC_RETRY_EXC_ERR:
          ...
          ldout(cct, 1) << __func__ << " qp" <<  response->qp_num 
        ...
        case IBV_WC_WR_FLUSH_ERR: 
          {
            ...
            if (qp) {
              ldout(cct, 20) << __func__ << " qp" <<  response->qp_num << " state is " 
                             << Infiniband::qp_state_string(qp->get_state()) << dendl;
            }
            if (qp && qp->is_dead()) {
              ldout(cct, 20) << __func__ << " qp" <<  response->qp_num 
                             << " outstanding SQ WR is flushed into CQ since QueuePair is dead " 
                             << dendl;
            } else {
              lderr(cct) << __func__ << " qp" <<  response->qp_num
        ...
        default:
          {
            lderr(cct) << __func__ << " qp" <<  response->qp_num
                       << " SQ WR return error, WCE status(" << response->status << "): " 
                       << Infiniband::wc_status_to_string(response->status) 
                       << " Opcode " << response->opcode << " wr_id: 0x" 
            ...
            if (conn && conn->is_connected()) {
              ldout(cct, 20) << __func__ << " qp" <<  response->qp_num 
                             ...
                             << conn->get_qp_state() << dendl;
              conn->fault();
            } else {
              ldout(cct, 1) << __func__ << " qp" << response->qp_num 
                            << " Disconnected, discard event" << dendl;
            }
          }
          break;
      }
    ...
    // 2) 'fin' message, wr_id points to the QueuePair.
    if (get_stack()->get_infiniband().get_memory_manager()->is_tx_buffer(chunk->buffer)) {
      ...
      ldout(cct, 1) << __func__ << " qp" << response->qp_num 
                                << " not tx buffer, chunk " << chunk << dendl;
      ceph_abort();
    }
  }
...
void RDMADispatcher::handle_rx_event(ibv_wc *cqe, int rx_number)
{
  ...
  post_backlog += rx_number - get_stack()->get_infiniband().post_chunks_to_srq(rx_number);

  for (int i = 0; i < rx_number; ++i) {
    ibv_wc* response = &cqe[i];
    Chunk* chunk = reinterpret_cast<Chunk *>(response->wr_id);
    ldout(cct, 25) << __func__ << " qp" << response->qp_num << " got chunk=" << chunk 
                   << " bytes:" << response->byte_len << " opcode:" << response->opcode << dendl;
    ...
    if (response->status == IBV_WC_SUCCESS) {
      ...
      if (!conn) {
        ldout(cct, 1) << __func__ << " qp" << response->qp_num << " may be dead. chunk " << chunk 
                      << " will be back." << dendl;
        get_stack()->get_infiniband().post_chunk_to_pool(chunk);
        perf_logger->dec(l_msgr_rdma_rx_bufs_in_use);
      } else {
...
int RDMAWorker::get_reged_mem(RDMAConnectedSocketImpl *o, std::vector<Chunk*> &c, size_t bytes)
{
  ...
  ldout(cct, 25) << __func__ << " need " << bytes << " bytes, reserve " << got << " registered  bytes, inflight " << dispatcher->inflight << dendl;
