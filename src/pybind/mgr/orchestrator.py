...
class InventoryDevice(object):
    ...
    def __init__(self, blank=False, type=None, id=None, size=None,
                 extended=None, metadata_space_free=None):
        self.blank = blank
        self.type = type  # 'ssd', 'hdd', 'nvme'
        self.id = id  # unique within a node (or globally if you like).
        self.size = size  # byte integer.
        # arbitrary JSON-serializable object
    ...
    def to_json(self):
        ...
        return dict(type=self.type, blank=self.blank, id=self.id,
                    size=self.size, extended=self.extended)
