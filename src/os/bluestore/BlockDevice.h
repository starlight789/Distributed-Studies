...
public:
 ...
 {}
  ...
  virtual int get_device_and_type(map<std::string, bool> &ls) {
    return -ENOTSUP;
  }

  virtual int get_device_and_type(pair<std::string, bool> &ls) {
    return -ENOTSUP;
  }
  ...
  virtual int aio_write(
    ...
    IOContext *ioc,
    bool buffered) = 0;
  virtual int flush(bool force_flush = true) = 0;
  ...
  virtual int open(const std::string& path, bool is_data_dev) = 0;
