...
class KernelDevice : public BlockDevice {
  ...
  // this device is used for bluestore data
  bool is_bluestore_data_block = false;
...
public:
  ...
  int get_device_and_type(map<std::string, bool> &ls);
  int get_device_and_type(pair<std::string, bool> &ls);
  ...
  int aio_write(uint64_t off, bufferlist& bl,
		IOContext *ioc,
		bool buffered) override;
  int flush(bool force_flush) override;
  int discard(uint64_t offset, uint64_t len) override;

  // for managing buffered readers/writers
  int invalidate_cache(uint64_t off, uint64_t len) override;
  int open(const std::string& path, bool is_data_dev) override;
  void close() override;
};
