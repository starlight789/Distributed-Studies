...
//#define CACHE_BLOB_BL  // not sure if this is a win yet or not... :/
  ...
  */
  ...
  struct Onode {
    ...
    Onode(Collection *c, const ghobject_t& o, const mempool::bluestore_cache_other::string& k)
      : c(c),
        oid(o),
        key(k),
        extent_map(this),
        nref(0),
        exists(false) {
      // record the memory that used by oid.name
      mempool::get_pool(mempool::pool_index_t(mempool::mempool_bluestore_cache_other)).adjust_count(1,
          (int)(oid.hobj.oid.name.capacity()));
    }
  ...
  private:
    ...
    /// forward lookups
    mempool::bluestore_cache_other::unordered_map<wrapper_ghobject_t, OnodeRef> onode_map;
...
private:
  ...
  uint64_t prefer_deferred_size = 0;
    ...
    struct MetaCache : public MempoolCache {
      ...
      virtual uint64_t _get_used_bytes() const {
        return mempool::bluestore_cache_other::allocated_bytes() +
               ...
               mempool::bluestore_inline_bl::allocated_items() * (sizeof(bufferptr)+2*sizeof(size_t)) +
               mempool::bluestore_onode_attrs::allocated_bytes() +
               mempool::bluestore_onode_attrs::allocated_items() * sizeof(bufferptr);
      }
...
private:
  ...
  // if poweroff is set as false, we force BlueStore to flush disks:
  //  1. when kv will exit
  //  2. when _kv_sync_thread wait for signal timeout
  inline void _no_poweroff_force_flush_all() {
    if (likely(cct->_conf->bdev_poweroff_safe))
      return;
    bdev->flush(true);
    KeyValueDB::Transaction synct = db->get_transaction();
    cct->_conf->bluestore_debug_omit_kv_commit ? 0 : db->submit_transaction_sync(synct);
  }
...
public:
  ...
  int get_devices_and_types(map<string, bool> &ls) override;
  int get_data_device_and_type(pair<string, bool> &device) override;
