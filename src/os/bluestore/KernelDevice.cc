...
int KernelDevice::open(const string& p, bool is_data_dev)
{
  path = p;
  is_bluestore_data_block = is_data_dev;
  int r = 0;
...
int KernelDevice::get_device_and_type(std::map<std::string, bool> &ls)
{
  if (!devname.empty()) {
    ls[devname] = rotational;
    return 0;
  } else {
    return -ENOTSUP;
  }
...
int KernelDevice::get_device_and_type(pair<std::string, bool> &ls)
{
  ls.first.clear();
  if (!devname.empty()) {
    ls.first = devname;
    ls.second = rotational;
    return 0;
  } else {
    return -ENOTSUP;
  }
...
int KernelDevice::flush(bool force_flush)
{
  // when using multiple aio threads, flush the disk directly in the thread
  if (!force_flush && is_bluestore_data_block) {
    dout(10) << __func__ << " skip flush force_flush=" << force_flush
             << " is_bluestore_data_block=" << is_bluestore_data_block
             << dendl;
    return 0;
  }
