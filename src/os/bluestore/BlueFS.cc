...
int BlueFS::get_devices_and_types(map<string, bool> &ls)
{
  for (unsigned i = 0; i < MAX_BDEV; ++i) {
    if (bdev[i]) {
      bdev[i]->get_device_and_type(ls);
    }
  }
  return 0;
}
...
int BlueFS::_flush_range(FileWriter *h, uint64_t offset, uint64_t length)
{
  ...
  if (h->file->deleted) {
    dout(0) << __func__ << "  deleted, no-op" << dendl;
    return 0;
  }
  ...
  if (h->file->fnode.ino == 1) {
    buffered = false;
  } else if (cct->_conf->bluestore_force_use_buffer_io) {
    buffered = true;
  } else {
...
void BlueFS::_maybe_compact_log(std::unique_lock<std::mutex>& l)
{
...
uint8_t BlueFS::choose_perfer_dev(uint8_t wanted_dev) {
  if (wanted_dev == BDEV_SLOW &&
      alloc[BDEV_DB] &&
      alloc[BDEV_DB]->get_free() > cct->_conf->bluestore_volume_selection_db_reserved) {
    dout(10) << __func__ << " use BDEV_BD instead of BDEV_SLOW, db free size: "
             << alloc[BDEV_DB]->get_free() << dendl;
    return BDEV_DB;
  }
  return wanted_dev;
}
  ...
  bool overwrite)
{
  ...
  if (dirname.length() > 5) {
    ...
    if (boost::algorithm::ends_with(dirname, ".slow")) {
      file->fnode.prefer_bdev = choose_perfer_dev(BlueFS::BDEV_SLOW);
