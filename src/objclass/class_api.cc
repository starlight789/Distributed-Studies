...
int cls_cxx_map_get_vals(cls_method_context_t hctx, const string &start_obj,
                         ...
                         map<string, bufferlist> *vals, bool *more, bool skip_cache)
{
  PrimaryLogPG::OpContext **pctx = (PrimaryLogPG::OpContext **)hctx;

  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_get_vals(hctx, start_obj, filter_prefix, max_to_get,
                                                       vals, more);
  }
...
// v1 don't decode outdata
int cls_cxx_map_get_vals_v1(cls_method_context_t hctx, const string &start_obj,
                            const string &filter_prefix, uint64_t max_to_get,
                            bufferlist* vals)
{
  PrimaryLogPG::OpContext **pctx = (PrimaryLogPG::OpContext **)hctx;
  ...
  int ret = (*pctx)->pg->do_osd_ops(*pctx, ops);
  if (ret < 0)
    return ret;

  vals->swap(op.outdata);
  return ret;
}

int cls_cxx_map_read_header(cls_method_context_t hctx,
                            bufferlist *outbl,
                            bool skip_cache)
{
  PrimaryLogPG::OpContext **pctx = (PrimaryLogPG::OpContext **)hctx;
  ...
  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_read_header(hctx, outbl);
  }
                        ...
                        bool skip_cache)
{
  ...
  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_get_val(hctx, key, outbl);
  }
                        ...
                        bool skip_cache)
{
  ...
  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_set_val(hctx, key, inbl);
  }
...
int cls_cxx_map_write_header(cls_method_context_t hctx, bufferlist *inbl, bool skip_cache)
{
  ...
  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_write_header(hctx, inbl);
  }
...
int cls_cxx_map_remove_keys(cls_method_context_t hctx, const vector<string>& keys, bool skip_cache)
{
  PrimaryLogPG::OpContext **pctx = (PrimaryLogPG::OpContext **)hctx;

  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_remove_keys(hctx, keys);
  }
...
int cls_cxx_map_remove_key(cls_method_context_t hctx, const string &key, bool skip_cache)
{
  ...
  // process header in cache
  if ((*pctx)->merged_cache && !skip_cache) {
    return (*pctx)->merged_cache->cls_cxx_map_remove_key(hctx, key);
  }
...
bool cls_check_oid_exist(cls_method_context_t hctx) {
  PrimaryLogPG::OpContext *ctx = *(PrimaryLogPG::OpContext **)hctx;
