...
int Objecter::_calc_target(op_target_t *t, Connection *con, bool any_change)
{
  ...
  ldout(cct,30) << __func__ << "  base pi " << pi << " base pool " << t->base_oloc.pool
  ...
  // apply tiering
  // 根据读写操作，分别设置需要操作的 tier 池
  t->target_oid = t->base_oid;
  t->target_oloc = t->base_oloc;
  if ((t->flags & (CEPH_OSD_FLAG_IGNORE_OVERLAY | CEPH_OSD_FLAG_SKIP_CACHE)) == 0) {
    // try to find cache pool
  ...
  } else if ((t->flags & CEPH_OSD_FLAG_SKIP_CACHE) != 0 && pi->is_tier()) {
    // try to find backend pool
    t->target_oloc.pool = pi->tier_of;
    pi = osdmap->get_pg_pool(t->target_oloc.pool);
    if (!pi) {
      t->osd = -1;
      return RECALC_OP_TARGET_POOL_DNE;
