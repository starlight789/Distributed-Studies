...
bool compatible_platform_command(const cmdmap_t& cmdmap, const string& key, const string& value) { 
  auto itr = cmdmap.find(key);
  if (itr != cmdmap.end()) {
    try {
      std::string v_str = boost::get<std::string>(itr->second);
      if (v_str == value) {
         return true;
      }
    } catch (boost::bad_get&) {
      throw bad_cmd_get(key, cmdmap);
    }
  }
  return false;
}
...
void OSDMonitor::encode_pending(MonitorDBStore::TransactionRef t)
{
...

  {
    ...
    // clean inappropriate pg_upmap/pg_upmap_items (if any)
    {
      ...
      if (int(pgs_to_check.size()) < g_conf->mon_clean_pg_upmaps_per_chunk * 2) {
  ...
  map<string, string> temp_metadata;
  // metadata, too!
  for (map<int, bufferlist>::iterator p = pending_metadata.begin();
    ...
    temp_metadata.clear();
    try {
      bufferlist::iterator m_p = p->second.begin();
      decode(temp_metadata, m_p);
    } catch (buffer::error& e) {
      dout(0) << __func__ << " ERROR: osd." << p->first << " metadata is corrupt" << dendl;
      continue;
    }
    t->put(OSD_METADATA_PREFIX, stringify(p->first), p->second);

    // check slow disk
    if (osd_metadata_have_slow_disk(temp_metadata)) {
      osd_slow_disk_metadatas[p->first] = std::move(temp_metadata);
    } else {
      osd_slow_disk_metadatas.erase(p->first);
    }
  }
  for (set<int>::iterator p = pending_metadata_rm.begin();
    ...
    osd_slow_disk_metadatas.erase(*p);
  }
  pending_metadata.clear();
  pending_metadata_rm.clear();
  ...
  if (!osd_slow_disk_metadatas.empty()) {
    osd_slow_disk_health_check(osd_slow_disk_metadatas, next);
  }
...
bool OSDMonitor::preprocess_query(MonOpRequestRef op)
{
  ...
  case MSG_OSD_LICENSE:
    return preprocess_license(op);

  case MSG_OSD_SLOW_DISK:
    return preprocess_osd_slow_disk(op);
...
bool OSDMonitor::prepare_update(MonOpRequestRef op)
{
  ...
  case MSG_OSD_LICENSE:
    return prepare_license(op);

  case MSG_OSD_SLOW_DISK:
    return prepare_osd_slow_disk(op);
...
void OSDMonitor::reset_osd_pending_state(const unsigned mask, unsigned new_state, const int from)
{
  const unsigned want_state = new_state & mask;
...
bool OSDMonitor::prepare_full(MonOpRequestRef op)
{
  ...
  reset_osd_pending_state(mask, m->state, from);
...
{
  dout(10) << "build_incremental [" << from << ".." << to << "] with features " << std::hex
...
bool OSDMonitor::preprocess_license(MonOpRequestRef op)
{
  op->mark_osdmon_event(__func__);
  MOSDLicense *m = static_cast<MOSDLicense*>(op->get_req());
  int from = m->get_orig_source().num();
  set<string> state;
  ...
  if (!session)
    goto ignore;
  if (!session->is_capable("osd", MON_CAP_X)) {
    dout(0) << "MOSDLicense from entity with insufficient privileges:"
	    << session->caps << dendl;
    goto ignore;
  }

  if (!osdmap.exists(from)) {
    dout(7) << __func__ << " ignoring license message from nonexistent "
	    << m->get_orig_source_inst() << dendl;
    goto ignore;
  }
  ...
  if ((!osdmap.is_up(from) &&
      ...
      (osdmap.is_up(from) &&
       osdmap.get_inst(from) != m->get_orig_source_inst())) {
    dout(7) << __func__ << " ignoring license message from down "
	    << m->get_orig_source_inst() << dendl;
    goto ignore;
  }
...
bool OSDMonitor::prepare_license(MonOpRequestRef op)
{
  op->mark_osdmon_event(__func__);
  const MOSDLicense *m = static_cast<MOSDLicense*>(op->get_req());
  const int from = m->get_orig_source().num();
...
bool OSDMonitor::preprocess_osd_slow_disk(MonOpRequestRef op)
{
  op->mark_osdmon_event(__func__);
  MOSDSlowDisk *m = static_cast<MOSDSlowDisk*>(op->get_req());
  ...
  if (!osdmap.exists(from) || osdmap.get_addr(from) != m->get_orig_source_inst().addr ||
      osdmap.is_down(from)) {
  ...
  dout(10) << __func__ << " recv osd." << from << " slow disk report " << *m << dendl;
  return false;
}

// the calculation is not exact, but false positives are not allowed
// if mon drop OSD, the cluster may not have enough space to recover data
bool OSDMonitor::have_enough_space_to_drop_osd(const PGMapDigest& digest, int from) {
  // if this osd is out, rados do not need to recover data when MON drop this osd
  if (osdmap.is_out(from)) {
    ldout(cct, 0) << __func__ << " osd." << from << " is out " << dendl;
    return true;
  }

  set<int> raw_rules;
  int r = osdmap.crush->get_rules_by_osd(from, &raw_rules);
  if (r < 0) {
    lderr(cct) << __func__ << " failed to get rules of osd." << from << ", " << cpp_strerror(r)
               << dendl;
    return false;
  }
  ldout(cct, 5) << __func__ << " osd." << from << " in cursh rules " << raw_rules << dendl;

  // Check whether there is enough space to recover data under each crush rule which contains this osd
  for (int ruleno: raw_rules) {
    // skip crush rule without pool
    if (!osdmap.does_rule_is_used_by_pool(ruleno)) {
      ldout(cct, 5) << __func__ << " cursh rule " << ruleno << " don't have pool" << dendl;
      continue;
    }

    // get weight of osd
    // if the weight of this osd is 0, the cluster is abnormal and this osd node should not be dropped
    int ow = osdmap.get_osd_adjusted_crush_weight(ruleno, from);
    if (ow <= 0) { // should not be happened
      lderr(cct) << __func__ << " ERROR: failed to get the adjusted crush weight of osd." << from
                 << " ret=" << ow << dendl;
      return false;
    }

    // get the weight of the parent of osd
    int parent;
    auto type = osdmap.crush->get_rule_failure_domain(ruleno);
    if (type > 0) { // it's bucket
      parent = osdmap.crush->get_parent_of_type(from, type, ruleno);
      if (parent == 0) {
        lderr(cct) << __func__ << " ERROR: unable to get parent of raw osd."
                   << from << " of rule " << ruleno << dendl;
        return false;
      }
    } else { // it's osd
      int r = osdmap.crush->find_root_of_rule(ruleno, parent);
      if (r < 0) {
        lderr(cct) << __func__ << " ERROR: failed to get the root of raw osd."
                   << from << " in rule " << ruleno << dendl;
        return false;
      }
    }

    // get weight of parent
    int pw = osdmap.calc_osd_parent_adjusted_weight(parent);
    if (pw <= 0) {
      lderr(cct) << __func__ << " ERROR: failed to get the weight of parent " << parent
                 << " or all osds are out, ret=" << pw << dendl;
      return false;
    }

    ldout(cct, 5) << __func__ << " in rule " << ruleno << ", the weight of osd." << from << " is "
                  << ow << " the type of parent is " << type << " weight is " << pw << dendl;
    if (pw == ow) {
      ldout(cct, 5) << __func__ << " the weight of parent is " << pw
                    << " which equal to the weight of osd." << from << dendl;
      return false;
    }

    int64_t avail = digest.get_rule_avail(ruleno);
    if (avail <= 0) {
        lderr(cct) << __func__ << " failed to get the avail of rule " << ruleno
                   << " or avail is 0, avail=" << avail << dendl;
        return false;
    }
    int64_t used = digest.get_rule_used_space(osdmap, ruleno);
    if (used < 0) {
      lderr(cct) << __func__ << " failed to get the used  of rule " << ruleno
                 << " used=" << avail << dendl;
      return false;
    } else if (used == 0) {
      ldout(cct, 5) << __func__ << " used of rule " << ruleno << " is 0 " << dendl;
      continue;
    }
    ldout(cct, 5) << __func__ << " ruleno " << ruleno << " available space is " << avail
                  << " used space is " << used << dendl;

    // crsuh weight can be approximated as the space osd
    // used_ratio =  used / (used + avail)
    // so, need:
    // ow * used_ratio <= (pw - ow) * (1 - used_ratio) * osd_slow_disk_drop_osd_max_pool_used_percent
    // The formula can be derived as follow:
    // ow * used <= (pw - ow) * avail * osd_slow_disk_drop_osd_max_pool_used_percent
    // used/avail <= (pw - ow) * osd_slow_disk_drop_osd_max_pool_used_percent / ow
    // used/avail <= (pw/ow - 1) * osd_slow_disk_drop_osd_max_pool_used_percent
    if ((double)used / (double) avail >
        (((double) pw / (double) ow - 1.0) * g_conf->osd_slow_disk_drop_osd_max_pool_used_percent)) {
      ldout(cct, 5) << __func__ << " crush rule " << ruleno
                    << " don't have enough space to recover osd." << from << dendl;
      return false;
    }
  }
...
bool OSDMonitor::can_drop_osd_slow_disk(int from) {
  // enable drop slow disk
  if (!g_conf->mon_slow_disk_check_enable_drop_disk) {
    dout(5) << __func__ << " mon_slow_disk_check_enable_drop_disk is false, can not mark osd."
            << from << " down " << dendl;
    return false;
  }
  ...
  if (!can_mark_down(from)) {
    dout(5) << __func__ << " can not mark osd." << from << " down " << dendl;
    return false;
  }

  // up == in
  if (osdmap.get_num_up_osds() != osdmap.get_num_in_osds()) {
    dout(5) << __func__ << " up " << osdmap.get_num_up_osds() << " != in "
            << osdmap.get_num_in_osds() << ", can not mark osd." << from << " down "<< dendl;
    return false;
  }

  // if the OSDMap has just been updated, mon can not drop osd, because the PGMap may not
  // be updated in time
  if (ceph_clock_now() - osdmap.get_modified() < g_conf->osd_mon_report_interval * 10 + 10) {
    dout(5) << __func__ << " osdmap is updated recently, at " << osdmap.get_modified()
            << ", can not mark osd." << from << " down " << dendl;
    return false;
  }

  // no temp pg
  if (osdmap.get_num_pg_temp() != 0) {
    dout(5) << __func__ << " have temp pg, can not mark osd." << from << " down " << dendl;
    return false;
  }

  // all pg is active+clean
  MgrStatMonitor *mgr_status = mon->mgrstatmon();
  if (mgr_status == nullptr ||
      mgr_status->get_active_clean_pgs_num() != mgr_status->get_pg_num()) {
    dout(5) << __func__ << " not all pg are active+clean, can not mark osd." << from
            << " down " << dendl;
    return false;
  }

  // whether any osd is being mark down
  for (auto i = pending_inc.new_state.begin();  i != pending_inc.new_state.end(); ++i) {
    if (i->second & CEPH_OSD_UP) {
      dout(5) << __func__ << " osd." << i->first << " may be marked as down" << dendl;
      return false;
    }
  }

  // have available space?
  const PGMapDigest& digest = mgr_status->get_digest();
  set<int64_t> pools;
  double  used_percent;
  osdmap.get_pool_ids_by_osd(cct, from, &pools);
  for (auto poolid : pools) {
    // check the min_size of pool
    const pg_pool_t* pool = osdmap.get_pg_pool(poolid);
    if (pool == nullptr) {
      dout(0) << __func__ << " ERROR: failed to get the info of pool " << poolid << dendl;
      return false;
    } else if (pool->get_size() <= pool->get_min_size()) {
      dout(5) << __func__ << " pool " << poolid << " size is " << pool->get_size()
              <<  " min_size is " << pool->get_min_size()
              << ", can not mark osd." << from << " down " << dendl;
      return false;
    }
    // check the used percent of pool
    int r = digest.get_pool_used_percent(osdmap, poolid, used_percent);
    if (r < 0) {
      dout(0) << __func__ << " ERROR: failed to get used percent of pool " << poolid
              << " ret=" << r << dendl;
      return false;
    }
    if (used_percent >= g_conf->osd_slow_disk_drop_osd_max_pool_used_percent) {
      dout(5) << __func__ << " pool " << poolid << " percent_used is "
              <<  used_percent << " exceeds " << g_conf->osd_slow_disk_drop_osd_max_pool_used_percent
              << ", can not mark osd." << from << " down " << dendl;
      return false;
    } else {
      dout(5) << __func__ << " osd." << from << " pool " << poolid << " percent_used is "
              <<  used_percent << dendl;
    }
  }
  // have enough space to recover data after drop this osd?
  if (!have_enough_space_to_drop_osd(digest, from)) {
    dout(5) << __func__ << " do not have enough space to recover data after drop osd. " << from
            << ", can not mark osd." << from << " down " << dendl;
    return false;
  }

  dout(0) << __func__ << " can mark osd." << from << " down " << dendl;
  return true;
}

bool OSDMonitor::prepare_osd_slow_disk(MonOpRequestRef op)
{
  op->mark_osdmon_event(__func__);
  MOSDSlowDisk *m = static_cast<MOSDSlowDisk*>(op->get_req());
  int from = m->get_orig_source().num();

  dout(10) << __func__ << " osd." << from << " from " << m->get_orig_source_inst()
           << " slow disk report " << *m << dendl;

  auto iter = osd_slow_disk_metadatas.find(from);
  if (pending_metadata_rm.find(from) != pending_metadata_rm.end()) {
    dout(5) << __func__ << " osd." << from << " is in pending remove metadata queue" <<dendl;
    goto noupdate;
  }
  if (iter == osd_slow_disk_metadatas.end() &&
      !osd_metadata_have_slow_disk(m->metadata) &&
      !(osdmap.get_state(from) & CEPH_OSD_SLOW_DISK_DROP)) {
    // no slow disk in metadata
    dout(20) << __func__ << " no slow disk in the metadata of osd." << from << dendl;
    goto noupdate;
  } else if (osd_metadata_have_slow_disk_need_drop(m->metadata) && can_drop_osd_slow_disk(from)) {
    // may need to drop the slow disk
    assert(osdmap.is_up(from));
    dout(0) << __func__ << " WARN: osd." << from << " has slow disk, mark it down" << dendl;
    pending_inc.new_state[from] = CEPH_OSD_UP;
    // set osd drop state
    reset_osd_pending_state(CEPH_OSD_SLOW_DISK_DROP, CEPH_OSD_SLOW_DISK_DROP, from);
  } else if (iter != osd_slow_disk_metadatas.end() && iter->second == m->metadata) {
    // disk state do not change
    dout(20) << __func__ << " osd." << from << " slow disk don't change" << dendl;
    goto noupdate;
  } else {
    // update osd metadata
    bufferlist osd_metadata;
    ...
    pending_metadata[from] = osd_metadata;
    // clear drop state
    reset_osd_pending_state(CEPH_OSD_SLOW_DISK_DROP, 0, from);
    dout(5) << __func__ << " update osd." << from << " metadata" << dendl;
  }
  wait_for_finished_proposal(op, new C_ReplyMap(this, op, m->version));
  return true;
...
// osd slow disk status
void OSDMonitor::osd_slow_disk_health_check(const map<int, map<string, string>> &metadatas,
                                            health_check_map_t &checks) {
  map<int, string> warn, warn_long, warn_drop;
  string out;
  auto m_iter = osd_slow_disk_metadatas.begin();
  while (m_iter != osd_slow_disk_metadatas.end()) {
    // when the osd recovers, it reports its disk status again
    if (!osdmap.is_up(m_iter->first)) {
      dout(10) << __func__ << " osd." << m_iter->first << " is not up" << dendl;
      m_iter = osd_slow_disk_metadatas.erase(m_iter);
      continue;
    }
    // slow disk
    out = osd_get_slow_disks_from_metadata(m_iter->second, OSD_SLOW_DISK_WARN_TYPE_WARN);
    if (!out.empty()) {
      dout(5) << __func__ << " osd." << m_iter->first << " warn slow disk is " << out << dendl;
      warn.emplace(m_iter->first, std::move(out));
    }

    // slow disk long
    out = osd_get_slow_disks_from_metadata(m_iter->second, OSD_SLOW_DISK_WARN_TYPE_WARN_LONG);
    if (!out.empty()) {
      dout(5) << __func__ << " osd." << m_iter->first << " warn slow disk long is " << out << dendl;
      warn_long.emplace(m_iter->first, std::move(out));
    }

    // need to be dropped
    out = osd_get_slow_disks_from_metadata(m_iter->second, OSD_SLOW_DISK_WARN_TYPE_WARN_DROP);
    if (!out.empty()) {
      dout(5) << __func__ << " osd." << m_iter->first << " warn slow disk drop is " << out << dendl;
      warn_drop.emplace(m_iter->first, std::move(out));
    }
    ++m_iter;
  }
  ...
  if (warn.size()) {
    ostringstream ss;
    ss << warn.size() << " osd(s) have slow disk";
    auto& d = checks.add("OSD_SLOW_DISK", HEALTH_WARN, ss.str());
    for (auto iter = warn.begin(); iter != warn.end(); ++iter) {
      ostringstream ss;
      ss << "osd." << iter->first << " has slow disk, slow disk is [" << iter->second << "]";
  ...
  if (warn_long.size()) {
    ostringstream ss;
    ss << warn_long.size() << " osd(s) are stuck in slow disk for too long";
    auto& d = checks.add("OSD_SLOW_DISK_LONG", HEALTH_WARN, ss.str());
    for (auto iter = warn_long.begin(); iter != warn_long.end(); ++iter) {
      ostringstream ss;
      ss << "osd." << iter->first << " is stuck in slow state for more than "
                   << cct->_conf->osd_slow_disk_check_warn_long_interval
                   << " seconds, slow disk is [" << iter->second << "]";
      d.detail.push_back(ss.str());
    }
  }
  ...
  if (warn_drop.size()) {
    ostringstream ss;
    ss << warn_drop.size() << " osd(s) have slow disk, they need to be dropped";
    auto& d = checks.add("OSD_SLOW_DISK_DROP", HEALTH_ERR, ss.str());
    for (auto iter = warn_drop.begin(); iter != warn_drop.end(); ++iter) {
      ostringstream ss;
      ss << "osd." << iter->first << " need to be dropped, slow disk is [" << iter->second << "]";
      d.detail.push_back(ss.str());
    }
  }
}
                                                 ...
                                                 stringstream& ss)
{
  ...
  if (boost::algorithm::ends_with(prefix, "enable")) {
    ...
    bool force = false;
    if (!cmd_getval(cct, cmdmap, "yes_i_really_mean_it", force))
      force = compatible_platform_command(cmdmap, "force", "--yes-i-really-mean-it");
...
int OSDMonitor::prepare_command_osd_remove(int32_t id)
{
  ...
  osd_slow_disk_metadatas.erase(id);
				      ...
				      const cmdmap_t& cmdmap)
{
  ...
  if (prefix == "osd setcrushmap" ||
    ...
    bool sure = false;
    if (!cmd_getval(cct, cmdmap, "yes_i_really_mean_it", sure))
      sure = compatible_platform_command(cmdmap, "sure", "--yes-i-really-mean-it");
    ...
    bool force = false;
    if (!cmd_getval(cct, cmdmap, "yes_i_really_really_mean_it", force))
      force = compatible_platform_command(cmdmap, "sure", "--yes-i-really-really-mean-it");

    bool force_no_fake = false;
    cmd_getval(cct, cmdmap, "yes_i_really_really_mean_it_not_faking", force_no_fake);
    if (poolstr2 != poolstr ||
	(!force && !force_no_fake)) {
