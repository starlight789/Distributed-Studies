...
void rgw_bucket_dir_entry_meta::dump(Formatter *f) const
{
  ...
  if (has_tags) {
    encode_json("tags", tags_bl.to_str(), f);
  }
}
...
{
  JSONDecoder::decode_json("total_size", total_size, obj);
  JSONDecoder::decode_json("total_size_rounded", total_size_rounded, obj);
  JSONDecoder::decode_json("num_entries", num_entries , obj);
  JSONDecoder::decode_json("actual_size", actual_size , obj);
}
...
{
  encode_json("ver", ver, f);
  encode_json("master_ver", master_ver, f);
  encode_json("stats", stats, f);
  encode_json("new_instance", new_instance, f);
  encode_json("tag_timeout", tag_timeout, f);
  encode_json("max_marker", max_marker, f);
  encode_json("instance_entry", new_instance, f);
  encode_json("syncstopped", syncstopped, f);
  encode_json("stats_by_storageclass", storageclass_stats, f);
}

void rgw_bucket_dir_header::decode_json(JSONObj *obj) {
  JSONDecoder::decode_json("ver", ver, obj);
  JSONDecoder::decode_json("master_ver", master_ver, obj);
  JSONDecoder::decode_json("stats", stats, obj);
  JSONDecoder::decode_json("new_instance", new_instance, obj);
  JSONDecoder::decode_json("tag_timeout", tag_timeout, obj);
  JSONDecoder::decode_json("max_marker", max_marker, obj);
  JSONDecoder::decode_json("instance_entry", new_instance, obj);
  JSONDecoder::decode_json("syncstopped", syncstopped, obj);
  JSONDecoder::decode_json("stats_by_storageclass", storageclass_stats, obj);
}
...
{
  string status_str;
  JSONDecoder::decode_json("reshard_status", status_str, obj);
  if (status_str == "none") {
    ...
    reshard_status = CLS_RGW_RESHARD_DONE;
  } else {
    reshard_status = CLS_RGW_RESHARD_NONE;
  }
