...
// namespace
void cls_rgw_namespace_prepare_op(librados::ObjectWriteOperation& o, RGWModifyOp op, string& tag,
                                 const string& obj_name, const string& parent_oid,
                                 const string& child_name, bool exclude);

void cls_rgw_namespace_dir_prepare_op(librados::ObjectWriteOperation& o, RGWModifyOp op,
                                      const string& optag, const string& parent_name,
                                      const string& child_name,ceph::real_time& ut, bool exclude);

void cls_rgw_namespace_dir_complete_op(librados::ObjectWriteOperation& o, RGWModifyOp op,
                                       int64_t poolid, uint64_t epoch, const string& optag,
                                       const string& child_name, ceph::real_time ut,
                                       bool force_delete);

void cls_rgw_namespace_dir_make(librados::ObjectWriteOperation& o, bool need_complete);

int cls_rgw_namespace_dir_del(librados::IoCtx& io_ctx, const string& dir_oid);

void cls_rgw_namespace_complete_op(librados::ObjectWriteOperation& o, RGWModifyOp op,
                                  const string& file_name, int64_t poolid, uint64_t epoch,
                                  const string& tag, uint64_t size, ceph::real_time* ut,
                                  bool force_delete, bool appendable);
...
template <class T>
inline void cls_rgw_namespace_encode_suggestion(char op, const string& child_name, T& ent, bufferlist& updates) {
  updates.append(op);
  encode(child_name, updates);
  encode(ent, updates);
}

void cls_rgw_namespace_suggest_changes(librados::ObjectWriteOperation& o, bufferlist& updates);
 ...
 * skip_ns       - if true, skip object with namespace when list objects
 *
...
int cls_rgw_usage_log_read_total(librados::IoCtx& io_ctx, string& oid, string& user,
                                 ...
                                 bool *is_truncated);
int cls_rgw_usage_log_read_current_readop(librados::IoCtx& io_ctx, string& oid, string& user,
                             uint64_t end_epoch, map<rgw_user_bucket, rgw_usage_log_entry>& usage);
int cls_rgw_usage_log_read_readop(librados::IoCtx& io_ctx, string& oid, string& user,
                            uint64_t start_epoch, uint64_t end_epoch, uint32_t max_entries,
...
int cls_rgw_usage_log_trim_readop(librados::IoCtx& io_ctx, const string& oid, string& user,
                           uint64_t start_epoch, uint64_t end_epoch);
int cls_rgw_usage_log_trim_total(librados::IoCtx& io_ctx, const string& oid, string& user, string& bucket);
...
int cls_rgw_lc_get_entry(librados::IoCtx& io_ctx, const string& oid,
                         std::string& marker, cls_rgw_lc_entry& entry);
int cls_rgw_lc_rm_entries(librados::IoCtx& io_ctx, string& oid, vector<cls_rgw_lc_entry>& entries);
int cls_rgw_lc_set_entries(librados::IoCtx& io_ctx, string& oid, vector<cls_rgw_lc_entry>& entries);
