...
int OSDMap::calc_osd_parent_adjusted_weight(int parent_id) const {
  map<int, int> osd_crush_weights;
  int sum = crush->get_osds_weight_of_root(parent_id, osd_crush_weights);
  if (sum <= 0) {
    return sum;
  }
  sum = 0;
  for (auto &iter : osd_crush_weights) {
    sum += (int) ((float(iter.second)) * get_weightf(iter.first));
  }
  return sum;
}
...
{
  ldout(cct, 20) << __func__ << " overfull " << overfull
                 << " underfull " << underfull
  ...
  const pg_pool_t *pool = get_pg_pool(pg.pool());
  if (!pool) {
    ldout(cct, 20) << __func__ << " don't find pool " << pg.pool() << dendl;
    return false;
  }
  int rule = pool->get_crush_rule();
  if (rule < 0) {
    ldout(cct, 20) << __func__ << " don't find fule " << rule << dendl;
    return false;
  }
  ...
  for (auto osd : *orig) {
    if (overfull.count(osd)) {
      ldout(cct, 20) << __func__ << " osd  " << osd << " in overfull " << dendl;
      any = true;
      break;
    } else {
      ldout(cct, 20) << __func__ << " osd  " << osd << " not in overfull " << dendl;
    }
  }
  if (!any) {
    ldout(cct, 20) << __func__ << " the osd of pg " << pg << " not in overfull" << dendl;
    return false;
  }
  ...
  int r = crush->try_remap_rule(
    ...
    *orig,
    out);
  if (r < 0) {
    ldout(cct, 20) << __func__ << " ret=" << r << dendl;
    return false;
  }
  if (*out == *orig) {
    ldout(cct, 20) << __func__ << " out==orig" << dendl;
    return false;
  }
  ldout(cct, 20) << __func__ << " find new upmap, orig is " << *orig << " new " << *out << dendl;
  return true;
}
  ...
  std::random_device::result_type *p_seed)
{
  ...
  if (num_up_osd != num_in_osd) {
    lderr(cct) << __func__ << " early exit, osd_up(" << num_up_osd << ") != osd_in("
               << num_in_osd << ")" << dendl;
    return 0;
  }
  ...
  auto aggressive = cct->_conf->osd_calc_pg_upmaps_aggressively;
  auto fast_aggressive = aggressive && cct->_conf->osd_calc_pg_upmaps_aggressively_fast;
  auto local_fallback_retries = cct->_conf->osd_calc_pg_upmaps_local_fallback_retries;
...
void OSDMap::check_health(health_check_map_t *checks) const
{
  ...
  // license status
  {
    set<int> offline, offline_error, expire_warn, expire_error;
    for (int i = 0; i < max_osd; ++i) {
      if (exists(i) && is_up(i) && is_in(i)) {
       if (osd_state[i] & CEPH_OSD_LICENSE_OFFLINE_LONG)
          offline_error.emplace(i);
       else if (osd_state[i] & CEPH_OSD_LICENSE_OFFLINE)
          offline.emplace(i);
       if (osd_state[i] & CEPH_OSD_LICENSE_SOON_EXPIRE)
          expire_error.emplace(i);
       else if (osd_state[i] & CEPH_OSD_LICENSE_NEAR_EXPIRE)
          expire_warn.emplace(i);
      }
    }
    if (offline.size()) {
      ostringstream ss;
      ss << offline.size() << " osd(s) license offline";
      auto& d = checks->add("OSD_LICENSE_OFFLINE", HEALTH_WARN, ss.str());
      for (auto& i: offline) {
        ostringstream ss;
        ss << "osd." << i << " is license offline";
        d.detail.push_back(ss.str());
      }
    }
    if (offline_error.size()) {
      ostringstream ss;
      ss << offline_error.size() << " osd(s) license offline too long";
      auto& d = checks->add("OSD_LICENSE_OFFLINE_LONG", HEALTH_ERR, ss.str());
      for (auto& i: offline_error) {
        ostringstream ss;
        ss << "osd." << i << " license offline too long";
        d.detail.push_back(ss.str());
      }
    }
    if (expire_warn.size()) {
      ostringstream ss;
      ss << expire_warn.size() << " osd(s) license will expire";
      auto& d = checks->add("OSD_LICENSE_EXPIRE_WARN", HEALTH_WARN, ss.str());
      for (auto& i: expire_warn) {
	ostringstream ss;
	ss << "osd." << i << " license will expire";
	d.detail.push_back(ss.str());
      }
    }
    if (expire_error.size()) {
      ostringstream ss;
      ss << expire_error.size() << " osd(s) license will expire soon";
      auto& d = checks->add("OSD_LICENSE_EXPIRE_ERROR", HEALTH_ERR, ss.str());
      for (auto& i: expire_error) {
	ostringstream ss;
	ss << "osd." << i << " license will expire soon";
	d.detail.push_back(ss.str());
      }
    }
  }
