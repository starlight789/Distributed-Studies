...
public:
  ...
  struct MergedOp {
    ...
    OpRequestRef op;
    MergedOp(OpRequestRef _op): op(_op) {}
  };
  ...
  struct OpContext {
    ...
    OpRequestRef op;
    CLSMergedProcess *merged_cache = nullptr;
    list<MergedOp> merged_ops;
    ...
    ~OpContext() {
      ...
      for (list<pair<boost::tuple<uint64_t, uint64_t, unsigned>,
           pair<bufferlist*, Context*> > >::iterator i = pending_async_reads.begin();
    ...
    void save_merged_ops(OpRequestRef _op);
...
protected:
  ...
  // try merge rgw index prepare and index complete
  void merge_op_requests(const hobject_t& hoid, list<OpRequestRef>& ops) {
    OpRequestRef* last_can_merged_op = nullptr;
    auto iter = ops.begin();
    int last_merged_num = 0;
    int last_merged_type = 0;

    while (iter != ops.end()) {
        OpRequestRef& op = *iter;
        int merge_type = op->get_merge_op_type();
        const MOSDOp *m = static_cast<const MOSDOp*>(op->get_req());
        if (merge_type == 0) { // can't merge
            last_can_merged_op = nullptr;
            lgeneric_subdout(cct, osd, 10) << __func__ << " " << hoid
                                           << " op " << op->get_reqid()
                                           << " m " << *m
                                           << " can not merge" << dendl;
            ++iter;
            ...
            last_merged_num = 0;
            continue;
        } else if (last_can_merged_op == nullptr ||
                   merge_type != last_merged_type ||
                   last_merged_num > cct->_conf->osd_merge_batch_max_size) {
            last_can_merged_op = &op;
            ++iter;
            last_merged_type = merge_type;
            last_merged_num = op->merged_ops_num();
            lgeneric_subdout(cct, osd, 10) << __func__ << " " << hoid
                                           << " can merge op " << op->get_reqid()
                                           << " m " << *m
                                           << " type " << merge_type << dendl;
            continue;
        }
        lgeneric_subdout(cct, osd, 10) << __func__ << " " << hoid
                                       << " start merge op " << op->get_reqid()
                                       << " m " << *m
                                       << " to " << (*last_can_merged_op)->get_reqid()
                                       << " type " << merge_type << dendl;

        (*last_can_merged_op)->merge_op(op);
        last_merged_num = op->merged_ops_num();
        iter = ops.erase(iter);
    }
  }
    ...
    if (!to_req.empty()) {
      ...
      for (auto &&p: to_req) {
        if (write_blocked_by_scrub(p.first->obs.oi.soid.get_head())) {
          ...
          if (cct->_conf->try_osd_merge_ops && !p.second.empty()) {
            lgeneric_subdout(cct, osd, 10) << __func__ << " " << p.first->obs.oi.soid
                                           <<  " before merge " << p.second.size()
                                           << " ops"  << dendl;
            merge_op_requests(p.first->obs.oi.soid, p.second);
            lgeneric_subdout(cct, osd, 10) << __func__ << " " << p.first->obs.oi.soid
                                           <<  " after merge " << p.second.size()
                                           << " ops"  << dendl;
          }
...
protected:
  ...
  void log_op_stats(OpRequestRef& op, utime_t &now, uint64_t inb, uint64_t outb);

  // merged request

  void execute_ctx_merged(OpContext *ctx);

  int prepare_transaction_merged(OpContext *ctx,
                                 ObjectContextRef &obc,
                                 MergedOp &m_op,
                                 MOSDOp *m,
                                 bool &write_success,
                                 bool &read_success);
  void finish_success_write(OpContext *ctx);
  void response_merged_requests(OpContext *ctx);
  void response_read_or_failed_merged_requests(OpContext *ctx, bool have_read_succ);
  void recode_write_error_merged_requests(OpContext *ctx, bool read_succ);
  void reply_completed_dup_ops(OpRequestRef op, int r, eversion_t v, version_t uv);
...
public:
  ...
  int do_osd_ops(OpContext *ctx, vector<OSDOp>& ops, bool reset_subop_num = true);
