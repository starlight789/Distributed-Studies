...
{
   return *_dout << " OpRequest "  << " ";
}

OpRequest::OpRequest(Message *req, OpTracker *tracker) :
  TrackedOp(tracker, req->get_recv_stamp()),
  rmw_flags(0), original_flags(0), request(req),
  ...
  hitset_inserted(false)
{
  ...
  dout(25) << reqid << " [" << initiated_at << "] new op ref=" << nref << dendl;
}

void OpRequest::get() {
  TrackedOp::get();
  dout(25) << reqid << " [" << initiated_at << "] after get ref=" << nref << dendl;
}

void OpRequest::put() {
  dout(25) << reqid << " [" << initiated_at << "] before put ref=" << nref << dendl;
  if (nref == 1) {
    merged_reqs.clear();
  }
  TrackedOp::put();
...
// for merge
int OpRequest::get_merge_op_type() {
  return rmw_flags & (CEPH_OSD_RMW_FLAG_MERGE_RGW_TYPE);
}
bool OpRequest::origin_may_write() {
  if (!is_merged_op) {
    return may_write();
  }
  return original_flags & (CEPH_OSD_RMW_FLAG_CLASS_WRITE | CEPH_OSD_RMW_FLAG_WRITE);
}
bool OpRequest::origin_may_cache() {
  if (!is_merged_op) {
    return may_cache();
  }
  return original_flags & CEPH_OSD_RMW_FLAG_CACHE;
}
bool OpRequest::origin_may_read() {
  if (!is_merged_op) {
    return may_read();
  }
  return original_flags & (CEPH_OSD_RMW_FLAG_CLASS_READ | CEPH_OSD_RMW_FLAG_READ);
}
...
void OpRequest::set_merge_op_type(int flag) { set_rmw_flags(flag); }
...
// not include this op
// 1. merge flags
// 2. save merged ops to merged_reqs
void OpRequest::merge_op(OpRequest::Ref& op) {
  ostringstream ss;
  ss << "merged_to " << reqid;
  op->mark_op_merged(ss.str());

  // backup original flags
  if (!original_flags) {
    original_flags = rmw_flags;
  }

  // merge all flags 
  rmw_flags |= op->rmw_flags;

  merged_reqs.push_back(op);
  if (op->is_merged()) {
    // recovery original flags
    op->rmw_flags = op->original_flags;
    op->is_merged_op = false;
    merged_reqs.splice(merged_reqs.end(), op->merged_reqs);
  }
  is_merged_op = true;
}
