...
void PG::queue_recovery(bool is_backfill)
{
  if (!is_primary() || !is_peered()) {
    ...
    osd->queue_for_recovery(this, is_backfill);
  }
}
...
bool PG::sched_scrub()
{
  ...
  if ((cct->_conf->osd_scrub_auto_repair
      ...
      || (cct->_conf->osd_scrub_unittest
          && cct->_conf->osd_scrub_auto_repair
          && get_pgbackend()->auto_repair_supported())) {
    if (time_for_deep || (cct->_conf->osd_scrub_unittest && scrubber.must_deep_scrub)) {
...
void PG::scrub_compare_maps() 
{
  ...
  for (const auto& i : acting_recovery_backfill) {
    if (i == pg_whoami) continue;
    dout(10) << __func__ << " replica " << i << " has "
  ...
  if (acting.size() > 1) {
    ...
    dout(10) << __func__ << "   osd." << acting[0] << " has "
...
std::pair<ghobject_t, bool> PG::_delete_some(ObjectStore::Transaction *t, const ghobject_t& _next)
{
  dout(10) << __func__ << " next " << _next << dendl;
