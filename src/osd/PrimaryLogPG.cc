...
void PrimaryLogPG::OpContext::save_merged_ops(OpRequestRef _op) {
  if (_op && _op->is_merged()) {
    merged_ops.emplace_back(_op);
    for (auto m_op = _op->merged_reqs.begin(); m_op != _op->merged_reqs.end(); ++m_op) {
      merged_ops.emplace_back(*m_op);
    }
    merged_cache = make_merge_cache_processor(_op->get_merge_op_type(), pg->get_cct());
    assert(merged_cache);
  }
}
...
void PrimaryLogPG::do_op(OpRequestRef& op)
{
  ...
  // inject time out
  if (cct->_conf->osd_debug_inject_op_timeout) {
    uint32_t num = ceph::util::generate_random_number(0, 100);
    if (num < cct->_conf->osd_debug_inject_op_timeout_percentage) {
       dout(0) << __func__ << " inject timeout, sleep "
               << cct->_conf->osd_debug_inject_op_timeout << dendl;
       sleep(cct->_conf->osd_debug_inject_op_timeout);
    }
  }

  // check op timeout
  uint32_t timeout_seconds = cct->_conf->osd_client_op_timeout;
  // TODO: for now, only allow to set timeout for rgw requests
  if (timeout_seconds && pool.info.only_used_by_rgw) {
    utime_t recv_time = m->get_recv_stamp();
    if (op->is_merged() && !op->merged_reqs.empty()) {
        recv_time = op->merged_reqs.back()->get_req()->get_recv_stamp();
    }
    if (ceph_clock_now() - recv_time > timeout_seconds) {
      dout(0) << __func__ << " ERROR: op " << *m << " timeout, received at "
              << recv_time << dendl;
      osd->reply_op_error(op, -ETIMEDOUT);
      return;
    }
  }
  ...
  if (op->may_write() || op->may_cache()) {
    ...
    if (got) {
      ...
      if (already_complete(version)) {
        reply_completed_dup_ops(op, return_code, version, user_version);
      } else {
        dout(10) << " waiting for " << version << " to commit" << dendl;
        // always queue ondisk waiters, so that we can requeue if needed
        waiting_for_ondisk[version].push_back(make_tuple(op, user_version, return_code));
  ...
  if (is_localread) {
    dout(20) << __func__ << " current op has flag CEPH_OSD_FLAG_BALANCE_READS or CEPH_OSD_FLAG_LOCALIZE_READS,"
             << "and current OSD is not primary." << dendl;
  }
  int r = find_object_context(
    oid, &obc, can_create,
    m->has_flag(CEPH_OSD_FLAG_MAP_SNAP_CLONE),
    &missing_oid, is_localread);
  ...
  // 如果这个pg存在agent_state
  if (agent_state) {
    // 对相应的 PG 设置 flush 模式和 evict 模式，计算一个 PG 的 flush_mode 和 evic_mode 的参数值
  ...
  if (m->has_flag(CEPH_OSD_FLAG_SKIPRWLOCKS)) {
    ...
    dout(20) << __func__ << " " << *m << " waiting for rw locks " << dendl;
...
void PrimaryLogPG::log_op_stats(OpContext *ctx)
{
  ...
  if (ctx->op->is_merged()) {
    osd->logger->inc(l_osd_merged_op_after);
    for (auto m_op = ctx->merged_ops.begin(); m_op != ctx->merged_ops.end(); ++m_op) {
      if (m_op->early_result < 0) {
        continue;
      }
      osd->logger->inc(l_osd_merged_op_before);
      if (m_op->user_modify) {
        // write
        log_op_stats(m_op->op, now, inb, m_op->bytes_read);
        // bytes_written recorded to the first op
        inb = 0; 
      } else {
        // read
        log_op_stats(m_op->op, now, 0, m_op->bytes_read);
      }
    }
  } else {
    log_op_stats(ctx->op, now, inb, outb);
  }
}

void PrimaryLogPG::log_op_stats(OpRequestRef& op, utime_t &now, uint64_t inb, uint64_t outb)
{
  const MOSDOp *m = static_cast<const MOSDOp*>(op->get_req());
  ...
  if (op->origin_may_read() && op->origin_may_write()) {
  ...
  } else if (op->origin_may_write() || op->origin_may_cache()) {
...
void PrimaryLogPG::execute_ctx_merged(OpContext *ctx)
{
  FUNCTRACE(cct);
  dout(20) << __func__ << " start " << ctx << dendl;
  assert(ctx->op->is_merged());
  assert(!ctx->user_at_version);

  ctx->reset_obs(ctx->obc);
  ctx->update_log_only = false; // reset in case finish_copyfrom() is re-running execute_ctx
  ObjectContextRef obc = ctx->obc;
  const hobject_t& soid = obc->obs.oi.soid;
  bool update_log_only = false;
  ...
  // before we finally apply the resulting transaction.
  ctx->op_t.reset(new PGTransaction);
  version_t last_projected_version = projected_last_update.version;

  // prepare all ops
  for (auto m_op_iter = ctx->merged_ops.begin(); m_op_iter != ctx->merged_ops.end(); ++m_op_iter) {
    osd_reqid_t reqid = m_op_iter->op->get_reqid();
    MOSDOp *m = static_cast<MOSDOp*>(m_op_iter->op->get_nonconst_req());
    tracepoint(osd, prepare_tx_enter, reqid.name._type, reqid.name._num, reqid.tid, reqid.inc);

    // set version
    if (m_op_iter->op->origin_may_write() || m_op_iter->op->origin_may_cache()) {
      // cannot merge the ops of snapshot-enabled pools
      assert(m->get_snap_seq() == 0 && m->get_snaps().empty());

      ctx->at_version = eversion_t(get_osdmap()->get_epoch(), ++last_projected_version);
      m_op_iter->at_version = ctx->at_version;
      ctx->mtime = m->get_mtime();
      ctx->merged_cache->cls_cxx_map_snap();
    }
    ctx->user_at_version = m_op_iter->user_at_version = obc->obs.oi.user_version;

    dout(30) << __func__ << " merge op user_at_version " << m_op_iter->user_at_version << dendl;

    // prepare transaction
    int result = prepare_transaction_merged(ctx, obc, *m_op_iter, m, write_success, read_success);
    assert(result != -EINPROGRESS);

    tracepoint(osd, prepare_tx_exit, reqid.name._type, reqid.name._num, reqid.tid, reqid.inc);

    // record error pg log
    if (m_op_iter->update_log_only) {
      ctx->merged_cache->cls_cxx_map_rollback();
      update_log_only = true;
      assert(m_op_iter->op->origin_may_write());
      ctx->log.push_back(pg_log_entry_t(pg_log_entry_t::ERROR, soid, m_op_iter->at_version,
            eversion_t(), 0, reqid, utime_t(), result));
    } else if (m_op_iter->op->origin_may_write() || m_op_iter->op->origin_may_cache()) {
      ctx->merged_cache->cls_cxx_map_apply();
    }

    dout(20) << __func__ << *m << " at_verison " << m_op_iter->user_at_version
             << " ov " << obc->obs.oi.version 
             << " ret=" << result << dendl;
  }

  dout(10) << __func__ << " prepare finish: write_success " << write_success
           << " read_success " << read_success
           << " update_log_only " << update_log_only
           << dendl;
  // only read requests
  if (!write_success && !update_log_only) {
    if (read_success) {
      unstable_stats.add(ctx->delta_stats);
    }
    response_read_or_failed_merged_requests(ctx, read_success);
    return;
  }
  ...
  if ((ctx->delta_stats.num_bytes > 0 ||
    ...
    // drop all requests, include read request
    dout(5) << __func__ << " full, dropping all requests (bad client)" << dendl;
    close_op_ctx(ctx);
    return;
  }
  ...
  // have write
  if (write_success) {
    int result = ctx->merged_cache->cls_cxx_map_flush((cls_method_context_t)&ctx);
    if (result < 0) {
      dout(0) << __func__ << " failed to flush cls merged cache result=" << result << dendl;
      reply_ctx(ctx, result);
      return;
    }
    assert(!ctx->op_t->empty());
    ctx->user_modify = true;
    finish_success_write(ctx);
  }
  ...
  if (!write_success && update_log_only) {
    ...
    recode_write_error_merged_requests(ctx, read_success);
    return;
  }
...
int PrimaryLogPG::prepare_transaction_merged(OpContext *ctx,
                                             ObjectContextRef &obc,
                                             MergedOp &m_op,
                                             MOSDOp *m,
                                             bool &write_success,
...
{
  assert(!m->ops.empty());
  OpRequestRef &op = m_op.op;
  ctx->user_modify = false;

  dout(20) << __func__ << " prepare message " << *m
           << " at_version " << m_op.at_version
           << dendl;

  // inject error for test
  if (unlikely(cct->_conf->osd_debug_merge_inject_write_error > 0 ||
               cct->_conf->osd_debug_merge_inject_read_error > 0)) {
    int rd = generate_random_number(0, 100);
    if (op->origin_may_write()) {
      if (rd <= cct->_conf->osd_debug_merge_inject_write_error) {
        m_op.early_result = -EINVAL;
        dout(0) << __func__ << " do_osd_ops inject_write_error ret=" << m_op.early_result
                << dendl;
      }
    } else if (op->origin_may_read() && rd <= cct->_conf->osd_debug_merge_inject_read_error) {
      m_op.early_result = -EINVAL;
      dout(0) << __func__ << " do_osd_ops inject_read_error ret=" << m_op.early_result
              << dendl;
    }
  }

  if (!m_op.early_result) {
    m_op.early_result = do_osd_ops(ctx, m->ops, false);
    m_op.user_modify = ctx->user_modify;
  }

  if (m_op.early_result < 0) {
    // all ops failed
    if (op->origin_may_write() && get_osdmap()->require_osd_release >= CEPH_RELEASE_KRAKEN) {
      m_op.update_log_only = true;
    }
    dout(1) << __func__ << " do_osd_ops failed ret=" << m_op.early_result
            << " update_log_only " << m_op.update_log_only << dendl;
    return m_op.early_result;
  }
  ...
  if (!m_op.user_modify) {
    if (op->origin_may_write()) {
      if (get_osdmap()->require_osd_release >= CEPH_RELEASE_KRAKEN) {
        m_op.update_log_only = true;
        dout(0) << __func__ << " not user_modify ret=" << m_op.early_result
                << " update_log_only " << m_op.update_log_only << dendl;
      }
    } else {
      read_success = true;
      dout(20) << __func__ << " read_success ret=" << m_op.early_result
               << " update_log_only " << m_op.update_log_only << dendl;
    }
    return m_op.early_result;
  }
  ...
  m_op.user_at_version = std::max(info.last_user_version, ctx->new_obs.oi.user_version) + 1;
  ctx->user_at_version = m_op.user_at_version;
  ...
  if (m_op.at_version.version > m_op.user_at_version) {
    ctx->user_at_version = m_op.user_at_version = m_op.at_version.version;
  }
  ctx->new_obs.oi.user_version = ctx->user_at_version;

  osd_reqid_t reqid = op->get_reqid();
  if (ctx->new_obs.exists) {
    ctx->new_obs.oi.version = m_op.at_version;
    ctx->new_obs.oi.prior_version = ctx->obs->oi.version;
    ctx->new_obs.oi.last_reqid = reqid;
    if (ctx->mtime != utime_t()) {
      ctx->new_obs.oi.mtime = ctx->mtime;
      dout(10) << " set mtime to " << ctx->new_obs.oi.mtime << dendl;
      ctx->new_obs.oi.local_mtime = ceph_clock_now();
    } else {
      dout(10) << " mtime unchanged at " << ctx->new_obs.oi.mtime << dendl;
    }
  }

  int log_op_type = ctx->new_obs.exists ? pg_log_entry_t::MODIFY : pg_log_entry_t::DELETE;

  // append to log
  ctx->log.push_back(pg_log_entry_t(log_op_type, obc->obs.oi.soid, m_op.at_version,
                                    ctx->obs->oi.version,
                                    m_op.user_at_version, reqid,
                                    ctx->mtime, 0));

  write_success = true;

  dout(20) << __func__ << " write_success ret=" << m_op.early_result
           << " update_log_only " << m_op.update_log_only
           << " exists " << ctx->new_obs.exists
           << " at_version " << m_op.at_version
           << " pre version " << ctx->obs->oi.version
           << " user at version " << m_op.user_at_version
           << " reqid " << reqid
           << dendl;
...
void PrimaryLogPG::finish_success_write(OpContext *ctx) {
  ObjectContextRef obc = ctx->obc;
  const hobject_t& soid = obc->obs.oi.soid;

  if ((ctx->new_obs.exists && ctx->new_obs.oi.is_omap()) &&
      (!obc->obs.exists || !obc->obs.oi.is_omap())) {
  ...
  if (ctx->new_obs.exists) {
    ...
    // reset cached oi
    ctx->new_obs.oi = object_info_t(soid);
  }
  // apply new object state.
  obc->obs = ctx->new_obs;

  if (soid.is_head() && !obc->obs.exists) {
    obc->ssc->exists = false;
    obc->ssc->snapset = SnapSet();
  } else {
    obc->ssc->exists = true;
...
void PrimaryLogPG::response_merged_requests(OpContext *ctx) {
  dout(10) << __func__ << " start " << dendl;
  assert(!ctx->data_off);

  for (auto m_op = ctx->merged_ops.begin(); m_op != ctx->merged_ops.end(); ++m_op) {
    int result = m_op->early_result;
    if (result == -EAGAIN || result == -EINPROGRESS) {
      dout(0) << __func__ << " skip " <<  m_op->op->get_reqid() << " ret=" << result << dendl;
      continue;
    }
    MOSDOp *m = static_cast<MOSDOp*>(m_op->op->get_nonconst_req());
    MOSDOpReply *reply = new MOSDOpReply(m, 0, get_osdmap()->get_epoch(), 0, false);

    if (result >= 0 && m_op->op->origin_may_write()) { // success write
      reply->set_reply_versions(m_op->at_version, m_op->user_at_version);
      m_op->op->mark_commit_sent();
      result = 0;
    } else {
      if (m_op->update_log_only) { // write failed
        reply->set_reply_versions(m_op->at_version, m_op->user_at_version);
      } else { // read
        for (auto p = m->ops.begin(); p != m->ops.end() && result >= 0; ++p) {
          if (p->rval < 0 && !(p->op.flags & CEPH_OSD_OP_FLAG_FAILOK)) {
            result = p->rval;
            break;
          }
          ctx->bytes_read += p->outdata.length();
          m_op->bytes_read += p->outdata.length();
        }
        if (result >= 0) {
      ...
      reply->get_header().data_off = 0;
    }
    reply->add_flags(CEPH_OSD_FLAG_ACK | CEPH_OSD_FLAG_ONDISK);
    reply->set_result(result);

    dout(20) << __func__ << " sending reply on " << *m << " reply " << *reply << dendl;
    osd->send_message_osd_client(reply, m->get_connection());
  }
}

void PrimaryLogPG::response_read_or_failed_merged_requests(OpContext *ctx, bool have_read_succ) {
  dout(10) << __func__ << " have_read_succ " << have_read_succ << dendl;
  response_merged_requests(ctx);
  if (have_read_succ && !ctx->ignore_log_op_stats) {
    log_op_stats(ctx);
    publish_stats_to_osd();
  }
  close_op_ctx(ctx);
...
void PrimaryLogPG::recode_write_error_merged_requests(OpContext *ctx, bool read_succ)
{
  dout(10) << __func__ << " start" << dendl;
  assert(!ctx->log.empty());
  ...
  struct OnComplete {
    ...
    OnComplete(
      ...
      : pg(pg),
      ctx(ctx),
      have_read_succ(read_succ) {}
    void operator()() {
      ldpp_dout(pg, 20) << "finished " << __func__ << dendl;
      pg->response_read_or_failed_merged_requests(ctx, have_read_succ);
    }
  };

  mempool::osd_pglog::list<pg_log_entry_t> log_entries(std::move_iterator(std::begin(ctx->log)),
      std::move_iterator(std::end(ctx->log)));
  ObcLockManager lock_manager;
  submit_log_entries(
    log_entries,
    std::move(lock_manager),
    boost::optional<std::function<void(void)> >(OnComplete(this, ctx, read_succ)));
}

void PrimaryLogPG::reply_completed_dup_ops(OpRequestRef op, int r, eversion_t v, version_t uv) {
  osd->send_error_op_reply(op, r, v, uv);
  if(op->is_merged()) {
    eversion_t version;
    version_t user_version;
    int return_code = 0;
    for(auto& m_op : op->merged_reqs) {
      bool got = check_in_progress_op(m_op->get_reqid(), &version, &user_version, &return_code);
      if (!got) {
        // may be read op
        dout(0) << __func__ << " can not find dup " <<  m_op->get_reqid()
                 << " maybe read version " << v
                 << " user version " << uv
                 << dendl;
        assert(m_op->origin_may_read() && !m_op->origin_may_write());
        osd->send_error_op_reply(m_op, -EAGAIN, v, uv);
      } else {
        dout(0) << __func__ << " find dup " <<  m_op->get_reqid()
                 << " version " << version
                 << " user_version " << user_version << dendl;
        osd->send_error_op_reply(m_op, return_code, version, user_version);
      }
    }
  }
}
  ...
  bool ignore_transaction)
{
  ...
  if (!obs.exists) {
    ...
    // Reset mtime, promote lead to update mtime with current time.
    // When create obj, set it to null
    ctx->new_obs.oi.mtime = utime_t();
    --ctx->delta_stats.num_whiteouts;
  }
}
...
int PrimaryLogPG::do_osd_ops(OpContext *ctx, vector<OSDOp>& ops, bool reset_subop_num)
{
  int result = 0;
...
void PrimaryLogPG::eval_repop(RepGather *repop)
{
  ...
  if (repop->all_committed) {
    ...
    if (it != waiting_for_ondisk.end()) {
      ...
      for (list<tuple<OpRequestRef, version_t, int> >::iterator i =
				      ...
				      bool is_localread)
{
  ...
  if (oid.snap == CEPH_NOSNAP) {
    ObjectContextRef obc = get_object_context(oid, can_create, NULL, is_localread);
    if (!obc) {
      if (pmissing)
        *pmissing = oid;
      return -ENOENT;
  ...
  if (map_snapid_to_clone) {
    ...
    if (oid.snap > ssc->snapset.seq) {
      ...
      ObjectContextRef obc = get_object_context(head, false, NULL, is_localread);
      dout(10) << __func__ << " " << oid << " @" << oid.snap
	       << " snapset " << ssc->snapset
	       << " maps to head" << dendl;
      *pobc = obc;
	...
	return -EAGAIN;
      ...
      ObjectContextRef obc = get_object_context(oid, false, NULL, is_localread);
  ...
  ObjectContextRef obc = get_object_context(soid, false, NULL, is_localread);
  if (!obc || !obc->obs.exists) {
    if (pmissing)
      *pmissing = soid;
    put_snapset_context(ssc);
...
void PrimaryLogPG::apply_and_flush_repops(bool requeue)
{
  ...
  while (!repop_queue.empty()) {
	...
	repop->op = OpRequestRef();
      ...
      map<eversion_t, list<tuple<OpRequestRef, version_t, int> > >::iterator p =
  ...
  if (requeue) {
    ...
    if (!waiting_for_ondisk.empty()) {
      for (map<eversion_t, list<tuple<OpRequestRef, version_t, int> > >::iterator i =
	     waiting_for_ondisk.begin();
	   i != waiting_for_ondisk.end();
	   ++i) {
	for (list<tuple<OpRequestRef, version_t, int> >::iterator j =
	       i->second.begin();
	     j != i->second.end();
	     ++j) {
	  derr << __func__ << ": op " << *(std::get<0>(*j)->get_req()) << " waiting on "
	       << i->first << dendl;
	}
      }
...
int PrimaryLogPG::rep_repair_primary_object(const hobject_t& soid, OpRequestRef op)
{
  ...
  assert(!pool.info.is_erasure());
  if (!is_primary()) {
    dout(0) << __func__ << " replica object error, bad_object_info_t:" << soid << dendl;
    osd->reply_op_error(op, -EAGAIN);
    return -EAGAIN;
  }
