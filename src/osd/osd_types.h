...
// for slow disk check
#define OSD_SLOW_DISK_WARN_TYPE_NONE      0
#define OSD_SLOW_DISK_WARN_TYPE_WARN      1
#define OSD_SLOW_DISK_WARN_TYPE_WARN_LONG 2
#define OSD_SLOW_DISK_WARN_TYPE_WARN_DROP 3
#define OSD_SLOW_DISK_WARN_TYPE_LAST      4

typedef uint64_t  slow_disk_warn_t;

const string slow_disk_state_key = "slow_disk_state_";
const string DEV_PREFIX  = "/dev/";

inline string split_block_device_name(const string &disk) {
    auto n = disk.find(DEV_PREFIX);
    if (n == string::npos) {
        return disk;
    }
    return disk.substr(n + DEV_PREFIX.length());
}

// slow disk state to string
string osd_slow_disk_alarm_warn_str(slow_disk_warn_t _t);
string osd_slow_disk_alarm_warn_str(const string &_t);
bool osd_metadata_have_slow_disk(const map<string, string> &pm);
...
enum {
  ...
  CEPH_OSD_RMW_FLAG_MERGE_RGW_TYPE    = (1 << 12),
};
