...
enum {
  ...
  l_osd_merged_op_before,
  l_osd_merged_op_after,
...
public:
  ...
  void send_error_op_reply(OpRequestRef op, int err, eversion_t v, version_t uv);
...
private:
  ...
  list<pair<epoch_t, PGRef> > awaiting_throttle_high_priority;
  list<pair<epoch_t, PGRef> > backfill_awaiting_throttle;
  list<pair<epoch_t, PGRef> > backfill_awaiting_throttle_high_priority;

  utime_t defer_recovery_until;
  uint64_t recovery_ops_active;
  uint64_t recovery_ops_reserved;
  bool recovery_paused;
  int  recovery_start_second_of_day;
  int  recovery_end_second_of_day;
  int  backfill_start_second_of_day;
  int  backfill_end_second_of_day;
#ifdef DEBUG_RECOVERY_OIDS
  map<spg_t, set<hobject_t> > recovery_oids;
#endif

  // backfill is part of recovery, but we should distinguish backfill from recovery, and give
  // recovery higher priority
  bool _can_recover_or_backfill_now(uint64_t *available_pushes,
                                    bool high_priority,
                                    ...
                                    int begin_time,
                                    int end_time);
  bool _recover_now(uint64_t *available_pushes, bool high_priority);
  bool _backfill_now(uint64_t *available_pushes, bool high_priority);
  void _maybe_queue_recovery();
  void _queue_for_recovery(pair<epoch_t, PGRef> p, uint64_t reserved_pushes);

public:
  void init_recovery_time_interval();
  ...
  void clear_queued_recovery(PG *pg) {
    ...
    awaiting_throttle_high_priority.remove_if(
      [pg](decltype(awaiting_throttle_high_priority)::const_reference awaiting ) {
        return awaiting.second.get() == pg;
      });
    backfill_awaiting_throttle.remove_if(
      [pg](decltype(backfill_awaiting_throttle)::const_reference awaiting ) {
        return awaiting.second.get() == pg;
      });
    backfill_awaiting_throttle_high_priority.remove_if(
      [pg](decltype(backfill_awaiting_throttle_high_priority)::const_reference awaiting ) {
        return awaiting.second.get() == pg;
      });
  }

  // delayed pg activation
  void queue_for_recovery(PG *pg, bool is_backfill) {
    Mutex::Locker l(recovery_lock);
    if (!is_backfill) {
      if (pg->is_forced_recovery_or_backfill()) {
        awaiting_throttle_high_priority.push_front(make_pair(pg->get_osdmap()->get_epoch(), pg));
      } else if(pg->recover_with_high_priority()) {
        awaiting_throttle_high_priority.push_back(make_pair(pg->get_osdmap()->get_epoch(), pg));
      } else {
        awaiting_throttle.push_back(make_pair(pg->get_osdmap()->get_epoch(), pg));
      }
    } else {
      if (pg->is_forced_recovery_or_backfill()) {
        backfill_awaiting_throttle_high_priority.push_front(make_pair(pg->get_osdmap()->get_epoch(), pg));
      } else if(pg->recover_with_high_priority()) {
        backfill_awaiting_throttle_high_priority.push_back(make_pair(pg->get_osdmap()->get_epoch(), pg));
      } else {
        backfill_awaiting_throttle.push_back(make_pair(pg->get_osdmap()->get_epoch(), pg));
      }
    }
    _maybe_queue_recovery();
  }
  ...
  // -- auto-adjuest the speed of recovery and backfill --
private:
  typedef enum {
    RECOVERY_SPEED_HOLD = 1,
    RECOVERY_SPEED_INC,
    RECOVERY_SPEED_DEC
  } recover_adjust_t;

  static const char* get_recover_adjust_type(recover_adjust_t t) {
    switch(t) {
    case RECOVERY_SPEED_HOLD:
      return "hold";
    case RECOVERY_SPEED_INC:
      return "increase";
    case RECOVERY_SPEED_DEC:
      return "decrease";
    }
    return "???";
  }

  disk_type_t data_disk_type = TYPE_SATA_NONE;
  utime_t last_recovery_speed_check;
  // pair<count, sum of cost>
  pair<uint64_t, uint64_t> last_osd_op_perf = {0, 0};
  pair<uint64_t, uint64_t> last_osd_r_op_perf = {0, 0};
  pair<uint64_t, uint64_t> last_osd_w_op_perf = {0, 0};
  pair<uint64_t, uint64_t> last_osd_rw_op_perf = {0, 0};
  // keep these configuration in mem to avoid we have to calc these config based on the type of disk
  uint64_t recovery_speed_increase_r_op_lat_max;
  uint64_t recovery_speed_increase_w_op_lat_max;
  uint64_t recovery_speed_increase_rw_op_lat_max;
  uint64_t recovery_speed_decrease_r_op_lat_max;
  uint64_t recovery_speed_decrease_w_op_lat_max;
  uint64_t recovery_speed_decrease_rw_op_lat_max;
  uint64_t osd_max_backfills_temp = 1;
  uint64_t osd_recovery_max_active_temp = 1;
  float osd_recovery_sleep_temp = 1.0;
  // pause recover when this osd is processing client requests
  atomic<bool> recovery_paused_by_client_op_inflight = false;

  bool adjust_recover_speed(recover_adjust_t _type);
  recover_adjust_t calc_recover_adjust_type(pair<uint64_t, uint64_t> op_perf,
                                            pair<uint64_t, uint64_t> r_op_perf,
                                            pair<uint64_t, uint64_t> w_op_perf,
                                            pair<uint64_t, uint64_t> rw_op_perf);

  inline uint64_t get_osd_recovery_max_active() {
    return cct->_conf->osd_recovery_speed_self_adaption ? osd_recovery_max_active_temp :
           cct->_conf->osd_recovery_max_active;
  }
  void pause_recovery_with_client_op_inflight() {
    recovery_paused_by_client_op_inflight.store(true);
  }
  bool recovery_is_paused_with_client_op_inflight() {
    return recovery_paused_by_client_op_inflight;
  }
  void unpause_recovery_with_client_op_inflight() {
    recovery_paused_by_client_op_inflight.store(false);
  }

public:
  void init_disk_info(pair<string, bool>& disk_info);
  void init_recovery_auto_config();
  void handle_recovery_auto_adjust_config_change();
  float get_osd_recovery_sleep();
  void auto_adjust_recover_speed_process();
...
private:
  ...
  // -- license --
  Mutex license_lock;
  Cond_monotonic license_cond;
  bool license_stop = false;
  const double OSD_LICENSE_TICK_INTERVAL = { 600.0 };
  const double OSD_LICENSE_TICK_TIMEOUT = { 86400.0 };
  License *osd_license = nullptr;
  utime_t last_license_verify_time = utime_t();
  std::atomic<unsigned> license_state = {0};

  struct LicenseThread : public Thread {
    OSD *osd;
    explicit LicenseThread(OSD *o) : osd(o) {}
    void *entry() override {
      osd->license_entry();
      return NULL;
    }
  ...
  int write_local_object(const string &oid, const bufferlist &bl);
  int read_local_object(const string &oid, bufferlist &bl);
  void license_entry();
  int license_verify(bool is_init, double duration_sec);
  void send_license_status_update();

  // -- slow disk check --
  map<string, string> osd_metadata;
  SlowDiskAlarmSystem slow_disk_check;
  std::atomic<uint64_t> last_slow_disk_state = 0;
  utime_t last_slow_disk_report_mon_time;
  void send_slow_disk_status_update();
...
private:
  ...
  float get_osd_recovery_sleep_from_config();
