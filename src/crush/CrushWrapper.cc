                               ...
                               const vector<int>& up)
{
  ...
  ldout(cct, 20) << __func__ << " rule_id " << rule_id << " pool size " << pool_size
                << " up " << up << dendl;
...
int CrushWrapper::find_root_of_rule(unsigned ruleno, int &root) const
{
...
// get the crush weight of osd or bucket
int CrushWrapper::get_item_weight(unsigned ruleno, int id) const
{
  int root, r;
  r = find_root_of_rule(ruleno, root);
  if (r < 0)
    return r;

  if (root >= 0) { // root is osd
    if (root == id) {
      // return the weight for the first instance it finds.
      return get_item_weight(id);
    } else {
      return -ENOENT;
    }
  }
  ...
  while (!q.empty()) {
    ...
    for (unsigned j=0; j<b->size; ++j) {
      ...
      if (item_id >=0 && item_id == id) { // it's osd
        return crush_get_bucket_item_weight(b, j);
      } else if (item_id < 0) {
        q.push_back(item_id);
      }
    }
...
int CrushWrapper::get_osds_weight_of_root(int root, map<int,int> &pmap) const
{
  list<int> q;
  int sum = 0;
  pmap.clear();
  ...
  while (!q.empty()) {
    ...
    for (unsigned j=0; j<b->size; ++j) {
      int item_id = b->items[j];
      if (item_id >= 0) { //it's an OSD
        int w = crush_get_bucket_item_weight(b, j);
        pmap[item_id] = w;
        sum += w;
      } else {
	q.push_back(item_id);
