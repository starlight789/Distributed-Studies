...
extern "C" int rados_init(const char *meta_pool, struct ceph_rados_context *ceph_rados, 
                            const char * const path)
{
  int ret = ceph_rados->rados.init("admin");
  if(ret < 0) return ret;
  ret = ceph_rados->rados.conf_read_file(path);
  if(ret < 0) return ret;
  ret = ceph_rados->rados.connect();
  if(ret < 0) return ret;
  ret = ceph_rados->rados.ioctx_create(meta_pool, ceph_rados->io_ctx);
  if(ret < 0) return ret;
  ceph_rados->io_ctx.set_namespace(CEPHFS_USER_NAMESPACE);
  return 0;
}

extern "C" int rados_user_index_create(const char *meta_pool, const char * const path)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  if(ret < 0) {
    return ret;
  }

  librados::ObjectWriteOperation op_create;
  op_create.create(true);
  ret = crc.io_ctx.operate(CEPHFS_USER_OBJ, &op_create);
  if(ret < 0) {
    return ret;
  }
  return 0;
...
extern "C" int rados_user_create(const char *meta_pool, const char * const path, const char *usr_id, 
                        const char *usr_email, uint64_t file_quota, uint64_t space_quota)
{
  ...
  op.write_full(bl);
  op.mtime(&timestamp);
  ret = crc.io_ctx.operate(string(usr_id), &op);
  if (ret < 0) {
    return ret;
  }
  ...
  bl.append("");
  values[usr_id] = bl;
  ret = crc.io_ctx.omap_set(CEPHFS_USER_OBJ, values);
  if (ret < 0) {
    return ret;
  }
  return 0;
...
extern "C" int rados_user_change(const char *meta_pool, const char * const path, const char *usr_id,
                                 const char *usr_email, uint64_t file_quota, uint64_t space_quota)
{
  time_t timestamp = time(NULL);
  ...
  cephfs_user cu;
  ret = rados_user_stat(meta_pool, path, usr_id, &cu);
  if(ret < 0) {
    return ret;
  }

  cu.email = string(usr_email);
  cu.file_quota = file_quota;
  cu.space_quota = space_quota;
  bufferlist bl;
  encode(cu, bl);
...
extern "C" int rados_user_stat(const char *meta_pool, const char * const path, const char *usr_id, 
                                struct cephfs_user *cu)
{
  if(meta_pool == NULL || path == NULL || usr_id == NULL || cu == NULL) { 
    return -EINVAL;
  }
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  ...
  bufferlist bl;
  ret = crc.io_ctx.read(string(usr_id), bl, default_op_size, 0);
  if (ret < 0) {
    return ret;
  }
...
extern "C" int rados_user_info(const char *meta_pool, const char * const path, const char *usr_id,
                     char **usr_email, char **create_time, uint64_t *files, uint64_t *space_used)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  if(ret < 0) {
  ...
  char *tmp = (char *)(malloc(strlen(out.email_str()) + 1));
  memset(tmp, 0, strlen(out.email_str()) + 1);
  memcpy(tmp, out.email_str(), strlen(out.email_str()));
  *usr_email = tmp;
  char *tmp2 = (char *)(malloc(strlen(out.magic_str()) + 1));
  memset(tmp2, 0, strlen(out.magic_str()) + 1);
  memcpy(tmp2, out.magic_str(), strlen(out.magic_str()));
  *create_time = tmp2;

  //*usr_email = out.email_str();
  //*create_time = out.magic_str();
  *files = out.files;
  *space_used = out.space_used;
  return 0;
}

extern "C" int rados_user_rm(const char *meta_pool, const char * const path, const char *usr_id)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  if(ret < 0) {
  ...
  set<string> out_keys;
  ret = crc.io_ctx.omap_get_keys(string(usr_id), "", LONG_MAX, &out_keys);
  if(!out_keys.empty()) {
    return -ENOTEMPTY;
  }

  set<string> keys;
  keys.insert(string(usr_id));

  ret = crc.io_ctx.omap_rm_keys(CEPHFS_USER_OBJ, keys);
  if (ret < 0) {
    return ret;
  }

  ret = crc.io_ctx.remove(string(usr_id));
  if (ret < 0) {
    return ret;
  }
  return 0;
...
extern "C" int rados_user_list(const char *meta_pool, const char * const path, char ***arr, 
                                int *arr_len)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  ...
  set<string> out_keys;
  ret = crc.io_ctx.omap_get_keys(CEPHFS_USER_OBJ, "", LONG_MAX, &out_keys);
  if (ret < 0) {
    return ret;
  }
  *arr_len = out_keys.size();
  *arr = (char**) malloc(out_keys.size() * sizeof(char*));
  set<string>::const_iterator iter = out_keys.begin();
  int i = 0;
  for(; iter != out_keys.end(); iter++, i++) {
    (*arr)[i] = (char*) malloc((*iter).length() + 1);
    memset((*arr)[i], 0, (*iter).length() + 1);
    memcpy((*arr)[i], (*iter).c_str(), (*iter).length());
  }
  return 0;
}

extern "C" int rados_dir_create(const char *meta_pool, const char * const path, const char *usr_id,
                                 const char *dir_name)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  ...
  uint64_t psize;
  time_t pmtime;
  ret = crc.io_ctx.stat(string(usr_id), &psize, &pmtime);
  if(ret < 0) {
    return ret;
  } 
  map<string, bufferlist> values;
  bufferlist bl;
  bl.append("");
  values[string(dir_name)] = bl;
        
  ret = crc.io_ctx.omap_set(string(usr_id), values);
  if (ret < 0) {
    return ret;
  }
  return 0;
...
extern "C" int rados_dir_rm(const char *meta_pool, const char * const path, const char *usr_id, 
                            const char *dir_name)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  ...
  set<string> keys;
  keys.insert(string(dir_name));

  ret = crc.io_ctx.omap_rm_keys(string(usr_id), keys);
  if (ret < 0) {
    return ret;
  }
  return 0;
...
extern "C" int rados_dir_list(const char *meta_pool, const char * const path, const char *usr_id, 
                              char ***arr, int *arr_len)
{
  struct ceph_rados_context crc;
  int ret = rados_init(meta_pool, &crc, path);
  if(ret < 0) {
