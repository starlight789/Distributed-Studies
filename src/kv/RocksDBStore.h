...
public:
  ...
  RocksDBStore(CephContext *c, const string &path, map<string,string> opt, void *p) :
    ...
    disableWAL(false),
    delete_range_threshold(cct->_conf->rocksdb_delete_range_threshold)
  {}
      ...
      iterate_upper_bound(make_slice(bounds.upper_bound))
    {
        ...
        if (db->cct->_conf->osd_rocksdb_iterator_bounds_enabled) {
          if (bounds.lower_bound) {
            lgeneric_dout(db->cct, 20) << " iterator set lower bound " << dendl;
            options.iterate_lower_bound = &iterate_lower_bound;
          }
          if (bounds.upper_bound) {
            lgeneric_dout(db->cct, 20) << " iterator set upper bound " << dendl;
...
err:
  ...
  WholeSpaceIterator get_wholespace_iterator(IteratorOpts opts = 0,
                                             IteratorBounds bounds = IteratorBounds()) override;
};
