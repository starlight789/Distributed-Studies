...
void librados::ObjectWriteOperation::cmpxattr(const char *name, uint8_t op, const bufferlist& v)
{
  ::ObjectOperation *o = &impl->o;
  o->cmpxattr(name, op, CEPH_OSD_CMPXATTR_MODE_STRING, v);
}

void librados::ObjectWriteOperation::cmpxattr(const char *name, uint8_t op, uint64_t v)
{
  ::ObjectOperation *o = &impl->o;
  bufferlist bl;
  encode(v, bl);
...
static int translate_flags(int flags)
{
  ...
  if (flags & librados::OPERATION_SKIP_CACHE)
    op_flags |= CEPH_OSD_FLAG_SKIP_CACHE;

  return op_flags;
}

int librados::IoCtx::operate(const std::string& oid, librados::ObjectWriteOperation *o, bool skip_cache)
{
  int flags = skip_cache ? CEPH_OSD_FLAG_SKIP_CACHE : 0;
  object_t obj(oid);
  return io_ctx_impl->operate(obj, &o->impl->o, (ceph::real_time *)o->impl->prt, flags);
}
...
int librados::IoCtx::operate(const std::string& oid, librados::ObjectReadOperation *o, int flags, bufferlist *pbl)
{
  object_t obj(oid);
...
int librados::Rados::get_base_pool(int64_t pool_id, int64_t* base_tier)
{
  return rados_pool_get_base_tier((rados_t)client, pool_id, base_tier);
}
