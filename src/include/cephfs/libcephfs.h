...
/**
 * Init a rados context with a pool name
 * @param meta_pool the pool name to create context
 * @param rados librados::Rados to be used
 * @param io_ctx librados::IoCtx to be userd for manipulating rados object
 * @param path the conf path
 * 
 * Return 0 if the user is successfully stored.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_init(const char *meta_pool, struct ceph_rados_context *ceph_rados, const char * const path);


/**
 * Create a obj to store usr index omap
 * @param meta_pool the pool name to manipulate
 * @param path the conf path
 *
 * Return 0 if the user index ojb is successfully created, -EEXIST if the user exists.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_user_index_create(const char *meta_pool, const char * const path);

/**
 * Store a user info to a rados object and index omap
 * @param meta_pool the pool name to manipulate
 * @param usr_id the user name to store
 * @param usr_email the user email address
 * @param file_quota the files can writed to a user
 * @param space_quota the space allocated to a user
 * @param path the conf path
 *
 * Return 0 if the user is successfully stored, -EEXIST if the user exists.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_user_create(const char *meta_pool, const char * const path, const char *usr_id, 
            const char *usr_email, uint64_t file_quota, uint64_t space_quota);

/**
 * Change a user info to a rados object and index omap
 * @param meta_pool the pool name to manipulate
 * @param usr_id the user name to change
 * @param usr_email the user email address
 * @param file_quota the files can writed to a user
 * @param space_quota the space allocated to a user
 * @param path the conf path
 *
 * Return 0 if the user is successfully stored, -ENODEN if the user does not exists.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_user_change(const char *meta_pool, const char * const path, const char *usr_id, 
            const char *usr_email, uint64_t file_quota, uint64_t space_quota);

/**
 * Request a user stat from a rados object, used by cplusplus context.
 * @param meta_pool the pool name to manipulate
 * @param user_id user name to read
 * @param cu user info to be stored
 * @param path the conf path
 * 
 * Return 0 if the user is successfully read, -ENOENT if the user does not exists.
 */
int rados_user_stat(const char *meta_pool, const char * const path, const char *usr_id, 
					struct cephfs_user *cu);

/**
 * Request a user stat from a rados object.
 * @param meta_pool the pool name to manipulate
 * @param user_id user name to read
 * @param cu user info to be stored
 * @param path the conf path
 * 
 * Return 0 if the user is successfully read, -ENOENT if the user does not exists.
 * Caller should free *usr_email and *create_time.
 */
int rados_user_info(const char *meta_pool, const char * const path, const char *usr_id, char **usr_email, 
            char **create_time, uint64_t *files, uint64_t *space_used);

/**
 * Remove the user from index omap and rados objects.
 * @param meta_pool the pool name to manipulate
 * @param usr_id the user id to be remove
 * @param path the conf path
 * 
 * Return 0 if the user is successfully removed, -ENOENT if the user does not exists.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_user_rm(const char *meta_pool, const char * const path, const char *usr_id);

/**
 * List all users from index omap
 * @param meta_pool the pool name to manipulate
 * @param arr the set of users to be read from index omap
 * @param arr_len the length of arr
 * @param path the conf path
 * 
 * Return 0 if the user is successfully removed, -ENOENT if is not any user.
 * And other error codes if there is a fatal error of some sort.
 * It is the caller's responsibility to arrange for it to be freed, such as by calling free.
 * Caller should free *arr, and free all **arr count by arr_len.
 */
int rados_user_list(const char *meta_pool, const char * const path,  char ***arr, int *arr_len);

/**
 * Store a user info to user omap
 * @param meta_pool the pool name to manipulate
 * @param usr_id the user name to store the dir path
 * @param dir_name the dir path to store
 * @param path the conf path
 *
 * Return 0 if the dir path is successfully stored, -ENOENT if the user does not exists.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_dir_create(const char *meta_pool, const char * const path, const char *usr_id,
                      const char *dir_name);

/**
 * Remove the dir from user omap.
 * @param meta_pool the pool name to manipulate
 * @param usr_id the user id to be remove the dir path
 * @param dir_name the dir path to remove
 * @param path the conf path
 * 
 * Return 0 if the dir is successfully removed from user omap, -ENOENT if the user does not exists.
 * And other error codes if there is a fatal error of some sort.
 */
int rados_dir_rm(const char *meta_pool, const char * const path, const char *usr_id, 
                  const char *dir_name);

/**
 * List all dir path stored in an user omap
 * @param meta_pool the pool name to manipulate
 * @param usr_id the user to be listed
 * @param arr the set of dirs to be read from index omap
 * @param arr_len the length of arr
 * @param path the conf path
 * 
 * Return 0 if the user is successfully listed, -ENOENT if is not any user.
 * And other error codes if there is a fatal error of some sort.
 * It is the caller's responsibility to arrange for it to be freed, such as by calling free.
 * Caller should free *arr, and free all **arr count by arr_len.
 */
int rados_dir_list(const char *meta_pool, const char * const path, const char *usr_id, char ***arr, 
                    int *arr_len);
