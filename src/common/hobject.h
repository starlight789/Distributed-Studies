...
struct wrapper_ghobject_t {
  const ghobject_t *obj = nullptr;
  wrapper_ghobject_t(const ghobject_t *obj) : obj(obj) {}

  friend bool operator>(const wrapper_ghobject_t& l, const wrapper_ghobject_t& r) {
    return cmp(*(l.obj), *(r.obj)) > 0;
  }
  friend bool operator>=(const wrapper_ghobject_t& l, const wrapper_ghobject_t& r) {
    return cmp(*(l.obj), *(r.obj)) >= 0;
  }
...
namespace std {
  template<> struct hash<wrapper_ghobject_t> {
    size_t operator()(const wrapper_ghobject_t &r) const {
      return std::hash<ghobject_t>{}(*r.obj);
    }
  };
} // namespace std
