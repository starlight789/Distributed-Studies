...
# define bendl std::endl; }
  ...
  public:
    ...
    int mempool;
    unsigned alloc_len; // alloc size = size of data + size of struct raw
    ...
    explicit raw(unsigned l, int mempool=mempool::mempool_buffer_anon)
      : data(NULL), len(l), nref(0), mempool(mempool), alloc_len(l) {
      mempool::get_pool(mempool::pool_index_t(mempool)).adjust_count(1, alloc_len);
    }
    raw(char *c, unsigned l, int mempool=mempool::mempool_buffer_anon)
      : data(c), len(l), nref(0), mempool(mempool), alloc_len(l) {
      mempool::get_pool(mempool::pool_index_t(mempool)).adjust_count(1, alloc_len);
    }
    raw(char *c, unsigned l, int mempool, unsigned alloc_len)
      : data(c), len(l), nref(0), mempool(mempool), alloc_len(alloc_len) {
      mempool::get_pool(mempool::pool_index_t(mempool)).adjust_count(1, alloc_len);
    }
    virtual ~raw() {
	...
	return;
      ...
      mempool::get_pool(mempool::pool_index_t(mempool)).adjust_count(-1, -(int)alloc_len);
      mempool = pool;
      mempool::get_pool(mempool::pool_index_t(pool)).adjust_count(1, alloc_len);
    }
...
public:
  ...
  class buffer::raw_combined : public buffer::raw {
    size_t alignment;
  public:
    raw_combined(char *dataptr, unsigned l, unsigned alloc_len, unsigned align,
		 int mempool)
      : raw(dataptr, l, mempool, alloc_len),
	alignment(align) {
      inc_total_alloc(len);
	...
	align = sizeof(size_t);
      ...
      constexpr size_t rawlen = round_up_to(sizeof(buffer::raw_combined),
          alignof(buffer::raw_combined));
      size_t datalen = round_up_to(len, alignof(buffer::raw_combined));
      size_t alloc_len = rawlen + datalen;

#ifdef DARWIN
      char *ptr = (char *) valloc(alloc_len);
#else
      char *ptr = 0;
      int r = ::posix_memalign((void**)(void*)&ptr, align, alloc_len);
      if (r)
	throw bad_alloc();
      alloc_len = round_up_to(alloc_len, align);
	...
	throw bad_alloc();
      ...
      return new (ptr + datalen) raw_combined(ptr, len, alloc_len, align, mempool);
    }
  ...
  buffer::raw* buffer::copy(const char *c, unsigned len, int mempool) {
    raw* r = buffer::create_aligned(len, sizeof(size_t), mempool);
    memcpy(r->data, c, len);
    return r;
  }
  ...
  buffer::raw* buffer::create_aligned(unsigned len, unsigned align, int mempool) {
    return create_aligned_in_mempool(len, align, mempool);
  }
  ...
  buffer::ptr::ptr(const char *d, unsigned l, int mempool) : _off(0), _len(l)    // ditto.
  {
    _raw = copy(d, l, mempool);
    _raw->nref++;
    bdout << "ptr " << this << " get " << _raw << bendl;
  }
