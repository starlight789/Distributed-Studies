...
void TokenBucketThrottle::set_limit(uint64_t avg, uint64_t capacity) {
  Mutex::Locker timer_locker(*m_timer_lock);
  // chcek again to avoid other thread modify
  if (avg == m_avg && capacity == m_throttle.max) {
    return;
  }
  cancel_timer();
  m_throttle.max = capacity;
  m_avg = avg;
  schedule_timer();
}
